...
onmessage = (event) => {
  const {seqList, rolesList, chainStartsList, monomersDict, alphabet, polymerType, start, end}:
    SeqToMolfileWorkerData = event.data;
  ...
  for (let rowI = start; rowI < end; ++rowI) {
    try {
      ...
      const chainStarts = chainStartsList ? chainStartsList[rowI] : undefined;
      resMolList[rowI - start] = monomerSeqToMolfile(seq, monomersDict, alphabet, polymerType, roles, chainStarts);
    } catch (err: any) {
