...
7. **Codegen** — if adding a single-objective optimizer with sync+async API, follow the driver pattern (see `src/optimization/CLAUDE.md`): hand-write only the async source; run `npm run update-codegen`; commit both `*-driver.ts` and `*-driver-sync.ts`; `npm run codegen:check` must pass
