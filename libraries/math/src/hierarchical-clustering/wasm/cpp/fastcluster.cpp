...
bool fc_isnan(float x) { return x != x; }
...
//   labels = allocated integer array of size n for result
//
void cutree_cdist(int n, const int* merge, float* height, float cdist, int* labels) {
...
//   1 = invalid method
//
int hclust_fast(int n, float* distmat, int method, int* merge, float* height) {
  ...
  else if (method == HCLUST_METHOD_WEIGHTED) {
    // best average distance
    float* members = new float[n];
    for (int i=0; i<n; i++) members[i] = 1;
    NN_chain_core<METHOD_METR_WEIGHTED, t_float>(n, distmat, members, Z2);
    delete[] members;
  }
