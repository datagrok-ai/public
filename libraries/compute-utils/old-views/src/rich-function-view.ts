...
import {Validator, ValidationResult, isValidationPassed, getErrorMessage, makePendingValidationResult, mergeValidationResults, getValidationIcon} from './shared-utils/validation';
import {EXPERIMENTAL_TAG, viewerTypesMapping} from '../../shared-utils/consts';
...
import {getDefaultValue, properUpdateIndicator} from '../../function-views/src/shared/utils';
import {getPropViewers} from '../../shared-utils/utils';
import {UiUtils} from '../../old-components';
import {richFunctionViewReport} from '../../shared-utils/export';
...
export class RichFunctionView extends FunctionView {
  ...
  public async loadInputsOverrides() {
    ...
    await Promise.all(inputParams.map(async (param) => {
      if (param.property.options.input) {
        ...
        const call = await func.prepareAsync({params: JSON.parse(param.property.options.inputOptions || '{}')});
        await call.call();
  ...
  private async processCustomDataUpload() {
    ...
    const funcCall = await func.prepareAsync({params: {'func': this.func}});
    await funcCall.call();
