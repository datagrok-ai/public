...
export class PinnedColumn {
  ...
  constructor(colGrid : DG.GridColumn) {
    ...
    this.m_handlerKeyDown = rxjs.fromEvent<KeyboardEvent>(eCanvasThis, 'keydown').subscribe((e : KeyboardEvent) => {
      ...
      setTimeout(() =>{
        ...
        try { grid.overlay.dispatchEvent(ee); } catch (ex) {
  ...
  public onMouseDblClick(e : MouseEvent) : void {
    ...
    const gridColName = this.m_colGrid.name;
    if (column !== null) {
      if (column.semType === DG.SEMTYPE.MOLECULE) {
        ...
        const headerThis = this;
        dialog.onOK(() => {
          const gc = grid.cell(gridColName, nRow);
          if (gc !== null) // set on grid cell to triger value edited event
            gc.setValue(b ? sketcher.getMolFile() : sketcher.getSmiles(), true);
  ...
  public onMouseDown(e : MouseEvent) : void {
    ...
    */
    ...
    if (nRowGrid >= 0) {
      ...
      const cell = grid.cell(this.m_colGrid.name, nRowGrid);
      const renderer = getRenderer(cell);
      if (renderer !== null) { renderer.onMouseDownEx(cell, e, this.m_arXYMouseOnCellDown[0], this.m_arXYMouseOnCellDown[1]); } else {
    ...
    */
    ...
    if (this.m_nRowGridDragging >= 0) {
      ...
      if (!bCtrl && !bRangeSel && nRowGrid === this.m_nRowGridDragging) { //click on the same row which will become active
        ...
        } else { bitsetSel.setAll(false, false); }
  ...
  paint(g : CanvasRenderingContext2D | null, grid : DG.Grid) : void {
    ...
    const str = TextUtils.trimText(this.m_colGrid.name, g, nW);
    ...
    const nWW = nW*window.devicePixelRatio;
    //const nHH = nHRowGrid;
