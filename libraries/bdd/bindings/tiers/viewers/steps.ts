...
export const setProperty = When('user sets {string} of {element} to {string}', (page: Page, caption: string, target: ElementRef, value: string) =>
...
export const setProperties = When('user sets properties of {element}:', (page: Page, target: ElementRef, table: string[][]) =>
...
export const propertyShouldBe = Then('{string} of {element} should be {string}', (page: Page, caption: string, target: ElementRef, value: string) =>
...
export const propertyShouldNotBe = Then('{string} of {element} should not be {string}', (page: Page, caption: string, target: ElementRef, value: string) =>
...
export const pickFromAreaContextMenu = When('user picks {string} from the context menu of the {string} area of {element}',
...
export const rightClickArea = When('user right-clicks on the {string} area of {element}', async (page: Page, area: string, target: ElementRef) => {
  await v.openContextMenuOf(page, target, area);
...
export const clickArea = When('user clicks on the {string} area of {element}', async (page: Page, area: string, target: ElementRef) => {
  const c = v.centerOf(await v.hitArea(page, target, area, true));
  await page.mouse.click(c.x, c.y);
}, {tier: 'ui'});

export const doubleClickArea = When('user double-clicks on the {string} area of {element}', async (page: Page, area: string, target: ElementRef) => {
  const c = v.centerOf(await v.hitArea(page, target, area, true));
  await page.mouse.dblclick(c.x, c.y);
}, {tier: 'ui'});

export const hoverArea = When('user hovers over the {string} area of {element}', async (page: Page, area: string, target: ElementRef) => {
...
export const resizeTo = When('user resizes {element} to {int} by {int}', (page: Page, target: ElementRef, width: number, height: number) =>
  v.resize(page, target, width, height), {tier: 'ui', description: 'width by height in pixels; "restores the size" undoes it'});

export const resizeWidth = When('user resizes {element} to {int} wide', (page: Page, target: ElementRef, width: number) =>
  v.resize(page, target, width, null), {tier: 'ui'});

export const restoreSize = When('user restores the size of {element}', (page: Page, target: ElementRef) => v.restoreSize(page, target), {tier: 'ui'});

// --- the canvas ------------------------------------------------------------------------------------

export const takeSnapshot = When('user takes a snapshot of {element}', async (page: Page, target: ElementRef) => {
...
export const repainted = Then('{element} should have repainted', (page: Page, target: ElementRef) => v.expectRepainted(page, target),
  {description: 'the canvas differs from the snapshot taken before the last change; the new state becomes the snapshot'});

export const lessInk = Then('{element} should have less ink than before', (page: Page, target: ElementRef) => v.expectInk(page, target, 'less'),
  {description: 'fewer painted pixels than the snapshot before the last change'});

export const moreInk = Then('{element} should have more ink than before', (page: Page, target: ElementRef) => v.expectInk(page, target, 'more'));

export const painted = Then('{element} should be painted', (page: Page, target: ElementRef) => v.expectInk(page, target, 'some'),
  {description: 'the canvas has painted pixels'});

// --- events and errors -----------------------------------------------------------------------------

export const listenFor = Given('user listens for {string} event on {element}', (page: Page, event: string, target: ElementRef) =>
  v.listenFor(page, target, event), {tier: 'api', description: 'a viewer event by name (d4-boxplot-reset-view); assert with "should have fired"'});

export const eventFired = Then('{string} event should have fired on {element}', (page: Page, event: string, target: ElementRef) =>
  v.expectFired(page, target, event));
