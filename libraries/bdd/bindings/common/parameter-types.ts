...
defineParameterType({name: 'widget', regexp: /(?:.+? (?:viewer|widget)|(?:the )?grid)(?: in .+?)?/,
  description: 'an element phrase that names a viewer or a widget ("box plot viewer", "results widget"), or the platform\'s grid ("grid") — steps that need one accept nothing else'});
