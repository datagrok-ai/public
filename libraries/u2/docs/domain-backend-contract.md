...
degrading silently. Which of them a table has is **declared, never guessed** — `support` says so
and the handle installs exactly that set, so `member === undefined` and `support.<flag> === false`
are always the same answer. The memory backend mirrors all of them but `watch`.
