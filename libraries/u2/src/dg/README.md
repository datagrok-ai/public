...
| `domains.search(src, options?)` | `DomainSearch`, a ribbon search box two-way with `src.search` — the schema's searchable columns, AND-ed with the query; written after a pause (`debounceMs`, 300), at once on Enter, cleared by Escape; a search while the session is dirty is not run (H6) |
| `domains.filters(src, options?)` | `DomainFilters`, the query box (`mode: 'query'`, completion over the table's columns and, over the platform, their values) or the condition builder (`'builder'`), two-way with `src.query`; a change while the session is dirty goes through `confirmDiscard` first; text stays text, a tree stays a tree |
