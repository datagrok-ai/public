...
import {drainPendingDeletes} from './projects';
  ...
  return base.extend<{page: Page}, {shared: {page: Page | null; dirty: boolean}}>({
    ...
    page: async ({browser, contextOptions, shared}, use) => {
      if (!shared.page) {
        ...
        // a raw contextOptions context drops the base config's clipboard grant, which copy flows need on plain-HTTP CI
        const context = await browser.newContext(
          {...contextOptions, permissions: contextOptions.permissions ?? base.info().project.use.permissions});
