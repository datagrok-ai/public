...
export default async function globalSetup(_config: FullConfig) {
  ...
  try {
    ...
    // This module now lives in @datagrok-libraries/test, so __dirname points at the
    // lib, not the run dir. The runner (playwright-runner.ts) spawns Playwright with
    // cwd = the package dir, so write auth relative to process.cwd() — every run dir
    // (playwright-public or a package's playwright/) gets its own auth files.
    ...
    //   scripts/*     → path.resolve(cwd, '.auth.json')
    // CWD when Playwright runs under `grok test` is the package directory,
    // so all three resolve to siblings inside the run dir.
