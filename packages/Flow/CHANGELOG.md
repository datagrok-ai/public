...
* Added: Annotations can be pinned (right-click → Pinned, saved with the flow) — a pinned note cannot be moved or resized and dragging it pans the canvas, so a zoomed-in view full of annotations stays navigable
* Added: Copy in the node context menu (feeds the same clipboard as Ctrl+C — copies the whole selection when the node is part of one)
* Improved: The home screen offers Workflow demo, Interactive viewers, and Blank canvas (Bio Molecules card removed; the Sequence demo still ships as a flow script); the Workflow demo template now opens with autorun on
...
* Improved: Removed the success balloons ("Flow execution completed" after every 30 ms run, "Added OpenFile node for: …" on drop) — the green node states, edge counts, and the node itself already say it
* Fixed: A successful Open File run no longer overwrites a user-renamed node title — only machine-stamped titles ("Open File: <file>") restamp from the path
* Improved: "Import from a table's history" now asks for confirmation before replacing a non-empty flow (the import is not undoable)
* Improved: Context panel header — node kind and function chips merged into one line with vertical separators, user-facing kind words ("Function", not "func"), the package never named twice, a placeholder and a visible resting border on Description, and the Sheet Name row only shown for .xlsx paths
* Improved: A collapsed errored node gets a red edge accent (its info line is folded away); viewer nodes' auto-caption reads "Displays a scatter plot" instead of "Runs a function"
* Improved: Pass-through output rows are labeled after the input they forward ("Table →") instead of an anonymous arrow, so chaining tables past a node's declared outputs no longer takes guesswork
* Improved: Smaller polish — the view tab says "Flow" (was "FuncFlow"), panel labels fit two-word captions ("Group By Columns"), the viewer pane's title field reads "Chart title" (the header already has the node Title), the drag-out menu shows each function's category right-aligned and muted, a new group names itself after its first member ("Open File +2"), and the result-row ⊕ explains what it does
* Fixed: The panel's Execution section could contradict the canvas — a run finishing while the user typed in the panel left "Error (28ms)" on display after the node had already rerun green; the section now re-renders in place from the live state (the full-panel rebuild stays focus-guarded)
* Improved: A halted run now explains itself — nodes the run never reached read "Skipped — \"Open File: x.csv\" failed" instead of sitting silently grey, and an explicit run pops one red "Run stopped" balloon naming the failing node
* Improved: Error messages drop the platform's "Operation caused an exception (…)" wrapper and map server-side absolute file paths back to the user's System:AppData/… form
* Improved: A clipped node title reveals its full text on hover
* Improved: Saving reads as a journey with an end — the OK button says "Save & publish…" when the dashboard step follows, publishing ends with a "Dashboard published" balloon, and the defaults no longer leak "MyFuncFlow"/"Generated by FuncFlow"
* Improved: Join Tables speaks user language — "Left table/Right table", "Left keys/Right keys", "Columns from left/right" (curated caption overrides; parameter names and saved flows unchanged)
* Improved: A new annotation opens ready to type (placeholder selected); the canvas context menu gains Paste and Tidy up layout; importing a script says "press Run to compute" instead of just counting nodes; the preview pin tilts when loose and stands upright when pinned
* Fixed: The raw "Offset is outside the bounds of the DataView" parse error that could intermittently surface on a failed file open is mapped to a plain-language message; imported Open File nodes are titled with their file from import time (so "Skipped/Run stopped" messages name the actual file); outputs kept from before a failure are labeled "Last successful outputs" and dimmed; the Join Tables column rows' table combo says "Left table/Right table" instead of "table1/table2"
...
* Fixed: A column name, Log label, or string input default containing a quote produced a SyntaxError in the generated script; all user strings are now JSON-escaped at emission (Select Column(s), Log, `//input:` header defaults)
* Fixed: The instrumented run always reports `run-complete` — statements between instrumented steps (SetVar registration, semantic-type detection) could throw and abandon the run with no end event; the body is now wrapped in try/finally with a de-duplicated final emit
* Fixed: Opening a saved flow after a function's parameter was renamed/removed threw mid-load and silently abandoned the rest of the document; such connections are now warn-skipped like unknown node types, and a document node without `pos` no longer crashes the loader
* Fixed: Double-clicking a recent-flow row (or any two concurrent loads) interleaved two deserializations into one merged, corrupt canvas — loads are now serialized through a mutex
* Fixed: Stop now actually cancels the in-flight call, releases a run paused at a breakpoint, and marks the abandoned run so its late `__ff_stash` writes can't overwrite the next run's live values (stale data presented as fresh to single-node reruns and the column picker)
* Fixed: An edit made while a save round-trip was in flight was silently marked "saved" (Save greyed out, closing lost the edit) — the saved baseline is now the snapshot of the payload actually sent; a failed save also no longer proceeds to add-to-space / dashboard publish
* Fixed: Resource leaks on view close — `detach()` now releases the run's event subscription and this flow's `__ffFlowLive` DataFrame clones, stops a running guide (its interval, resize listener, and on-screen card previously survived as zombie UI), cancels the deferred editor init (fast preview cycling used to build editors nothing ever destroyed), disposes the shown node's custom editors, unhooks the preview-cell handler, closes the port-preview popup, drops unsaved uploaded-file bytes, and resets `grok.shell.o`; `FlowEditor.destroy()` now removes the suggestion-menu's window listeners (previously permanent). The creation-script dialog detaches its embedded view on close
* Fixed: Keyboard shortcuts (Delete, Ctrl+A/C/V/G, arrows, Escape) fired in every live editor at once — pressing Delete in the creation-script dialog also deleted the selected nodes in the main view behind it; the editor the user last pressed inside now owns the shortcuts
* Fixed: A guide step whose wait ignores the abort signal kept its card and highlights on screen after the guide was stopped or the view closed
* Fixed: Deleting an output node while its view tab was being activated mounted a TableView into the removed pane (leaked view, zero-size dock trap)
* Fixed: The expression editor's wired-but-uncomputed notice was reworded back to state the table has not been computed yet — it had lost the documented distinction from the unconnected state
* Improved: Replaced hand-rolled implementations with platform API — `DG.Utils.openFile`/`download` for .flow open/export, `DG.Dialog.getOpenDialogs()` instead of `.d4-dialog` DOM sniffing; execution-status colors and warning text now use the design tokens; removed the debug `testDialog` platform function, dead `graph-utils.ts`, dead guide predicates, dead CSS blocks, and fixed wrong `var(--blue-1, …)` fallbacks
* Improved: Migrated to newly exposed js-api surface (added in core alongside this change): `Func.tags` (tags live on Func, not just Script) and `Property.isOptional` replace the private `grok_Script_Get_Tags`/`grok_Property_Get` interop reads; `Property.choices` and `DG.Project.showSaveDialog` replace their untyped casts

* Save dialog: Reworked after a UX audit. The space binding is a regular form row (Space / value / Choose…-Change… link / × to clear) instead of an "Add to space…" button beside a status sentence; the picker is framed with a "New space…"/"New subspace…" footer link that says what it will actually create, and its sub-dialog says "Will be created under…" with a Create button. The Dashboard section keeps a stable height across its states, shows a spinner while a run started from the dialog is going (Save is disabled until it ends), and distinguishes "run failed" from "run produced no tables" instead of silently resetting; result tables render as icon rows with muted dimensions. A previously published dashboard is an explicit "Publish: update existing / as new" choice applied only on Save (the old "publish as new" link unbound instantly and vanished). The duplicate-name warning sits under Name on a reserved line (no layout shift), states the consequence, and a fresh flow pre-uniquifies its template name so the dialog no longer opens with a warning the user didn't cause
...
* Inputs: Aggregations are picked as rows of (aggregation, column) combos, with the column choices filtered by what the aggregation can consume; an Aggregate node whose list names no column is reported as unready and excluded from runs
* Inputs: The platform's own Add New Column node edits its `expression` with the same inline formula editor — column autocomplete, function list, live validation — instead of a bare text box with a pencil that opened the editor in a dialog. The pencil is gone (a custom editor replaces the whole row, and the inline editor carries its own "Edit in dialog"), and a formula naming a column that isn't there now blocks the node with the reason
* Inputs: A row condition that is not a true/false expression now blocks the node instead of failing at run time. The formula editor validates asynchronously (deciding a formula's type means evaluating it), marks itself, and the node reports `Requires: Condition — The formula is int, not a true/false condition`. The check fails open when it has no verdict — a flow loaded from disk and never opened must stay runnable
* Fixed: A node kept showing the previous run's error after a later run succeeded — execution state is merged per node, so the failed run's `error`/`stack` rode along into the "Completed" one. A stale node still keeps its error, since that IS the last thing that happened to it
* Fixed: The context panel rebuilt itself while a dialog it had opened was still up, destroying the live `FuncCall` behind it — so "Edit in dialog" → OK wrote the formula into an editor nothing was listening to. The panel no longer refreshes under an open modal, and a validity verdict on mount no longer counts as a parameter edit
* Fixed: The formula editor stored a literal `[object Object]` as the condition. `FuncCallParam.onChanged` emits the changed PARAMETER, not its value, so the text has to be read back off the call — found by driving the real UI
* Fixed: Helm Input was registered as a node type but missing from the toolbox Inputs section, which is a hand-maintained list
* Fixed: Node headers humanize a camelCase function name (`filterRows` → `Filter Rows`). A `//friendlyName:` annotation does not survive package publishing, so the raw name was showing through
* Catalog (bio): Added Extract Sequence Region, To Atomic Level Column, Molecules to HELM Column, Split to Monomers Columns, Convert Sequence Notation, Tag as Macromolecule, Motif Search, Apply Antibody Numbering, Generate Sequences, and To Atomic Level Single sequence; dropped the void / same-frame originals they replace, and the `immunumAntibodyNumbering` engine
* Catalog (assay): Added Add Curve Statistic, Add Aggregated Curve Statistic and Fit Dose-Response Curves; the name-addressed `addStatisticsColumn` / `addAggrStatisticsColumn` originals stay out
* Inputs: Added a Helm Input node — a String Input tagged `semType: Macromolecule`, which is all it takes for the value editor to become Helm's registered HELM web editor, exactly as Sketcher Input does for molecules. It emits an ordinary `//input: string` line, so a HELM sequence stays a string downstream
* Tests: Fixed two Phase 1 MPO assertions that pinned an exact requirement-message count. A blank `columnMapping` also surfaces under its own plain label, so the count tracked message plumbing rather than the run gate, and `find` settled on the plain label and never saw the profile lookup resolve
...
* Catalog (cheminformatics): Added Filter by Substructure, Similarity To, Diverse Subset, Chemical Similarity Search, To InChI / To InChI Keys, MPO Score by Profile, Apply Reaction, To SDF, Chemical Space Columns, and the two Chemspace lookups; dropped the entries that could not work on a canvas (Morgan fingerprints, synthon search, the four content-taking file importers) and replaced `findSimilar` / `getSimilarities` / `getDiversities` / `searchSubstructure` / `getInchis` / `getInchiKeys` with twins that take the table their column belongs to
* Property panel: Fixed a wrapped function (`FUNC_WRAPPERS`) showing different parameters on the node than in the panel — the panel iterated the function's raw signature while the node was built from the wrapper, so captions disagreed, the panel's table list came out empty (no column picker), and it offered editors for parameters the wrapper folds away. Both now go through `effectiveFuncInputs`
* Property panel: A `choices: Pkg:funcName(args)` reference now resolves to the function's actual return value. Core strips the first character and the trailing `)` from a single-entry choices string, so the combo used to offer the literal `hem:getMpoProfileNames(` as its only option; the parser reconstructs the reference from the damage, and an empty result clears the list rather than leaving it
* Nodes: A required choice parameter is seeded with the value its combo actually displays. A non-nullable choice input has no blank option, so DG renders the first choice — but nothing was stored, so the node reported the parameter as missing while the panel plainly showed a value (it only "fixed itself" if you changed the selection and changed it back). An unresolved `choices:` reference is still never seeded
* Nodes: The "Needs input" hint now recomputes on a parameter edit. Only `onGraphChanged` refreshed it, which parameter edits don't fire, so filling a required value left the amber hint up until something else re-rendered the node
* Property panel: Added a column-mapping editor for `Chem:mpoScoreByProfile` — one **numeric** column choice per profile property, auto-filled from the upstream table's column names by similarity (exact → normalized → containment). It only fills blanks, so an explicit choice survives a table change, and switching profiles drops the previous one's entries. With no profile or no computed table it renders a single amber notice instead of a row of unfillable combos
* Nodes: Added `FUNC_NODE_VALIDATORS`, a per-function readiness check for requirements the generic ones can't express. First user: an MPO node whose mapping doesn't cover every property of the chosen profile is now unready — it raises the "Needs input" hint naming the unmapped properties, and the run gate skips it and everything downstream (a partial mapping used to score over the properties it could resolve and return a plausible but wrong number). Fails CLOSED while the profile's properties are still being read — an unverifiable mapping must not run — and the background lookup clears the state on its own, so it is a brief "reading the profile…" rather than a dead end
* Property panel: Custom editors can now read sibling input values, the upstream table's columns, node properties (so an async lookup can be cached where a synchronous validator — and a reload — can see it), and — via `watch` — react to a sibling being edited. Needed because `refreshShownNode` deliberately skips itself while focus is inside the panel, which is exactly when a combo the editor depends on gets changed (picking a different MPO profile left the old properties on screen until you re-selected the node)
...
* Suggestions: A semType-specialized input node no longer outranks the general one when a bare type is dragged (a string drag offered the sketcher)
...
* Context panel: replaced every hand-rolled editor (boxed textareas, raw number/checkbox/select controls) with native Datagrok inputs — Title/Description, Param Name, qualifiers (Nullable, Caption, Min/Max, Show Slider, Type/SemType filters, Choices), Output Type, utility/constant properties, and the multi-table column-row table chooser now all share the platform input style; the "Open editor" header button became `ui.button`
* Toolbox: the collections tab strip is now **icon-only** — the same Browse-tree glyphs (folder, brackets-curly, database, sitemap, star), evenly spread, with the names in tooltips and aria-labels — freeing the strip for five tabs
* Toolbox: added a **Spaces** tab — browse spaces and subspaces plus their content (flows, scripts, queries, stored files); double-click or drag content onto the canvas to use it; the Save dialog's space browser was parameterized (`SpacePicker` content mode) instead of duplicated, and dropping any function on the canvas now registers uncataloged ones on the fly
...
* Guides: full audit + overhaul of all 33 tutorials/how-tos (every step played through and screenshot-reviewed by persona agents) — fixed the flagship tutorial's dead-end "move the Table Output" step (outputs dock into the OUTPUTS strip and can't be dragged), reuse-script pointing at a disabled Save on an empty canvas, how-set-param/how-preview ending without showing their answer (gates satisfied by stale UI), how-run finishing with an unexplained input dialog, and decoy highlights on flows with no outputs; steps whose action is already done now skip silently without leaving holes in the step numbering, and texts got success cues + a de-jargon pass
* Guides engine: the instruction card now avoids covering highlighted elements and open dialogs (and anchors beside a dialog instead of hiding under it), large targets highlight with an outline instead of an orange wash over every row, clipped targets scroll into view at step start, the Start panel hides while a guide runs, stale tooltips clear between steps, and the toolbox search prefill is cleared when a guide ends
* Canvas: new nodes are placed in **free space** near the viewport center instead of on top of the previous node (which hid the sockets the user was about to wire)
* Context panel: the Connections pane refreshes when a wire is added/removed — no more stale "MISSING — required" rows on a node that just got connected
* Guides: added "How do I use a file from my computer?" (the Files-tab folder button) and "How do I group nodes?" how-tos
* Toolbox: the Files tab got an **open-local-file** button (the platform's folder icon) on its hint row (no extra vertical space) — it opens the native file picker filtered to the formats the platform can read (CSV family, d42, and every registered file importer) and adds an Uploaded File node per picked file, same as dropping files onto the canvas
