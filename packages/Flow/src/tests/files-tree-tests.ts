/** The Files tree stamps stable `data-testid`s on connection/folder/file rows.
 *  Needs a live stand (lists connections + files). */
...
category('Flow: files tree', () => {
  test('a dropped file titles its Open File node with the file name', async () => {
    ...
    try {
      ...
      // the path write arms the live-by-default autorun — detach resets the scheduler
      view.detach();
      await sleep(120);
      probe.flow?.destroy?.();
      view.root.remove();
  ...
  test('connections, folders and files carry name-based test-ids', async () => {
    ...
    try {
      // connections load async; the Demo connection is always present
