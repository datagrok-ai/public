...
```typescript
{
  ...
  annotations: [{id, pos, size, text, color}],                       // optional (back-compat)
...
- **`files/Workflow Demo.flow` / `files/Sequence demo.flow`** — raw flow JSON docs, loaded by the Start panel's template cards (`FLOW_TEMPLATES` in [funcflow-view.ts](src/funcflow-view.ts), read via `_package.files.readAsText`).
