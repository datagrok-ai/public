...
Dropping a file from the OS onto the canvas adds an **Uploaded File** node — `Flow:readUploadedFile(fileId, fileName)`, a regular package function opted into the catalog via `meta.includeInFlow: true`, so it round-trips through creation scripts and `.ffjson` with zero special-casing.
...
- **On save** (entity save, creation-script save, `.ffjson` export): `persistPendingUploads` uploads each file as a **connection-less `DG.FileInfo`** — bytes land in the server's GUID-addressed blob store (`files/data/{id}`, the same mechanism Compute uses for replayable file inputs) — and rewrites the node's `fileId` to the real id. Registering the FileInfo *entity* needs two datlas fixes in `files_service.dart#saveFileInfo`: the connection null-guard, and `addToUserProject: true` for connection-less files (without the user-project relation the author can't find/share the entity they created). On older servers this degrades to blob-only (replay works, permission sync is skipped with a console warning).
