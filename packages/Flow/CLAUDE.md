...
| `compiler-tests.ts` | Flow: topological sort / script emitter / validator / func wrappers | order, cycles, emitted headers + body, instrumented mode, validation rules, Select Table fail-fast guard, Output ⇄ SetVar same-contract emission; FUNC_WRAPPERS: node exposes wrapper inputs, compile folds them into the real call args (plain + instrumented), the reshaped call runs on the platform, creation-emit warns and skips |
...
- **Parsing**: CSV/TSV/TXT natively, `.d42` via `DataFrame.fromByteArray`, everything else through registered `file-handler` functions (xlsx, sdf, …); multi-table files yield their first table.
