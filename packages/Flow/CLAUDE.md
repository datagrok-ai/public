...
├── serialization/
│   ├── flow-schema.ts            # .flow v2 type definitions (Rete-native, no LiteGraph payload)
...
- `@grok.decorators.fileViewer({fileViewer: 'flow'})` → `viewFuncFlow(file)` opens `.flow` files
...
| `panel-tests.ts` | Flow: property panel | `stringChoiceOptions` (choices/nullable/current-preservation) + `propertyChoices` reading live func-input choices; input overrides: hidden inputs stay data-carrying but render nowhere, a registered custom editor replaces the default input (storage/`isValid` routing), OpenFile's `fullPath` file picker round-trips a path, panel-only inputs (`PANEL_ONLY_FUNC_INPUTS`: OpenFile `fullPath`/`sheetName`) keep their slot + required check but render no node socket/pass-through row while the panel still edits them, editor-shortcut inputs carry the inline open-editor pencil (gated on `onEditFuncParams`, off-list inputs stay bare) |
...
| `files-tree-tests.ts` | Flow: files tree | name-based test-ids on connection/folder/file rows; lazy expand loads + stamps a connection's files (Demo → demog.csv); a dropped file's Open File node is titled with the file name |
| `uploaded-file-tests.ts` | Flow: uploaded files | pending registry (add/read/remove, 100 MB cap asserted without allocating), csv parse + table naming, sdf parse through the registered `file-handler` func (discovery by tag-or-role — a pending sdf runs through `readUploadedFile` too), the server-file contract `OpenFile(path)` opens an sdf, `readUploadedFile` from memory (no server round-trip), actionable lost-pending error, catalog registration via `meta.includeInFlow: true`, server blob round-trip (persist → `readAsBytes` → node function → entity cleanup), creation-script import/emit round-trip |
| `property-input-tests.ts` | Flow: property input / to semantic value | the node starts `dynamic` and leads `findNodeTypesProducingOutput` only for a function-input drag (`targetIsFuncInput`; the exact-matching Input node still leads plain drags); connecting to Gasteiger's `mol` adopts string + `semType: Molecule` (sketcher hosting, seeded declared default, auto param name/title); re-routing to `contours` re-picks int (string qualifiers dropped, numeric ones appear, the stale SMILES replaced by the int default) and user renames survive the next re-adoption; a Macromolecule param routes to the HELM box; a utility target adopts nothing and the manual `applyPropertyInputShape` path works; the panel renders Type + exactly the adopted family's qualifier rows (and no Type row on dedicated inputs); the emitted header carries the mimicked lines/qualifiers and `DG.Script.create` parses them (semType survives); save/load and duplicate restore the adopted shape; adopting Biologics' assaysByOrganism `organism` keeps its `query("…")` choices reference + connection — in the plain AND the JSON-escaped (`uery(\"…\"`) form the platform hands out — and the value editor resolves the live organism list with zero reported edits (header omits `choices:`); a mangled func-call reference (`hem:getMpoProfileNames(`) on a plain String Input resolves in the editor too; To Semantic Value: shape (dynamic in, `semantic_value` out, `Molecule` default, `semType` required), Value Output auto-types on connect, emission carries `DG.SemanticValue.fromValueType` with the configured semType, and the emitted script runs end to end |
| `input-value-tests.ts` | Flow: input values | `resolveInputValue` per type (scalars/lists/map/dataframe by name + `transientValue` priority), `inputBlockReason` wording, header emission guard (table names stay out, scalar defaults stay, a brace-carrying default stays out — the param line survives bare), `runAutorun` unconfigured-skips / configured-runs-silently (no dialog), explicit Run with configured scalar needs no dialog, `autorunBlockers`, the node-body DG editor renders + panel→node sync, editor edits report `params-changed` exactly once (guarded) |
...
| `inline-preview-tests.ts` | Flow: inline preview | `supportsInlinePreview` (viewer/widget/graphics outputs only), default-off + size fallback, save/load round-trip of choice+size, the title-bar ⊞ toggle (capable nodes only, flips property, container appears at 300×300, off deletes the property), the context-menu item, live-root mounting through the bridge (claim marker, collapse/toggle-off release, placeholder without content), drag-resize persistence via ResizeObserver, layout estimates including the preview, the bottom panel's hosted-note handoff (`buildPreview` never steals a claimed root), `OutputPreviewPanel.refresh`, controller ownership (`inlinePreviewRoot`/`syncInlinePreviewOwnership`, stale keeps the last root), graphics outputs (SVG/PNG element built + cached per summary, no ownership marker, panel copy coexists, mounted in the node), the portal (content mounts once — no transformed ancestor, hover changes nothing, portal tracks the container across zoom/pan, grip present; collapse/toggle-off/no-content release), the overlap-interleave regression (portal DOM order follows node paint order; the covered portal carries an evenodd clip hole, the top one none, and elementFromPoint in the overlap hits the covering card — click-through), the fixed-position repro+fix (a fixed probe inside the node lands far from its viewport coords — the popup bug — while in the portal it lands exactly, at k=1 and 2), right-click routing (no node menu over preview content, node menu still on the card), a live Select Table → Scatter Plot run mounting the real viewer with client geometry unscaled at k=0.5/2, a live shell-attached run clicking the REAL axis selector (`d4-column-selector-column`) asserting its popup opens next to the viewer and is visible at k=1 and 1.5 and that plot right-click opens the viewer's menu, a live Gasteiger Partial Charges run rendering its graphics in the node (skips with a warning when JKG is down, probed via Chem:TestPythonRunning), autorun persistence (`metadata.settings.autorun` serializes; a flow saved live reopens with the scheduler on and the bolt lit, without it off; the toggle records/removes the setting), and the run-pending feedback (`inlinePreviewPending` truth table; an empty preview swaps its hint for a loader while pending and back; a kept preview gets/loses the recalculating overlay) |
| `start-template-tests.ts` | Flow: start templates | the home-screen cards are exactly Workflow demo + Interactive viewers + Blank canvas (the Sequence demo card is asserted absent); the deployed Workflow Demo script carries `autorun`; every carded template has a card (`ff-start-template-<slug>`); the deployed Interactive Viewers script entity parses (canonical name + sketcher `//input` line, `metadata.settings.autorun` carried) and fully deserializes (19 nodes / 15 wires — an unregistered func would be silently skipped, so the counts are the regression); the card click loads the whole flow, hides the start panel, and re-enables autorun — that half probes AppData file serving first (reads the pre-existing Workflow Demo.flow) and skips with a console warning when the stand cannot serve files, since the DB-backed test still covers the graph |
| `render-molecule-tests.ts` | Flow: render molecule | `Flow:renderMolecule` is registered + catalog-opted-in and runs through the platform; the widget's canvas keeps a full-resolution square backing store (>= 800, dpr-scaled by Chem) while CSS scales the display size to the container (styles are applied AFTER `grok.chem.canvasMol`, which pins `style.width/height` and rewrites the backing size — set-before is silently undone), inline styles not a funcflow.css class (the widget renders where Flow's stylesheet may not be loaded); an empty molecule yields a blank widget without throwing; the node's widget output supports the in-node preview and the `molecule` input is `semType: Molecule` (sketcher-compatible) |
...
| `clipboard-tests.ts` | Flow: clipboard | `duplicateNodes` (nodes + internal connections copied, copies selected, originals not, output `paramName` deduped, 30px offset), single-node duplicate keeps label/description, `copySelection`/`pasteClipboard` round-trip (repeat pastes fan out 30px·n, latest paste is the selection), the Ctrl+C/Ctrl+V keyboard path, empty copy leaves the clipboard untouched, right-click keeps the multi-selection (real pointer dispatch), Escape clears the selection |
| `annotation-tests.ts` | Flow: annotations | annotation drag carries the nodes inside it (center-inside, captured at drag start — real pointer dispatch), a node dragged out detaches from the next drag, fully-contained annotations move along while partially-overlapping ones don't, waypoints of connections between carried nodes travel too, strip-pinned output rows are never carried, Delete removes the last-clicked annotation (clicking elsewhere disarms; with nodes selected the nodes win); styling: the 13-color palette is unique and border-matched (a post-expansion color resolves its own border, not the blue fallback), the title font size defaults to 13 / applies as px / falls back on malformed values / stays out of `toDoc` at the default, color+size round-trip through serialize/deserialize, the `setAnnotationColor`/`setAnnotationFontSize`/`setAnnotationPinned` editor setters apply and fire `onGraphChanged` exactly once per real change, the right-click menu offers Color and Title size radio groups plus the Pinned checkbox, and a pinned annotation stays put (direct drag inert, resize handle inert, a carrier leaves it behind) and round-trips through serialize/deserialize (`pinned` omitted when false) |
| `group-tests.ts` | Flow: groups | `createGroup` wraps members in a frame (output rows and already-grouped nodes filtered; frame refits to the member bbox), frame drag carries the members (frame follows via the nodetranslated→refit path), minimize hides member views + internal wires and renders per-boundary-socket card dots with the boundary wire's path endpoint re-anchored to the dot (SVG-path assertion), card drag moves the hidden members + wire anchors, maximize restores views/wires/frame wherever the card traveled, ungroup keeps nodes+wires and un-hides, member deletion shrinks → dissolves the group, Ctrl+G / Ctrl+Shift+G, Ctrl+A skips hidden members, the card's aggregate `data-status` (running > errored > stale > all-completed), serialize/deserialize round-trip (memberIds remapped via idMap, minimized state re-applied) |
| `output-views-tests.ts` | Flow: output views | status-bar tab strip (Canvas + one per table output; scalar outputs stay out; SetVar-with-table gets a tab, scalar SetVar doesn't — Q2); add/remove/rename tracking + fallback-to-Canvas; empty state (run message + Run button, canvas restore); lazy TableView creation on activation, refresh rebinds the SAME tv (root identity), stale dot keeps the table; ribbon/toolbox swap + restore (containment assertions — the Dart toolbox getter wraps); layouts persist by paramName through `entityBodyText` → `parseFlowBody` → `loadFromDoc` across node-id remapping (scatter plot re-created); a completed Open File run restamps the node title from its path (typing alone doesn't); the tab strip follows the outputs-strip order (`outputOrder` ranks, Canvas chip stays first) |
| `output-strip-tests.ts` | Flow: output strip | no orphan outputs (last-wire removal and feeding-node removal both remove the row; unrelated removals don't); strip column mounts OUTSIDE the canvas viewport (vertical label, ≤50px); an output node renders as a 40×24 chip INSIDE the strip (one-letter type, tooltip names output + source, no canvas card); chips are screen-space (pan/zoom change neither size nor position); analytic wire endpoints stay at the canvas edge across pan/zoom (`listenChipSocket`); chip click selects the node; a ≥5px chip drag reorders the outputs (rank persisted as `outputOrder`, `//output:` emission follows, sub-threshold press stays a click); `bindOutputToStrip` (dataframe → wired Table Output named after the slot + unique suffixing, scalar → Value Output auto-typed, chip letter follows); `.ff-strip-droptarget` during an output drag; minimap/zoomToFit exclusion; destroy removes strip + wrapper |
...
**Configured values** ([utils/input-values.ts](src/utils/input-values.ts)): every input node carries a
value editor **on the node body** (the `value` Rete control → `InputValueControl` →
`DgControlComponent` mounts a real DG input, built once and re-attached across React re-renders so
focus survives) and, via the same `buildInputValueEditor`, as the **Value** row of the panel's Input
Configuration pane — the two sync through `notifyNodeParamsChanged` (bridge method
`notifyParamsChanged`; panel edits call `control.sync()`, node edits trigger
`refreshShownNode`). The value lives in `properties['defaultValue']` (serialized; scalars keep the
classic `//input: int n = 5` header emission — `NON_HEADER_DEFAULT_TYPES` keeps
dataframe/file/map/blob values OUT of headers). A Table Input uses `ui.input.table` (open tables +
live tracking + the built-in open-local-file folder icon); a table opened that way exists only inside
the input, so the picked DataFrame is also kept in the runtime-only `FlowNode.transientValue` —
after a reload the serialized name re-resolves from the workspace (`resolveInputValue`). Scalars use
`ui.input.forProperty` over a `DG.Property` built from the node's qualifiers (choices/min/max/
nullable); column/list/map/dynamic edit as strings; blob has no inline value.

**Semantic specializations of String Input.** Sketcher Input and Helm Input are String Inputs whose `semType` qualifier is pre-set to `Molecule` and `Macromolecule` respectively. That qualifier is the whole mechanism: `inputValueProperty` passes it into the `DG.Property`, and `ui.input.forProperty` matches it against the `valueEditor` funcs packages register on (propertyType, semType) — Chem's sketcher, Helm's HELM web editor. Do NOT hand-build such an editor; tag the property and let the registry supply it. They emit an ordinary `//input: string … {semType: …}` line, so everything downstream is unchanged. `isSpecializedInput` (node-factory) reads the `semType` property generically, so a bare `string` drag still leads with the plain String Input and any new specialization is ranked correctly for free.
  - **The one sanctioned exception is the NODE body of Molecule/Macromolecule string inputs** (`hostsInlineSketcher` / `hostsHelmEditor` in scheme.ts — read the `semType` property, so a user-tagged String Input qualifies too; `buildInputValueEditor(…, {host: 'node'})` is what `InputValueControl` passes — the panel's Value row keeps the standard editors). Editors mount **inline in the card, never in a portal** — that was tried and scrapped: sketcher chrome is not built to be zoomed OR to live in a screen-space overlay.
    - **Sketcher Input** (`buildInlineSketcherEditor`, `ff-inline-sketcher`): a **compact molecule preview** (`ff-sketcher-compact` — `grok.chem.canvasMol` render, or a "Sketch" invitation when empty) that **expands in place into an inplace `DG.chem.Sketcher` on click — no dialog** (`ff-sketcher-full`, a fixed **500×500** box — `INLINE_SKETCHER_SIZE`, mirrored in funcflow.css; lazily built; a Done button folds back). **The sketcher exists only at native zoom** — its chrome is not built to be scaled (a constant-screen-size variant and a screen-space-portal variant were both tried and scrapped): expanding calls `editorBridge.focusNodeForEditing(id, w, h)` (`FlowEditor` — snaps zoom to 1 and pans so the box is fully visible, 16px margin, top-left wins when the viewport is smaller), and a light 200ms poll watching `editorBridge.getZoom()` folds it back the moment the user zooms away (or the control unmounts). It stores **SMILES** (a molfile is multiline and would corrupt the emitted header default) and echoes programmatic sets back through `onChanged` asynchronously and canonicalized, so edits report only after a real user interaction (a `touched` flag armed by pointer/key/paste events, reset on every `sync()`) — without it, merely loading a flow marked it dirty. Sync uses `setSmiles`/`setMolFile`, never `setMolecule` (its SMARTS sniff is a sync Chem call that throws until Chem loads). Compact is row-sized, so the sketcher input never affects layout estimates.
    - **Async-bind retry** (scalar branch of `buildInputValueEditor`): a semType-routed editor materializes asynchronously — a `stringValue` set on the still-unbound proxy THROWS, so the load-time sync of a stored value was silently lost and a loaded flow's helm rendered (and edited from) empty. `syncValue` now retries the application on a 200ms interval until the editor binds (skipped for empty values; each attempt under the `syncing` guard so it never reports an edit).
    - **Helm Input** (`wrapInHelmBox`, `ff-helm-box`): the registry-built HELM editor sits in a **resizable in-card box** (native CSS `resize: both`; default `INLINE_HELM_*` 300×250; size persists in `EDITOR_BOX_SIZE_PROP` via a ResizeObserver — cosmetic, no params-changed — and serializes with the flow; `editorBoxSize` feeds the `estimateNodeWidth/Height` branches). The HELM input pins its editor host to a fixed 250×250 with inline styles and never tracks a container on its own — `wrapInHelmBox` stretches `getInput()` to `100%` with inline `!important` styles (the documented consumer pattern; the input's own `onSizeChanged` then re-fits the canvas), retried on a 200ms interval until the asynchronously-materialized editor proxy binds.
    - `.ff-node:has(.ff-sketcher-expanded)` / `:has(.ff-helm-box)` lift the 280px card cap.
  - **Rich value editors report objects, not strings**: Helm's input `value` is a `SeqValueBase` — the scalar branch of `buildInputValueEditor` stores `input.stringValue` whenever a string input's changed event carries a non-string (storing `v` raw serialized as `[object Object]`). The Helm editor dialog's OK firing `onChanged` at all is a Helm-package fix (`HelmInput.showEditorDialog` routes through `setStringValue` + `fireChanged`) — before it, a sketched HELM read back empty.

**Runs and autorun**: `ExecutionController.configuredInputValues` resolves each script param from
its input node and feeds `func.prepare(values)` — the parameter dialog appears only for params still
missing (prefilled otherwise), and `runAutorun` skips only when an input in the run set has
`inputBlockReason(node) != null` (was: any input node). Instrumented emission reports a
`node-complete` for input steps (`emitInputStepComplete`) with the param's value summary, so a
configured input shows "Done · 200 × 11", its wire gets the count label, clicking it previews the
parameter, and its stash feeds slice re-runs. When autorun is ON but blocked,
`ExecutionController.autorunBlockers()` (validation errors + unconfigured pending inputs) drives the
ribbon bolt's **amber pad + corner dot** (`.ff-autorun-blocked`, `updateAutorunIndicator`) and the
bolt tooltip lists the exact blockers. Empty/unresolvable editors get an amber underline
(`.ff-value-missing`, toggled in the shared builder) so the two cues read as one. AI tools:
`getFlowNodeDetails` exposes `value`/`valueBlocks` for input nodes; `setFlowNodeInputs` accepts
`{value: …}` on them.
...
| Sketcher Input | `string` | as String Input, pre-tagged `semType: Molecule`; the node body shows a compact preview that expands into an inplace sketcher (see above) |
| Helm Input | `string` | as String Input, pre-tagged `semType: Macromolecule`; the node body hosts the HELM editor in a resizable in-card box (see above) |
...
| Property Input | starts `dynamic`, then whatever it mimics | derived per adopted type (see below) |

**Property Input** ([input-nodes.ts](src/rete/nodes/input-nodes.ts), `PROPERTY_INPUT_TYPE`) is the
universal input: it **mimics the function input it connects to**. On every `connectioncreated` whose
source is a Property Input and whose target is a DG-function node's parameter
(`FlowEditor.maybeAdoptPropertyInput`, wrapper-aware via `effectiveFuncInputs`),
`adoptInputProperty` copies the target `DG.Property` onto the node: `properties['propertyType']` +
the type family's qualifier keys (semType/choices for strings, min/max/showSlider for numerics,
typeFilter/semTypeFilter for columns — `QUALIFIERS_BY_TYPE`, kept in exact lockstep with the
dedicated input nodes so the panel rows and header qualifiers just work), the declared default
(seeded only when the node holds no value, and **reset when the adopted type changes** — a SMILES
riding into an int would emit a broken header), and the param name/title (auto-followed via the
`mimicParam` marker until the user renames them — renames survive re-adoption). Everything downstream
is the ordinary input-node machinery reading those properties: `inputValueProperty` →
`ui.input.forProperty` (so a Molecule string gets the sketcher, a Macromolecule the HELM box —
`hostsInlineSketcher`/`hostsHelmEditor` fire on the adopted semType), header emission, run gating.
`applyPropertyInputShape` re-derives the live shape (dgOutputType + output socket + qualifier keys)
from `propertyType` — idempotent, and called from every path that assigns properties
post-construction: adoption, the panel's Type combo, `deserializeFlow`, `materializeClip`
(duplicate/paste). After a shape change `FlowEditor.rebuildValueEditor` throws the cached inline
editor away so the next render builds one for the new type. **Non-function targets** (utilities,
viewer options) adopt nothing — the panel's Input Configuration pane shows a **Type combo**
(Property Input only) for manual setup; picking a type applies the same shape function. In the
**reverse drag-out suggestion menu**, Property Input ranks **first** when the dragged input belongs
to a function node (`SuggestionContext.targetIsFuncInput`, rank −1 in
`findNodeTypesProducingOutput`) — pick it and the wire's `connectioncreated` adopts the parameter;
for non-function drags it ranks with the built-ins and the exact-matching Input node still leads.

**Choices references** (this applies to EVERY input node, not just Property Input): a `choices`
qualifier that is a **reference** — a mangled `Pkg:func()` call or a `query("…")` string
(`isChoicesRefString` in [choice-refs.ts](src/utils/choice-refs.ts); core strips the first char and
trailing `)`, e.g. Biologics' Assays by Organism `organism` param; the platform can additionally
hand it **JSON-escaped** (`uery(\"SELECT …\"`), so detection tolerates `\"`, adoption unescapes
BEFORE detecting, and `resolveChoicesRef` unescapes at entry — an escape-blind detector read the
live query ref as a literal list and never captured the connection) — is stored (unescaped) in
`properties['choices']` and resolved **live by the value editor**: `inputValueProperty` passes the
reference as the combo's single item, and the scalar branch of `buildInputValueEditor` swaps in the
resolved list via `loadChoicesRefItems` (in
[choice-input-processor.ts](src/panel/choice-input-processor.ts), whose core `resolveChoicesRef` is
shared with the func-node panel's `processChoiceInput`) — under the `syncing` guard, so the swap
never reads as a user edit. A **query reference needs a connection to run on**: Property Input
adoption captures the owning `DG.DataQuery`'s connection id in `properties['choicesConnection']`
(serialized; deleted whenever `choices` is dropped or replaced by a non-reference), and the resolver
looks it up via `grok.dapi.connections.find`. References **never ride the emitted header** — their
quotes/parens/commas would corrupt the `{…}` options block, so `buildInputLine` skips the
`choices:` qualifier for them.
...
- **Drag a chip up/down to reorder the outputs** (`beginChipReorder`): a ≥5px vertical pull
  lifts the chip (`.ff-output-row-dragging`, body cursor `grabbing`) and makes it **ride under
  the pointer** — a `translateY` relative to its current slot (`staticTop`, re-measured
  untransformed after every re-insertion) — while neighbors re-slot live via `insertBefore`;
  release writes each chip's index into the node's
  **`outputOrder` property** (`commitChipOrder`) — properties serialize, so the order survives
  save/load. `getOutputNodes()` (public) sorts by `outputOrderRank` (unranked → end, stable) and
  is the single source of order for chip stacking, `chipSocketPos` wire endpoints, the view's
  `tableOutputs()` tab order (`syncTabs` re-appends chips when the order differs), and the
  emitted `//output:` header lines (`buildHeaderLines` sorts output steps by the same rank —
  unranked flows keep topological order). A sub-threshold press stays a click; a real drag
  suppresses the release's `click` via a self-clearing `suppressChipClick`.
...
- **No output node without a connection, ever**: rows are auto-created by publishing a value, so
  losing the last wire auto-removes the row (`scheduleOrphanOutputCheck` on `connectionremoved` —
  covers direct wire deletion AND the feeding node's removal). The check is **deferred and held
  during a connection drag** (`connectionDragActive`): picking an existing wire's endpoint removes
  it before the re-route drop re-adds it, and an eager check would kill the chip mid-gesture; a
  drag dropped nowhere flushes the check and the orphan goes. A still-connected output is never
  touched by unrelated wire removals.
...
| To Semantic Value | `let v = DG.SemanticValue.fromValueType(value, '<semType>')` — semType is a string property, default `Molecule`; output socket type `semantic_value`; no creation-script form (warn & skip) |
...
- **In-node preview** (viewer nodes AND any func whose output socket is `viewer`/`widget`/`graphics`
  ...
  `FlowEditor.setZoom(k)`/`getZoom()` are public for tests. **Run feedback**: while a run in
  progress still has the node ahead of it (`ExecutionController.inlinePreviewPending` — part of the
  run set, neither completed nor errored; reached via `isInlinePreviewPending` on the callbacks +
  bridge), an EMPTY preview shows `ui.loader()` instead of the "Run the flow" hint
  (`data-empty="loading"`, `data-loading` on the placeholder — a fresh full run clears the captured
  value, which used to read as a blank box), and a KEPT (stale) preview gets a
  `ui.setUpdateIndicator` "Recalculating..." overlay on the portal; both resolve on the node's
  completion re-render. One live DOM root cannot be in two places: whoever hosts it stamps
...
`FuncNode` is built dynamically per `DG.Func`. Header uses `func.friendlyName` (split by `|`, last segment). Color comes from `ROLE_COLORS` map. Generates `await grok.functions.call('Pkg:funcName', {...})`. **Function wrappers** (`FUNC_WRAPPERS` in [utils/func-input-overrides.ts](src/utils/func-input-overrides.ts), `{[nqName]: {inputs, mapInputs}}`) reshape an awkward signature into Flow-friendly node inputs: the node (sockets, pass-throughs, seeds, `requiredInputs`, panel, suggestion matching/auto-wiring) is built from the wrapper's `inputs` — turned into real `DG.Property`s by `wrapperProperties` (`nullable: false` unless `optional`, else the base Property defaults nullable and would kill the "Needs input" gate) — while the graph compiler runs the resolved input expressions through `mapInputs` into `CompiledStep.callInputs`, which both script emitters pass to `grok.functions.call` (`step.inputs` stays keyed by the exposed slots for pass-through/stash resolution). Registered: `core:AppendTables` (`tables: dataframe_list` is unwirable) exposes `table1`/`table2` → `tables: [table1, table2]`. Wrapped nodes have **no creation-script equivalent** (the creation emitter warns and skips them). **Hidden inputs** (`HIDDEN_FUNC_INPUTS` in [utils/func-input-overrides.ts](src/utils/func-input-overrides.ts), `{[nqName]: {[inputName]: true}}` — e.g. AddNewColumn's `subscribeOnChanges`/`errorBehavior`) are read onto `node.hiddenInputs` and hidden **visually only**: the node component skips their (unconnected) socket rows + pass-throughs and the property panel skips their editor rows, but the slot, seeded value, compilation, and creation-script import/emit stay untouched — data-sync scripts legitimately carry `subscribeOnChanges = true` and must round-trip. Only hide inputs with a declared default. Primitive inputs seed `inputValues` with the **declared default** — `getParamDefault(prop)` = `defaultValue ?? initialValue`, read defensively, with one pair of wrapping quotes stripped (`unquoteDefault` — annotation defaults arrive double-encoded like `"'inner'"`), string-encoded bools/numbers coerced to their declared type (`'false'` must not compile to `true`) — else the type's zero value. At compile time an **untouched optional scalar** (blank string / null on an input the func declares optional) is **omitted from the call** so the function's own default applies — passing `''` where the default is null changes behavior (OpenFile forwards ANY non-null `sheetName` to the importer as a second argument, crashing one-arg importers: "importSdf 1 input parameters, 2 passed"); additionally OpenFile's `sheetName` is only ever emitted for `.xlsx` paths. In the panel, every DG input is initialized from the stored value via the guarded `stringValue` setter (`PropertyPanel.initInputValue`) because `ui.input.forProperty` / the `value` init option don't reliably load the editor.

#### Writing a Flow-friendly function — metadata traps

Learned the hard way while building the phase catalogs; every one of these fails at publish time or silently produces the wrong parameter.

- **A column output must not declare a `semType`** unless it also declares `{action: join(...)}`. The platform's output handler builds its frame only for a column WITH an action, then does `for (var column in df?.columns)` to stamp the semType — iterating null and throwing on every call (`xamgle/lib/src/features/functions/functions.dart`). Tag the column in code instead; the declared output semType only exists to stamp it.
- **`options.type` on a column parameter is not a filter unless `type: 'column'` is also declared.** `@param({options: {type: 'numerical'}})` generates `numerical concentrationCol` — the parameter's own type — turning a column slot into a plain value. Always pair it with `@param({type: 'column', ...})`.
- **No `;` or `{` in a `description`.** `;` separates options, so everything after one is parsed as a bogus option of its own.
- **Never spell an `input:` / `name:` annotation inside a code comment in `package.ts`.** The server scans that file for those lines and tries to parse the comment as a real parameter; a `numerical` in prose is enough to reject the whole package with `Wrong parameter type`.
- **`choices` must be a literal array.** The generator only reads literals: an enum reference emits `["dynamic","dynamic"]`, anything else emits nothing, and an empty `choices:` makes the server reject the package with `RangeError: Value not in range: 1`.
...
- **Ports**: `createNode` appends one **exec-in** (`__exec_in`, accepts many predecessors via `multipleConnections`) and one **exec-out** (`__exec_out`) of socket type `order` to *every* node — defined in [scheme.ts](src/rete/scheme.ts) (`EXEC_IN_KEY` / `EXEC_OUT_KEY` / `ORDER_SOCKET_TYPE` / `isExecKey`). They render as gray squares at the node's **top corners** (KNIME flow-variable style); the data-port rows exclude them. **Visibility**: the squares are always in the DOM (edges must keep their endpoints — hidden via `opacity`, never `display:none`) but only *visible* when the node's exec row is **wired** (`data-wired`, recomputed on every node re-render via `editorBridge.isSocketConnected`), the **node is hovered**, or an **order drag is in progress** — `beginConnectHints` adds `.ff-connecting-order` to the container when the picked socket is an exec port (so the source square can't vanish mid-drag and every target square is visible), removed in `endConnectHints`. Invisible squares also drop `pointer-events`, so a click near the title corners can't silently start an order drag. Tooltips are plain-language ("Run order: drag a wire from here to another node's order square…"), not just "order".
...
The catalog is **allowlist-based** (`shouldIncludeFunc`): a function enters only when **any** holds — **`meta.includeInFlow: true`** declared on the function (the author opt-in; meta surfaces as `func.options.includeInFlow`, honoured as boolean or string); it is a **saved flow** (`isWorkflowFunc`) or a **query** (`DG.DataQuery`, except from a `DEV_TEST_PACKAGES` package — Dbtests alone ships ~290 test queries) — user artifacts included by kind, with their own toolbox panes; or its `nqName` ∈ [`INCLUDED_FUNC_NQNAMES`](src/rete/included-funcs.ts), the curated, hand-editable include list (~264 package/core functions). **`meta.includeInFlow: false` opts out unconditionally** (checked first, wins over the list — Flow's own `openCreationScriptFlowDialog` uses it). A zero-signature func (no inputs and no outputs) is skipped regardless. The list was frozen on 2026-07-20 from the catalog the previous deny pipeline (structural + role/tag rules + a denylist, ~2,357 → ~285) produced on a live server — see [docs/func-catalog-snapshot.md](docs/func-catalog-snapshot.md) for how that shape was derived. **For a function you own, prefer declaring `meta.includeInFlow: true` in its own metadata over editing the list.** New user-authored scripts need the meta tag to appear (saved flows and queries don't).
...
Dropping a file from the OS onto the canvas adds an **Uploaded File** node — `Flow:readUploadedFile(fileId, fileName)`, a regular package function opted into the catalog via `meta.includeInFlow: true`, so it round-trips through creation scripts and `.flow` with zero special-casing.

- **Unsaved flow**: the bytes live in an in-memory pending registry (`pending:<uuid>` id, 100 MB cap — `MAX_UPLOAD_SIZE`); `readUploadedFile` checks it first, so runs need no server.
- **On save** (entity save, creation-script save, `.flow` export): `persistPendingUploads` uploads each file as a **connection-less `DG.FileInfo`** — bytes land in the server's GUID-addressed blob store (`files/data/{id}`, the same mechanism Compute uses for replayable file inputs) — and rewrites the node's `fileId` to the real id. Registering the FileInfo *entity* needs two datlas fixes in `files_service.dart#saveFileInfo`: the connection null-guard, and `addToUserProject: true` for connection-less files (without the user-project relation the author can't find/share the entity they created). On older servers this degrades to blob-only (replay works, permission sync is skipped with a console warning).
- **Sharing**: `syncFlowFilePermissions(script)` parses the flow body (`uploadedFileIdsFromFlowBody`) and grants read on each file entity to the script's view/edit grantees. It runs **globally** from the `flowShareSync` **autostart** function in `package.ts` (subscribed to `grok.events.onEntityShared`, js-api exposure of `d4-entity-shared`) — so sharing a flow from Browse with no Flow view open still syncs — and after every save from the editor. A sharer without rights on someone else's file gets a per-file warning, the rest still grant.
- **Drop routing**: while the Flow view is active, `grok.events.onFileDragEnter` (js-api exposure of `d4-file-drag-enter`) is `preventDefault`-ed, which suppresses the platform's "drop to open" overlay so the canvas `drop` handler receives `dataTransfer.files`.
- **Parsing**: CSV/TSV/TXT natively, `.d42` via `DataFrame.fromByteArray`, everything else through registered `file-handler` functions (xlsx, sdf, …); multi-table files yield their first table. Importers are discovered with the platform's own matching — `DG.Func.find({meta: {role: 'file-handler'}})`, which resolves the kebab-case name against tag OR camelCased role (`fileHandler` is what packages actually register); `find({tags: ['file-handler']})` finds none of them. Server files never come through here — the Open File node's `OpenFile(fullPath)` runs the full platform handler stack for any extension (tested with sdf).
...
Compiler reads `node.properties` and emits `{type:..; semType:..; nullable: true; caption: ..; choices: [..]; min:..; max:..; showSlider: true}`. Description appended as `[description]`. Default values inlined as `//input: type name = <value>` — but ONLY when header-safe (`headerSafeDefault`): the platform's ScriptParser (`_validateParamLine`) takes the span from the line's FIRST `{` to its LAST `}` as the options block, with no string-awareness or escaping, so a brace-carrying default (a HELM string) corrupts the whole line — it eats the `{semType: …}` qualifiers and fails validation; every run path goes through `DG.Script.create`, so this broke Run/autorun/save alike. Unsafe defaults (braces, newlines) are omitted from the header; runs still feed them via `configuredInputValues`, only the saved script's dialog prefill is lost.
...
- **ExecutionController** ([execution/execution-controller.ts](src/execution/execution-controller.ts)) — orchestrates runs. Subscribes to `funcflow.exec.<runId>`, updates `ExecutionState`, drives `ExecutionVisualizer`, pushes outputs into `OutputPreviewPanel`. Exposes callbacks `onBreakpointHit`, `onRunEnd`, `onNodeStateChanged`. On `node-complete`, besides the pinned-node re-show, an **unpinned** preview is fed the completed node when it is the **sole selected** node — a run (autorun included) of the node the user is looking at opens/updates its preview exactly as clicking it would, no unselect/select round-trip. The view's `onNodeStateChanged` handler mirrors this for the context panel: `PropertyPanel.updateExecState(nodeId, state)` swaps in the fresh state (focus-guarded rebuild), so the Execution section tracks the run live while the node stays selected.
...
Ribbon **bolt icon** (`data-testid` ribbon/autorun; `.ff-autorun-toggle` = off: 0.8 opacity, default weight (outline glyph); `+ .ff-autorun-on` = on: blue + **font-weight 600**, which renders the FA bolt filled — the weight must NOT apply to the off state; dynamic tooltip) toggles `AutorunScheduler` (view-owned, off by default). **The mode persists with the flow**: `toggleAutorun` records it in `flowSettings.autorun` (→ `metadata.settings.autorun` in the `.flow`; deleted when off, so saves stay tidy) and marks the flow dirty; `doLoadFromDoc` re-applies it via `applyAutorunSetting`, which drives the same toggle path — so a flow saved live reopens live (and kicks its pending nodes exactly like a ribbon click). **Turning it on immediately schedules `ExecutionController.pendingNodes()`** (every node without a completed result — never-run/stale/errored — plus downstream) via `AutorunScheduler.kick(dirty)`, so a fresh flow runs at once instead of waiting for the first edit. The scheduler accumulates the invalidated ids from `applyGraphEdit` and, `AUTORUN_DEBOUNCE_MS` (1 s) after the last edit, calls `ExecutionController.runAutorun(dirty, settings)`:

- **Live-by-default nodes**: `AUTORUN_FUNC_NAMES` (`['OpenFile', 'AddNewColumn']`, case-insensitive simple names) and `AUTORUN_NODE_TYPES` (`['Viewers/*']`, `dgTypeName` patterns, trailing `*` = prefix) at the top of autorun.ts — **edit these lists to change what's live** — plus the author opt-in **`meta.autorun: true`** declared on the function itself (surfaces as `func.options`, value may arrive as a string; e.g. Flow's own `readUploadedFile`, so a dropped local file parses at once). `isAutorunByDefault(node)` consolidates all three (+ a `properties.viewerType` fallback). With the toggle OFF, `onEdit` admits an edit only when its affected cone touches a live node, and the dirty set collects **only the live node ids** (never the cone); the fired run goes through `ExecutionController.runLiveNodes`, which executes **exactly those nodes**, each gated on satisfied inputs (`readyForLiveRun`: no missing requirements — which, since `requiredInputs` covers every non-optional no-default input, includes an Open File with a blank `fullPath`, so a bare toolbox/suggestion drop never live-runs `OpenFile('')` — + every connected input fed by a captured live value) — nothing else on the canvas ever runs uninvited, and there is NO full-run fallback. The scheduler's run callback gets a `liveOnly` flag (true = toggle off) so the view routes to `runLiveNodes` vs `runAutorun`. Entry points beyond edits: **`node-added` of a live node schedules** (toggle off only — a file dragged from the files tree creates a ready Open File; readiness is re-checked at fire time so a bare toolbox drop quietly no-ops), and **`kickLive(ids)`** is called by `loadFromDoc` so a freshly loaded flow's Open Files run at once (viewers with uncaptured upstream drop out at the readiness gate). `addOpenFileNode` also reports its programmatic `fullPath` write via `notifyNodeParamsChanged` — any programmatic param write must.
...
- Outcomes: `'started'` (dirty consumed) / `'busy'` (run in progress → retry next interval, set kept) / `'skipped'` (validation errors, or the run would prompt for script inputs — an input node **without a resolvable configured value** inside the run set → wait for the next edit, set kept; configured inputs feed the prepared call and never block — see Input Nodes).
- Silent by design: no toasts, no dialog, no `autoSelectFirstOutputNode`; the UI side effects are re-opening the output preview (content only, no selection change) if invalidation had closed it, and — like every run — the `node-complete` of the **sole selected** node feeding its preview (see Execution Visualization), so an autorun of the node the user is looking at shows its output without an unselect/select round-trip.
- `FuncFlowView.detach()` resets the scheduler (and disarms auto-pin) so nothing fires into a closed view.
- **Save button state** (`saveAvailability` / `updateSaveButtonState`): the ribbon Save button greys **and goes non-clickable** (class `ff-ribbon-btn-disabled` → `pointer-events: none` + steel colors) only while the canvas is **empty** — Save is also the dashboard-publish gateway, so an unchanged (freshly opened) flow keeps it clickable and the tooltip carries the dirty/clean state instead. The baseline is a serialization recorded by `markSaved()` on save, on `bindScript`'s load (a server entity is already saved), on `newFlow`, and once the editor is ready (empty scratch flow). **`currentSnapshot()` strips `created`/`modified`/`author`** — `serializeFlow` stamps fresh timestamps every call, so without that every comparison reads "changed" (Save never greys; same flow saveable back-to-back). `updateSaveButtonState` runs on **both** `onGraphChanged` (structural: add/remove/clear + annotations) **and** `onGraphEdited` — the latter is the *only* signal for **parameter edits** (`notifyNodeParamsChanged` fires `onGraphEdited`, not `onGraphChanged`; node *drags* fire neither). The "why" tooltip lives on a wrapper `div.ff-save-btn-wrap` (a disabled button fires no hover; `pointer-events:none` passes the hover to the wrapper). The click handler also no-ops when disabled (keyboard-activation belt-and-braces). Only the non-creation Save button is tracked (`this.saveButton`).
...
- **Persistence**: `.flow` gains an optional `outputViews: {[paramName]: {layout}}` section
...
- **Save dialog + dashboard publish**: the ribbon **Save** pill always opens the combined
  ...
  ordering, and sharing. **Project binding**: the saved project's id persists in the `.flow`
  ...
  output. **The producing call must be namespace-qualified** (`user:FlowName(...)`, not
  `FlowName(...)` — an unqualified name resolves only for its author and breaks on share): the dapi
  UPDATE path (`scripts.save` with an existing id) returns the entity WITHOUT its namespace, so
  `saveToServer` re-fetches the saved entity (`scripts.find`) before rebinding `boundScript`, and
  `ensureBoundScriptQualified` re-checks (`nqName` contains `:`) right before stamping.
  Stamping is skipped (static snapshots) when the flow is unsaved or takes inputs without a
  ...
  canvas — a freshly opened (unchanged) flow must still open the dialog to publish; the tooltip
  tracks the dirty state.

## File Format

`.flow` v2 — Rete-native. A v1 → v2 migrator lives at `tools/migrate-ffjson-v1-to-v2.py` (run with `py tools/migrate-ffjson-v1-to-v2.py path/to/flow.flow`); the demo files in `files/` were converted with it. The runtime loader rejects anything other than `version: '2.0'`.
...
```typescript
{
  ...
  annotations: [{id, pos, size, text, color, fontSize?}],            // optional (back-compat)
  groups: [{id, title, description, memberIds, minimized, pos}],     // optional (back-compat)
...
### Bundled demos: `files/*.flow` vs `scripts/*.flow`

Two forms of each of the three demos — **Workflow Demo**, **Sequence demo**, and **Interactive
Viewers** (a live-viewers showcase: sketcher input, chem properties, seven in-node-previewed
viewers, saved with autorun on so it opens live) — ship in the package:
- **`files/*.flow`** — raw flow JSON docs, loaded by the Start panel's template cards (`FLOW_TEMPLATES` in [funcflow-view.ts](src/funcflow-view.ts), read via `_package.files.readAsText`; cards: Workflow demo and Interactive viewers — both saved with `autorun: true` so they open live; Sequence demo ships but has no card).
- **`scripts/*.flow`** — the same graphs as **first-class flow scripts** (the platform auto-registers everything in `scripts/`; the `flow` scriptHandler declares `extensions: flow`). Each file is a `.flow` body: the annotation header (`//name` / `//language: flow` / `//tags: flow` / execution-ordered `//output:` lines) followed by the flow JSON document — exactly what `flowScriptText` writes. They are **hand-committed** (generated from the files/ JSON demos), so they can drift; the **Flow: bundled flow scripts** test category regenerates the canonical body from each `.flow` via `flowScriptText` and asserts the header/output-ordering the committed files carry. If a future change alters that output (e.g. header ordering), the test fails — regenerate the `.flow` files to match. **Size cap**: a script body is a 256KB DB column — an oversized `scripts/*.flow` (e.g. a Biostructure viewer look embedding its structure) fails the WHOLE package publish with `value too long for type character varying(262144)`.
...
| `guide-content.ts` | The 6 tutorials (flagship `load-data-add-column` first — the Start-panel tour, now ending on the result tab; `publish-dashboard` — run → result tab → Save → Create dashboard → the core Save-project dialog; `interface-tour` walks every toolbox/ribbon/canvas/context-panel control, auto-adding a sample node as a prerequisite for the canvas/panel steps) + how-to questions (`TUTORIALS`, `QUESTIONS`). How-tos include `how-visualize` (demog → Scatter/Bar/Pie viewers, editing columns), `how-join` (two demog tables → Join Tables → output, filling keys via the column picker), and post-launch feature coverage pinned by the content test: `how-autorun` (bolt toggle, incremental reruns), `how-out-of-date` (downstream-only invalidation, amber tab dots), `how-func-editor` (the “Open editor” dialog round-trip, gated on `.d4-flow-function-funccall-editor`), `how-reuse-flow` (saved flows → Workflows section), `how-rerun-node` (single-node rerun from captured values), `how-open-result` (result tabs), `how-table-layouts` (layout persistence), `how-publish-dashboard`, `how-update-dashboard` (project binding / publish-as-new). The interface tour includes Autorun, the output panel, and the result tabs; Save/Open steps describe the combined save dialog with its Dashboard section (`.flow` import/export lives in the Flow menu). Output-tab steps gate on chip attributes: `.ff-view-tab[data-state="ready"]`, `[data-param="canvas"][data-active="true"]`. |
...
the single `.ff-node-infoline` (`data-kind="summary"`), only when no run status and no manual
`description` claim the line; viewer nodes short-circuit to "Displays a <viewer type>") and via ribbon
**Advanced → Describe this flow…**.
...
| Node group | `ff-group` (+ `data-group-id`, `data-minimized`, `data-status`), `ff-group-caret`, `ff-group-title`, `ff-group-desc`, `ff-group-status` |
...
- **Custom function editors** ([panel/func-editor-launcher.ts](src/panel/func-editor-launcher.ts) + [utils/func-editor-utils.ts](src/utils/func-editor-utils.ts)) — a function that ships its **own editor dialog** (`shouldUseFunctionEditor`: an `editor:` meta, surfacing as `func.options.editor`, or the explicit `EXPLICITLY_SUPPORTED_EDITABLE_FUNCTIONS` allowlist — e.g. `core:AddNewColumn`) gets a light-blue **"Open editor" button in the Input Parameters pane header** (`ff-prop-func-editor`, rendered by `decorateEditorHeader` only when the view wired `onEditFuncParams`). Inputs that practically REQUIRE that editor additionally carry an **inline pencil option inside the input itself** (`EDITOR_SHORTCUT_INPUTS` in func-editor-utils.ts, `nqName:inputName` pairs — currently `core:AddNewColumn:expression` and `Chem:descriptorsDocker:selected`, the list grows; `hasEditorShortcut` implies `shouldUseFunctionEditor`): `PropertyPanel.addEditorShortcut` adds it via `input.addOptions` (`ff-prop-input-editor-<param>`), firing the **exact same** `onEditFuncParams` handler as the header button — the pencil is pure discoverability, no separate code path. Clicking it runs `FuncEditorLauncher.open(node)`, which follows the **column picker's table ladder for every dataframe input at once**: all table inputs must be connected (balloon otherwise) → each resolves via `cloneForNode` (captured result) → anything missing gets one confirm dialog, then `produceTableForNode` slice runs. A `FuncCall` is then `func.prepare()`d from: the resolved tables, live captured values for other connected inputs (`ExecutionController.liveValue`, the `__ffFlowLive` registry), and the panel `inputValues` (column names resolved to real columns of their seeded table via `tableParamForColumn` — the same `columnTables`→suffix→first ladder as the compiler; `string_list` split to an array). `createFuncCallEditor(fc)` opens the platform dialog (`fc.edit()`, dialog polled via `pollDialogCreation`; the run action is intercepted by `d4-before-run-action` + `fc.status = 'Canceled'` so the values are saved without executing; the `d4-flow-function-funccall-editor` class locks the table selectors). On resolve, `applyEditorResult` writes values back into `node.inputValues`: **connected inputs are never overridden**, dataframe inputs skipped, only panel-editable slots touched, and columns come back as **name strings** (`editorValueToPanelValue`: column→name, column/string lists→comma-separated names, plain `list` stays an array, dataframes rejected). The view re-renders the panel on completion, holds the autorun scheduler for the whole round-trip, and reports the writeback via `notifyNodeParamsChanged`. **Never let a Flow run execute while the dialog is open**: the dialog intercepts the global `d4-before-run-action` (fired by EVERY client funccall) matching by func — see the Autorun section for the full race; `open()` waits for run-idle and passes `ignoreEvent` to `createFuncCallEditor`. Tested in [tests/func-editor-tests.ts](src/tests/func-editor-tests.ts).
  ...
  - **Connections** pane: shows **only what's wired** (connected Inputs / Pass-through / Outputs — disconnected slots are visible on the canvas already), each row naming its **far end** — `IN table (dataframe) ← Open File · result` / `OUT result (dataframe) → Join Tables · table1, …` (`endpointText`: node title `·` slot label; a pass-through source renders as its base input name). Exec ports are excluded from those groups and render in a **Run order** group as `runs after/before <node>` rows (`data-conn=__exec_in/__exec_out`). A **Missing** group lists required inputs neither connected nor filled (`missingRequiredInputs`) and required properties left empty (`missingRequiredProps`), amber-styled (`funcflow-conn-missing`, `data-missing`). Collapsed by default, **expanded when something is missing**; "Nothing connected yet" when empty. Connected rows carry `data-conn=<key>`.
- **Per-input overrides** ([utils/func-input-overrides.ts](src/utils/func-input-overrides.ts)), all keyed by `Func.nqName` like the exclusion denylist: **`HIDDEN_FUNC_INPUTS`** skips the row entirely (see Function Nodes — visual-only, data layer untouched); **`PANEL_ONLY_FUNC_INPUTS`** hides the row on the NODE body only (no socket/pass-through row unless wired — `nodeHiddenInputsOf` feeds `FlowNode.hiddenInputs`) while the panel keeps its editor — `core:OpenFile`'s `fullPath` + `sheetName`, so the node is a compact "this file" card; **`CONDITIONAL_FUNC_INPUTS`** (`{[nqName]: {[input]: (node) => boolean}}`) gates a PANEL row on the node's current values — OpenFile's `sheetName` renders only for an `.xlsx` path (the compiler applies its own matching guard at emission); **`FUNC_INPUT_CAPTIONS`** (`{[nqName]: {[input]: caption}}`) replaces implementation-flavored display captions ("Keys1" → "Left keys" on JoinTables) on the node sockets, pass-through labels, and panel rows — purely cosmetic, slot keys/values/compilation stay on the real names (`inputCaptionOf`); **`CUSTOM_FUNC_INPUT_EDITORS`** (`{[nqName]: {[inputName]: factory}}`) replaces the default editor with a bespoke one — the factory returns `{element, getValue, setValue, isValid?, onChanged?}`, the panel assigns `onChanged` (routed through the same `changeReporter`, gated on `isValid()`), calls `setValue(stored)`, and storage stays `inputValues[name]`, so compile/required-input checks see no difference. Registered: `core:OpenFile` `fullPath` → `ui.input.file` (`filePathEditor`) storing the picked `FileInfo.fullPath` **string**; its `setValue` rebuilds a `FileInfo` via `DG.FileInfo.fromString(path, '')` (synchronous) rather than `stringValue`, whose async server resolution validates hand-typed paths (that path needs core's `FileInput._setString` null-connection guard, fixed in d4 `file_input.dart`).
- Editor helpers: **every** editor row is a native DG input — no hand-rolled textareas/selects/
  checkboxes anywhere in the panel. Func primitives via `createPropertyInput` (`forProperty`);
  ad-hoc `properties` rows (Title, Input/Output config, Utility/Constants) via the DG-backed
  helpers `createTextarea` (ui.input.string — the name is historical), `createTextAreaRow`
  (ui.input.textArea, Description only), `createNumberInput` (ui.input.int/float), `createToggle`
  (ui.input.bool), `createCombo` (ui.input.choice); the multi-table column row's table chooser is a
  caption-less compact `ui.input.choice` inside the name input's `addOptions` area
  (`.funcflow-col-table-select`). All resolve tooltips through `propTip` (explicit → shared
  `PROP_TOOLTIPS`). Rows keep their `data-param` identity via `dgRow`/`propRow` — tests and guides
  address fields as `[data-param="X"] input|textarea|select` (see `paramFieldSelector`). Since the
  rows are standalone inputs (not one `ui.form`), the panel CSS provides the shared form geometry:
  a fixed 90px label column and full-width editors (`.funcflow-property-panel .ui-input-root >
  label.ui-input-label` / `> .ui-input-editor` in funcflow.css) — without it every editor starts
  wherever its label ends and the pane reads ragged. Long captions go in via the `caption` arg
  (identity stays the long name — e.g. 'Choices (comma-sep)' shows as "Choices").
...
- **Sections are a platform `ui.accordion`** (`FunctionBrowser.accordion`, rebuilt each render): the keyed accordion (`TOOLBOX_ACCORDION_KEY = 'funcflow.toolbox'`) restores/persists expanded pane state **itself** in `localStorage['Accordion:funcflow.toolbox']`, keyed by **pane name** — no manual expand bookkeeping. Pane content builds **lazily on first expand** (`addSection`'s `getContent`); category counts use `addCountPane` badges (the pane name stays the bare title). Per-connection query groups are a **nested accordion** (`FunctionBrowser.queriesAccordion`, key `funcflow.toolbox.queries`). Pane headers keep `data-section` + `ff-browser-section-<title>`; an expanded header carries the `expanded` class (what `untilSectionExpanded` polls). **Search forces all panes open on an *unkeyed* accordion** so forced states never persist. Only the group-by mode still lives in `funcflow.browser.v1`. Headers pass `allowDragOut: false` (pane drag-out would fight item drag-to-canvas). Tests must **expand a pane before querying its items** (lazy content).
...
- **Rules are data-driven from catalog signatures** (no per-package lists): R1 — a column with semType *S* + a catalog func declaring `column {semType: S}` and a dataframe input → suggest, wire the table, **prefill the column param** (Molecule → Chem descriptors/properties/…, Macromolecule → Bio toAtomicLevel/MSA/…, PDB_ID → structure fetch; `FEATURED_FUNCS` float the canonical ops). R2 — the clicked preview cell's semType matches a scalar input (`string {semType: Molecule}` → Chembl/SureChembl searches) → suggest with the **value prefilled**. R3 — ≥2 selected table-bearing nodes → every func with ≥2 dataframe inputs (Join/Link boosted), **wired to both**; selecting two tables is the most explicit intent signal, so `SCORE.twoTables` (115) outranks even a fully-boosted semType hit (100 + 6 + 2 + 3) — combiners always lead the list. R4 — a single table → the top `COMMON_NEXT_ORDER` steps (capped at `COMMON_NEXT_CAP` so plumbing never swamps the 10 slots) + viewers picked by column stats (≥2 numeric → Scatter Plot, categorical → Bar Chart, always Grid) + Table Output. R5 — scalar-only focus → Value Output. Empty canvas → Table Input / Open File. Scores in the exported `SCORE` map; domain-continuation and selected-table bonuses on top; dedup by typeName keeps the max.
- **Refresh triggers** (debounced 250 ms in the pane): node select/deselect, **`onSelectionChanged`** (the editor callback for selection changes that bypass `nodepicked` — marquee, Ctrl+A / Ctrl+Shift+A, modifier-click removals, programmatic selects; required because the marquee's release is swallowed by `stopImmediatePropagation` before any container listener sees it), `onGraphChanged` + `onGraphEdited` (params), `ExecutionController.onNodeStateChanged` (fresh captured values), canvas `pointerup` (empty-canvas-click deselects), and preview-cell clicks.

### Catalog inclusion ([node-factory.ts](src/rete/node-factory.ts))

`shouldIncludeFunc(func)` admits a function into the catalog when **any**: `meta.includeInFlow: true` on the function (opt-in); it is a **saved flow** (`isWorkflowFunc`) or a **query** (`DG.DataQuery`, unless its package ∈ `DEV_TEST_PACKAGES`) — included by kind; or `func.nqName` ∈ [`INCLUDED_FUNC_NQNAMES`](src/rete/included-funcs.ts) — the curated, hand-editable include list frozen from what the old deny pipeline produced live (~2,357 raw → ~264 listed package/core functions). `meta.includeInFlow: false` wins over everything. Everything not listed and not opted in — dev/test packages, panels, sketchers, dialog wrappers, scalar helpers — simply stays out. Locked in by [tests/function-browser-tests.ts](src/tests/function-browser-tests.ts); the shape of the original list is documented in [docs/func-catalog-snapshot.md](docs/func-catalog-snapshot.md).
...
- **Selection — mirrors the platform's `selectRows` / `areaSelector` (d4 `viewer_utils.dart`)**: plain click selects a node exclusively; **Shift+click adds** to the selection; **Ctrl+click toggles**; **Ctrl+Shift+click removes**; **Shift+drag** on empty canvas draws a marquee (platform look: black stroke, lightBlue 50% fill) that **adds** every intersecting node — **Ctrl held at mouse-up removes** them instead; the marquee never replaces the selection. **Ctrl+A / Ctrl+Shift+A** select / deselect every node. Empty-canvas click clears (rete default). Implementation: rete's `nodepicked` can only ADD, so `accumulating.active()` admits any modifier (plus clicks on an already-selected node — group drag must keep the group) and the removal half (Ctrl toggle-off, Ctrl+Shift remove, plain-click collapse of a multi-selection) runs in a capture-phase window `pointerup` tracker on a clean release (< 4px movement — a drag keeps the group). The marquee's `onUp` calls `stopImmediatePropagation()` so the AreaPlugin's always-on window pointerup listener doesn't count the release as an empty-canvas click and `unselectAll` what was just selected. **Re-click dedupe is host-state-aware**: re-picking the already-selected node (click again, grab to drag — `nodepicked` and the strip-chip click both) skips `onNodeSelected` only while the host's `isNodeContextCurrent(node)` callback says its panels still show the node; the view's implementation returns false when the context panel isn't the shell's current object anymore (a tab switch replaced it), when it shows a different node, or when the node has a renderable captured output the unpinned preview isn't showing — so a re-click *restores* a stale context instead of being swallowed by "already selected". No callback (headless/test editors) → same-pick never re-fires. Tests: [selection-tests.ts](src/tests/selection-tests.ts).
...
- **"Needs input" hint** (U6/U5b): when a node is idle/stale and a *requirement* is unmet, an amber hint line ("Requires: table, molecules") replaces the status line and the status dot turns amber (`data-attention="true"`). Requirements are two kinds: **required inputs** — **any** non-optional input with no declared default, neither connected nor filled (`FuncNode.requiredInputs`; optionality = Dart `FuncParam.isOptional` read reflectively via `grok_Property_Get` / `nullable` / options `{optional: true}` — see `isInputOptional`; bools and list-likes are exempt since they always hold a value, and a required numeric seeds `null` rather than 0 so "unset" stays detectable; viewers list `['table']`; **annotation override**: a leading **(dataframe, column)** input pair is forced required even when declared nullable — platform mis-annotation is rampant (Chem:bitbirchClusteringTopMenu marks both `table` and `molecules` nullable) and such a function is meaningless without its data) — and **required properties** (`FlowNode.requiredProps`: panel values that must be set, e.g. Select Column's `columnName`, Select Columns' `columnNames`, Select Table's `tableName`; Add Table View adds `requiredInputs:['table']`). Computed at render time by `nodeMissingRequirements(node, isConnected)` = `missingRequiredInputs` ⊕ `missingRequiredProps` ([scheme.ts](src/rete/scheme.ts)) — the **same** predicate the run-readiness gate uses (a hinted node is exactly one the run skips). `FuncFlowView.refreshNodeHints()` re-renders all nodes on `onGraphChanged` **and on a `params-changed` edit** (which does NOT fire `onGraphChanged` — without it, filling a required value left the hint up until something else happened to re-render the node), so it tracks both wiring and values live. **Adding a required input/prop to a new node type automatically flags it in the hint and excludes it from runs.** A requirement the generic checks can't express (e.g. "this JSON must cover every property of the chosen profile") goes in **`FUNC_NODE_VALIDATORS`** ([utils/func-input-overrides.ts](src/utils/func-input-overrides.ts)), keyed by `nqName` and returning the labels of what's missing; `FuncNode` stamps it on the node as `extraValidator`, so `scheme.ts` never imports the panel. Such a validator must be **synchronous** (it runs on every render and in the run gate). When readiness depends on something only an async lookup can answer, **fail closed and resolve in the background**: return a "reading…" requirement, start the lookup (deduped in a `WeakMap` keyed by node identity — the validator runs on every render), and on completion cache the answer and call `node.editorBridge?.notifyParamsChanged(id)` so the hint and the run gate recompute. Failing *open* on the unknown is what let an unconfigured MPO node run: a flow never opened in the panel and never run had nothing cached, so the check waved it through. Cache such answers in a module-level `WeakMap`, **not** in `node.properties` — those serialize, so a lookup landing would dirty a flow nobody edited.
...
- **Context menu (right-click on a node)**: Run up to here & preview (when `onPreviewNode` is wired — same slice-compile as the output-port menu, just more discoverable) · **Rerun this node only** (when `onRerunNode` is wired AND `canRerunNode(id)` — re-executes just this node from captured upstream values, see the live-value registry above) · Collapse/Expand · Duplicate · Copy (fills the same clipboard as Ctrl+C; a node inside a multi-selection copies the whole selection) · Delete. Right-click on a connection: Delete connection. Built with `DG.Menu.popup().item(...).separator().show({causedBy: ev})` so the platform handles positioning, dismissal, and styling. Trigger comes from `area-plugin`'s `contextmenu` signal — `data.context` is `'root'` / a `FlowNode` / a `FlowConnection` and we branch on it.
- **Delete key (or Backspace)**: removes every selected node and all connections touching it. With no nodes selected, it removes the **last-clicked annotation** instead (`activeAnnotationId` — armed on any pointerdown inside an annotation, shown as the same blue ring nodes get via `.ff-annotation-active`, disarmed by a press anywhere else or Escape; node selection always wins). The handler is registered on `window.keydown` and skips events whose target is an `<input>`/`<textarea>`/`<select>` or contenteditable (the annotation title) so typing never deletes things.
- **Arrow keys** (same keydown handler): nudge every selected node by `NODE_NUDGE_STEP` (10 canvas units); **Ctrl/Cmd+arrow pans the canvas** by `CANVAS_PAN_STEP` screen px instead (content shifts opposite to the arrow so the view "moves" with it). Nudges raise `suppressSnap` around their `area.translate` calls: the drag snap applies to any *picked* node and a clicked node stays picked, so without it a 10px step got re-rounded to the 20px grid (537 → 547 → 540) or captured by an alignment guide — the node looked stuck. Minimized-group members and strip-pinned output rows are excluded from the nudge.
- **Copy / paste / Esc** (same keydown handler): **Ctrl+C** snapshots the selected nodes — deep-copied payloads plus every connection whose both endpoints are selected (data, pass-through, and order edges) — into the per-editor clipboard (`GraphClip`; skipped when a live text selection exists, so copying text still works); **Ctrl+V** materializes it offset 30px·pasteCount down-right, dedupes output `paramName`s / SetVar `variableName`s against the shared variable namespace (`dedupeVariableName` — the validator would flag duplicates as errors), and selects the copies so they're immediately draggable as a group; **Escape** clears the selection. The context menu's **Duplicate** goes through the same `duplicateNodes(ids)` — right-clicking a node inside a multi-selection duplicates the whole selection with its internal connections. This relies on the non-primary-button guard (`guardNonPrimary` in `installPointerDownTracker`) swallowing BOTH pointerdown and pointerup of right/middle clicks on nodes and sockets in the capture phase: the node view ignores non-left buttons, so the gesture would otherwise bubble to the area, whose selectable extension counts pointerdown→pointerup with <4 moves as an empty-canvas click and unselects all (its pointerup listener is on `window`, hence both halves). The `contextmenu` DOM event is separate and still opens the menu.
- **Annotations carry their contents** ([rete/annotation.ts](src/rete/annotation.ts) + `annotationCargo` in [flow-editor.ts](src/rete/flow-editor.ts)): dragging an annotation body also moves every node whose **center** sits inside the rect (output rows excluded), annotations **fully contained** in it, minimized group **cards** whose center is inside (their hidden members travel too — a card must never separate from its members), and the waypoints of connections between carried nodes. The cargo is computed at **drag start** (stateless capture — nothing persisted, nothing to go stale): a node dragged out simply isn't inside at the next grab. Carried nodes go through `area.translate`, which skips the snap interception (only *picked* nodes snap), so relative geometry is preserved. **Styling** lives in the annotation's right-click menu: a 13-color palette (`ANNOTATION_COLORS`, Material 100-level bg / 700-level border pairs — `borderForBackground` falls back to blue for unknown colors) and a **Title size** group (`ANNOTATION_TITLE_SIZES`, 11–26px; default 13, `ANN_DEFAULT_FONT_SIZE`). Both are **radio groups** (`radioGroup` in `IMenuItemOptions` — bare `check` renders independent checkboxes; use `radioGroup` for one-of-many menu choices). The menu also has a **Pinned** checkbox: a pinned annotation cannot be moved (body drags fall through to the canvas pan — the point: navigating a zoomed-in view full of annotations) or resized (handle inert + hidden via `[data-pinned]` CSS), and a carrier annotation leaves pinned ones behind (`annotationCargo` skips them); title editing and Delete-arming still work. All three go through public `FlowEditor.setAnnotationColor`/`setAnnotationFontSize`/`setAnnotationPinned`, which fire `onGraphChanged` (a restyled note is an unsaved change). `fontSize` (omitted at the default 13) and `pinned` (omitted when false) serialize in the annotation doc — same tidy-save pattern as `autorun`.
- **Node groups** ([rete/node-group.ts](src/rete/node-group.ts) + the `node groups` section in [flow-editor.ts](src/rete/flow-editor.ts)): select ≥2 nodes → **Ctrl+G** or context-menu *Group selected* wraps them in a titled frame; **Ctrl+Shift+G** / *Ungroup* dissolves it (members stay). **Editor-level like annotations — the graph stays flat**: members remain real nodes/connections, so compiler, execution, invalidation, autorun, and script emission are untouched. The **expanded frame** (background element behind the nodes, like annotations) auto-fits the member bbox (`fitGroupFrame`, rAF-coalesced refits on `nodetranslated`) and drags by title bar or body — the editor translates the members (+ internal waypoints) and the frame follows via the refit path. The **caret** (or double-press on the card) minimizes: member views and fully-internal connection wrappers get `.ff-group-hidden` (the wrapper element persists across React re-renders), boundary wires re-anchor to **card-edge dots** (one per distinct boundary socket; `listenGroupableSocket` wraps the DOM socket watcher — same pattern as the output strip's `chipSocketSubs` — swallowing DOM positions while the node hides and pushing `groupSocketAnchor` card anchors instead, refreshed on card drags via `notifyGroupSockets`), and the card shows title + description + an **aggregate status dot** (running > errored > stale > all-completed; refreshed from `updateNode`, which every `ExecutionVisualizer.highlightNode` goes through). Card drags translate the hidden members live, so **maximize simply un-hides** — contents track the card's travels; `area.update('node', …)` on each member makes the DOM watcher re-measure and release the card anchors. Double-press detection is pointer-based in the drag handler (`preventDefault` on pointerdown suppresses the browser's compatibility `dblclick`): title → rename, description → edit, elsewhere on a minimized card → maximize. Membership is explicit (drag members freely inside; *Remove from group* on the node menu); a member's `noderemoved` shrinks the group, empty dissolves it; *Tidy layout* re-runs `layoutGraph` over just the members anchored at the bbox top-left. Consistency: Ctrl+A / rect-select / snap targets skip hidden members, `zoomToFit` switches to a manual model-coordinate fit when a minimized group exists (hidden views measure at 0,0), the minimap draws the card instead of hidden members, `autoLayout` maximizes all groups first, `guardNonPrimary` covers `.ff-group`/`.ff-annotation` so right-clicks don't clear the selection, and empty-`.flow`-load groups re-apply minimized state (`createGroup` with `opts.minimized`, late-mounting internal wires hidden by the `tagConnectionElement` hook). Persisted as the optional `groups` field (memberIds remapped through the loader's idMap). Tests: [group-tests.ts](src/tests/group-tests.ts).
- **Drag from toolbox**: `ui.makeDroppable` on the canvas container — accepts `DG.FileInfo` (creates an `OpenFile` node with `inputValues['fullPath']`, titled `<label>: <fileName>` so several stay tellable apart — cosmetic, mirrors the Uploaded File node's `file.name` title; a later **completed run restamps** the title from the current path (`refreshOpenFileTitle` in the `onNodeStateChanged` fan-out) — but ONLY while the title is still machine-stamped (`<display name>: <file>`); a user-renamed node keeps its name (restamping unconditionally silently erased custom titles on every run)) or `DG.Func` (looks up the registered factory by func name). The node is placed **at the drop pointer** (`args.dropEvent` → `addNodeByTypeAtDrop` → `addNodeByTypeAt(typeName, clientX, clientY)`), matching the native-drag behavior; only double-click / programmatic adds fall back to center. Native HTML5 drag from the FunctionBrowser carries the typeName via the `FF_DRAG_MIME` data type; the canvas drop handler reads it and calls `addNodeAt` with the drop point.
