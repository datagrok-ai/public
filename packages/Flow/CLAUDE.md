...
| `panel-tests.ts` | Flow: property panel | `stringChoiceOptions` (choices/nullable/current-preservation) + `propertyChoices` reading live func-input choices; input overrides: hidden inputs stay data-carrying but render nowhere, a registered custom editor replaces the default input (storage/`isValid` routing), OpenFile's `fullPath` file picker round-trips a path |
