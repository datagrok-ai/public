...
- **Selection — mirrors the platform's `selectRows` / `areaSelector` (d4 `viewer_utils.dart`)**: plain click selects a node exclusively; **Shift+click adds** to the selection; **Ctrl+click toggles**; **Ctrl+Shift+click removes**; **Shift+drag** on empty canvas draws a marquee (platform look: black stroke, lightBlue 50% fill) that **adds** every intersecting node — **Ctrl held at mouse-up removes** them instead; the marquee never replaces the selection. **Ctrl+A / Ctrl+Shift+A** select / deselect every node. Empty-canvas click clears (rete default). Implementation: rete's `nodepicked` can only ADD, so `accumulating.active()` admits any modifier (plus clicks on an already-selected node — group drag must keep the group) and the removal half (Ctrl toggle-off, Ctrl+Shift remove, plain-click collapse of a multi-selection) runs in a capture-phase window `pointerup` tracker on a clean release (< 4px movement — a drag keeps the group). The marquee's `onUp` calls `stopImmediatePropagation()` so the AreaPlugin's always-on window pointerup listener doesn't count the release as an empty-canvas click and `unselectAll` what was just selected. Tests: [selection-tests.ts](src/tests/selection-tests.ts).
