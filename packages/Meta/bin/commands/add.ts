...
function addPackageVersion(name: string, packageVersion: string, dependency: string, dependencyVersion: string) {
...
export function add(args: CreateArgs) {
  ...
  if (nOptions && !Object.keys(args).slice(1).every(op => ['package', 'ver', 'dep', 'depver'].includes(op))) return false;

  addPackageVersion(args.package, args.ver, args.dep, args.depver);
  return true;
}
