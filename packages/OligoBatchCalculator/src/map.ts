export const map: {[index: string]: {[index: string]: {"name": string, "molecularWeight": number}}} = {
  "BioSpring Code For ASO Gapmers": {
    "5": {
      "name": "2'MOE-5Me-rU",
      "molecularWeight": 378.27
    },
    "6": {
      "name": "2'MOE-rA",
      "molecularWeight": 387.29
    },
    "7": {
      "name": "2'MOE-5Me-rC",
      "molecularWeight": 377.29
    },
    "8": {
      "name": "2'MOE-rG",
      "molecularWeight": 403.28
    },
    "9": {
      "name": "5-Methyl-dC",
      "molecularWeight": 303.21
    },
    "*": {
      "name": "ps linkage",
      "molecularWeight": 16.07
    },
    "A": {
      "name": "dA",
      "molecularWeight": 313.21
    },
    "C": {
      "name": "dC",
      "molecularWeight": 289.18
    },
    "G": {
      "name": "dG",
      "molecularWeight": 329.21
    },
    "T": {
      "name": "dT",
      "molecularWeight": 304.2
    },
  },
  "Janssen GCRS code For ASO Gapmers": {
    "moeT": {
      "name": "2'MOE-5Me-rU",
      "molecularWeight": 378.27
    },
    "moeA": {
      "name": "2'MOE-rA",
      "molecularWeight": 387.29
    },
    "moe5mC": {
      "name": "2'MOE-5Me-rC",
      "molecularWeight": 377.29
    },
    "(5m)moeC": {
      "name": "2'MOE-5Me-rC",
      "molecularWeight": 377.29
    },
    "moeG": {
      "name": "2'MOE-rG",
      "molecularWeight": 403.28
    },
    "5mC": {
      "name": "5-Methyl-dC",
      "molecularWeight": 303.28
    },
    "(5m)C": {
      "name": "5-Methyl-dC",
      "molecularWeight": 303.28
    },
    "ps": {
      "name": "ps linkage",
      "molecularWeight": 16.07
    },
    "A": {
      "name": "Adenine",
      "molecularWeight": 313.21
    },
    "dA": {
      "name": "Adenine",
      "molecularWeight": 313.21
    },
    "C": {
      "name": "Cytosine",
      "molecularWeight": 289.18
    },
    "dC": {
      "name": "Cytosine",
      "molecularWeight": 289.18
    },
    "G": {
      "name": "Guanine",
      "molecularWeight": 329.21
    },
    "dG": {
      "name": "Guanine",
      "molecularWeight": 329.21
    },
    "T": {
      "name": "Tyrosine",
      "molecularWeight": 304.2
    },
    "dT": {
      "name": "Tyrosine",
      "molecularWeight": 304.2
    },
    "rA": {
      "name": "Adenine",
      "molecularWeight": 329.21
    },
    "rC": {
      "name": "Cytosine",
      "molecularWeight": 305.18
    },
    "rG": {
      "name": "Guanine",
      "molecularWeight": 345.21
    },
    "rU": {
      "name": "Uracil",
      "molecularWeight": 306.17
    }
  },
  "BioSpring Code For 2'-OMe and 2'-F modified siRNA": {
    "1": {
      "name": "2'-fluoro-U",
      "molecularWeight": 308.16
    },
    "2": {
      "name": "2'-fluoro-A",
      "molecularWeight": 331.2
    },
    "3": {
      "name": "2'-fluoro-C",
      "molecularWeight": 307.18
    },
    "4": {
      "name": "2'-fluoro-G",
      "molecularWeight": 347.19
    },
    "5": {
      "name": "2'OMe-rU",
      "molecularWeight": 320.2
    },
    "6": {
      "name": "2'OMe-rA",
      "molecularWeight": 343.24
    },
    "7": {
      "name": "2'OMe-rC",
      "molecularWeight": 319.21
    },
    "8": {
      "name": "2'OMe-rG",
      "molecularWeight": 359.24
    },
    "*": {
      "name": "ps linkage",
      "molecularWeight": 16.07
    }
  },
  "Axolabs Code For 2'-OMe and 2'-F modified siRNA": {
    "Uf": {
      "name": "2'-fluoro-U",
      "molecularWeight": 308.16
    },
    "Af": {
      "name": "2'-fluoro-A",
      "molecularWeight": 331.2
    },
    "Cf": {
      "name": "2'-fluoro-C",
      "molecularWeight": 307.18
    },
    "Gf": {
      "name": "2'-fluoro-G",
      "molecularWeight": 347.19
    },
    "u": {
      "name": "2'OMe-rU",
      "molecularWeight": 320.2
    },
    "a": {
      "name": "2'OMe-rA",
      "molecularWeight": 343.24
    },
    "c": {
      "name": "2'OMe-rC",
      "molecularWeight": 319.21
    },
    "g": {
      "name": "2'OMe-rG",
      "molecularWeight": 359.24
    },
    "s": {
      "name": "ps linkage",
      "molecularWeight": 16.07
    }
  },
  "Janssen GCRS code For 2'-OMe and 2'-F modified siRNA": {
    "fU": {
      "name": "2'-fluoro-U",
      "molecularWeight": 308.16
    },
    "fA": {
      "name": "2'-fluoro-A",
      "molecularWeight": 331.2
    },
    "fC": {
      "name": "2'-fluoro-C",
      "molecularWeight": 307.18
    },
    "fG": {
      "name": "2'-fluoro-G",
      "molecularWeight": 347.19
    },
    "mU": {
      "name": "2'OMe-rU",
      "molecularWeight": 320.2
    },
    "mA": {
      "name": "2'OMe-rA",
      "molecularWeight": 343.24
    },
    "mC": {
      "name": "2'OMe-rC",
      "molecularWeight": 319.21
    },
    "mG": {
      "name": "2'OMe-rG",
      "molecularWeight": 359.24
    },
    "ps": {
      "name": "ps linkage",
      "molecularWeight": 16.07
    }
  }
};