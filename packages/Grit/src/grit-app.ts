...
export class GritApp {
  ...
  async refresh(): Promise<void> {
    ...
    const rows = this.currentProject == null ? [] : await gritDb.issue.query()
      .where('project_id', '=', this.currentProject.id).orderBy('number', true);
  ...
  async showIssue(issue: IssueRow): Promise<void> {
    ...
    // One query brings the issue with its comments embedded; the builder's expand()
    // types the `comment` array (no hand-written intersection type).
    const expanded = await gritDb.issue.query()
      .where({project_id: issue.project_id, number: issue.number})
  ...
  formatAuditEntry(a: DG.DomainAuditEntry): string {
  ...
  async patch(issue: IssueRow, values: Partial<IssueRow>): Promise<void> {
    const updated = await gritDb.issue.updateWithRetry(issue.id, () => values);
    if (updated != null)
      Object.assign(issue, values, {version: updated.version});
  }

  async nextNumber(projectId: string): Promise<number> {
    const [top] = await gritDb.issue.aggregate({
      filter: [{property: 'project_id', operator: '=', value: projectId}],
      measures: [{fn: 'max', column: 'number', as: 'last'}],
    });
    return ((top?.last as number | null) ?? 0) + 1;
  }
