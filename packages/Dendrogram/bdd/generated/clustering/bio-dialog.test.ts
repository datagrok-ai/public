...
test.describe("Hierarchical clustering from the Bio menu", () => {
  ...
  test("Hierarchical clustering from the Bio menu", {tag: ["@journey", "@realizes:dendrogram.cp.hier-clustering-bio-sequence-path"]}, async ({browser}) => {
    ...
    await session.step(14, "And user opens FASTA_PT_activity dataset", () => openDataset(page, ds("FASTA_PT_activity")));
    await run.scenario("The dialog opens on the sequence column with every distance and linkage", async () => {
      ...
      await session.step(18, "And the \"cell type of sequence\" reading of grid should be \"sequence\"", () => readingReads(page, "cell type of sequence", el("grid"), "sequence"));
      await session.step(19, "When user picks \"Bio > Analyze > Hierarchical Clustering...\" from the top menu", () => pickFromTopMenu(page, "Bio > Analyze > Hierarchical Clustering..."));
      ...
      await session.step(21, "And Table input in \"Hierarchical Clustering\" dialog should have value \"FASTA_PT_activity\"", () => shouldHaveValue(page, el("Table input in \"Hierarchical Clustering\" dialog"), "FASTA_PT_activity"));
      await session.step(22, "And editor of Features input in \"Hierarchical Clustering\" dialog should have text \"(1) sequence\"", () => shouldHaveText(page, el("editor of Features input in \"Hierarchical Clustering\" dialog"), "(1) sequence"));
    ...
    await run.scenario("Euclidean distance and ward linkage attach a tree with a leaf for every sequence", async () => {
      ...
      await session.step(33, "When user picks \"Bio > Analyze > Hierarchical Clustering...\" from the top menu", () => pickFromTopMenu(page, "Bio > Analyze > Hierarchical Clustering..."));
      ...
      await session.step(38, "And the \"tree leaves\" reading of grid should be 99", () => readingIs(page, "tree leaves", el("grid"), 99));
    ...
    await run.scenario("Removing the tree, then manhattan distance and complete linkage attach a tree of another height", async () => {
      ...
      await session.step(61, "Then the task bar should have finished \"Creating dendrogram\"", () => taskBarFinished(page, "Creating dendrogram"));
      await session.step(62, "And the \"tree leaves\" reading of grid should be 99", () => readingIs(page, "tree leaves", el("grid"), 99));
      await session.step(63, "And the \"tree height\" reading of grid should not be as remembered", () => readingNotAsRemembered(page, "tree height", el("grid")));
    ...
    await run.scenario("Assign Clusters on the sequence tree adds a cluster number to every sequence", async () => {
      await session.step(68, "When user clicks on \"Remove Dendrogram\" icon", () => clickOn(page, el("\"Remove Dendrogram\" icon")));
      await session.step(69, "And user picks \"Bio > Analyze > Hierarchical Clustering...\" from the top menu", () => pickFromTopMenu(page, "Bio > Analyze > Hierarchical Clustering..."));
      await session.step(70, "And user clicks on OK button in \"Hierarchical Clustering\" dialog", () => clickOn(page, el("OK button in \"Hierarchical Clustering\" dialog")));
      await session.step(71, "Then the task bar should have finished \"Creating dendrogram\"", () => taskBarFinished(page, "Creating dendrogram"));
      await session.step(72, "And the \"tree leaves\" reading of grid should be 99", () => readingIs(page, "tree leaves", el("grid"), 99));
      await session.step(73, "When user clicks on \"Assign Clusters\" icon", () => clickOn(page, el("\"Assign Clusters\" icon")));
      await session.step(74, "Then \"Assign Clusters\" dialog should be visible", () => shouldBe(page, el("\"Assign Clusters\" dialog"), "visible"));
      await session.step(75, "When user enters \"5\" into Clusters input in \"Assign Clusters\" dialog", () => enterInto(page, "5", el("Clusters input in \"Assign Clusters\" dialog")));
      await session.step(76, "Then Threshold input in \"Assign Clusters\" dialog should have a value between 11.60 and 11.61", () => shouldHaveValueBetween(page, el("Threshold input in \"Assign Clusters\" dialog"), 11.6, 11.61));
      await session.step(77, "When user clicks on Assign button in \"Assign Clusters\" dialog", () => clickOn(page, el("Assign button in \"Assign Clusters\" dialog")));
      await session.step(78, "Then the \"Assign Clusters\" dialog should close", () => dialogCloses(page, "Assign Clusters"));
      await session.step(79, "And the table should have a column \"Cluster (11.60)\"", () => hasColumn(page, "Cluster (11.60)"));
      await session.step(80, "And \"Cluster (11.60)\" column should have no missing values", () => columnComplete(page, "Cluster (11.60)"));
      await session.step(81, "And the newest column matching \"^Cluster \\(\" should have 5 distinct values", () => newestMatchingDistinct(page, "^Cluster \\(", 5));
    ...
    await run.scenario("A threshold above the tree's height is refused by the dialog", async () => {
      await session.step(85, "When user clicks on \"Assign Clusters\" icon", () => clickOn(page, el("\"Assign Clusters\" icon")));
      await session.step(86, "And user enters \"100\" into Threshold input in \"Assign Clusters\" dialog", () => enterInto(page, "100", el("Threshold input in \"Assign Clusters\" dialog")));
      await session.step(87, "Then Threshold input in \"Assign Clusters\" dialog should be invalid", () => shouldBe(page, el("Threshold input in \"Assign Clusters\" dialog"), "invalid"));
      await session.step(88, "When user closes \"Assign Clusters\" dialog", () => close(page, el("\"Assign Clusters\" dialog")));
      await session.step(89, "Then the \"Assign Clusters\" dialog should close", () => dialogCloses(page, "Assign Clusters"));
