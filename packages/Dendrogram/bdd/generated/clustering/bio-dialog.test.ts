...
test.describe("Hierarchical clustering from the Bio menu", () => {
  ...
  test("Hierarchical clustering from the Bio menu", {tag: ["@journey", "@realizes:dendrogram.cp.hier-clustering-bio-sequence-path"]}, async ({browser}) => {
    ...
    await run.scenario("Euclidean distance and ward linkage attach a tree with a leaf for every sequence", async () => {
      ...
      await session.step(35, "And the \"tree leaves\" reading of grid should be 99", () => readingIs(page, "tree leaves", el("grid"), 99));
    ...
    await run.scenario("The sequence tree follows the grid, and the grid follows the tree", async () => {
      await session.step(41, "When user clicks on the \"cell 3 of sequence_id\" area of grid", () => clickArea(page, "cell 3 of sequence_id", el("grid")));
      await session.step(42, "Then row 3 should be current", () => currentRowIs(page, 3));
      await session.step(43, "And the \"tree current node\" reading of grid should be \"2\"", () => readingReads(page, "tree current node", el("grid"), "2"));
      await session.step(44, "When user hovers over the \"leaf 10\" area of grid", () => hoverArea(page, "leaf 10", el("grid")));
      await session.step(45, "Then the \"tree mouse over node\" reading of grid should be \"10\"", () => readingReads(page, "tree mouse over node", el("grid"), "10"));
      await session.step(46, "And row 11 should be under the mouse", () => mouseOverRowIs(page, 11));
    ...
    await run.scenario("Removing the tree, then manhattan distance and complete linkage attach a tree of another height", async () => {
      await session.step(50, "When user remembers the \"tree height\" reading of grid", () => rememberReading(page, "tree height", el("grid")));
      await session.step(51, "And user clicks on \"Remove Dendrogram\" icon", () => clickOn(page, el("\"Remove Dendrogram\" icon")));
      await session.step(52, "Then grid should not report a \"tree leaves\" reading", () => noSuchReading(page, el("grid"), "tree leaves"));
      await session.step(53, "When user picks \"Bio > Analyze > Hierarchical Clustering...\" from the top menu", () => pickFromTopMenu(page, "Bio > Analyze > Hierarchical Clustering..."));
      await session.step(54, "And user selects \"manhattan\" in Distance input in \"Hierarchical Clustering\" dialog", () => selectIn(page, "manhattan", el("Distance input in \"Hierarchical Clustering\" dialog")));
      await session.step(55, "And user selects \"complete\" in Linkage input in \"Hierarchical Clustering\" dialog", () => selectIn(page, "complete", el("Linkage input in \"Hierarchical Clustering\" dialog")));
      await session.step(56, "Then Distance input in \"Hierarchical Clustering\" dialog should have value \"manhattan\"", () => shouldHaveValue(page, el("Distance input in \"Hierarchical Clustering\" dialog"), "manhattan"));
      await session.step(57, "And Linkage input in \"Hierarchical Clustering\" dialog should have value \"complete\"", () => shouldHaveValue(page, el("Linkage input in \"Hierarchical Clustering\" dialog"), "complete"));
      await session.step(58, "When user clicks on OK button in \"Hierarchical Clustering\" dialog", () => clickOn(page, el("OK button in \"Hierarchical Clustering\" dialog")));
      await session.step(59, "Then the \"tree leaves\" reading of grid should be 99", () => readingIs(page, "tree leaves", el("grid"), 99));
      await session.step(60, "And the \"tree height\" reading of grid should not be as remembered", () => readingNotAsRemembered(page, "tree height", el("grid")));
    ...
    await run.scenario("Assign Clusters on the sequence tree adds a cluster number to every sequence", async () => {
      await session.step(65, "When user clicks on \"Remove Dendrogram\" icon", () => clickOn(page, el("\"Remove Dendrogram\" icon")));
      await session.step(66, "And user picks \"Bio > Analyze > Hierarchical Clustering...\" from the top menu", () => pickFromTopMenu(page, "Bio > Analyze > Hierarchical Clustering..."));
      await session.step(67, "And user clicks on OK button in \"Hierarchical Clustering\" dialog", () => clickOn(page, el("OK button in \"Hierarchical Clustering\" dialog")));
      await session.step(68, "Then the \"tree leaves\" reading of grid should be 99", () => readingIs(page, "tree leaves", el("grid"), 99));
      await session.step(69, "When user clicks on \"Assign Clusters\" icon", () => clickOn(page, el("\"Assign Clusters\" icon")));
      await session.step(70, "Then \"Assign Clusters\" dialog should be visible", () => shouldBe(page, el("\"Assign Clusters\" dialog"), "visible"));
      await session.step(71, "When user enters \"5\" into Clusters input in \"Assign Clusters\" dialog", () => enterInto(page, "5", el("Clusters input in \"Assign Clusters\" dialog")));
      await session.step(72, "Then Threshold input in \"Assign Clusters\" dialog should have a value between 11.05 and 11.07", () => shouldHaveValueBetween(page, el("Threshold input in \"Assign Clusters\" dialog"), 11.05, 11.07));
      await session.step(73, "When user clicks on Assign button in \"Assign Clusters\" dialog", () => clickOn(page, el("Assign button in \"Assign Clusters\" dialog")));
      await session.step(74, "Then the \"Assign Clusters\" dialog should close", () => dialogCloses(page, "Assign Clusters"));
      await session.step(75, "And the table should have a column \"Cluster (11.06)\"", () => hasColumn(page, "Cluster (11.06)"));
      await session.step(76, "And \"Cluster (11.06)\" column should have no missing values", () => columnComplete(page, "Cluster (11.06)"));
      await session.step(77, "And the newest column matching \"^Cluster \\(\" should have 5 distinct values", () => newestMatchingDistinct(page, "^Cluster \\(", 5));
    ...
    await run.scenario("A threshold above the tree's height is refused by the dialog", async () => {
      ...
      await session.step(82, "And user enters \"100\" into Threshold input in \"Assign Clusters\" dialog", () => enterInto(page, "100", el("Threshold input in \"Assign Clusters\" dialog")));
      await session.step(83, "Then Threshold input in \"Assign Clusters\" dialog should be invalid", () => shouldBe(page, el("Threshold input in \"Assign Clusters\" dialog"), "invalid"));
      await session.step(84, "When user closes \"Assign Clusters\" dialog", () => close(page, el("\"Assign Clusters\" dialog")));
      await session.step(85, "Then the \"Assign Clusters\" dialog should close", () => dialogCloses(page, "Assign Clusters"));
