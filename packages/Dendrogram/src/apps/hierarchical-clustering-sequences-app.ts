...
export class HierarchicalClusteringSequencesApp {
  ...
  async loadData(df?: DG.DataFrame): Promise<void> {
    if (!df) {
      const csv: string = await _package.files.readAsText('data/Prots.csv');
  ...
  private async buildView(): Promise<void> {
    ...
    this.tv.path = this.tv.basePath = `/func/${_package.name}.hierarchicalClusteringSequencesApp`;
    this.tv.dataFrame.onSemanticTypeDetected.subscribe(async () => {
      await hierarchicalClusteringUI(this.df, ['AlignedSequence'], DistanceMetric.Euclidean, 'ward');
    });
  }
}
