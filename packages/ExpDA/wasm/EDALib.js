...
var EDALib = undefined;

async function initEDALib() {
  if (EDALib === undefined) {
    console.log("Wasm not Loaded, Loading");
    EDALib = await exportEDALib();
    EDALib.principalComponentAnalysis = principalComponentAnalysis;
    EDALib.error = error;
    EDALib.partialLeastSquareRegression = partialLeastSquareRegression;
    EDALib.generateDataset = generateDataset;
    EDALib.normalizeDataset = normalizeDataset;
    EDALib.trainLSSVM = trainLSSVM;
    EDALib.predictByLSSVM = predictByLSSVM;
    EDALib.trainAndAnalyzeLSSVM = trainAndAnalyzeLSSVM;
  } else {
