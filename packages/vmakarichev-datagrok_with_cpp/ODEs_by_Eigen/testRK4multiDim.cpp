...
// Implementation of the function testRK4multiDim() - test of Runge-Kutta method of the order 4: multi-dimensional case
...
{
	cout << "Test of Runge-Kutta method of the order 4: multi-dimensional case.\n";
	...
	cout << oneStepExplicitSolver(f, times, N, yInitial.data(), DIM, getNextPointRK4, solution) << endl;
	...
	for (int i = 0; i < N; i++)
	{
		...
		cout << setw(7) << setiosflags(ios::right) << times[i]
			<< setw(19) << solution[2 * i]
			<< setw(18) << solution[2 * i + 1]
			<< setw(18) << yExact(0)
			<< setw(18) << yExact(1)
			<< setw(22) << fmax(fabs(solution[2 * i] - yExact(0)), fabs(solution[2 * i + 1] - yExact(1)))
			<< endl;
	}
