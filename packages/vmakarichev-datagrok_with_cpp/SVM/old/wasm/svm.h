...
namespace svm {
	...
	*/
	...
	/*  Predict labels of the target data using linear kernel model and precomputed weigths.	      
		  ...
		  weights - weights of the model (precomputed earlier)
		  targetData - target data
		  labels - target labels
		  targetSamplesCount - number of target samples	*/
	template<typename Float>
	int predictByLSSVMwithLinearKernelWithPrecomputedWeights(int featuresCount,
		Float* means, Float* stdDevs, Float* weights,
		Float* targetData, Float* labels, int targetSamplesCount)
	{
		...
		// matrix for normalized target data
		Matrix<Float, Dynamic, Dynamic, RowMajor> X(targetSamplesCount, featuresCount);		
		...
		// bias of the model
		Float bias = weights[featuresCount];
		...
		for (int i = 0; i < targetSamplesCount; i++)
		{
			// put target data to model
			Float cost = bias + w.dot(X.row(i));
		...
		return NO_ERRORS;
	} // predictByLSSVMwithLinearKernelWithPrecomputedWeights
		...
		Float* targetData, Float* labels, int targetSamplesCount)
	{
		...
		{
		case LINEAR: // the case of linear kernel
			return predictByLSSVMwithLinearKernelWithPrecomputedWeights(featuresCount,
				means, stdDevs, precomputedWeights,	targetData, labels, targetSamplesCount);
		default:
			return UNKNOWN_KERNEL_TYPE;
		}
	} // predictByLSSVM
