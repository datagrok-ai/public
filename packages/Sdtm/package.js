class SDTMPackage extends DG.Package {
    ...
    startApp(context) {
        ...
        function addQuotes(str) { return str.includes(' ') ? `"${str}"` : str; }

        function paramToInPattern(param) { return (param.length > 0) ? 'in (' + param.map(addQuotes).join(',') + ')' : null; }
        ...
        function addFilters(updateResults = true) {
            ...
            let studiesSelector = selector('SDTM:Studies', {
                'race': paramToInPattern(raceParam)}, 'study', 'Studies', studiesParam, (studies) => {
                studiesParam = studies;
                removeFilters(studiesSelector);
                let testsSelector = selector('SDTM:Tests', {'studies': paramToInPattern(studiesParam),
                    'race': paramToInPattern(raceParam)}, 'test', 'Tests', testsParam, (tests) => {
                    testsParam = tests;
                    removeFilters(testsSelector);
                    let methodSelector = selector('SDTM:Methods', {
                        'studies': paramToInPattern(studiesParam),
                        'tests': paramToInPattern(testsParam),
                        'race': paramToInPattern(raceParam)}, 'method', 'Methods', methodParam, (method) => {
                        methodParam = method;
                        if (updateResults)
                            queryUpdateResult();
                    });
