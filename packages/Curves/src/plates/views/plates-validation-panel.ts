...
export function renderValidationResults(plate: Plate, template: PlateTemplate): HTMLElement {
  ...
  for (const plateCol of plateColumns) {
    ...
    if (!isRequired) {
      validationData.push({
        'Template Field': '—',
        'Expected Type': '—',
        'Found Column': plateCol.originalName,
  ...
  if (validationData.length === 0)
    return ui.divText('No well properties are defined in the selected template to validate against.');

  return ui.divV([
    ui.h2('Template Validation'),
