...
export class PlateWidget extends DG.Widget {
 ...
 (layerName === PLATE_OUTLIER_WELL_NAME && pane.name.includes('Outliers'))
  ...
  /**
* [MODIFIED] This popup now allows users to enter a custom role name.
*/
  private showRoleAssignmentPopup(): void {
    ...
    const assignAction = () => {
      ...
      for (const i of selectedIndexes)
        (roleCol as DG.Column<string>).set(i, roleName);
  ...
  /**
* [MODIFIED] This method now adds color swatches to layout tabs and prioritizes
* the 'Role' column for coloring wells on the summary grid.
*/
  refresh() {
    this.tabs.clear();
    this.grids.clear();
    ...
    for (const type of [LayerType.ORIGINAL, LayerType.LAYOUT, LayerType.DERIVED]) {
      for (const layerName of layerInfo[type].layers) {
        ...
        const pane = this.tabs.panes[this.tabs.panes.length - 1];
        if (type === LayerType.LAYOUT) {
          const layoutCol = this.plate.data.col(layerName);
          if (layoutCol && layoutCol.isCategorical) {
            const colors = layoutCol.meta.colors;
            const categories = layoutCol.categories;
            const swatchContainer = ui.divH([], {style: {marginRight: '8px', gap: '4px', alignItems: 'center'}});

            for (const category of categories) {
              if (category == null) continue;
              const color = DG.Color.toHtml(colors.getColor(layoutCol.toList().indexOf(category)));
              const swatch = ui.div('', {
                style: {
                  width: '10px',
                  height: '10px',
              ...
              ui.tooltip.bind(swatch, category);
              swatchContainer.appendChild(swatch);
            }
            pane.header.prepend(swatchContainer);
          }
        }
      }
    }
