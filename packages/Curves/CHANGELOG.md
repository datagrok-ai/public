...
* GROK-17637: Known limitation: a legacy x-space statistic name (`interceptX`) that a fit function has no field of its own for falls back to the parameter slot the pre-typed API read, and that slot is not converted out of log space. A custom JS fit function with three or more parameters therefore reports it in fit space under a logarithmic x axis, on the plot and in an extracted column alike. Every built-in fit function resolves the name to a real field and is unaffected
...
* GROK-17637: Tests: Saving a project from a table with a Column level option and reopening it restores that option and keeps it outranking the data - modelled on the Chem project tests, verified to fail when the explicit override is removed
