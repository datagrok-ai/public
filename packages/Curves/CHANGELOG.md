...
* Fit options: Fixed a Column or Dataframe level change having no effect on a cell where the same property had been set at cell level. The cell-level write did not record that the cell now overrides the property, so the pass that clears those overrides was skipped
