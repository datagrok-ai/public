# SignalsLink — Third-Party Libraries

The `@datagrok/signals-link` package is distributed under the MIT license that
covers the rest of the `public/` repository (see [`../../LICENSE.md`](../../LICENSE.md)).

This package is a thin REST-API client for the Revvity Signals ELN; it
...
These libraries are not bundled into SignalsLink's `dist/` — they are provided
...
The package calls a customer-configured **Revvity Signals ELN** endpoint via
`grok.dapi.fetchProxy`. No Revvity SDK or JS code is bundled or redistributed
by this plugin; access to the Revvity Signals service is governed entirely by
the customer's own Revvity license.
...
as part of the published SignalsLink plugin and impose no obligation on users
of the plugin.
