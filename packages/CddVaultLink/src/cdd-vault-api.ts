...
interface ProtocolQueryParams {
  ...
  modified_before?: string;
  modified_after?: string;
  runs_modified_before?: string;
  runs_modified_after?: string;
  plates?: string;
  molecules?: string;
  offset?: number;
  page_size?: number;
...
export async function queryVaults(): Promise<ApiResponse<Vault[]>> {
  return request('GET', '/api/v1/vaults');
}

/** Get all available projects for vault */
export async function queryProjects(vaultId: number): Promise<ApiResponse<Project[]>> {
  return request('GET', `/api/v1/vaults/${vaultId}/projects`);
}
...
export async function queryProtocols(vaultId: number): Promise<ApiResponse<ProtocolQueryResult>> {
...
export async function queryBatchesAsync(vaultId: number, params: BatchQueryParams): Promise<ApiResponse<ExportStatus>> {
  params.async = true;
  const paramsStr = paramsStringFromObj(params);
