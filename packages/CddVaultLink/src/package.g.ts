...
//input: int protocol { category: Protocol; nullable: true; description: Protocol id }
//input: int run { category: Protocol; nullable: true; description: Specific run id }
//input: int page_size { nullable: true; description: Page size for preview (defaults to PREVIEW_ROW_NUM if omitted) }
//output: dataframe result
//meta.cache: all
//meta.cache.invalidateOn: 0 0 * * *
//editor: Cddvaultlink:CDDVaultSearchEditor
export async function cDDVaultSearch(vaultId: number, structure?: string, structure_search_type?: any, structure_similarity_threshold?: number, protocol?: number, run?: number, page_size?: number) : Promise<any> {
  return await PackageFunctions.cDDVaultSearch(vaultId, structure, structure_search_type, structure_similarity_threshold, protocol, run, page_size);
}
