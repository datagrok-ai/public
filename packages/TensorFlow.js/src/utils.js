...
export function processFeatures(dataframe) {
    let array2d = [];
    let val;
    for (let i = 0; i < dataframe.rowCount; i++) {
        ...
        for (let col of dataframe.columns) {
            ...
            if (col.type === DG.TYPE.STRING) {
                ...
                val = col.categories.findIndex((cat) => cat === val);
...
export function processLables(column) {
    let labels = [];
    if (column.type === DG.TYPE.STRING) {
        labels = tf.tensor1d(column.toList().map((value) => column.categories.findIndex((cat) => cat === value)), 'float32');
...
export async function loadModel(modelArray) {
