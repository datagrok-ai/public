...
Feature: Pareto front objectives
  ...
  its canvas. Axes and labels are claimed on the nested scatter plot as well as on the parent
  properties, so a broken forwarding handler cannot pass.
  ...
  Scenario: The viewer's properties come in their categories, and only numeric columns are offered
    ...
    When user clicks on "..." button in "Minimize" property in context panel
    Then the "rows" reading of grid viewer in "Select columns..." dialog should be 16
    And the "text of cell 1 of __name" reading of grid viewer in "Select columns..." dialog should be "highway.mpg"
    When user types "model" into Search input in "Select columns..." dialog
    Then the "rows shown" reading of grid viewer in "Select columns..." dialog should be 0
    When user clicks on CANCEL button in "Select columns..." dialog
    Then "Minimize" property of pareto front viewer should be "highway.mpg, price"
  ...
  Scenario: An axis and the labels chosen by hand turn their automatic choice off
    ...
    And "X Column Name" property of scatter plot viewer in pareto front viewer should be "horsepower"
  ...
  Scenario: Cancelling the objective picker preserves the viewer's objectives
    ...
    And "Maximize" property of pareto front viewer should be ""
    And no errors should have been logged
