...
test.describe("Pareto front objectives", () => {
  ...
  test("Pareto front objectives", {tag: ["@journey", "@eda", "@realizes:eda.viewer.pareto-front"]}, async ({browser}) => {
    ...
    await run.scenario("Maximizing what is minimized is refused with a warning, and choosing again clears it", async () => {
      ...
      await session.step(62, "And pareto front viewer should contain text \"Cannot minimize and maximize features at the same time: \\\"highway.mpg\\\", \\\"price\\\"\"", () => shouldContainText(page, el("pareto front viewer"), "Cannot minimize and maximize features at the same time: \"highway.mpg\", \"price\""));
      await session.step(63, "When user clicks on \"...\" button in \"Maximize\" property in context panel", () => clickOn(page, el("\"...\" button in \"Maximize\" property in context panel")));
      await session.step(64, "And user clicks on None label in \"Select columns...\" dialog", () => clickOn(page, el("None label in \"Select columns...\" dialog")));
      await session.step(65, "And user clicks on OK button in \"Select columns...\" dialog", () => clickOn(page, el("OK button in \"Select columns...\" dialog")));
      await session.step(66, "Then \"Maximize\" property of pareto front viewer should be \"\"", () => propertyShouldBe(page, "Maximize", el("pareto front viewer"), ""));
      await session.step(67, "And pareto front viewer should not contain text \"Cannot minimize and maximize\"", () => shouldNotContainText(page, el("pareto front viewer"), "Cannot minimize and maximize"));
    ...
    await run.scenario("An axis and the labels chosen by hand turn their automatic choice off", async () => {
      await session.step(71, "Given \"Axes\" category in context panel is expanded", () => isExpanded(page, el("\"Axes\" category in context panel")));
      await session.step(72, "When user selects \"horsepower\" in \"X Axis\" property in context panel", () => selectIn(page, "horsepower", el("\"X Axis\" property in context panel")));
      await session.step(73, "Then \"X Axis\" property of pareto front viewer should be \"horsepower\"", () => propertyShouldBe(page, "X Axis", el("pareto front viewer"), "horsepower"));
      await session.step(74, "And \"Auto Axes Selection\" property of pareto front viewer should not be \"true\"", () => propertyShouldNotBe(page, "Auto Axes Selection", el("pareto front viewer"), "true"));
      await session.step(75, "And \"X Column Name\" property of scatter plot viewer in pareto front viewer should be \"horsepower\"", () => propertyShouldBe(page, "X Column Name", el("scatter plot viewer in pareto front viewer"), "horsepower"));
      await session.step(76, "And scatter plot viewer in pareto front viewer should be painted", () => painted(page, el("scatter plot viewer in pareto front viewer")));
      await session.step(77, "Given \"Labels\" category in context panel is expanded", () => isExpanded(page, el("\"Labels\" category in context panel")));
      await session.step(78, "When user clicks on \"...\" button in \"Label Columns\" property in context panel", () => clickOn(page, el("\"...\" button in \"Label Columns\" property in context panel")));
      await session.step(79, "And user clicks on None label in \"Select columns...\" dialog", () => clickOn(page, el("None label in \"Select columns...\" dialog")));
      ...
      await session.step(81, "Then \"Label Columns\" property of pareto front viewer should be \"\"", () => propertyShouldBe(page, "Label Columns", el("pareto front viewer"), ""));
      await session.step(82, "And \"Auto Labels Selection\" property of pareto front viewer should not be \"true\"", () => propertyShouldNotBe(page, "Auto Labels Selection", el("pareto front viewer"), "true"));
      await session.step(83, "And the \"labels shown\" reading of scatter plot viewer in pareto front viewer should be 0", () => readingIs(page, "labels shown", el("scatter plot viewer in pareto front viewer"), 0));
      await session.step(84, "And no errors should have been logged", () => noErrors(page));
    });
    await run.scenario("An empty numeric column is not offered as an objective", async () => {
      await session.step(87, "When user adds a calculated column \"empty\" with formula \"If(true, null, 0)\"", () => addCalculated(page, "empty", "If(true, null, 0)"));
      await session.step(88, "Then \"empty\" column should have type \"int\"", () => columnType(page, "empty", "int"));
      await session.step(89, "And \"empty\" column should have missing values", () => columnIncomplete(page, "empty"));
      await session.step(90, "When user clicks on \"...\" button in \"Maximize\" property in context panel", () => clickOn(page, el("\"...\" button in \"Maximize\" property in context panel")));
      await session.step(91, "Then \"Select columns...\" dialog should be visible", () => shouldBe(page, el("\"Select columns...\" dialog"), "visible"));
      await session.step(92, "And the \"rows\" reading of grid viewer in \"Select columns...\" dialog should be 16", () => readingIs(page, "rows", el("grid viewer in \"Select columns...\" dialog"), 16));
    ...
    await run.scenario("Cancelling the objective picker preserves the viewer's objectives", async () => {
      await session.step(96, "When user clicks on CANCEL button in \"Select columns...\" dialog", () => clickOn(page, el("CANCEL button in \"Select columns...\" dialog")));
      await session.step(97, "Then \"Select columns...\" dialog should be absent", () => shouldBe(page, el("\"Select columns...\" dialog"), "absent"));
      await session.step(98, "And \"Maximize\" property of pareto front viewer should be \"\"", () => propertyShouldBe(page, "Maximize", el("pareto front viewer"), ""));
