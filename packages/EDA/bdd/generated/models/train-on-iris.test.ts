...
test.describe("Training a model to classify iris species", () => {
  ...
  test("Training a model to classify iris species", {tag: ["@journey", "@eda", "@realizes:ml.menu.models.train-model", "@realizes:eda.model.softmax", "@realizes:eda.model.xg-boost"]}, async ({browser}) => {
    ...
    await run.scenario("Softmax classifies the species and retrains as its hyperparameters move", async () => {
      ...
      await session.step(48, "Then Rate input should have a value between 9.5 and 10.5", () => shouldHaveValueBetween(page, el("Rate input"), 9.5, 10.5));
      await session.step(49, "And \"rate\" table row should not contain text \"rate2\"", () => shouldNotContainText(page, el("\"rate\" table row"), "rate2"));
      await session.step(50, "When user drags the slider of Penalty input to 0.5", () => dragSliderTo(page, el("Penalty input"), 0.5));
      await session.step(51, "Then Penalty input should have a value between 0.48 and 0.52", () => shouldHaveValueBetween(page, el("Penalty input"), 0.48, 0.52));
      await session.step(52, "And \"penalty\" table row should not contain text \"penalty0.10\"", () => shouldNotContainText(page, el("\"penalty\" table row"), "penalty0.10"));
    ...
    await run.scenario("XGBoost classifies the species and retrains as its clickers and sliders move", async () => {
      await session.step(58, "When user selects \"Eda: XGBoost\" in \"Model Engine\" input", () => selectIn(page, "Eda: XGBoost", el("\"Model Engine\" input")));
      await session.step(59, "And user enters \"20\" into Iterations input", () => enterInto(page, "20", el("Iterations input")));
      await session.step(60, "And user enters \"6\" into \"Max Depth\" input", () => enterInto(page, "6", el("\"Max Depth\" input")));
      await session.step(61, "And user enters \"0.3\" into Rate input", () => enterInto(page, "0.3", el("Rate input")));
      await session.step(62, "And user enters \"1\" into Lambda input", () => enterInto(page, "1", el("Lambda input")));
      await session.step(63, "And user enters \"0\" into Alpha input", () => enterInto(page, "0", el("Alpha input")));
      await session.step(64, "Then \"Eda: XGBoost\" heading should be visible", () => shouldBe(page, el("\"Eda: XGBoost\" heading"), "visible"));
      await session.step(65, "And \"Accuracy\" table row should be visible", () => shouldBe(page, el("\"Accuracy\" table row"), "visible"));
      await session.step(66, "And \"iterations\" table row should contain text \"20\"", () => shouldContainText(page, el("\"iterations\" table row"), "20"));
      await session.step(67, "When user hovers over Iterations input", () => hoverOver(page, el("Iterations input")));
      await session.step(68, "And user clicks on plus icon in Iterations input", () => clickOn(page, el("plus icon in Iterations input")));
      await session.step(69, "Then \"iterations\" table row should contain text \"21\"", () => shouldContainText(page, el("\"iterations\" table row"), "21"));
      await session.step(70, "When user hovers over \"Max Depth\" input", () => hoverOver(page, el("\"Max Depth\" input")));
      await session.step(71, "And user clicks on minus icon in \"Max Depth\" input", () => clickOn(page, el("minus icon in \"Max Depth\" input")));
      await session.step(72, "Then \"maxDepth\" table row should contain text \"5\"", () => shouldContainText(page, el("\"maxDepth\" table row"), "5"));
      await session.step(73, "When user drags the slider of Rate input to 0.5", () => dragSliderTo(page, el("Rate input"), 0.5));
      await session.step(74, "Then Rate input should have a value between 0.48 and 0.52", () => shouldHaveValueBetween(page, el("Rate input"), 0.48, 0.52));
      await session.step(75, "And \"eta\" table row should not contain text \"eta0.30\"", () => shouldNotContainText(page, el("\"eta\" table row"), "eta0.30"));
      await session.step(76, "When user drags the slider of Lambda input to 50", () => dragSliderTo(page, el("Lambda input"), 50));
      await session.step(77, "Then Lambda input should have a value between 48 and 52", () => shouldHaveValueBetween(page, el("Lambda input"), 48, 52));
      await session.step(78, "And \"lambda\" table row should not contain text \"lambda1\"", () => shouldNotContainText(page, el("\"lambda\" table row"), "lambda1"));
      await session.step(79, "When user drags the slider of Alpha input to 40", () => dragSliderTo(page, el("Alpha input"), 40));
      await session.step(80, "Then Alpha input should have a value between 38 and 42", () => shouldHaveValueBetween(page, el("Alpha input"), 38, 42));
      await session.step(81, "And \"alpha\" table row should not contain text \"alpha0\"", () => shouldNotContainText(page, el("\"alpha\" table row"), "alpha0"));
      await session.step(82, "And \"Accuracy\" table row should be visible", () => shouldBe(page, el("\"Accuracy\" table row"), "visible"));
