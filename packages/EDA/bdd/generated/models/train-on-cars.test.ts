...
test.describe("Training a model to predict the price of a car", () => {
  ...
  test("Training a model to predict the price of a car", {tag: ["@journey", "@eda", "@realizes:ml.menu.models.train-model", "@realizes:eda.model.linear-regression", "@realizes:eda.model.pls-regression", "@realizes:eda.model.xg-boost"]}, async ({browser}) => {
    ...
    await run.scenario("XGBoost retrains as its clickers and sliders move", async () => {
      ...
      await session.step(88, "Then \"eta\" table row should contain text \"eta0.30\"", () => shouldContainText(page, el("\"eta\" table row"), "eta0.30"));
      await session.step(89, "And \"lambda\" table row should contain text \"lambda1\"", () => shouldContainText(page, el("\"lambda\" table row"), "lambda1"));
      await session.step(90, "And \"alpha\" table row should contain text \"alpha0\"", () => shouldContainText(page, el("\"alpha\" table row"), "alpha0"));
      await session.step(91, "When user drags the slider of Rate input to 0.5", () => dragSliderTo(page, el("Rate input"), 0.5));
      await session.step(92, "Then Rate input should have a value between 0.48 and 0.52", () => shouldHaveValueBetween(page, el("Rate input"), 0.48, 0.52));
      await session.step(93, "And \"eta\" table row should not contain text \"eta0.30\"", () => shouldNotContainText(page, el("\"eta\" table row"), "eta0.30"));
      await session.step(94, "When user drags the slider of Lambda input to 50", () => dragSliderTo(page, el("Lambda input"), 50));
      await session.step(95, "Then Lambda input should have a value between 48 and 52", () => shouldHaveValueBetween(page, el("Lambda input"), 48, 52));
      await session.step(96, "And \"lambda\" table row should not contain text \"lambda1\"", () => shouldNotContainText(page, el("\"lambda\" table row"), "lambda1"));
      await session.step(97, "When user drags the slider of Alpha input to 40", () => dragSliderTo(page, el("Alpha input"), 40));
      await session.step(98, "Then Alpha input should have a value between 38 and 42", () => shouldHaveValueBetween(page, el("Alpha input"), 38, 42));
      await session.step(99, "And \"alpha\" table row should not contain text \"alpha0\"", () => shouldNotContainText(page, el("\"alpha\" table row"), "alpha0"));
      await session.step(100, "And \"R squared\" table row should be visible", () => shouldBe(page, el("\"R squared\" table row"), "visible"));
