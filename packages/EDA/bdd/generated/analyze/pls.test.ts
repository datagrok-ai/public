...
test.describe("Partial least squares regression", () => {
  ...
  test("Partial least squares regression", {tag: ["@journey", "@eda", "@realizes:ml.menu.analyze.pls"]}, async ({browser}) => {
    ...
    await run.scenario("With price taken back out, RUN adds three PLS components", async () => {
      ...
      await session.step(43, "Then \"PLS\" dialog should be hidden", () => shouldBe(page, el("\"PLS\" dialog"), "hidden"));
      await session.step(44, "And 3 new columns should have been added", () => newColumnsCount(page, 3));
      await session.step(45, "And the table should have a column \"PLS1\"", () => hasColumn(page, "PLS1"));
      await session.step(46, "And the table should have a column \"PLS2\"", () => hasColumn(page, "PLS2"));
      await session.step(47, "And the table should have a column \"PLS3\"", () => hasColumn(page, "PLS3"));
      await session.step(48, "And \"PLS1\" column should have no missing values", () => columnComplete(page, "PLS1"));
      await session.step(49, "And \"PLS2\" column should have no missing values", () => columnComplete(page, "PLS2"));
      await session.step(50, "And \"PLS3\" column should have no missing values", () => columnComplete(page, "PLS3"));
      await session.step(51, "And \"PLS1\" column should have at least 2 distinct values", () => distinctValues(page, "PLS1", 2));
      await session.step(52, "And \"PLS2\" column should have at least 2 distinct values", () => distinctValues(page, "PLS2", 2));
      await session.step(53, "And \"PLS3\" column should have at least 2 distinct values", () => distinctValues(page, "PLS3", 2));
