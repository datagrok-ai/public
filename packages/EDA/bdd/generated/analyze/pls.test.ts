...
test.describe("Partial least squares regression", () => {
  ...
  test("Partial least squares regression", {tag: ["@journey", "@eda", "@realizes:ml.menu.analyze.pls"]}, async ({browser}) => {
    ...
    await run.scenario("With price taken back out, RUN adds three PLS components", async () => {
      ...
      await session.step(50, "And \"PLS2\" column should have no missing values", () => columnComplete(page, "PLS2"));
      await session.step(51, "And \"PLS3\" column should have no missing values", () => columnComplete(page, "PLS3"));
      await session.step(52, "And \"PLS1\" column should have at least 2 distinct values", () => distinctValues(page, "PLS1", 2));
      await session.step(53, "And \"PLS2\" column should have at least 2 distinct values", () => distinctValues(page, "PLS2", 2));
      await session.step(54, "And \"PLS3\" column should have at least 2 distinct values", () => distinctValues(page, "PLS3", 2));
