...
test.describe("Group comparison of a filtered table", () => {
  ...
  test("Group comparison of a filtered table", {tag: ["@journey", "@eda", "@realizes:ml.menu.analyze.group-comparison.control-comparisons", "@known-failure"]}, async ({browser}) => {
    ...
    await run.scenario("Control comparisons count only the women when the table is filtered to them", async () => {
      ...
      await session.step(20, "And user clicks on Run button in \"Control comparisons\" dialog", () => clickOn(page, el("Run button in \"Control comparisons\" dialog")));
      await session.step(21, "Then the top menu command should have completed", () => commandCompleted(page));
      await session.step(22, "And second grid viewer should be bound to table \"Control comparisons result\"", () => boundTable(page, el("second grid viewer"), "Control comparisons result"));
      await session.step(23, "And the \"text of cell 1 of Group\" reading of second grid viewer should be \"Black\"", () => readingReads(page, "text of cell 1 of Group", el("second grid viewer"), "Black"));
      await session.step(24, "And the \"text of cell 1 of n\" reading of second grid viewer should be \"104\"", () => readingReads(page, "text of cell 1 of n", el("second grid viewer"), "104"));
      await session.step(25, "And the \"text of cell 2 of n\" reading of second grid viewer should be \"2823\"", () => readingReads(page, "text of cell 2 of n", el("second grid viewer"), "2823"));
      await session.step(26, "And the \"text of cell 3 of n\" reading of second grid viewer should be \"279\"", () => readingReads(page, "text of cell 3 of n", el("second grid viewer"), "279"));
    }, {knownFailure: true});
    run.finish();
  });
});
