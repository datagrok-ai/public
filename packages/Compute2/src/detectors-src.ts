/* eslint-disable */
// TypeScript source of ../detectors.js — compiled by `tsc -p tsconfig.detectors.json` (plain
// script output, no bundler: the platform evaluates detectors.js as a global script and scans
// its annotation comments, so the emitted file must stay module-free with comments intact).
declare const DG: typeof import('datagrok-api/dg');
declare const grok: typeof import('datagrok-api/grok');
declare const ui: typeof import('datagrok-api/ui');
...
function openComputeRun(id: string): Promise<any> {
  return DG.Func.byName('Compute2:OpenWorkflowRun').prepare({id: id}).call();
}

function computeRunEditorOf(call: any): string | null {
...
class ComputeRunHandler extends DG.ObjectHandler {
  ...
  subtitleText(call: any, author: string): string {
    const tags = ((call.options['tags'] || []) as string[]).map((t) => '#' + t).join(' ');
    return [this.modelName(call), this.startedText(call), author, tags].filter((s) => s).join(' · ');
  }

  async authorOf(call: any): Promise<string> {
    if (call.author && call.author.friendlyName)
      return call.author.friendlyName;
    try {
      const full: any = await grok.dapi.functions.calls.allPackageVersions()
  ...
  async patchHostItem(icon: HTMLElement, call: any): Promise<void> {
  ...
  tryPatchOnce(icon: HTMLElement, call: any): boolean {
  ...
  renderCard(x: any): HTMLElement {
    const call = DG.toJs(x);
    const subtitle = ui.divText(this.subtitleText(call, (call.author && call.author.friendlyName) || ''),
      {style: {color: 'var(--grey-4)'}});
    void this.authorOf(call).then((author) => {
      subtitle.textContent = this.subtitleText(call, author);
    });
    const card = ui.divV([
      ui.divH([ui.iconFA('play-circle'), ui.divText(this.getCaption(call), {style: {fontWeight: 'bold'}})],
  ...
  renderProperties(x: any): HTMLElement {
    const call = DG.toJs(x);
    const info = ui.div([]);
    const fill = (author: string) => {
      ui.empty(info);
      const map: {[key: string]: string} = {'Model': this.modelName(call), 'Started': this.startedText(call)};
      if (author)
        map['Author'] = author;
      const tags = ((call.options['tags'] || []) as string[]).join(', ');
      if (tags)
        map['Tags'] = tags;
  ...
  async renderPreview(x: any): Promise<any> {
    const call = DG.toJs(x);
  ...
  init(): void {
    this.registerParamFunc('Open run', (x: any) => void openComputeRun(DG.toJs(x).id));
  }
}
