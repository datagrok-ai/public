...
import {clickArea, hoverArea, noBalloons, noErrors, pointerAway, readingIs, readingReads, repainted, takeSnapshot, viewerAdded, viewerCount} from '@datagrok-libraries/bdd/bindings/tiers/viewers/steps';
...
test.describe("Inspect peptide statistics in tooltips", () => {
  ...
  test("Inspect peptide statistics in tooltips", {tag: ["@journey"]}, async ({browser}) => {
    ...
    await run.scenario("WebLogo header tooltips replace their statistics and clear the highlight on leaving", async () => {
      await session.step(92, "When user moves the pointer away from Sequence Variability Map viewer", () => pointerAway(page, el("Sequence Variability Map viewer")));
      await session.step(93, "And user hovers over the \"A at 2\" area of grid", () => hoverArea(page, "A at 2", el("grid")));
      await session.step(94, "Then Count table row in tooltip should contain text \"14 (14.000%)\"", () => shouldContainText(page, el("Count table row in tooltip"), "14 (14.000%)"));
      await session.step(95, "And the \"highlighted rows\" reading of grid should be 14", () => readingIs(page, "highlighted rows", el("grid"), 14));
      await session.step(96, "When user hovers over the \"N at 4\" area of grid", () => hoverArea(page, "N at 4", el("grid")));
      await session.step(97, "Then Count table row in tooltip should contain text \"74 (74.000%)\"", () => shouldContainText(page, el("Count table row in tooltip"), "74 (74.000%)"));
      await session.step(98, "And the \"highlighted rows\" reading of grid should be 74", () => readingIs(page, "highlighted rows", el("grid"), 74));
      ...
      await session.step(101, "And the \"highlighted rows\" reading of grid should be 0", () => readingIs(page, "highlighted rows", el("grid"), 0));
    ...
    await run.scenario("Cluster tooltips and selection use the computed cluster membership", async () => {
      ...
      await session.step(108, "And the \"members total\" reading of Logo Summary Table viewer should be 100", () => readingIs(page, "members total", el("Logo Summary Table viewer"), 100));
      await session.step(109, "And the cluster summaries should match the source rows of table \"peptides\"", () => clusterSummaries(page, "peptides"));
      await session.step(110, "When user hovers over the \"cluster 1\" area of Logo Summary Table viewer", () => hoverArea(page, "cluster 1", el("Logo Summary Table viewer")));
      await session.step(111, "Then the cluster statistics in tooltip should match cluster \"1\" of table \"peptides\"", () => clusterStatistics(page, el("tooltip"), "1", "peptides"));
      await session.step(112, "When user hovers over the \"cluster 2\" area of Logo Summary Table viewer", () => hoverArea(page, "cluster 2", el("Logo Summary Table viewer")));
      await session.step(113, "Then the cluster statistics in tooltip should match cluster \"2\" of table \"peptides\"", () => clusterStatistics(page, el("tooltip"), "2", "peptides"));
      await session.step(114, "When user clicks on the \"cluster 1\" area of Logo Summary Table viewer", () => clickArea(page, "cluster 1", el("Logo Summary Table viewer")));
      await session.step(115, "Then only rows where \"Cluster (MCL)\" is \"1\" should be selected", () => onlyOfSelected(page, "Cluster (MCL)", "1"));
      await session.step(116, "And the \"selected clusters\" reading of Logo Summary Table viewer should be \"1\"", () => readingReads(page, "selected clusters", el("Logo Summary Table viewer"), "1"));
      await session.step(117, "When user expands Distribution pane in context panel", () => expand(page, el("Distribution pane in context panel")));
      await session.step(118, "Then the cluster statistics in Distribution pane in context panel should match cluster \"1\" of table \"peptides\"", () => clusterStatistics(page, el("Distribution pane in context panel"), "1", "peptides"));
      await session.step(119, "When user clicks on the \"cluster 2\" area of Logo Summary Table viewer", () => clickArea(page, "cluster 2", el("Logo Summary Table viewer")));
      await session.step(120, "Then only rows where \"Cluster (MCL)\" is \"2\" should be selected", () => onlyOfSelected(page, "Cluster (MCL)", "2"));
      await session.step(121, "And the \"selected clusters\" reading of Logo Summary Table viewer should be \"2\"", () => readingReads(page, "selected clusters", el("Logo Summary Table viewer"), "2"));
      await session.step(122, "When user expands Distribution pane in context panel", () => expand(page, el("Distribution pane in context panel")));
      await session.step(123, "Then the cluster statistics in Distribution pane in context panel should match cluster \"2\" of table \"peptides\"", () => clusterStatistics(page, el("Distribution pane in context panel"), "2", "peptides"));
      await session.step(124, "When user clears the row selection", () => clearSelection(page));
      await session.step(125, "Then no rows should be selected", () => noneSelected(page));
      await session.step(126, "And the \"selected clusters\" reading of Logo Summary Table viewer should be \"\"", () => readingReads(page, "selected clusters", el("Logo Summary Table viewer"), ""));
