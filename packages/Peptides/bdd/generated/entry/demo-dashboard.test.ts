...
test.describe("Peptide SAR demo dashboard", () => {
  ...
  test("The demo builds a working dashboard on Simple peptides", async ({browser}) => {
    ...
    await session.step(29, "And the table should have a column \"Cluster (MCL)\"", () => hasColumn(page, "Cluster (MCL)"));
    await session.step(30, "And the open tableview should have 1 Sequence Variability Map viewer", () => viewerCount(page, 1, "Sequence Variability Map"));
    await session.step(31, "And the open tableview should have 1 Most Potent Residues viewer", () => viewerCount(page, 1, "Most Potent Residues"));
    await session.step(32, "And the open tableview should have 1 MCL viewer", () => viewerCount(page, 1, "MCL"));
    await session.step(33, "And the open tableview should have 1 Logo Summary Table viewer", () => viewerCount(page, 1, "Logo Summary Table"));
    await session.step(34, "And the \"positions\" reading of Sequence Variability Map viewer should be 15", () => readingIs(page, "positions", el("Sequence Variability Map viewer"), 15));
    await session.step(35, "And the \"activity scaling\" reading of Sequence Variability Map viewer should be \"-lg\"", () => readingReads(page, "activity scaling", el("Sequence Variability Map viewer"), "-lg"));
    ...
    await session.step(38, "And the \"positions\" reading of Most Potent Residues viewer should be 15", () => readingIs(page, "positions", el("Most Potent Residues viewer"), 15));
    await session.step(39, "And Logo Summary Table viewer should be painted", () => painted(page, el("Logo Summary Table viewer")));
    await session.step(40, "And the \"members total\" reading of Logo Summary Table viewer should be 647", () => readingIs(page, "members total", el("Logo Summary Table viewer"), 647));
    await session.step(41, "And scatter plot viewer in MCL viewer should be painted", () => painted(page, el("scatter plot viewer in MCL viewer")));
