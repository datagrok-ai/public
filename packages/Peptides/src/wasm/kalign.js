const initKalign = (() => {
  ...
  return function(initKalign) {
    initKalign = initKalign || {};

    const Module = typeof initKalign !== 'undefined' ? initKalign : {};
    ...
    let moduleOverrides = objAssign({}, Module);
    let arguments_ = [];
    ...
    var FS = {
      ...
      open: function(path, flags, mode, fd_start, fd_end) {
        ...
        const stream = FS.createStream(
          {node: node, path: FS.getPath(node), id: node.id, flags: flags, mode: node.mode, seekable: true, position: 0, stream_ops: node.stream_ops, node_ops: node.node_ops, ungotten: [], error: false},
          fd_start,
          fd_end,
        );
        if (stream.stream_ops.open) {
      ...
      const EXPANSION_RULES_2 = {
        ...
        '%U': function(date) {
          ...
          if (compareByDay(firstSunday, endDate) < 0) {
            const februaryFirstUntilEndMonth = __arraySum(__isLeapYear(endDate.getFullYear()) ? __MONTH_DAYS_LEAP : __MONTH_DAYS_REGULAR, endDate.getMonth() - 1) - 31;
        ...
        '%W': function(date) {
          ...
          const firstMonday = janFirst.getDay() === 1 ? janFirst : __addDays(janFirst, janFirst.getDay() === 0 ? 1 : 7 - janFirst.getDay() + 1);
          const endDate = new Date(date.tm_year + 1900, date.tm_mon, date.tm_mday);
    ...
    return initKalign.ready;
  };
})();
if (typeof exports === 'object' && typeof module === 'object') module.exports = initKalign;
else if (typeof define === 'function' && define['amd']) {
  define([], function() {
    return initKalign;
  });
} else if (typeof exports === 'object') exports['initKalign'] = initKalign;
