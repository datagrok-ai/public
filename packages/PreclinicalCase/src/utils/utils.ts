...
import {ACT_TRT_ARM, AGETXT, COL_HAS_ERRORS_POSTFIX, ERRORS_POSTFIX,
  HAS_VALIDATION_ERRORS_COL, PLANNED_TRT_ARM, SEX, SPECIES, SUBJECT_ID,
  VIOLATED_RULES_COL} from '../constants/columns-constants';
...
export async function readClinicalFile(file: DG.FileInfo): Promise<DG.DataFrame | null> {
...
export function studyConfigToMap(studyConfig: StudyConfig): {[key: string]: any} {
  ...
  for (const key of Object.keys(studyConfig)) {
    if (key !== 'other' && key !== 'fieldsDefinitions') {
      ...
      map[formattedKey] = (studyConfig as any)[key];
    }
  }
  if (studyConfig.other) {
