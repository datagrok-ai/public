...
// NB: registration has no undo — the handler stays active for the session.
...
const typeOf = (h) => { try { return h.type; } catch (_) { return null; } }; // base handlers may throw on .type
DG.ObjectHandler.list().find((h) => typeOf(h) === 'grit.issue').renderGrid(grid, {items: df});
