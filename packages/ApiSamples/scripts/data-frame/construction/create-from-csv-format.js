...
grok.shell.info(`Format detected on 'gdpPercap': ${table.getCol('gdpPercap').tags[DG.TAGS.FORMAT]}`);
grok.shell.info(`Format detected on 'lifeExp': ${table.getCol('lifeExp').tags[DG.TAGS.FORMAT]}`);
grok.shell.info(`Max significant digits detected for 'lifeExp': ${table.getCol('lifeExp').tags[DG.TAGS.SOURCE_PRECISION]}`);
