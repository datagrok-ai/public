...
let countries = await grok.data.query('ApiSamples:Countries', {});
let customersView = null;

grok.shell.addTableView(countries);
countries.onCurrentRowChanged.subscribe(async (_) => {
  let t = await grok.data.query('ApiSamples:CustomersInCountry', {country: countries.currentRow['country']});
