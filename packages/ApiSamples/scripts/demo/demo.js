...
// viewers
hist = view.addViewer('histogram', {'valueColumnName': 'weight'});

// markup viewer
let text = '# Summary generated by JavaScript';

function summary(col) {
  return `## ${col.name}\nType: ${col.type}\n\nMin: ${col.min}\n\nMax: ${col.max}\n\n`;
}

grok.shell.tables.forEach(t => text = text + '\n# ' + t.name + '\n\n' + t.columns.toList().map(summary).join('\n\n'));

markup = view.addViewer('markup', {'content': text});

// events
demog.onCurrentRowChanged.subscribe((_) => grok.shell.info('current row changed'));
grok.events.onEvent('d4-current-view-changed').subscribe((_) => grok.shell.info('view changed'));
...
grok.shell.info('Script executed.');
