...
* Added data-frame/bitset/bit-array.js — `DG.BitArray` built in JS (`create`/`set`/`trueCount`/`getSelectedIndexes`) and applied to a DataFrame filter through `copyFrom`, `toBitArray` and `BitSet.fromBitArray`
