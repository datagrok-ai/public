...
export class PackageFunctions {
  ...
  static async translationPanel(textfile: DG.FileInfo) : Promise<DG.Widget> {
  ...
  @grok.decorators.func({
    meta: {
      ...
      supportedDistanceFunctions: 'Common Items',
    },
    name: 'Stem Column',
    outputs: [{type: 'object', name: 'result'}],
  })
  static stemColumnPreprocessingFunction(
    @grok.decorators.param({options: {semType: 'Text'}}) col: DG.Column,
      metric: string,
    @grok.decorators.param({type: 'int', options: {min: '0', max: '100', optional: true, initialValue: '1'}}) minimumCharactersCount: number) {
  ...
  @grok.decorators.func({name: 'Radial Coloring'})
  static radialColoring(col1: DG.Column, col2: DG.Column) {
  ...
  static distance(@grok.decorators.param({options: {semType: 'Text'}}) query: string): DG.Widget {
  ...
  static similar(
    @grok.decorators.param({options: {semType: 'Text'}}) query: string): DG.Widget {
    const df = grok.shell.t;
    const source = df.currentCol;
  ...
  static async sentenceEmbeddingsPreprocessingFunction(
    @grok.decorators.param({options: {semType: 'Text'}}) col: DG.Column,
      metric: string) {
    const embeddings: Float32Array[] = await PackageFunctions.getEmbeddings(col.toList());
    return {entries: embeddings, options: {}};
  }


  @grok.decorators.func({outputs: [{type: 'string', name: 'result'}]})
  static async getEmbeddings(
    @grok.decorators.param({type: 'list<string>'}) sentences: string[]): Promise<Float32Array[]> {
  ...
  static sentenceSearchTopMenu(): void {
