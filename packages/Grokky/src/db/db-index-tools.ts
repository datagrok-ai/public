...
export async function genDBConnectionMeta(connection: DG.DataConnection, schemas: string[]) {
  ...
  //const enrichedMeta = await postProcessDBConnectionMeta(out);
  //return enrichedMeta;
  return out;
}

// Helper schemas for structured LLM output
const ConnectionCommentSchema = {
  type: 'object',
  properties: {
...
async function queryLLM(systemPrompt: string, userPrompt: string, outputSchema: object): Promise<any> {
  const client = ClaudeRuntimeClient.getInstance();
  const result = await client.query(`${systemPrompt}\n\n${userPrompt}`, {outputSchema});
 ...
 */
export async function postProcessDBConnectionMeta(connectionMeta: DBConnectionMeta): Promise<DBConnectionMeta> {
  ...
  const connectionComment = await queryLLM(
  ...
  for (const schema of result.schemas) {
    ...
    const schemaComment = await queryLLM(
    ...
    for (const table of schema.tables) {
      ...
      const tableComment = await queryLLM(
      ...
      const columnComments = await queryLLM(
...
Focus on non-obvious columns, semantic types, and domain-specific meanings.`,
      ...
      for (const colComment of columnComments.columns) {
        const col = table.columns.find((c: any) => c.name === colComment.name);
        if (col) {
  ...
  for (let i = 0; i < result.relations.length; i++) {
    ...
    const relationComment = await queryLLM(
...
4. Whether this is a primary/recommended join path (true) or a legacy/unusual join (false)`,
  ...
  const hiddenRelations = await queryLLM(
