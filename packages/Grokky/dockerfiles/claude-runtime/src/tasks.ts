 ...
 *  only this admission, the turn streams over the browser WebSocket. Fail-open: an unclaimed
 *  task delays a turn by CLAIM_WAIT_MS, never blocks it. */
...
export function waitForClaim(taskId: string, sessionId: string, signal: AbortSignal): Promise<boolean> {
  ...
  return new Promise((resolve) => {
    ...
    const timer = setTimeout(() => done(false), CLAIM_WAIT_MS);
