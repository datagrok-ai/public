...
|---|---|
| `chemblIdToSmilesTs(id: string)` | CHEMBL ID (e.g. `'CHEMBL1185'`) → canonical SMILES. Tagged as a `converter` for the `(CHEMBL[0-9]+)` regex — string fields auto-resolve. **Only goes ID → SMILES**; for InChI key / other-ID → ChEMBL ID, use `Chem:mapIdentifiersTransform` from the toolkit skill. |
...
| `getChemblCompoundsByOrganism(maxNumberOfMolecules: int = 1000, organism: string = 'Shigella')` | Pulls compounds active against targets of the given organism. Returns DataFrame. Used as a `hitTriageDataSource`. |
| `getChemblCompounds(maxNumberOfMolecules: int = 1000)` | Pulls a generic ChEMBL compound sample. Used as a `hitTriageDataSource`. |
...
| Function | What it does |
|---|---|
| `molTrackApp(path?: string)` | Opens the MolTrack app. `path` (URL) routes to sub-views: `'Compound' \| 'Batch' \| 'Register' \| 'Search' \| 'Saved Searches' \| 'Bulk' \| 'Assay' \| 'Schema'`. URL params `corporate_compound_id` / `corporate_batch_id` jump to a specific entity. browsePath `Chem`. |
| `molTrackAppTreeBrowser(appNode: object, browseView: object)` | Wires the tree-browser sidebar (auto). |
...
| `getMoltrackPropPanelById(id: string)` | "Databases \| MolTrack" context panel for a `Grok ID` cell. |
...
### Demo data sources (registered as `hitTriageDataSource` / `pepTriageDataSource`)

| Function | What it does |
|---|---|
| `demoFileIngest()` | 100-molecule demo dataset. |
| `demoFileIngest1()` | 5000-molecule demo dataset. |
| `demoFileIngest2(numberOfMolecules: int)` | Variable-size demo dataset. |
| `demoPeptideSequences(peptideCount: int)` | Demo HELM peptide sequences for PeptiHit. |
...
| `hitDesignVidPanel(vid: string)` | "Hit Design V-iD" context panel — shows campaigns for a V-iD. |
| `gasteigerCellRenderer()` | PNG cell renderer for Gasteiger charge images. |
...
| `dataToCurvesTopMenu()` | UI wrapper around `dataToCurves`. Top menu: Data → Curves → Data to Curves. |
| `addStatisticsColumn(table: dataframe, colName: string, propName: <fit-stat>, seriesNumber: int)` | Adds a column with one fit statistic for a given series index (0-based). |
| `addAggrStatisticsColumn(table: dataframe, colName: string, propName: <fit-stat>, aggrType: 'avg' \| 'min' \| 'max' \| 'med' \| 'sum' \| 'stdev')` | Same but aggregated across all series in each cell. |
...
### Curve format converters (registered with role `curveConverter`)
...
| Function | What it does |
|---|---|
| `previewPzfx(file: file)` | Opens `.pzfx` as a curve grid (file viewer). |
| `pzfxFileHandler(bytes: list<int>)` | Parses `.pzfx` → list of DataFrames (file handler). |
