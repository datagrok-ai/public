...
Functions tagged `vectorFunc: 'true'` (e.g. `getProperties`, `getDescriptors`,
`getToxicityRisks`, `getStructuralAlerts`, `convertMoleculeNotation`,
`getInchis`, `getMorganFingerprints`, `Admetica:getAdmeProperties`,
`Docking:getAutodockResults`, `Boltz1:docking`) return a `DG.DataFrame`
and are usable from the Add-New-Column dialog. Functions whose `outputs`
declare `action: 'join(table)'` automatically append their result columns
to the input table — do not also do it yourself.
...
| `convertMoleculeNotation(molecule: column<Molecule>, targetNotation: <notation>, kekulize?: bool = false)` | Vector-form notation conversion. Returns a new `DG.Column`. |
| `convertNotation(data: dataframe, molecules: column<Molecule>, targetNotation: <notation>, overwrite?: bool, join?: bool, kekulize?: bool)` | Top-menu version: appends or overwrites the column. Top menu: Chem → Transform → Convert Notation. |
| `recalculateCoords(table: dataframe, molecules: column<Molecule>, method: 'OCL' \| 'CoordGen' = 'OCL', join: bool = true)` | Recompute 2D layout. Top menu: Chem → Transform → Recalculate Coordinates. |
| `canonicalize(molecule: string)` | → canonical SMILES (string). |
| `getInchis(molecules: column<Molecule>)` / `getInchiKeys(molecules: column<Molecule>)` | Vector InChI / InChI key. Vector-func. |
| `addInchisTopMenu(table: dataframe, molecules: column<Molecule>)` / `addInchisKeysTopMenu(...)` | Append as a new column. Top menu: Chem → Calculate → To InchI / To InchI Keys. |
...
| Function | What it does |
|---|---|
| `addChemPropertiesColumns(table: dataframe, molecules: column<Molecule>, MW?: bool = true, HBA?: bool = true, HBD?: bool = true, logP?: bool = true, logS?: bool = true, PSA?: bool = true, rotatableBonds?: bool = true, stereoCenters?: bool = true, moleculeCharge?: bool = false)` | OCL-based properties → appended as columns. Top menu: Chem → Calculate → Chemical Properties. |
| `getProperties(molecules: column<Molecule>, selected?: list<string>)` | Vector form. `selected` is a subset of the property names above; omit/empty for all. Returns a DataFrame (joinable). |
...
| `getDescriptors(molecules: column<Molecule>, selected?: list<string>)` | RDKit descriptor set (via Chem docker). Returns DataFrame. Use `chemDescriptorsTree()` for the legal `selected` names. |
| `chemDescriptors(table: dataframe, molecules: column<Molecule>, descriptors: list<string>)` | Same, appended to `table`. |
| `chemDescriptorsTree()` | Returns the tree of available descriptor groups (for UI). |
| `descriptorsDocker()` | Opens the descriptors picker dialog. Top menu: Chem → Calculate → Descriptors. |
| `calculateDescriptorsTransform(table: dataframe, molecules: column<Molecule>, selected: list<string>)` | Same but tagged `transform` for data-sync projects. |
| `getMorganFingerprints(molColumn: column<Molecule>)` | Returns a Column of `DG.BitSet` fingerprints. |
...
| `addChemRisksColumns(table: dataframe, molecules: column<Molecule>, mutagenicity?: bool = true, tumorigenicity?: bool = false, irritatingEffects?: bool = false, reproductiveEffects?: bool = false)` | OCL toxicity risk flags. Top menu: Chem → Calculate → Toxicity Risks. |
| `getToxicityRisks(molecules: column<Molecule>, risks?: list<string>)` | Vector form. `risks` is a subset of the risk names above. |
| `structuralAlertsTopMenu(table: dataframe, molecules: column<Molecule>, pains: bool = true, bms: bool = false, sureChembl: bool = false, mlsmr: bool = false, dundee: bool = false, inpharmatica: bool = false, lint: bool = false, glaxo: bool = false)` | Apply rule-based structural alerts. Top menu: Chem → Analyze → Structural Alerts. |
| `getStructuralAlerts(molecules: column<Molecule>, alerts?: list<string>)` | Vector form. |
| `pharmacophoreFeaturesTopMenu(table: dataframe, molecules: column<Molecule>, donor: bool, acceptor: bool, hydrophobic: bool, aromatic: bool, positive: bool, negative: bool, halogenBond: bool)` | RDKit pharmacophore family flags. Top menu: Chem → Analyze → Pharmacophore Features. |
| `biochemPropsWidget()` | Opens the auto-discovery dialog for biochem calculators. Top menu: Chem → Calculate → Biochemical Properties. |
...
| `similarityMatrixTopMenu(table: dataframe, molecules: column<Molecule>, symbols: column, fingerprintType?: <fp>)` | Full pairwise Tanimoto matrix → opens as a new table. Top menu: Chem → Calculate → Similarity Matrix. |
...
| `SubstructureSearchTopMenu(molecules: column<Molecule>)` | Opens the filter sketcher. Top menu: Chem → Search → Substructure Search. |
...
| `similaritySearchTopMenu()` / `diversitySearchTopMenu()` | Top menu shortcuts. Top menu: Chem → Search. |
| `sortBySimilarity(value: object)` | Right-click action that sorts the grid by similarity to the picked molecule. |
| `useAsSubstructureFilter(value: object)` | Right-click action that adds the picked molecule as a substructure filter. |
...
| Function | What it does |
|---|---|
| `chemSpaceTopMenu(table: dataframe, molecules: column<Molecule>, methodName: 'UMAP' \| 't-SNE', similarityMetric: 'Tanimoto' \| 'Asymmetric' \| 'Cosine' \| 'Sokal' = 'Tanimoto', plotEmbeddings: bool = true, options?: object, preprocessingFunction?: func, clusterEmbeddings?: bool, clusterMCS?: bool)` | Projects molecules to 2D (UMAP / t-SNE) + scatter plot. Top menu: Chem → Analyze → Chemical Space. |
| `chemSpaceTransform(...)` | Transform-tagged version (for data-sync). |
...
| `rGroupsAnalysisMenu()` | Opens the R-group analysis dialog. Top menu: Chem → Analyze → R-Groups Analysis. |
...
| `mmpAnalysis(table: dataframe, molecules: column<Molecule>, activities: list<column<numerical>>, diffTypes: list<'delta'\|'ratio'>, scalings: list<'none'\|'lg'\|'-lg'>, fragmentCutoff?: float = 0.4)` | Matched Molecular Pairs analysis. Top menu: Chem → Analyze → Matched Molecular Pairs. |
| `mmpViewer()` | Returns `MatchedMolecularPairsViewer` for manual `view.addViewer`. |
| `activityCliffs(table: dataframe, molecules: column<Molecule>, activities: column<numerical>, similarity: float, methodName: 'UMAP' \| 't-SNE', similarityMetric: <metric>, preprocessingFunction?: func, options?: object, isDemo?: bool, isTest?: bool)` | Finds pairs of similar molecules with large activity differences. Top menu: Chem → Analyze → Activity Cliffs. |
| `addScaffoldTree()` | Adds a `ScaffoldTreeViewer` to the current view. Top menu: Chem → Analyze → Scaffold Tree. |
...
| `bitbirchClusteringTopMenu(table: dataframe, molecules: column<Molecule>, threshold?: float, fingerprintType?: <fp>)` | O(N) BitBIRCH clustering → cluster ID column. Top menu: Chem → Calculate → BitBIRCH Clustering. |
| `clusterMCSTopMenu(table: dataframe, molCol: column<Molecule>, clusterCol: column)` | Most-common substructure per cluster → appends MCS column. Top menu: Chem → Calculate → Cluster MCS. |
| `performClusterMCS(molCol: column<Molecule>, clusterCol: column)` | Vector form. |
| `elementalAnalysis(table: dataframe, molecules: column<Molecule>, radarViewer: bool, radarGrid: bool)` | Adds atom-count columns; optional radar viewer. Top menu: Chem → Analyze → Elemental Analysis. |
| `runElementalAnalysis(table: dataframe, molecules: column<Molecule>)` | Returns the list of added column names (transform-tagged). |
...
| Function | What it does |
|---|---|
| `_mpo()` | Opens the MPO Score dialog. Top menu: Chem → Calculate → MPO Score. |
...
| `mpoTransformFunction(df: dataframe, profileName: string, aggregation: <agg>, currentProperties: string, silent?: bool)` | Transform-tagged wrapper used by data-sync. |
| `mpoProfilesApp(path?: string)` | Opens the MPO Profiles app. |
| `getMapIdentifiers()` | Opens the ID-mapping dialog. Top menu: Chem → Calculate → Map Identifiers. |
| `mapIdentifiersTransform(table: dataframe, molecules: column<Molecule>, fromSource: <id-source>, toSource: <id-source>)` | Transform form: appends a column with mapped IDs. **This is also the right entry for "InChI key → ChEMBL ID" and similar reverse lookups** (chemblIdToSmilesTs goes only ID→SMILES, never the reverse). |
...
| `namesToSmiles(data: dataframe, names: column<string>)` | Looks up compound names → canonical SMILES via ChEMBL. **Requires the Chembl package + ChEMBL DB.** Use for any "drug name → SMILES" task (aspirin, ibuprofen, caffeine, ...) — wrap the list of names in a `DG.Column.fromStrings(...)` and call. Top menu: Chem → Transform → Names To Smiles. |
| `deprotect(table: dataframe, molecules: column<Molecule>, fragment: string)` | Removes the drawn fragment as a protecting group. Top menu: Chem → Transform → Reactions → Deprotect. |
| `removeWaterAndSaltsTopMenu(table: dataframe, molecules: column<Molecule>)` | Strip water/salts. Top menu: Chem → Transform → Reactions → Remove Water and Salts. |
| `transformationReactionsTopMenu()` | Run reaction SMARTS over a column of reactants. Top menu: Chem → Transform → Reactions → Transformation. |
| `twoComponentReactionTopMenu()` | Cross-product reaction between two columns. Top menu: Chem → Transform → Reactions → Two-Component Reaction. |
...
| `beautifyMols(mols: list<string>)` | Returns clean V3K molblocks. |
| `convertToV3KViaOCL(mols: list<string>)` | OCL-based V2K → V3K conversion. |
| `convertMolNotationAction(col: column<Molecule>)` | Right-click action wired to `convertNotation`. |
| `convertMixtureToSmiles(col: column)` | Right-click action for `ChemicalMixture` columns. |
| `copyAsAction(value: object)` / `copyAsSmiles` / `copyAsMolfileV2000` / `copyAsMolfileV3000` / `copyAsSmarts` / `copyAsImage` | Right-click "Copy as ..." actions. |
...
| `getAdmeProperties(table: dataframe, molecules: column<Molecule>, props?: list<string>)` | **Main entry point.** Predicts ADMET for a column → returns a DataFrame appended to `table`. `props` is a list of model names from `getModels()`; omit for all. Vector-func, joinable. |
| `getAdmePropertiesSingle(molecule: string)` | One molecule → DataFrame of ADMET predictions. |
| `admeticaMenu(table: dataframe, molecules: column<Molecule>, template: string, models: list<string>, addPiechart: bool, addForm: bool)` | Same as `getAdmeProperties` but with optional pie-chart and form viewers. Top menu: Chem → Admetica → Calculate. |
...
| `admeticaWidget(semValue: object)` | Per-molecule context panel: "Biology \| Admetica". |
