...
`datagrok-selection`; generic DataFrame cloning in `datagrok-df-and-columns`.
...
| Substructure (UI, < 50 k rows)                        | `... .updateOrAdd({type:DG.FILTER_TYPE.SUBSTRUCTURE, column, columnName, molBlock})` |
| Substructure (BitSet, any size)                       | `const bs = await grok.chem.searchSubstructure(col, query); t.filter.copyFrom(bs);` |
...
For large frames (> 50 k rows) the widget is laggy. Use the **BitSet path**
instead — no widget, just the filter mask is written. Also the right path for
headless / non-TableView calls:

```datagrok-exec
// Works for SMILES, SMARTS, and molblock alike.
const bs = await grok.chem.searchSubstructure(t.col('smiles'), 'c1ccccc1');
t.filter.copyFrom(bs);
```

Always `await` `grok.chem.searchSubstructure(...)` — without it, the rest of
the block sees the old filter state.
