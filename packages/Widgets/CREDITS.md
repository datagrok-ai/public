...
The `@datagrok/widgets` package is distributed under the MIT license that
covers the rest of the `public/` repository (see [`../../LICENSE.md`](../../LICENSE.md)).

This package is a small collection of demo widgets, viewers, filters, and
cell renderers; it contains no bundled third-party JavaScript runtime
libraries beyond what the Datagrok platform already provides as webpack
externals.
...
These libraries are not bundled into Widgets' `dist/` — they are provided once
...
as part of the published Widgets plugin and impose no obligation on users of
the plugin.
