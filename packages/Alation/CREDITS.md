# Alation — Third-Party Libraries

The `@datagrok/alation` package is distributed under the MIT license that
...
`package.json` (`cash-dom`, `dayjs`, `rxjs`) are webpack externals — they are
not bundled into Alation's `dist/`, but are provided once by the Datagrok
platform host and shared across all packages.

No copyleft (GPL/LGPL/MPL) component is bundled into the published Alation
plugin.
...
redistributed as part of the published Alation plugin and impose no
