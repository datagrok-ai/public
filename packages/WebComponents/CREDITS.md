# WebComponents — Third-Party Libraries

The `@datagrok/webcomponents` package is distributed under the MIT license
that covers the rest of the `public/` repository (see [`../../LICENSE.md`](../../LICENSE.md)).

This package is a small runtime-glue layer for Datagrok web components; it
...
platform) or are Datagrok-internal libraries (`@datagrok-libraries/*`) covered
by the repo-wide `LICENSE.md`.

---

## 2. Linked at runtime via the Datagrok platform (webpack externals)

These libraries are not bundled into WebComponents' `dist/` — they are
...
as part of the published WebComponents plugin and impose no obligation on
users of the plugin.
