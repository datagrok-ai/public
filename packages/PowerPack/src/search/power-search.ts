...
export function queriesSearch(s: string, host: HTMLDivElement): void {
  ...
  if (byRegExp.test(s)) {
    const byMatches = byRegExp.exec(s);
  ...
  for (const q of queries.filter((q) => varRegExp.exec(q.friendlyName) !== null)) {
    const varMatches = varRegExp.exec(q.friendlyName);
    if (varMatches == null || varMatches.length != 2)
      continue;

    const qPattern = new RegExp(q.friendlyName.replace(varRegExp, '([a-zA-Z0-9_]+)'));
    const matches = qPattern.exec(s);
    if (matches != null && matches.length == 2) {
      const varName = varMatches[1];
      const varValue = matches[1];
      q.apply({[varName]: varValue}).then((df: DG.DataFrame) => {
        const viewer: DG.Viewer = (byColumn == null) ?
          DG.Viewer.grid(df) :
