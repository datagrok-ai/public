...
import {getMyGroupFavorites, getAdminGroups} from './group-favorites';
...
function createSection(
  title: string, iconName: string, entities: DG.Entity[], root: HTMLElement,
...
export class WorkspaceTab {
  ...
  private async populate(root: HTMLElement, user: DG.User): Promise<void> {
    const [groupFavorites, adminGroups] = await Promise.all([
      getMyGroupFavorites(),
      getAdminGroups(),
    ]);

    // Container for group sections + manage link (for easy rebuild)
    const groupContainer = ui.divV([], 'pp-workspace-group-container');
    root.appendChild(groupContainer);

    const rebuildSections = async (): Promise<void> => {
      groupContainer.innerHTML = '';
      const freshFavorites = await getMyGroupFavorites();
      const freshAdminGroups = await getAdminGroups();
      this.renderGroupSections(groupContainer, freshFavorites, freshAdminGroups, rebuildSections);
    };

    this.renderGroupSections(groupContainer, groupFavorites, adminGroups, rebuildSections);
      ...
      for (const {group, entities, isAdmin} of groupFavorites) {
        if (entities.length > 0) {
          ...
          const onEdit = isAdmin ? () => {
            showManageFavoritesDialog(group, [...entities], rebuildSections);
          } : undefined;
          container.appendChild(
            createSection(group.friendlyName, 'star', entities, container, isAdmin ? group : undefined, onEdit),
          );
        }
      }
    }

    // "Manage favorites for another group..." link — for admin groups with no current pins
      ...
      const manageLink = ui.link('Manage favorites for another group...', () => {
        ...
        for (const g of extraGroups) {
          menu.item(g.friendlyName || g.name, () => {
            showManageFavoritesDialog(g, [], rebuildSections);
          });
        }
        menu.show();
      });
