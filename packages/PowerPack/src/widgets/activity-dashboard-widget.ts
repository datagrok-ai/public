...
export class ActivityDashboardWidget extends DG.Widget {
  ...
  async createRandomizedTipOfTheDay(): Promise<HTMLElement> {
    ...
    let tipIdx = 0;
    const demoApp = DG.Func.find({meta: {role: 'app'}, package: 'Tutorials', name: 'demoApp'})[0];
    const tutorialsApp = DG.Func.find({meta: {role: 'app'}, package: 'Tutorials', name: 'trackOverview'})[0];
