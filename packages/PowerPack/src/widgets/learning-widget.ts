...
export class LearningWidget extends DG.Widget {
  ...
  constructor() {
    super(ui.box(null, 'power-pack-activity-widget-content-learning-widget'));
    const wikiList = ui.element('ul');
    wikiList.style.columns = '2';
    wikiList.appendChild(ui.div(help.map(renderWiki)));

    const videoList = ui.element('ul');
    ...
    this.subs.push(tabs.onTabChanged.subscribe((tabPane: DG.TabPane) => {
      setTimeout(() => tabPane.name === 'DEMO' ? tabPane.content.firstElementChild?.classList.add('tutorial-widget-demo') :
        tabPane.content.firstElementChild?.classList.remove('tutorial-widget-demo'), 100);
    }));
  }
}
...
function renderPlaylist(p: PlayItem) {
  ...
  const link = ui.link(p.title, url, p.description, '');
  const root = ui.divH([img, link], {style: {alignItems: 'center'}});
