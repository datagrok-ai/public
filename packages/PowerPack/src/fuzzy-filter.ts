...
export class FuzzyFilter extends DG.Filter {
  ...
  async applyFilter() {
    ...
    const fuse = new Fuse(this.column?.toList() ?? [],
      {includeScore: true, isCaseSensitive: false, ignoreLocation: true,
        useExtendedSearch: true, includeMatches: true});

    const filterResult = fuse.search(this.input);
    this.bitset = DG.BitSet.create(this.dataFrame?.rowCount ?? 0, (_i: number) => false);
    for (const res of filterResult) {
