...
test.describe("Sensitivity analysis over a model", () => {
  ...
  test("Sensitivity analysis over a model", {tag: ["@journey", "@diffstudio", "@realizes:diffstudio.model.bioreactor"]}, async ({browser}) => {
    ...
    await run.scenario("The Edit toggle swaps the form for the equations, and back", async () => {
      await session.step(23, "When user clicks on Edit ribbon item", () => clickOn(page, el("Edit ribbon item")));
      await session.step(24, "Then code editor should be visible", () => shouldBe(page, el("code editor"), "visible"));
      await session.step(25, "And \"Process mode\" input should be absent", () => shouldBe(page, el("\"Process mode\" input"), "absent"));
      await session.step(26, "When user clicks on Edit ribbon item", () => clickOn(page, el("Edit ribbon item")));
      await session.step(27, "Then code editor should be absent", () => shouldBe(page, el("code editor"), "absent"));
      await session.step(28, "And \"Process mode\" input should be visible", () => shouldBe(page, el("\"Process mode\" input"), "visible"));
    });
    await run.scenario("Sensitivity opens a view of its own", async () => {
      await session.step(31, "When user clicks on Sensitivity ribbon item", () => clickOn(page, el("Sensitivity ribbon item")));
      await session.step(32, "Then the \"Bioreactor - comparison\" view should be current", () => viewIsCurrent(page, "Bioreactor - comparison"));
    });
    await run.scenario("A parameter is chosen by the switch beside its input", async () => {
      await session.step(35, "When user switches on \"FFox\" input", () => switchOn(page, el("\"FFox\" input")));
      await session.step(36, "Then \"FFox min\" input should be switched on", () => shouldBeSwitchedOn(page, el("\"FFox min\" input")));
      await session.step(37, "And \"FFox max\" input should be visible", () => shouldBe(page, el("\"FFox max\" input"), "visible"));
      await session.step(38, "When user switches off \"FFox min\" input", () => switchOff(page, el("\"FFox min\" input")));
      await session.step(39, "Then \"FFox\" input should be visible", () => shouldBe(page, el("\"FFox\" input"), "visible"));
    });
    await run.scenario("Running the analysis over three parameters puts its viewers on screen", async () => {
      await session.step(42, "When user switches on \"FFox\" input", () => switchOn(page, el("\"FFox\" input")));
      await session.step(43, "And user switches on \"FKox\" input", () => switchOn(page, el("\"FKox\" input")));
      await session.step(44, "And user switches on \"FFred\" input", () => switchOn(page, el("\"FFred\" input")));
      await session.step(45, "And user clicks on \"Run\" icon", () => clickOn(page, el("\"Run\" icon")));
      await session.step(46, "Then the current view should hold at least 4 viewers", () => viewHoldsViewers(page, 4));
