...
import {clickOn, selectIn, shouldBeSwitchedOn, shouldHaveValue, switchOn} from '@datagrok-libraries/bdd/bindings/common/steps';
...
test.describe("Fitting a model to data", () => {
  ...
  test("Fitting a model to data", {tag: ["@journey", "@diffstudio", "@realizes:diffstudio.model.bioreactor"]}, async ({browser}) => {
    ...
    await run.scenario("A parameter is varied, and its bounds appear with it", async () => {
      ...
      await session.step(35, "Then \"FFox (min)\" input should be switched on", () => shouldBeSwitchedOn(page, el("\"FFox (min)\" input")));
      await session.step(36, "And \"FFox (min)\" input should have value \"0.15\"", () => shouldHaveValue(page, el("\"FFox (min)\" input"), "0.15"));
      await session.step(37, "And \"FFox (max)\" input should have value \"0.25\"", () => shouldHaveValue(page, el("\"FFox (max)\" input"), "0.25"));
    });
    await run.scenario("The fit needs an output to aim at and a table to aim with", async () => {
      await session.step(40, "Given the \"System:AppData/DiffStudio/library/bioreactor-experiment.csv\" file is loaded as a table", () => loadTable(page, "System:AppData/DiffStudio/library/bioreactor-experiment.csv"));
      await session.step(41, "When user switches on \"Bioreactor\" input", () => switchOn(page, el("\"Bioreactor\" input")));
      await session.step(42, "And user selects \"bioreactor-experiment\" in \"Bioreactor\" input", () => selectIn(page, "bioreactor-experiment", el("\"Bioreactor\" input")));
      await session.step(43, "Then \"Bioreactor\" input should have value \"bioreactor-experiment\"", () => shouldHaveValue(page, el("\"Bioreactor\" input"), "bioreactor-experiment"));
      await session.step(44, "And \"argument\" input should have value \"t\"", () => shouldHaveValue(page, el("\"argument\" input"), "t"));
    });
    await run.scenario("Running the fit lowers the loss it reports, iteration by iteration", async () => {
      ...
      await session.step(48, "Then the table should have a \"RMSE by iterations\" column", () => hasColumn(page, "RMSE by iterations"));
      await session.step(49, "And the table should have 1 row", () => rowCount(page, 1));
      await session.step(50, "And every value of the \"FFox\" column should be between 0.15 and 0.25", () => columnWithin(page, "FFox", 0.15, 0.25));
      await session.step(51, "And the \"RMSE by iterations\" table should have at least 2 rows", () => nestedTableRows(page, "RMSE by iterations", 2));
      await session.step(52, "And the \"Loss\" column of the \"RMSE by iterations\" table should never increase", () => nestedSeriesDescends(page, "Loss", "RMSE by iterations"));
