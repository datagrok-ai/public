...
export namespace funcs {
  ...
  */
  export async function ivpModelBioreactor(mode: string , t0: number , t1: number , h: number , FFox: number , KKox: number , FFred: number , KKred: number , Ffree: number , Kfree: number , FKred: number , FKox: number , MEAthiol: number , DO2: number , yO2P: number , CYST: number , VL: number , filtration: number , qin: number , yO2in: number , T: number , P: number , switchTime: number ): Promise<DG.DataFrame> {
    return await grok.functions.call('DiffStudio:IvpModelBioreactor', { mode, t0, t1, h, FFox, KKox, FFred, KKred, Ffree, Kfree, FKred, FKox, MEAthiol, DO2, yO2P, CYST, VL, filtration, qin, yO2in, T, P, switchTime });
  }

  /**
  In-browser two-compartment pharmacokinetic-pharmacodynamic (PK-PD) simulation
  */
  export async function ivpModelPKPD(count: number , t0: number , t1: number , h: number , depot: number , centr: number , peri: number , eff: number , C2: number , C3: number , dose: number , KA: number , CL: number , V2: number , Q: number , V3: number , EC50: number , Rate: number ): Promise<DG.DataFrame> {
    return await grok.functions.call('DiffStudio:IvpModelPKPD', { count, t0, t1, h, depot, centr, peri, eff, C2, C3, dose, KA, CL, V2, Q, V3, EC50, Rate });
  }
}
