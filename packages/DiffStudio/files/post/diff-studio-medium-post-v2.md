...
In practice, packaging sets the circle of users, and that circle is usually one person: whoever wrote the script. Most of the people who need answers from an ODE model (biologists, clinicians, process engineers, students) never open the script. The model stays with the modeler.

This post is about a different workflow: you write the equations almost the way you would write them on paper, and you get an interactive model in the browser right away. Sliders for every parameter, live plots, fitting to experimental data, and a URL you can send to anyone, whether or not they have ever written code.
...
Press **F5** (or click **Refresh**). Two plots and a results table appear; there is nothing else to configure.
...
No solver selection, no tolerances, no plotting code. The default solver handles both stiff and non-stiff systems; you can override it later if you need to.

You can add comments with `//` right next to an equation. And the whole model is plain text: download it as an `.ivp` file, edit it in any editor, keep it in git, paste it into a chat.
...
Now try a few values of `beta`. In Python each one is an edit and a rerun; in Diff Studio it is a slider — but a slider labeled `beta` with no range is not much of an improvement. Fixing that is the next step.
...
You now have an interactive predator–prey app that took a couple of minutes and contains no code, only equations and labels. Anyone who understands what "prey growth rate" means can use it, and that is a much larger group than the one that can edit a `solve_ivp` call.
...
Set the dose to 250 mg and halve the clearance: two slider moves, and the peak concentration and the tail both change in front of you. A pharmacologist can do this without ever seeing the equations.
...
The case that convinced me of this was a bioreactor model I built from scratch. The mass balances were clear, but several kinetic constants were unknown — nothing in the literature matched our setup closely enough to borrow. What we did have was a set of experimental runs. I set the unknown constants as fitting parameters with rough physical bounds, loaded the measurements, and ran the fit. The constants came out, the curves matched the data, and the model went from a sketch to something we could use. What surprised me most was where it ran: on my laptop, in a browser tab, with no cloud or server involved. Diff Studio splits the optimization across Web Workers, so the thousands of model evaluations the fit needs ran in parallel on the laptop's own cores.
...
These three are the reasons people end up building a UI around a script. Here they come with the model.
...
Why ROS34PRw is the default and how it behaves on stiff benchmarks is a separate post; the Springer paper above covers the numbers for now.

---

## Try it

The whole idea in one sentence: **you write the math, and the platform builds the app around it** — sliders, fitting, sensitivity analysis, and a shareable URL included.

- **Run it:** [Diff Studio on public.datagrok.ai](https://public.datagrok.ai/apps/DiffStudio) — free, nothing to install. Start from a template, open a model from the Library, or follow the [interactive tutorial](https://public.datagrok.ai/apps/tutorials/Tutorials/Scientificcomputing/Differentialequations) inside the platform.
