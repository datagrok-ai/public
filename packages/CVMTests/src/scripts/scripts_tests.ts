...
for (const lang of languages) {
  ...
  category(`Scripts: ${lang} scripts`, () => {
    ...
    if (lang === 'Python') {
      ...
      // up to ~5 min end-to-end. The scripts carry #meta.timeout: 600 so the
      // server-side exec budget covers that; the client budget sits above it
      // so failures surface as the server's error, not a client timeout.
      ...
      test('Env: reuse existing environment', async () => {
        ...
        const result1 = await grok.functions.call('CVMTests:PythonEnvReuse',
          {'text': 'first call'});
        expect(result1.startsWith('ok:'), true);
        // Second call should reuse it (much faster)
        const t0 = Date.now();
        const result2 = await grok.functions.call('CVMTests:PythonEnvReuse',
          {'text': 'second call'});
