...
`CompartmentalPK` is a [package](https://datagrok.ai/help/develop/develop#packages) for the [Datagrok](https://datagrok.ai) platform
...
```bash
ReferenceError: Cannot access 'ctrlCompare' before initialization
...
```
...
```bash
Cannot access 'btnGenerateData' before initialization
...
```
