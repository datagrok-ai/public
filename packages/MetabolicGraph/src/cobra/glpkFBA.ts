...
export async function runGLPKFBA(network: CobraModelData, findSolutionSpaceEdges = false, start?: number, end?: number) {
    ...
    const {_glp_create_prob, _glp_set_prob_name, _glp_set_obj_dir, _glp_add_rows,
        ...
        _glp_init_smcp, _glp_scale_prob, _free, _malloc, getValue, setValue, stringToUTF8, UTF8ToString, lengthBytesUTF8} = glpk;
    ...
    class Model {
        ...
        // console.log(getValue(iaPointer + 4));
            ...
            _glp_scale_prob(lp, GLP_SF_AUTO)
        ...
        optimize (problem) {
            ...
            for (var i = 1; i <= _glp_get_num_cols(problem); i++) {
    ...
    if (findSolutionSpaceEdges) {
        for (let i = start; i < end; i+=1) {
            ...
            model.set_lone_objective(problem, j, direction);
            ...
            //const problem = model.buildGlpkProblem();
            const solution = model.slim_optimize(problem);
            if (!reactionNames)
