...
export function prop(molecule: DG.SemanticValue, propertyCol: DG.Column, host: HTMLElement, descriptions: { [colName: string]: string }) : HTMLElement {
  ...
  return ui.divH([addColumnIcon, ui.divText(val.toFixed(2), {style: {width: '42px', textAlign: 'right', flexShrink: '0', flexGrow: '0'}})], {style: {'position': 'relative'}});
}
...
export function buildComparisonTable(
  ...
  for (const name of Object.keys(currentValues)) {
    ...
    row.style.cssText = 'display:grid;grid-template-columns:42px 16px 42px 42px;align-items:baseline;column-gap:12px;';
