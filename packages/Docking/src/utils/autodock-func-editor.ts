...
export class AutodockBaseFuncEditor {
  ...
  constructor(semtype: DG.SemType) {
    this.gpfFiles = this.configFiles();
    ...
    this.macromoleculeInput = ui.choiceInput('Macromolecule', this.gpfFiles[0], this.gpfFiles, () => {        
  ...
  configFiles(): string[] {
    let targetsFiles: DG.FileInfo[] = [];
    grok.dapi.files.list(TARGET_PATH, true).then((files) => {
      targetsFiles = files;
    });
    const gpfFiles: string[] = [];
    for (let i = 0; i < targetsFiles.length; ++i) {
      if (targetsFiles[i].isFile && targetsFiles[i].name.endsWith('.gpf')) {
        gpfFiles[gpfFiles.length] = targetsFiles[i].path;
      }
    }
    return gpfFiles;
  }
