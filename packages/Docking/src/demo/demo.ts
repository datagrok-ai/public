...
export async function _demoBoltzFolding(): Promise<void> {
  const tv: DG.TableView = await openMoleculeDataset('System:AppData/Docking/demo_files/boltz_folding_demo.csv');
  const layoutString = await grok.dapi.files.readAsText('System:AppData/Docking/demo_files/boltz_folding_demo.layout');
  const layout = DG.ViewLayout.fromJson(layoutString);
  tv.loadLayout(layout);
...
// Constants
export const BOLTZ_HELP = `# Folding Demo
...
To run Boltz-1 for the entire column:
