...
test.describe("HELM sequences to structures, then to SMILES", () => {
  ...
  test("Build molecules from the HELM sequences, then write them out as SMILES", {tag: ["@guide", "@help:datagrok/solutions/domains/bio"]}, async ({browser}) => {
    ...
    await session.step(17, "And user clicks on OK button in \"To Atomic Level\" dialog", () => clickOn(page, el("OK button in \"To Atomic Level\" dialog")));
    await session.step(18, "Then the top menu command should have completed", () => commandCompleted(page));
    await session.step(19, "And a new column matching \"^molfile\\(HELM string\\)\" should have been added", () => newColumnMatching(page, "^molfile\\(HELM string\\)"));
    await session.step(20, "And every value of \"molfile(HELM string)\" column should contain \"V30 BEGIN CTAB\"", () => everyValueContains(page, "molfile(HELM string)", "V30 BEGIN CTAB"));
    ...
    await session.step(22, "Then \"Target Notation\" input in \"Convert Notation\" dialog should have value \"smiles\"", () => shouldHaveValue(page, el("\"Target Notation\" input in \"Convert Notation\" dialog"), "smiles"));
    await session.step(23, "When user clicks on OK button in \"Convert Notation\" dialog", () => clickOn(page, el("OK button in \"Convert Notation\" dialog")));
    await session.step(24, "Then the top menu command should have completed", () => commandCompleted(page));
    await session.step(25, "And a new column \"molfile(HELM string)_smiles\" should have been added", () => newColumnNamed(page, "molfile(HELM string)_smiles"));
    await session.step(26, "And every value of \"molfile(HELM string)_smiles\" column should match \"^[A-Za-z0-9@+()\\[\\]\\\\/%=#$.:-]+$\"", () => everyValueMatches(page, "molfile(HELM string)_smiles", "^[A-Za-z0-9@+()\\[\\]\\\\/%=#$.:-]+$"));
    await session.step(27, "When user picks \"Column Properties...\" from the context menu of the \"header molfile(HELM string)_smiles\" area of grid", () => pickFromAreaContextMenu(page, "Column Properties...", "header molfile(HELM string)_smiles", el("grid")));
    await session.step(28, "And user enters \"canonical_smiles\" into \"New name\" input in \"molfile(HELM string)_smiles\" dialog", () => enterInto(page, "canonical_smiles", el("\"New name\" input in \"molfile(HELM string)_smiles\" dialog")));
    await session.step(29, "And user clicks on OK button in \"molfile(HELM string)_smiles\" dialog", () => clickOn(page, el("OK button in \"molfile(HELM string)_smiles\" dialog")));
    await session.step(30, "Then the table should have a column \"canonical_smiles\"", () => hasColumn(page, "canonical_smiles"));
