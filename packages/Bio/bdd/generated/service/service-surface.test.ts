...
test.describe("The service surface other packages call", () => {
  ...
  test("The service surface other packages call", {tag: ["@journey", "@realizes:bio.service-surface"]}, async ({browser}) => {
    ...
    await run.scenario("A sequence handler is per column and reports the column's notation", async () => {
      ...
      await session.step(22, "When user calls \"Bio:getSeqHandler\" function with:", () => callWith(page, "Bio:getSeqHandler", [["sequence","column:HELM string"]]), [["sequence","column:HELM string"]]);
      ...
      await session.step(27, "When user calls \"Bio:getSeqHandler\" function with:", () => callWith(page, "Bio:getSeqHandler", [["sequence","column:fasta"]]), [["sequence","column:fasta"]]);
    ...
    await run.scenario("The HELM monomer list is exactly the monomers of the column", async () => {
      ...
      await session.step(34, "And user calls \"Bio:getHelmMonomers\" function with:", () => callWith(page, "Bio:getHelmMonomers", [["sequence","column:HELM string"]]), [["sequence","column:HELM string"]]);
