...
import {allLibrariesSelected, knownMonomer, loadedFrom, noLibraryOnServer, notLoadedFrom, unknownMonomer} from '../../bindings/monomer-libs.js';
import {bioInitialized} from '../../bindings/steps.js';
import {loggedIn} from '@datagrok-libraries/bdd/bindings/common/session';
import {check, clickOn, selectIn, shouldBe, shouldContainText, uncheck, uploadThrough} from '@datagrok-libraries/bdd/bindings/common/steps';
...
test.describe("Managing monomer libraries", () => {
  ...
  test("Managing monomer libraries", {tag: ["@journey", "@realizes:bio.menu.manage.monomer-libraries", "@realizes:bio.op.load_monomer_library", "@realizes:bio.op.save_monomer_library"]}, async ({browser}) => {
    ...
    await run.scenario("Add uploads a library file and its monomers become known", async () => {
      ...
      await session.step(41, "Then \"Select storage for new monomer library\" dialog should be visible", () => shouldBe(page, el("\"Select storage for new monomer library\" dialog"), "visible"));
      await session.step(42, "When user selects \"Files\" in Storage input in \"Select storage for new monomer library\" dialog", () => selectIn(page, "Files", el("Storage input in \"Select storage for new monomer library\" dialog")));
      await session.step(43, "And user clicks on OK button in \"Select storage for new monomer library\" dialog", () => clickOn(page, el("OK button in \"Select storage for new monomer library\" dialog")));
      await session.step(44, "Then \"bdd-test-lib.json\" checkbox should become visible", () => shouldBe(page, el("\"bdd-test-lib.json\" checkbox"), "visible"));
      await session.step(45, "And \"bdd-test-lib.json\" checkbox should be checked", () => shouldBe(page, el("\"bdd-test-lib.json\" checkbox"), "checked"));
      await session.step(46, "And the \"bio-monomer-lib-loaded\" custom event should have fired", () => customFired(page, "bio-monomer-lib-loaded"));
      await session.step(47, "And the monomer library should be loaded from \"bdd-test-lib.json\"", () => loadedFrom(page, "bdd-test-lib.json"));
      await session.step(48, "And \"BDD\" should be a known \"PEPTIDE\" monomer", () => knownMonomer(page, "BDD", "PEPTIDE"));
    ...
    await run.scenario("Delete unloads the library and removes its file after confirmation", async () => {
      ...
      await session.step(53, "When user clicks on \"Delete\" icon in \"bdd-test-lib.json\" checkbox", () => clickOn(page, el("\"Delete\" icon in \"bdd-test-lib.json\" checkbox")));
      await session.step(54, "Then \"Warning\" dialog should be visible", () => shouldBe(page, el("\"Warning\" dialog"), "visible"));
      await session.step(55, "And \"Warning\" dialog should contain text \"bdd-test-lib.json\"", () => shouldContainText(page, el("\"Warning\" dialog"), "bdd-test-lib.json"));
      await session.step(56, "When user clicks on OK button in \"Warning\" dialog", () => clickOn(page, el("OK button in \"Warning\" dialog")));
      await session.step(57, "Then \"bdd-test-lib.json\" checkbox should become hidden", () => shouldBe(page, el("\"bdd-test-lib.json\" checkbox"), "hidden"));
      ...
      await session.step(59, "And the monomer library should not be loaded from \"bdd-test-lib.json\"", () => notLoadedFrom(page, "bdd-test-lib.json"));
      await session.step(60, "And \"BDD\" should not be a known \"PEPTIDE\" monomer", () => unknownMonomer(page, "BDD", "PEPTIDE"));
