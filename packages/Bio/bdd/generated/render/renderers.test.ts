...
test.describe("Sequence cell renderers", () => {
  ...
  test("Sequence cell renderers", {tag: ["@journey", "@realizes:bio.rendering", "@realizes:bio.rendering.biln", "@realizes:bio.rendering.separator", "@realizes:bio.rendering.monomer", "@realizes:GROK-12164"]}, async ({browser}) => {
    ...
    await run.scenario("Converting HELM to separator gives the new column the separator renderer", async () => {
      ...
      await session.step(49, "Then 1 new column should have been added", () => newColumnsCount(page, 1));
      await session.step(50, "And a new column matching \"^separator\\(HELM string\\)\" should have been added", () => newColumnMatching(page, "^separator\\(HELM string\\)"));
      await session.step(51, "And \"separator(HELM string)\" column should have units \"separator\"", () => columnUnits(page, "separator(HELM string)", "separator"));
      await session.step(52, "And the \"cell type of separator(HELM string)\" reading of grid should be \"sequence\"", () => readingReads(page, "cell type of separator(HELM string)", el("grid"), "sequence"));
      await session.step(53, "And \"HELM string\" column should have units \"helm\"", () => columnUnits(page, "HELM string", "helm"));
      await session.step(54, "And the \"cell type of HELM string\" reading of grid should be \"helm\"", () => readingReads(page, "cell type of HELM string", el("grid"), "helm"));
    ...
    await run.scenario("Split to Monomers columns render with the monomer renderer", async () => {
      await session.step(58, "Given user opens filter_MSA dataset", () => openDataset(page, ds("filter_MSA")));
      await session.step(59, "When user picks \"Bio > Transform > Split to Monomers...\" from the top menu", () => pickFromTopMenu(page, "Bio > Transform > Split to Monomers..."));
      await session.step(60, "And user clicks on OK button in \"Split to Monomers\" dialog", () => clickOn(page, el("OK button in \"Split to Monomers\" dialog")));
      await session.step(61, "Then the top menu command should have completed", () => commandCompleted(page));
      await session.step(62, "And 17 new columns should have been added", () => newColumnsCount(page, 17));
      await session.step(63, "And 17 new columns matching \"^[0-9]+$\" should have been added", () => newColumnsMatching(page, 17, "^[0-9]+$"));
      await session.step(64, "And \"1\" column should have semantic type \"Monomer\"", () => columnSemType(page, "1", "Monomer"));
      await session.step(65, "And \"17\" column should have semantic type \"Monomer\"", () => columnSemType(page, "17", "Monomer"));
      await session.step(66, "And the \"cell type of 1\" reading of grid should be \"Monomer\"", () => readingReads(page, "cell type of 1", el("grid"), "Monomer"));
      await session.step(67, "And the \"cell type of 9\" reading of grid should be \"Monomer\"", () => readingReads(page, "cell type of 9", el("grid"), "Monomer"));
      await session.step(68, "And the \"cell 1 of 1\" area of grid should be painted", () => areaPainted(page, "cell 1 of 1", el("grid")));
