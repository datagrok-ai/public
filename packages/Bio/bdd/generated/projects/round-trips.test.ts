...
import {columnComplete, columnSemType, columnTag, columnUnits, columnsEqual} from '@datagrok-libraries/bdd/bindings/platform/columns';
...
test.describe("Bio results survive a project save and reopen", () => {
  ...
  test("Antibody numbering survives a save and reopen, and numbers the same again", {tag: ["@serial", "@realizes:GROK-19928", "@realizes:bio.project.round-trip"]}, async ({browser}) => {
    ...
    await session.step(59, "And \"AntibodyHC (aligned)\" column should list at least 100 position names", () => positionNamesCount(page, "AntibodyHC (aligned)", 100));
