...
test.describe("Bio results survive a project save and reopen", () => {
  ...
  test("Antibody numbering survives a save and reopen, and numbers the same again", {tag: ["@serial", "@realizes:GROK-19928", "@realizes:bio.project.round-trip"]}, async ({browser}) => {
    ...
    await session.step(51, "Then a new column \"AntibodyHC (aligned)\" should have been added", () => newColumnNamed(page, "AntibodyHC (aligned)"));
    await session.step(52, "When user saves the current view as project \"bdd-bio-numbering-{run}\"", () => saveAsProject(page, session.text("bdd-bio-numbering-{run}")));
    await session.step(53, "And user closes all views", () => closeAllViews(page));
    await session.step(54, "And user opens the \"bdd-bio-numbering-{run}\" project", () => openProject(page, session.text("bdd-bio-numbering-{run}")));
    await session.step(55, "Then the table should have 20 rows", () => rowCount(page, 20));
    await session.step(56, "And \"AntibodyHC\" column should have units \"fasta\"", () => columnUnits(page, "AntibodyHC", "fasta"));
    await session.step(57, "And \"AntibodyHC (aligned)\" column should have tag \".numberingScheme\" equal to \"kabat\"", () => columnTag(page, "AntibodyHC (aligned)", ".numberingScheme", "kabat"));
    await session.step(58, "And the \".positionNames\" tag of \"AntibodyHC (aligned)\" column should list at least 100 values", () => columnTagLists(page, ".positionNames", "AntibodyHC (aligned)", 100));
    await session.step(59, "And \"AntibodyHC (aligned)\" column should have no missing values", () => columnComplete(page, "AntibodyHC (aligned)"));
    await session.step(60, "And \"AntibodyHC\" column should carry at least 7 annotations", () => carriesAtLeast(page, "AntibodyHC", 7));
    await session.step(61, "When user picks \"Bio > Annotate > Apply Numbering Scheme...\" from the top menu", () => pickFromTopMenu(page, "Bio > Annotate > Apply Numbering Scheme..."));
    await session.step(62, "And user selects \"kabat\" in Scheme input in \"Apply Antibody Numbering\" dialog", () => selectIn(page, "kabat", el("Scheme input in \"Apply Antibody Numbering\" dialog")));
    await session.step(63, "And user clicks on OK button in \"Apply Antibody Numbering\" dialog", () => clickOn(page, el("OK button in \"Apply Antibody Numbering\" dialog")));
    await session.step(64, "Then a new column \"AntibodyHC (aligned) (2)\" should have been added", () => newColumnNamed(page, "AntibodyHC (aligned) (2)"));
    await session.step(65, "And \"AntibodyHC (aligned) (2)\" column should hold the same values as \"AntibodyHC (aligned)\" column", () => columnsEqual(page, "AntibodyHC (aligned) (2)", "AntibodyHC (aligned)"));
  ...
  test("A HELM table comes back painted from the same monomer library", {tag: ["@serial", "@realizes:GROK-19928", "@realizes:bio.project.round-trip"]}, async ({browser}) => {
    ...
    await session.step(22, "Given user is logged in", () => loggedIn(page));
    await session.step(23, "And the Bio package is initialized", () => bioInitialized(page));
    await session.step(70, "Given user opens filter_HELM dataset", () => openDataset(page, ds("filter_HELM")));
    await session.step(71, "Then the \"cell type of HELM string\" reading of grid should be \"helm\"", () => readingReads(page, "cell type of HELM string", el("grid"), "helm"));
    await session.step(72, "And the \"cell 1 of HELM string\" area of grid should be painted in at least 3 colors", () => areaColors(page, "cell 1 of HELM string", el("grid"), 3));
    await session.step(73, "When user saves the current view as project \"bdd-bio-helm-{run}\"", () => saveAsProject(page, session.text("bdd-bio-helm-{run}")));
    await session.step(74, "And user closes all views", () => closeAllViews(page));
    await session.step(75, "And user opens the \"bdd-bio-helm-{run}\" project", () => openProject(page, session.text("bdd-bio-helm-{run}")));
    await session.step(76, "Then the table should have 4 rows", () => rowCount(page, 4));
    await session.step(77, "And \"HELM string\" column should have semantic type \"Macromolecule\"", () => columnSemType(page, "HELM string", "Macromolecule"));
    await session.step(78, "And \"HELM string\" column should have units \"helm\"", () => columnUnits(page, "HELM string", "helm"));
    await session.step(79, "And the \"cell type of HELM string\" reading of grid should be \"helm\"", () => readingReads(page, "cell type of HELM string", el("grid"), "helm"));
    await session.step(80, "And the \"cell 1 of HELM string\" area of grid should be painted in at least 3 colors", () => areaColors(page, "cell 1 of HELM string", el("grid"), 3));
    await session.step(81, "And the monomer library should be loaded from \"HELMCoreLibrary.json\"", () => loadedFrom(page, "HELMCoreLibrary.json"));
    await session.step(82, "And \"dV\" should be a known \"PEPTIDE\" monomer", () => knownMonomer(page, "dV", "PEPTIDE"));
