...
test.describe("Transforming HELM and aligned columns", () => {
  ...
  test("An aligned column converts to HELM with its multi-letter monomers bracketed", {tag: ["@realizes:bio.calculate.extract-region", "@realizes:bio.transform.convert-notation", "@realizes:bio.transform.split-to-monomers"]}, async ({browser}) => {
    ...
    await session.step(65, "And a new column \"helm(MSA)\" should have been added", () => newColumnNamed(page, "helm(MSA)"));
    await session.step(66, "And \"helm(MSA)\" column should have units \"helm\"", () => columnUnits(page, "helm(MSA)", "helm"));
    await session.step(67, "And \"helm(MSA)\" column should have no missing values", () => columnComplete(page, "helm(MSA)"));
    await session.step(68, "And every value of \"helm(MSA)\" column should match \"^PEPTIDE1\\{(\\[[^\\]]+\\]|[A-Z*])(\\.(\\[[^\\]]+\\]|[A-Z*]))*\\}\\$\\$\\$\\$$\"", () => everyValueMatches(page, "helm(MSA)", "^PEPTIDE1\\{(\\[[^\\]]+\\]|[A-Z*])(\\.(\\[[^\\]]+\\]|[A-Z*]))*\\}\\$\\$\\$\\$$"));
    await session.step(69, "And the value of \"helm(MSA)\" column in row 1 should be \"PEPTIDE1{[meI].[hHis].[Aca].N.T.[dE].[Thr_PO3H2].[Aca].[D-Tyr_Et].[Tyr_ab-dehydroMe].[dV].E.N.[D-Orn].[D-aThr].*.[Phe_4Me]}$$$$\"", () => valueInRow(page, "helm(MSA)", 1, "PEPTIDE1{[meI].[hHis].[Aca].N.T.[dE].[Thr_PO3H2].[Aca].[D-Tyr_Et].[Tyr_ab-dehydroMe].[dV].E.N.[D-Orn].[D-aThr].*.[Phe_4Me]}$$$$"));
  ...
  test("Split to Monomers on a HELM column gives a column per position of the longest peptide", {tag: ["@realizes:bio.calculate.extract-region", "@realizes:bio.transform.convert-notation", "@realizes:bio.transform.split-to-monomers"]}, async ({browser}) => {
    ...
    await session.step(15, "Given user is logged in", () => loggedIn(page));
    await session.step(16, "And the Bio package is initialized", () => bioInitialized(page));
    await session.step(74, "Given user opens filter_HELM dataset", () => openDataset(page, ds("filter_HELM")));
    await session.step(75, "When user picks \"Bio > Transform > Split to Monomers...\" from the top menu", () => pickFromTopMenu(page, "Bio > Transform > Split to Monomers..."));
    await session.step(76, "Then \"Split to Monomers\" dialog should be visible", () => shouldBe(page, el("\"Split to Monomers\" dialog"), "visible"));
    await session.step(77, "And editor of Sequence input in \"Split to Monomers\" dialog should have text \"HELM string\"", () => shouldHaveText(page, el("editor of Sequence input in \"Split to Monomers\" dialog"), "HELM string"));
    await session.step(78, "When user clicks on OK button in \"Split to Monomers\" dialog", () => clickOn(page, el("OK button in \"Split to Monomers\" dialog")));
    await session.step(79, "Then the top menu command should have completed", () => commandCompleted(page));
    await session.step(80, "And 10 new columns should have been added", () => newColumnsCount(page, 10));
    await session.step(81, "And \"1\" column should have semantic type \"Monomer\"", () => columnSemType(page, "1", "Monomer"));
    await session.step(82, "And \"10\" column should have semantic type \"Monomer\"", () => columnSemType(page, "10", "Monomer"));
    await session.step(83, "And the value of \"1\" column in row 2 should be \"L\"", () => valueInRow(page, "1", 2, "L"));
    await session.step(84, "And the value of \"7\" column in row 2 should be \"T\"", () => valueInRow(page, "7", 2, "T"));
