@realizes:bio.int.empty-input-on-row-viewers
Feature: The current-row analyses on an empty sequence
  ...
  balloon, which does not come yet.
  ...
  # Known failure, GROK-16111 (open): no balloon, the viewer searches with the empty sequence.
  @known-failure
