...
category('detectorsBenchmark', () => {
  ...
  test('fastaDnaShorts50Few50', async () => {
    const et: number = await benchmarkDetectMacromolecule(10, bio.NOTATION.FASTA, bio.ALPHABET.DNA, 50, 50);
  });

  test('fastaDnaShorts50Many1E6', async () => {
      const et: number = await benchmarkDetectMacromolecule(10, bio.NOTATION.FASTA, bio.ALPHABET.DNA, 50, 1E6);
    },
    { /* skipReason: 'slow transmit large dataset to detector' */});

  test('fastaDnaLong1e6Few50', async () => {
    const et: number = await benchmarkDetectMacromolecule(10, bio.NOTATION.FASTA, bio.ALPHABET.DNA, 1E6, 50);
  });

  // -- separator --

  test('separatorDnaShorts50Few50', async () => {
    const et: number = await benchmarkDetectMacromolecule(10, bio.NOTATION.SEPARATOR, bio.ALPHABET.DNA, 50, 50, '/');
  });

  test('separatorDnaShorts50Many1E6', async () => {
      const et: number = await benchmarkDetectMacromolecule(10, bio.NOTATION.SEPARATOR, bio.ALPHABET.DNA, 50, 1E6, '/');
    },
    { /* skipReason: 'slow transmit large dataset to detector' */});

  test('separatorDnaLong1e6Few50', async () => {
    const et: number = await benchmarkDetectMacromolecule(10, bio.NOTATION.SEPARATOR, bio.ALPHABET.DNA, 1E6, 50, '/');
  });

  async function benchmarkDetectMacromolecule(
