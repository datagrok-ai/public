...
def _calculate_pi_single(smiles_string, mol_name='molecule'):
    try:
        ...
        if mol is None:
            return {'pI_mean': None, 'pI_std': None, 'charge_at_pH7': None, 'error': 'Invalid internal SMILES'}
...
def _calculate_pi_from_molblock_single(molblock_string, mol_name='molecule'):
    try:
        ...
        if mol is None:
            return {'pI_mean': None, 'pI_std': None, 'charge_at_pH7': None, 'error': 'Invalid MolBlock'}
...
for i in table.index:
    ...
    mol_results_list.append(_calculate_pi_from_molblock_single(molblock))
