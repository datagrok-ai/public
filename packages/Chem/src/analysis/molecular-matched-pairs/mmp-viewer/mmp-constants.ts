...
export const MATCHED_MOLECULAR_PAIRS_TOOLTIP_TRANS = `'Matched Molecular Pairs' grid contains all molecule pairs with current fragment substitution from 'Fragment Pairs' grid. Pinned row is a pair, containing current molecule from initial dataset. Click on any row to select rows with corresponding molecules in initial dataset, selected rows will be pinned on top.`;
export const MATCHED_MOLECULAR_PAIRS_TOOLTIP_FRAGS = `Click on a non-empty cell in the trellis plot above to see molecule pairs with corresponding fragment substitution. Click on any row to select rows with corresponding molecules in initial dataset, selected rows will be pinned on top.`;
