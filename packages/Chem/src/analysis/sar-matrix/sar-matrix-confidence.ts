...
/** Leave-one-out prediction of one observed cell within a single R-position's column slice. The
 *  held-out cell is blanked so the refit can't see it; returns null when its row or column has no
 *  other observation left in the slice. Fitting per position (not one global model over every column)
 *  keeps the reference-substituent "twin" cells of an all-reference compound in different slices so
...
/**
 * Leave-one-out cross-validated quality of the Free-Wilson fit that fills the matrix, validated per
