...
export async function structure3dWidget(molecule: string): Promise<DG.Widget> {
  ...
  ui.tools.waitForElementInDom(nglHost).then(() => {
    ...
    if (nglHost.closest('.dialog-floating')) {
      ...
      if (accPanel) {
        resizeSubscription = ui.onSizeChanged(accPanel).subscribe((_) => {
    ...
    const checkInterval = 100; // Check every 500ms
    const intervalId = setInterval(() => { 
      const isVisible = nglHost.offsetHeight > 0 && nglHost.offsetWidth > 0;
      if (!isVisible) {
