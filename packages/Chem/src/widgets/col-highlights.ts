...
export class CustomSketcherInput {
  ...
  constructor(molecule: string) {
    ...
    if (molecule) {
      ...
      PackageFunctions.isSmarts(molecule) ? this.sketcher.setSmarts(molecule) : this.sketcher.setMolFile(molecule);
    }
    this.root = ui.div(this.sketcher.root, 'chem-col-highlight-sketcher-input');
    this.root.addEventListener('click', (e) => {
      if (PackageFunctions.isSmarts(this.molecule!)) {
        e.stopPropagation();
        grok.shell.warning(`Smarts cannot be edited`);
      }
    }, true);
    this.sketcher.onChanged.subscribe(async () => {
      this.molecule = PackageFunctions.isSmarts(this.molecule!) ? await this.sketcher.getSmarts() : this.sketcher.getMolFile();
      this.onChanged.next();
    });
  }
