...
const searchTypeHints = {
  ...
  [SubstructureSearchType.STEREO_AGNOSTIC]: 'search structures matching sketched pattern ignoring stereochemistry',
...
export class SubstructureFilter extends DG.Filter {
  ...
  searchTypes = [SubstructureSearchType.CONTAINS, SubstructureSearchType.INCLUDED_IN,
    SubstructureSearchType.EXACT_MATCH, SubstructureSearchType.STEREO_AGNOSTIC,
    SubstructureSearchType.IS_SIMILAR,
  ...
  /** Rows edited since the last refresh, awaiting a bit patch. Filled on every cell edit and
   * drained once no search is running — immediately when idle, or from finishSearch() when the
   * in-flight search completes. We never restart or patch mid-search. */
  changedValuesRows = new Set<number>();

  get calculating(): boolean {return this.loader.style.display == 'initial';}
  set calculating(value: boolean) {this.loader.style.display = value ? 'initial' : 'none';}

  /** A full-column substructure search is currently in flight. */
  private get isSearchRunning(): boolean {
    return !!this.batchResultObservable && !this.batchResultObservable.closed;
  }

  /** The FILTER_SYNC_EVENT payload broadcast to peer filters on this column. */
  private filterSyncState(bitset: DG.BitSet | null = this.bitset) {
    return {bitset, molblock: this.currentMolecule, colName: this.columnName,
  ...
  get filterSummary(): string {
    return this.getFilterSummary(this.currentMolecule);
  }

  getSketcherMolecule(): string {
    const explicitMol = this.sketcher.sketcher?.explicitMol;
    if (explicitMol) {
      return DG.chem.isMolBlock(explicitMol.value) ?
        translateQueryAliasesV2000(explicitMol.value) : explicitMol.value;
    }
    return translateQueryAliasesV2000(this.sketcher.getMolFile());
  }

  isEmptyMolecule(molecule: string | null | undefined): boolean {
    if (molecule == null || molecule === '') return true;
  ...
  get isFiltering(): boolean {
    ...
    return filterEnabled && !this.isEmptyMolecule(molFile);
  }
  ...
  constructor() {
    ...
    this.sketcher.host.classList.add('chem-substructure-filter-sketcher');
  ...
  attach(dataFrame: DG.DataFrame): void {
    ...
    // On cell edits, queue the edited rows; they are matched + patched (O(k), not a full re-scan)
    // once no search is running — see _onCellsEdited / _drainChangedValuesRows.
    ...
    this.subs.push(grok.events.onCustomEvent(FILTER_SYNC_EVENT).subscribe((state: ISubstructureFilterState) => {
      if (state.colName === this.columnName && this.tableName == state.tableName && this.filterId !== state.filterId) {
        ...
        const updateMolecule = this.currentMolecule != state.molblock!;
        ...
        this.currentMolecule = state.molblock!;
  ...
  refresh() {
    ...
    ui.tools.waitForElementInDom(this.sketcher.root).then(() => {
      ...
      if (this.currentMolecule && molFile !== this.currentMolecule) {
        this.syncEvent = true;
        this.sketcher.setMolFile(this.currentMolecule);
      }
    });
  }
  ...
  detach() {
    ...
    if (this.column!.temp[CHEM_APPLY_FILTER_SYNC] && this.column!.temp[CHEM_APPLY_FILTER_SYNC].filterId === this.filterId) {
      ...
      grok.events.fireCustomEvent(FILTER_SYNC_EVENT, this.filterSyncState());
    };
  ...
  /** Queue the edited rows, then drain them now if idle. If a search is in flight we DON'T patch or
   * restart it (the column data it reads is mid-flight) — finishSearch() drains the queue once the
   * search completes. A peer filter owning the search patches and syncs the result to us, so we skip. */
  async _onCellsEdited(indexes: Int32Array | number[]): Promise<void> {
    ...
    for (const i of indexes)
      this.changedValuesRows.add(i);
    // a full search is running -> defer; finishSearch() will drain once it completes
    if (this.isSearchRunning)
      return;
    await this._drainChangedValuesRows();
  }

  /** Match the queued edited rows in a worker and patch their bits into the cached bitset, then sync
   * peers. Falls back to a full recompute when patching isn't possible (filter disabled, no cached
   * result, a recompute is already pending, or EXACT_MATCH — the worker compares raw strings against
   * the canonicalized column, which a raw edited cell value can't reproduce; see searchWithPatternFps). */
  async _drainChangedValuesRows(): Promise<void> {
    if (this.changedValuesRows.size === 0)
      return;
    try {
      // disabled/empty: nothing to apply now — mark dirty so a re-enable recomputes, drop the edits
      if (!this.isFiltering) {
        this.recalculateFilter = true;
        this.changedValuesRows.clear();
        return;
      }
      // no cached result / recompute already pending / EXACT_MATCH -> recompute the whole column
      ...
      // snapshot + clear so edits arriving during the awaits re-queue and get drained in finally
      const bitset = this.bitset; // capture before the await — a sketch change/peer sync can replace it
      const rows = Array.from(this.changedValuesRows);
      this.changedValuesRows.clear();
      ...
      grok.events.fireCustomEvent(FILTER_SYNC_EVENT, this.filterSyncState(bitset));
    } catch (e) {
      ...
      // edits that arrived while we matched/recomputed -> drain them too, but only if idle now
      // (a fallback _onSketchChanged above may have started a new search, which will drain on finish)
      if (this.changedValuesRows.size > 0 && !this.isSearchRunning)
        await this._drainChangedValuesRows();
    }
  }
  ...
  applyFilter(): void {
    ...
    if (activeFilterId !== this.filterId) {
      if (activeFilterId === -1) {
        this.column!.temp[CHEM_APPLY_FILTER_SYNC] =
          {filterId: this.filterId, summary: this.getFilterSummary(this.currentMolecule)};
      } else {
  ...
  saveState(): any {
    ...
    state.molBlock = this.currentMolecule;
  ...
  async _onSketchChanged(): Promise<void> {
    ...
    if (this.currentMolecule !== newMolecule)
      this.updateFilterUiOnSketcherChanged(newMolecule);
    ...
    if (!this.isFiltering) {
      ...
      .has(`${this.columnName}: ${this.getFilterSummary(newMolecule)}`) || this.isFilteringBySameStructure(newMolecule)) &&
      ...
      if (this.currentMolecule !== newMolecule && this.sketcher.align) {
        this.currentMolecule = newMolecule;
        this.setFilterScaffoldTag();
      }
      ...
      _package.logger.debug(`already filter by the same structure ${this.getFilterSummary(newMolecule)} , ${this.filterId}`);
      return;
    } else {
      ...
      _package.logger.debug(`starting filter by ${this.currentMolecule}, ${this.filterId}`);
      try {
        grok.events.fireCustomEvent(FILTER_SYNC_EVENT, this.filterSyncState());
        ...
        this.batchResultObservable = grok.events.onCustomEvent(this.progressEventName).subscribe((progress: number) => {
          _package.logger.debug(`progress: ${progress}, molfile: ${this.currentMolecule}, ${this.filterId}`);
          this.bitset = DG.BitSet.fromBytes(bitArray.buffer.buffer, this.column!.length);
          this.dataFrame?.rows.requestFilter();
  ...
  updateFilterUiOnSketcherChanged(newMolFile: string) {
    if (this.sketcher._mode !== DG.chem.SKETCHER_MODE.INPLACE) {
      if (!this.isEmptyMolecule(newMolFile)) {
        ...
        if (this.searchType === SubstructureSearchType.CONTAINS) {
          ...
          if (this.searchType === SubstructureSearchType.IS_SIMILAR)
            ...
            this.removeChildIfExists(this.searchOptionsDiv, this.similarityOptionsDiv,
              'chem-filter-similarity-options');
          }
        }
      } else {
        this.emptySketcherDiv.append(this.searchTypeInput.root);
  ...
  onSearchTypeChanged() {
    ...
    if (this.searchType !== SubstructureSearchType.CONTAINS)
      ...
      if (!this.isEmptyMolecule(this.getSketcherMolecule()) && this.sketcher._mode !== DG.chem.SKETCHER_MODE.INPLACE)
        this.sketcher.root.appendChild(this.optionsIcon);
    }
    if (this.searchType === SubstructureSearchType.IS_SIMILAR) {
      this.searchOptionsDiv.append(this.searchTypeInput.root);
  ...
  finishSearch(queryMolAndType: string) {
    ...
    const finish = () => {
      ...
      if (this.currentSearches.size === 0) {
        ...
        // edits made while the search was running: now that it's done, refresh just those rows
        if (this.changedValuesRows.size > 0)
          void this._drainChangedValuesRows();
      }
    };
  ...
  getFilterSummary(molfile: string): string {
    const smarts = this.moleculeToSmarts(molfile);
    return this.searchType === SubstructureSearchType.IS_SIMILAR ?
      `${smarts}_${this.searchType}_${this.similarityCutOff}_${this.fp}` :
      `${smarts}_${this.searchType}`;
  }
}
