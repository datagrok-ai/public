...
def calculate_logd(mol_str: str, index: int) -> float:
    ...
    try:
        ...
        sum_acids = sum(10**(7.4 - pka) for pka in acidic_pkas)
        sum_bases = sum(10**(pka - 7.4) for pka in basic_pkas)

        return logp - math.log10(1.0 + sum_acids + sum_bases)
    except Exception as e:
        print(f"ERROR on logD for row {index}: {e}")
        return None

molecule_data = table[molecules_column_name]
logd_values = [calculate_logd(m, i) for i, m in enumerate(molecule_data)]
result = pd.DataFrame({'logD @ pH 7.4': logd_values}, index=table.index)