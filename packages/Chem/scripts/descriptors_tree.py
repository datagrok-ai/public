...
for desc in descs:
  ...
  descriptor = {
    'name': name,
    'description': descriptors_params[name]['description'] if name in descriptors_params else '',
    'type': descriptors_params[name]['type'] if name in descriptors_params else 'string',
    'tags': descriptors_params[name]['tags'] if name in descriptors_params else []
  }
  if module_name in _descriptors_tree:
    ...
    _descriptors_tree[module_name] = {
      'version': version,
      'name': descriptors_modules[module_name]['name'] if ((module_name in descriptors_modules) and ('name' in descriptors_modules[module_name])) else module_name,
      'description': descriptors_modules[module_name]['description'] if module_name in descriptors_modules else '',
      'descriptors': [descriptor]}

tree = json.dumps(_descriptors_tree)
