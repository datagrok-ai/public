...
Feature: Names To Smiles over a column of compound names
  ...
  RDKit — so the lookup needs the ChEMBL database of the stand. On a table that already carries a
  canonical_smiles column the structures go into a column of their own, canonical_smiles (2)
  (GROK-20955).
  ...
  Scenario: The names go into a column of their own beside an existing canonical_smiles
    ...
    And a new column "canonical_smiles (2)" should have been added
    And the molecule in row 1 of "canonical_smiles (2)" column should be "CC(=O)Oc1ccccc1C(=O)O"
    And the value of "canonical_smiles" column in row 1 should be "CCO"
