...
Feature: The Similarity Search viewer and its properties
  ...
  and the three metrics (Tanimoto, Dice, Cosine) runs the search again and names itself in the
  viewer's header, Limit sets how many cards there are, Size sets their size, Molecule Properties
  adds the chosen columns to the cards, and Cutoff 1 leaves the cards that score 1.
  ...
  Scenario: Every metric runs the search again
    When user selects "Dice" in "Distance Metric" property
    ...
    When user selects "Cosine" in "Distance Metric" property
    Then the "header" reading of Chem Similarity Search viewer should be "Cosine, Morgan"
    When user selects "Tanimoto" in "Distance Metric" property
    Then the "header" reading of Chem Similarity Search viewer should be "Tanimoto, Morgan"
    And no error or warning balloon should have been shown
    And no errors should have been logged
