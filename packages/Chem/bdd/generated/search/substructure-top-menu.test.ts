...
import {clickOn, pressKeyIn, shouldBe, shouldNotBe, typeInto} from '@datagrok-libraries/bdd/bindings/common/steps';
...
test.describe("Substructure search from the Search menu", () => {
  ...
  test("Substructure search from the Search menu", {tag: ["@journey", "@realizes:chem.cp.substructure-search-top-menu"]}, async ({browser}) => {
    ...
    await run.scenario("The command prepares an empty card and opens its sketcher", async () => {
      ...
      await session.step(20, "And the \"cards\" reading of filter panel should contain \"canonical_smiles\"", () => readingContains(page, "cards", el("filter panel"), "canonical_smiles"));
      await session.step(21, "And the \"structure of canonical_smiles\" reading of filter panel should be \"\"", () => readingReads(page, "structure of canonical_smiles", el("filter panel"), ""));
    ...
    await run.scenario("Benzene keeps the molecules that contain it", async () => {
      await session.step(26, "When user types \"c1ccccc1\" into molecule input of sketcher dialog", () => typeInto(page, "c1ccccc1", el("molecule input of sketcher dialog")));
      await session.step(27, "And user presses Enter in molecule input of sketcher dialog", () => pressKeyIn(page, "Enter", el("molecule input of sketcher dialog")));
      await session.step(28, "And user clicks on OK button in sketcher dialog", () => clickOn(page, el("OK button in sketcher dialog")));
      await session.step(29, "Then 924 rows should pass the filter", () => filterPasses(page, 924));
      await session.step(30, "And the \"structure of canonical_smiles\" reading of filter panel should be \"c1ccccc1\"", () => readingReads(page, "structure of canonical_smiles", el("filter panel"), "c1ccccc1"));
      await session.step(31, "And the filter should pass exactly the molecules of \"canonical_smiles\" column containing \"c1ccccc1\"", () => filterPassesMatching(page, "canonical_smiles", "c1ccccc1"));
    ...
    await run.scenario("A gold atom keeps no molecule and deletes no row", async () => {
      await session.step(35, "When user clicks on the \"card canonical_smiles\" area of filter panel", () => clickArea(page, "card canonical_smiles", el("filter panel")));
      await session.step(36, "Then sketcher dialog should be visible", () => shouldBe(page, el("sketcher dialog"), "visible"));
      await session.step(37, "When user types \"[Au]\" into molecule input of sketcher dialog", () => typeInto(page, "[Au]", el("molecule input of sketcher dialog")));
      ...
      await session.step(40, "Then 0 rows should pass the filter", () => filterPasses(page, 0));
      await session.step(41, "And the filter should pass exactly the molecules of \"canonical_smiles\" column containing \"[Au]\"", () => filterPassesMatching(page, "canonical_smiles", "[Au]"));
      await session.step(42, "And the table should have 1000 rows", () => rowCount(page, 1000));
      await session.step(43, "And no errors should have been logged", () => noErrors(page));
    });
    await run.scenario("A second invocation starts from an empty card without asking for the column", async () => {
      await session.step(46, "When user picks \"Chem > Search > Substructure Search...\" from the top menu", () => pickFromTopMenu(page, "Chem > Search > Substructure Search..."));
      await session.step(47, "Then sketcher dialog should be visible", () => shouldBe(page, el("sketcher dialog"), "visible"));
      await session.step(48, "And \"Substructure search\" dialog should not be visible", () => shouldNotBe(page, el("\"Substructure search\" dialog"), "visible"));
