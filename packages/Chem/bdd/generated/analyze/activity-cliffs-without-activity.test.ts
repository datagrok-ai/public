...
test.describe("Activity Cliffs on a table with no numeric column", () => {
  ...
  test("OK clicked at once runs nothing and the dialog stays open", {tag: ["@regression", "@realizes:GROK-20915"]}, async ({browser}) => {
    ...
    await session.step(16, "And user opens smiles-only dataset", () => openDataset(page, ds("smiles-only")));
    await session.step(20, "Given user watches the OK button of the next dialog", () => watchNextOk(page));
    await session.step(21, "When user picks \"Chem > Analyze > Activity Cliffs...\" from the top menu", () => pickFromTopMenu(page, "Chem > Analyze > Activity Cliffs..."));
    await session.step(22, "And user clicks on OK button in \"Activity Cliffs\" dialog at once", () => clickAtOnce(page, el("OK button in \"Activity Cliffs\" dialog")));
    await session.step(23, "Then the OK button should have been disabled when it appeared and when it was clicked", () => okDisabledThroughout(page));
    await session.step(24, "And no error or warning balloon and no error should appear for 2 seconds", () => quietWindow(page, 2));
