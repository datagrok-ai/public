...
test.describe("The sketcher opened from a molecule cell", () => {
  ...
  test("The sketcher opened from a molecule cell", {tag: ["@journey", "@realizes:chem.cp.sketcher-cell-editor"]}, async ({browser}) => {
    ...
    await session.step(9, "And the molecule sketcher is \"OpenChemLib\"", () => sketcherIs(page, "OpenChemLib"));
    await session.step(10, "And the package autostarts have completed", () => autostartsCompleted(page));
    await session.step(11, "And user opens smiles-50 dataset", () => openDataset(page, ds("smiles-50")));
    await run.scenario("A double-click opens the sketcher on the cell's molecule", async () => {
      await session.step(14, "When user double-clicks on the \"cell 1 of canonical_smiles\" area of grid", () => doubleClickArea(page, "cell 1 of canonical_smiles", el("grid")));
      await session.step(15, "Then sketcher dialog should be visible", () => shouldBe(page, el("sketcher dialog"), "visible"));
      await session.step(16, "And molecule input of sketcher dialog should be visible", () => shouldBe(page, el("molecule input of sketcher dialog"), "visible"));
      await session.step(17, "And no errors should have been logged", () => noErrors(page));
    });
    await run.scenario("A molecule typed into the sketcher replaces the one in the cell", async () => {
      await session.step(20, "When user types \"C1CCCCC1\" into molecule input of sketcher dialog", () => typeInto(page, "C1CCCCC1", el("molecule input of sketcher dialog")));
      ...
      await session.step(23, "Then sketcher dialog should be absent", () => shouldBe(page, el("sketcher dialog"), "absent"));
      await session.step(24, "And the value of \"canonical_smiles\" column in row 1 should be \"C1CCCCC1\"", () => valueInRow(page, "canonical_smiles", 1, "C1CCCCC1"));
      await session.step(25, "And the table should have 50 rows", () => rowCount(page, 50));
    ...
    await run.scenario("Reopening the cell brings the sketcher up on the new molecule", async () => {
      await session.step(29, "When user double-clicks on the \"cell 1 of canonical_smiles\" area of grid", () => doubleClickArea(page, "cell 1 of canonical_smiles", el("grid")));
      await session.step(30, "Then sketcher dialog should be visible", () => shouldBe(page, el("sketcher dialog"), "visible"));
      ...
      await session.step(33, "And the value of \"canonical_smiles\" column in row 1 should be \"C1CCCCC1\"", () => valueInRow(page, "canonical_smiles", 1, "C1CCCCC1"));
