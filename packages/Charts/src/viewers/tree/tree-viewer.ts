...
export class TreeViewer extends EChartViewer {
    ...
    const handleZrMouseOut = () => {
      if (this.hoveredPath) {
        ...
        if (this.filteredPath) {
          const filterTree = this.getSelectionData(false);
          const filterDict = this.addParentPaths(filterTree);
          this.paintBranchByPath(this.filteredPath, null, filterDict);
        }
  ...
  handleTreeClick(pathString: string, color?: number): void {
    ...
    const filterTree = this.getSelectionData(false);
    const filterDict = this.addParentPaths(filterTree);
    this.paintBranchByPath(pathString, color, filterDict);
  }
  ...
  onPropertyChanged(p: DG.Property | null, render: boolean = true) {
    ...
    case 'filteredRowsColor':
      if (this.filteredPath) {
        const filterTree = this.getSelectionData(false);
        const filterDict = this.addParentPaths(filterTree);
        this.paintBranchByPath(this.filteredPath, this.filteredRowsColor, filterDict);
      }
      break;
  ...
  getSelectionData(select: boolean = true): SelectionData {
    const rowMask = select ? this.dataFrame.selection.clone() : this.dataFrame.filter.clone();
