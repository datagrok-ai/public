...
export class MultivariateAnalysisTutorial extends Tutorial {
  ...
  protected async _run() {
    ...
    this.describe('The multivariate uses a statistical method that resembles principal components regression. Instead of finding hyperplanes of maximum variance between the response and independent variables, it finds a linear regression model by projecting the predicted variables and the observable variables to a new space.\n\nIn the following example, we will predict a car price by its attributes, and see how different measures are related to each other and to the outcome.');
   ...
   //    'Features', this.t.columns.names().join(','));
    ...
    `Once you run the analysis, the following visualizations will appear:
    ...
    </ul> <br> Look into these findings to figure out which variables correlate with each other and which of them have a greater impact on the outcome.\nSee the context help on the right to learn more about multivariate analysis, or navigate to a documentation page by clicking the "Open in new tab" button.`,
