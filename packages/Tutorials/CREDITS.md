# Tutorials — Third-Party Libraries

The `@datagrok/tutorials` package is distributed under the MIT license that
covers the rest of the `public/` repository (see [`../../LICENSE.md`](../../LICENSE.md)).

This package provides the interactive tutorial framework and demo app; it
...
`@datagrok-libraries/tutorials`) covered by the repo-wide `LICENSE.md`.

---

## 2. Linked at runtime via the Datagrok platform (webpack externals)

These libraries are not bundled into Tutorials' `dist/` — they are provided
...
as part of the published Tutorials plugin and impose no obligation on users of
the plugin.
