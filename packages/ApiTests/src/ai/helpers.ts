...
export function expectPropAndLook(v: DG.Viewer, partial: {[k: string]: any}): void {
  for (var k of Object.keys(partial))
...
export function findProp(v: DG.Viewer, name: string): DG.Property | null {
  for (var p of v.getProperties() as DG.Property[])
...
export function subscribeAll(streams: Observable<any>[]): () => void {
  ...
  return () => { for (var sub of subs) sub.unsubscribe(); };
}
...
export async function expectFiresWithin(stream: Observable<any>, trigger: () => void, ms: number = 1500): Promise<void> {
  ...
  try {
    ...
    const timeout: Promise<boolean> = (async () => { await DG.delay(ms); return false; })();
    trigger();
