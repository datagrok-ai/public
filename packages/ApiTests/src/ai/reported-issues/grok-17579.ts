...
category('AI: GROK-17579: Bar chart tooltip hide', () => {
  ...
  // value round-trips through `setOptions` → `getOptions(true).look.showTooltip`,
  // and `getProperties()` does not list a `showTooltip` descriptor on a bar chart.
  // The only invariant we can pin from JS is that calling `setOptions({showTooltip: ...})`
  // does not throw. The rest of the fix surface is exercised by manual / Selenium QA.
  test('setOptions({showTooltip}) is non-throwing for arbitrary string', async () => {
    const v = DG.Viewer.barChart(demog(), {value: 'age', split: 'race'});
    expectNoThrow(() => v.setOptions({showTooltip: 'arbitrary-value-xyz'}));
  });
});
