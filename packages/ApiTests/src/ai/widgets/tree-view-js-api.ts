...
import {expectBoolGetSet, expectFiresWithin, subscribeAll, expectNoThrow} from '../helpers';
...
category('AI: Widgets: TreeView JS API', () => {
  ...
  test('onNodeExpanding / onChildNodeExpanding / onChildNodeExpandedChanged fire on first expand; ' +
    ...
    const root = ui.tree();
    // Group must be collapsed before subscribing so the first expand fires the events.
