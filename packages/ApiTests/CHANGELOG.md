...
GROK-20799: `dapi/domain-access.ts` (`Dapi: domain access`) pins `DomainTableClient.access()` — the `{can, fields}` shape with a restricted column absent from `fields`, the server-composed `travelableRelations` and `securingTable` (declared relation travelable for admin and not for an ungranted user; a master-mode junction secured by its delegate target), grant flips under a restricted user, and the `withAccess` rows of `query` / `get` / `queryDf` (bool columns tagged out of export); the handler, editor and app-framework suites read `access` / `~can_*`
