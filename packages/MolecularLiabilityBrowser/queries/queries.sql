...
--connection: MLB
select v_id from public.ptm_observed_v2
--end

--name: getJsonObsByVid
--connection: MLB
--input: string vid
select json from public.ptm_observed_v2
where v_id=@vid
--end
...
select json from public.jsons_new
where v_id=@vid
--end

--name: getIdMappings
--connection: MLB
select * from db_v2.gdb_id_mappings
--end

--name: getJsonByVid
--connection: MLB
--input: string vid = VR000030945
select encode(json_data, 'escape') from db_v2.json_files
where v_id=@vid
--end
...
--connection: MLB
--input: string vid = VR000030945
select encode(pdb_data, 'escape') from db_v2.pdb_files
where v_id=@vid
--end
