...
export const zoomValueAxis = When('user zooms into the value axis of {element}', async (page: Page, target: ElementRef) => {
  const loc = await viewers.viewerLocator(page, target);
...
export const narrowedRange = Then('{element} should show a narrowed value range', async (page: Page, target: ElementRef) => {
  const loc = await viewers.viewerLocator(page, target);
...
export const fullRange = Then('{element} should show the full value range again', async (page: Page, target: ElementRef) => {
  const loc = await viewers.viewerLocator(page, target);
  const full: number = await loc.evaluate((el) => (window as any).__bdd.viewerOf(el).root.__fullRange);
...
export const doubleClickEmptySpace = When('user double-clicks on empty plot space of {element}', async (page: Page, target: ElementRef) => {
