...
test.describe("A SQL query from creation to deletion", () => {
  ...
  test("A SQL query from creation to deletion", {tag: ["@journey", "@serial", "@realizes:views.queries"]}, async ({browser}) => {
    ...
    await run.scenario("Delete asks first, then removes the query from the server and the tree", async () => {
      ...
      await session.step(101, "And user hovers over Databases---Postgres---NorthwindTest---BDD-Q-life-renamed-{time} tree node inside browse tree", () => hoverOver(page, el(session.text("Databases---Postgres---NorthwindTest---BDD-Q-life-renamed-{time} tree node inside browse tree"))));
      await session.step(102, "And user picks \"Delete\" from the context menu of Databases---Postgres---NorthwindTest---BDD-Q-life-renamed-{time} tree node inside browse tree", () => pickFromContextMenu(page, "Delete", el(session.text("Databases---Postgres---NorthwindTest---BDD-Q-life-renamed-{time} tree node inside browse tree"))));
      await session.step(103, "Then \"Are you sure?\" dialog should be visible", () => shouldBe(page, el("\"Are you sure?\" dialog"), "visible"));
      await session.step(104, "And \"Are you sure?\" dialog should contain the text \"BDD-Q-life-renamed-{time}\"", () => shouldContainText(page, el("\"Are you sure?\" dialog"), session.text("BDD-Q-life-renamed-{time}")));
      await session.step(105, "When user clicks on DELETE button in \"Are you sure?\" dialog", () => clickOn(page, el("DELETE button in \"Are you sure?\" dialog")));
      await session.step(106, "Then the \"Are you sure?\" dialog should close", () => dialogCloses(page, "Are you sure?"));
      await session.step(107, "And 0 queries named \"BDD-Q-life-renamed-{time}\" should be on the server", () => queriesOnServer(page, 0, session.text("BDD-Q-life-renamed-{time}")));
      await session.step(108, "When user clicks on \"Refresh\" icon inside browse toolbar", () => clickOn(page, el("\"Refresh\" icon inside browse toolbar")));
      await session.step(109, "Then Databases---Postgres---NorthwindTest---BDD-Q-life-renamed-{time} tree node inside browse tree should be absent", () => shouldBe(page, el(session.text("Databases---Postgres---NorthwindTest---BDD-Q-life-renamed-{time} tree node inside browse tree")), "absent"));
