...
test.describe("A query's layout", () => {
  ...
  test("A query's layout", {tag: ["@journey", "@serial", "@realizes:views.queries"]}, async ({browser}) => {
    ...
    await run.scenario("The Layout tab waits for a run, then takes viewers from the toolbox", async () => {
      ...
      await session.step(27, "When user enters \"BDD-Q-layout-{time}\" into Name input", () => enterInto(page, session.text("BDD-Q-layout-{time}"), el("Name input")));
      ...
      await session.step(29, "And user clicks on Layout tab", () => clickOn(page, el("Layout tab")));
      await session.step(30, "Then \"Run query to get data and edit layout\" text should be visible", () => shouldBe(page, el("\"Run query to get data and edit layout\" text"), "visible"));
      await session.step(31, "When user clicks on play icon", () => clickOn(page, el("play icon")));
      ...
      await session.step(35, "When user moves the pointer away from play icon", () => pointerAway(page, el("play icon")));
      await session.step(36, "And user clicks on scatter plot icon in toolbox", () => clickOn(page, el("scatter plot icon in toolbox")));
      await session.step(37, "And user clicks on correlation plot icon in toolbox", () => clickOn(page, el("correlation plot icon in toolbox")));
      await session.step(38, "Then scatter plot viewer should be visible", () => shouldBe(page, el("scatter plot viewer"), "visible"));
      await session.step(39, "And correlation plot viewer should be visible", () => shouldBe(page, el("correlation plot viewer"), "visible"));
    ...
    await run.scenario("Save keeps the query with its layout, and the saved query runs with its viewers", async () => {
      ...
      await session.step(45, "Then 1 query named \"BDD-Q-layout-{time}\" should be on the server", () => queriesOnServer(page, 1, session.text("BDD-Q-layout-{time}")));
      await session.step(46, "And the query \"BDD-Q-layout-{time}\" on the server should have a layout", () => entityHasLayout(page, "query", session.text("BDD-Q-layout-{time}")));
      ...
      await session.step(54, "And the open tableview should have 1 scatter plot viewer", () => viewerCount(page, 1, "scatter plot"));
      await session.step(55, "And the open tableview should have 1 correlation plot viewer", () => viewerCount(page, 1, "correlation plot"));
    });
    run.finish();
  });
});
