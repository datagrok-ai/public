...
test.describe("A query result saved as a project", () => {
  ...
  test("A query result saved as a project", {tag: ["@journey", "@serial", "@realizes:views.queries"]}, async ({browser}) => {
    ...
    await run.scenario("A query result with a viewer is saved as a project", async () => {
      ...
      await session.step(26, "When user picks \"Run\" from the context menu of Databases---Postgres---NorthwindTest---PostgresByStringChoices tree node inside browse tree", () => pickFromContextMenu(page, "Run", el("Databases---Postgres---NorthwindTest---PostgresByStringChoices tree node inside browse tree")));
      await session.step(27, "And user selects \"France\" in \"Ship Country\" input in \"PostgresByStringChoices\" dialog", () => selectIn(page, "France", el("\"Ship Country\" input in \"PostgresByStringChoices\" dialog")));
      await session.step(28, "And user clicks on OK button in \"PostgresByStringChoices\" dialog", () => clickOn(page, el("OK button in \"PostgresByStringChoices\" dialog")));
      await session.step(29, "Then the current view should be a TableView view", () => currentViewType(page, "TableView"));
      await session.step(30, "And the table should have 77 rows", () => rowCount(page, 77));
      await session.step(31, "Given the toolbox pane is shown", () => toolboxPaneShown(page));
      await session.step(32, "When user clicks on trellis plot icon in toolbox", () => clickOn(page, el("trellis plot icon in toolbox")));
      await session.step(33, "Then the open tableview should have 1 trellis plot viewer", () => viewerCount(page, 1, "trellis plot"));
      await session.step(34, "When user clicks on Save button", () => clickOn(page, el("Save button")));
      await session.step(35, "Then \"Save project\" dialog should be visible", () => shouldBe(page, el("\"Save project\" dialog"), "visible"));
      await session.step(36, "When user enters \"BDD-Q-proj-{time}\" into Name text input in \"Save project\" dialog", () => enterInto(page, session.text("BDD-Q-proj-{time}"), el("Name text input in \"Save project\" dialog")));
      ...
      await session.step(39, "And 1 project named \"BDD-Q-proj-{time}\" should be on the server", () => projectsOnServer(page, 1, session.text("BDD-Q-proj-{time}")));
      await session.step(40, "And \"Share BDD-Q-proj-{time}\" dialog should be visible", () => shouldBe(page, el(session.text("\"Share BDD-Q-proj-{time}\" dialog")), "visible"));
      await session.step(41, "When user clicks on CANCEL button in \"Share BDD-Q-proj-{time}\" dialog", () => clickOn(page, el(session.text("CANCEL button in \"Share BDD-Q-proj-{time}\" dialog"))));
      await session.step(42, "Then the \"Share BDD-Q-proj-{time}\" dialog should close", () => dialogCloses(page, session.text("Share BDD-Q-proj-{time}")));
    ...
    await run.scenario("The saved project opens with its table and its viewer", async () => {
      await session.step(47, "When user closes all views", () => closeAllViews(page));
      await session.step(48, "And user opens the \"BDD-Q-proj-{time}\" project and waits for its table", () => openProjectWithTable(page, session.text("BDD-Q-proj-{time}")));
      await session.step(49, "Then the table should have 77 rows", () => rowCount(page, 77));
      await session.step(50, "And the open tableview should have 1 trellis plot viewer", () => viewerCount(page, 1, "trellis plot"));
