...
test.describe("Transformations saved with a query", () => {
  ...
  test("Transformations saved with a query", {tag: ["@journey", "@serial", "@realizes:views.queries"]}, async ({browser}) => {
    ...
    await run.scenario("A column added in the Transformations tab is in the result of the saved query", async () => {
      ...
      await session.step(45, "And the table should have 77 rows", () => rowCount(page, 77));
      await session.step(46, "And the table should have a column \"doubled\"", () => hasColumn(page, "doubled"));
      await session.step(47, "And the value of \"doubled\" column in row 1 should be \"2\"", () => valueInRow(page, "doubled", 1, "2"));
    ...
    await run.scenario("Removing the step takes the column out of the saved query's result", async () => {
      await session.step(52, "When user closes the current view", () => closeCurrentView(page));
      await session.step(53, "Then the current view should be a DataQueryView view", () => currentViewType(page, "DataQueryView"));
      await session.step(54, "When user clicks on Transformations tab", () => clickOn(page, el("Transformations tab")));
      await session.step(56, "And user hovers over last \"Remove step\" icon", () => hoverOver(page, el("last \"Remove step\" icon")));
      await session.step(57, "And user clicks on last \"Remove step\" icon", () => clickOn(page, el("last \"Remove step\" icon")));
      await session.step(58, "And user clicks on Save button", () => clickOn(page, el("Save button")));
      await session.step(59, "And user clicks on \"Run query...\" action in toolbox", () => clickOn(page, el("\"Run query...\" action in toolbox")));
      ...
      await session.step(62, "And the table should not have a column \"doubled\"", () => hasNoColumn(page, "doubled"));
