...
test.describe("A SQL query started from a table of the schema", () => {
  ...
  test("The editor opens on the table's select, runs it and saves it", {tag: ["@serial", "@realizes:views.queries"]}, async ({browser}) => {
    ...
    await session.step(17, "And no query named \"BDD-Q-sql-{time}\" is on the server", () => noQueryOnServer(page, session.text("BDD-Q-sql-{time}")));
    ...
    await session.step(28, "When user picks \"New SQL Query...\" from the context menu of Databases---Postgres---NorthwindTest---Schemas---public---products tree node inside browse tree", () => pickFromContextMenu(page, "New SQL Query...", el("Databases---Postgres---NorthwindTest---Schemas---public---products tree node inside browse tree")));
    await session.step(29, "Then the current view should be a DataQueryView view", () => currentViewType(page, "DataQueryView"));
    await session.step(30, "And Name input should have value \"products\"", () => shouldHaveValue(page, el("Name input"), "products"));
    await session.step(31, "And code editor should hold the code \"select * from public.products\"", () => holdsCode(page, el("code editor"), "select * from public.products"));
    ...
    await session.step(41, "When user enters \"BDD-Q-sql-{time}\" into Name input", () => enterInto(page, session.text("BDD-Q-sql-{time}"), el("Name input")));
    await session.step(42, "And user clicks on Save button", () => clickOn(page, el("Save button")));
    await session.step(43, "Then 1 query named \"BDD-Q-sql-{time}\" should be on the server", () => queriesOnServer(page, 1, session.text("BDD-Q-sql-{time}")));
