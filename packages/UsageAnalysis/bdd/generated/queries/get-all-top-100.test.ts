...
test.describe("Get All and Get Top 100 on a table of the schema", () => {
  ...
  test("Postgres opens the whole table and its first hundred rows [provider=Postgres, other=PostgresDart]", {tag: ["@realizes:views.queries"]}, async ({browser}) => {
    ...
    await session.step(29, "Given user watches the task bar", () => watchTaskBar(page));
    await session.step(31, "When user hovers over Databases---Postgres---NorthwindTest---Schemas---public---orders tree node inside browse tree", () => hoverOver(page, el("Databases---Postgres---NorthwindTest---Schemas---public---orders tree node inside browse tree")));
    await session.step(32, "And user picks \"Get All\" from the context menu of Databases---Postgres---NorthwindTest---Schemas---public---orders tree node inside browse tree", () => pickFromContextMenu(page, "Get All", el("Databases---Postgres---NorthwindTest---Schemas---public---orders tree node inside browse tree")));
    await session.step(33, "Then the task bar should have finished \"orders\"", () => taskBarFinished(page, "orders"));
    await session.step(34, "And table \"orders\" should have 830 rows", () => tableRows(page, "orders", 830));
    await session.step(35, "Given the toolbox pane is hidden", () => toolboxPaneHidden(page));
    ...
    await session.step(39, "When user hovers over Databases---Postgres---NorthwindTest---Schemas---public---orders tree node inside browse tree", () => hoverOver(page, el("Databases---Postgres---NorthwindTest---Schemas---public---orders tree node inside browse tree")));
    await session.step(40, "And user picks \"Get Top 100\" from the context menu of Databases---Postgres---NorthwindTest---Schemas---public---orders tree node inside browse tree", () => pickFromContextMenu(page, "Get Top 100", el("Databases---Postgres---NorthwindTest---Schemas---public---orders tree node inside browse tree")));
    await session.step(41, "Then the task bar should have finished \"orders\"", () => taskBarFinished(page, "orders"));
    await session.step(42, "And table \"orders (2)\" should have 100 rows", () => tableRows(page, "orders (2)", 100));
    await session.step(43, "And table \"orders\" should have 830 rows", () => tableRows(page, "orders", 830));
  ...
  test("PostgresDart opens the whole table and its first hundred rows [provider=PostgresDart, other=Postgres]", {tag: ["@realizes:views.queries", "@full-stand"]}, async ({browser}) => {
    ...
    await session.step(27, "Given Databases---PostgresDart---NorthwindTest---Schemas tree node inside browse tree is expanded", () => isExpanded(page, el("Databases---PostgresDart---NorthwindTest---Schemas tree node inside browse tree")));
    ...
    await session.step(29, "Given user watches the task bar", () => watchTaskBar(page));
    await session.step(31, "When user hovers over Databases---PostgresDart---NorthwindTest---Schemas---public---orders tree node inside browse tree", () => hoverOver(page, el("Databases---PostgresDart---NorthwindTest---Schemas---public---orders tree node inside browse tree")));
    await session.step(32, "And user picks \"Get All\" from the context menu of Databases---PostgresDart---NorthwindTest---Schemas---public---orders tree node inside browse tree", () => pickFromContextMenu(page, "Get All", el("Databases---PostgresDart---NorthwindTest---Schemas---public---orders tree node inside browse tree")));
    await session.step(33, "Then the task bar should have finished \"orders\"", () => taskBarFinished(page, "orders"));
    await session.step(34, "And table \"orders\" should have 830 rows", () => tableRows(page, "orders", 830));
    await session.step(35, "Given the toolbox pane is hidden", () => toolboxPaneHidden(page));
    await session.step(36, "And the browse panel is open", () => browsePanelOpen(page));
    await session.step(37, "Given user watches the task bar", () => watchTaskBar(page));
    await session.step(39, "When user hovers over Databases---PostgresDart---NorthwindTest---Schemas---public---orders tree node inside browse tree", () => hoverOver(page, el("Databases---PostgresDart---NorthwindTest---Schemas---public---orders tree node inside browse tree")));
    await session.step(40, "And user picks \"Get Top 100\" from the context menu of Databases---PostgresDart---NorthwindTest---Schemas---public---orders tree node inside browse tree", () => pickFromContextMenu(page, "Get Top 100", el("Databases---PostgresDart---NorthwindTest---Schemas---public---orders tree node inside browse tree")));
    await session.step(41, "Then the task bar should have finished \"orders\"", () => taskBarFinished(page, "orders"));
    await session.step(42, "And table \"orders (2)\" should have 100 rows", () => tableRows(page, "orders (2)", 100));
    await session.step(43, "And table \"orders\" should have 830 rows", () => tableRows(page, "orders", 830));
    await session.step(44, "And no errors should have been logged", () => noErrors(page));
