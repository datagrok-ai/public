...
test.describe("A visual query built on a table", () => {
  ...
  test("A visual query built on a table", {tag: ["@journey", "@serial", "@realizes:views.queries"]}, async ({browser}) => {
    ...
    await run.scenario("The builder groups and aggregates a table", async () => {
      ...
      await session.step(30, "When user hovers over Databases---Postgres---NorthwindTest---Schemas---public---customers tree node inside browse tree", () => hoverOver(page, el("Databases---Postgres---NorthwindTest---Schemas---public---customers tree node inside browse tree")));
      await session.step(31, "And user picks \"New Visual Query...\" from the context menu of Databases---Postgres---NorthwindTest---Schemas---public---customers tree node inside browse tree", () => pickFromContextMenu(page, "New Visual Query...", el("Databases---Postgres---NorthwindTest---Schemas---public---customers tree node inside browse tree")));
      await session.step(32, "Then the current view should be a DataQueryView view", () => currentViewType(page, "DataQueryView"));
      await session.step(33, "When user adds \"companyname\" to the \"Group-by\" row of the visual query", () => addToBuilderRow(page, "companyname", "Group-by"));
      await session.step(34, "Then the \"Group-by\" row of the visual query should hold \"companyname\"", () => builderRowHolds(page, "Group-by", "companyname"));
      await session.step(35, "When user adds \"customerid\" to the \"Aggregate\" row of the visual query", () => addToBuilderRow(page, "customerid", "Aggregate"));
      await session.step(36, "Then the \"Aggregate\" row of the visual query should hold \"values(customerid)\"", () => builderRowHolds(page, "Aggregate", "values(customerid)"));
    ...
    await run.scenario("The visual query is saved and runs as an ordinary query", async () => {
      await session.step(41, "When user enters \"BDD-Q-vq-{time}\" into Name input", () => enterInto(page, session.text("BDD-Q-vq-{time}"), el("Name input")));
      await session.step(42, "And user clicks on Save button", () => clickOn(page, el("Save button")));
      await session.step(43, "Then 1 query named \"BDD-Q-vq-{time}\" should be on the server", () => queriesOnServer(page, 1, session.text("BDD-Q-vq-{time}")));
      ...
      await session.step(47, "And the table should have 91 rows", () => rowCount(page, 91));
      await session.step(48, "And the table should have a column \"companyname\"", () => hasColumn(page, "companyname"));
    ...
    await run.scenario("Reopening the saved query shows the builder with its rows", async () => {
      await session.step(55, "When user closes all views", () => closeAllViews(page));
      await session.step(56, "Given the toolbox pane is hidden", () => toolboxPaneHidden(page));
      ...
      await session.step(59, "When user clicks on \"Refresh\" icon inside browse toolbar", () => clickOn(page, el("\"Refresh\" icon inside browse toolbar")));
      await session.step(61, "And user hovers over Databases---Postgres---NorthwindTest---BDD-Q-vq-{time} tree node inside browse tree", () => hoverOver(page, el(session.text("Databases---Postgres---NorthwindTest---BDD-Q-vq-{time} tree node inside browse tree"))));
      await session.step(62, "And user picks \"Edit...\" from the context menu of Databases---Postgres---NorthwindTest---BDD-Q-vq-{time} tree node inside browse tree", () => pickFromContextMenu(page, "Edit...", el(session.text("Databases---Postgres---NorthwindTest---BDD-Q-vq-{time} tree node inside browse tree"))));
      await session.step(63, "Then the current view should be a DataQueryView view", () => currentViewType(page, "DataQueryView"));
      await session.step(64, "And the \"Group-by\" row of the visual query should hold \"companyname\"", () => builderRowHolds(page, "Group-by", "companyname"));
      await session.step(65, "And the \"Aggregate\" row of the visual query should hold \"values(customerid)\"", () => builderRowHolds(page, "Aggregate", "values(customerid)"));
