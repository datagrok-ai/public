...
test.describe("Working with what a space holds", () => {
  ...
  test("Working with what a space holds", {tag: ["@journey", "@spaces", "@realizes:views.space"]}, async ({browser}) => {
    ...
    await run.scenario("A space with two files", async () => {
      ...
      await session.step(25, "Given Files tree node inside browse tree is expanded", () => isExpanded(page, el("Files tree node inside browse tree")));
      await session.step(26, "When user clicks on \"Files > Demo\" tree node inside browse tree", () => clickOn(page, el("\"Files > Demo\" tree node inside browse tree")));
      await session.step(27, "And user drags TSLA.csv link in gallery to BDD-Ops tree node inside browse tree", () => dragTo(page, el("TSLA.csv link in gallery"), el("BDD-Ops tree node inside browse tree")));
      ...
      await session.step(32, "And user drags acidiq.csv link in gallery to BDD-Ops tree node inside browse tree", () => dragTo(page, el("acidiq.csv link in gallery"), el("BDD-Ops tree node inside browse tree")));
      ...
      await session.step(36, "When user double-clicks on BDD-Ops tree node inside browse tree", () => doubleClickOn(page, el("BDD-Ops tree node inside browse tree")));
    ...
    await run.scenario("The search inside a space filters what it holds", async () => {
      await session.step(66, "When user enters \"acidiq\" into space search", () => enterInto(page, "acidiq", el("space search")));
      await session.step(67, "Then acidiq.csv link in gallery should be visible", () => shouldBe(page, el("acidiq.csv link in gallery"), "visible"));
      await session.step(68, "And BDD-Ops-renamed link in gallery should be absent", () => shouldBe(page, el("BDD-Ops-renamed link in gallery"), "absent"));
      await session.step(69, "When user enters \"aci\" into space search", () => enterInto(page, "aci", el("space search")));
      await session.step(70, "Then acidiq.csv link in gallery should be visible", () => shouldBe(page, el("acidiq.csv link in gallery"), "visible"));
      await session.step(71, "And BDD-Ops-renamed link in gallery should be absent", () => shouldBe(page, el("BDD-Ops-renamed link in gallery"), "absent"));
      await session.step(72, "When user enters \"zzz-no-such-file\" into space search", () => enterInto(page, "zzz-no-such-file", el("space search")));
      await session.step(73, "Then acidiq.csv link in gallery should be absent", () => shouldBe(page, el("acidiq.csv link in gallery"), "absent"));
      await session.step(74, "And BDD-Ops-renamed link in gallery should be absent", () => shouldBe(page, el("BDD-Ops-renamed link in gallery"), "absent"));
      await session.step(75, "When user clears space search", () => clearField(page, el("space search")));
      ...
      await session.step(77, "And BDD-Ops-renamed link in gallery should be visible", () => shouldBe(page, el("BDD-Ops-renamed link in gallery"), "visible"));
    });
    await run.scenario("A cancelled delete keeps the file", async () => {
      await session.step(80, "When user picks \"Delete...\" from the context menu of acidiq.csv link in gallery", () => pickFromContextMenu(page, "Delete...", el("acidiq.csv link in gallery")));
      await session.step(81, "Then \"Are you sure?\" dialog should be visible", () => shouldBe(page, el("\"Are you sure?\" dialog"), "visible"));
      await session.step(82, "When user clicks on CANCEL button in \"Are you sure?\" dialog", () => clickOn(page, el("CANCEL button in \"Are you sure?\" dialog")));
      await session.step(83, "Then \"Are you sure?\" dialog should be hidden", () => shouldBe(page, el("\"Are you sure?\" dialog"), "hidden"));
      await session.step(84, "And acidiq.csv link in gallery should be visible", () => shouldBe(page, el("acidiq.csv link in gallery"), "visible"));
    });
    await run.scenario("A copy in another space survives the original being deleted", async () => {
      await session.step(87, "When user drags acidiq.csv link in gallery to BDD-Ops-Copy tree node inside browse tree", () => dragTo(page, el("acidiq.csv link in gallery"), el("BDD-Ops-Copy tree node inside browse tree")));
      ...
      await session.step(90, "When user selects \"Copy\" in Move entity dialog", () => selectIn(page, "Copy", el("Move entity dialog")));
      ...
      await session.step(93, "When user double-clicks on BDD-Ops-Copy tree node inside browse tree", () => doubleClickOn(page, el("BDD-Ops-Copy tree node inside browse tree")));
      await session.step(94, "Then the \"BDD-Ops-Copy\" view should be current", () => viewIsCurrent(page, "BDD-Ops-Copy"));
      await session.step(95, "And acidiq.csv link in gallery should be visible", () => shouldBe(page, el("acidiq.csv link in gallery"), "visible"));
      await session.step(96, "When user double-clicks on BDD-Ops tree node inside browse tree", () => doubleClickOn(page, el("BDD-Ops tree node inside browse tree")));
      await session.step(97, "And user picks \"Delete...\" from the context menu of acidiq.csv link in gallery", () => pickFromContextMenu(page, "Delete...", el("acidiq.csv link in gallery")));
      await session.step(98, "And user clicks on DELETE button in \"Are you sure?\" dialog", () => clickOn(page, el("DELETE button in \"Are you sure?\" dialog")));
      await session.step(99, "Then acidiq.csv link in gallery should be absent", () => shouldBe(page, el("acidiq.csv link in gallery"), "absent"));
      await session.step(100, "And BDD-Ops-renamed link in gallery should be visible", () => shouldBe(page, el("BDD-Ops-renamed link in gallery"), "visible"));
      await session.step(101, "When user double-clicks on BDD-Ops-Copy tree node inside browse tree", () => doubleClickOn(page, el("BDD-Ops-Copy tree node inside browse tree")));
      await session.step(102, "Then the \"BDD-Ops-Copy\" view should be current", () => viewIsCurrent(page, "BDD-Ops-Copy"));
      await session.step(103, "And acidiq.csv link in gallery should be visible", () => shouldBe(page, el("acidiq.csv link in gallery"), "visible"));
    });
    await run.scenario("A single click previews the file in place", async () => {
      await session.step(106, "When user clicks on acidiq.csv link in gallery", () => clickOn(page, el("acidiq.csv link in gallery")));
      await session.step(107, "Then \"Toggle entity preview\" icon should be visible", () => shouldBe(page, el("\"Toggle entity preview\" icon"), "visible"));
      await session.step(108, "And grid should be visible", () => shouldBe(page, el("grid"), "visible"));
    });
    await run.scenario("A file opens as a table", async () => {
      await session.step(111, "When user double-clicks on acidiq.csv link in gallery", () => doubleClickOn(page, el("acidiq.csv link in gallery")));
