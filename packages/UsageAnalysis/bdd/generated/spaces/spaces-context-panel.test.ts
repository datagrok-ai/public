...
test.describe("What the context panel says about a space", () => {
  ...
  test("What the context panel says about a space", {tag: ["@journey", "@spaces", "@realizes:views.space"]}, async ({browser}) => {
    ...
    await run.scenario("Two children to switch between", async () => {
      ...
      await session.step(24, "When user picks \"Create Child Space...\" from the context menu of BDD-CP-Root tree node inside browse tree", () => pickFromContextMenu(page, "Create Child Space...", el("BDD-CP-Root tree node inside browse tree")));
      await session.step(25, "And user enters \"BDD-CP-One\" into Name input in Create Space dialog", () => enterInto(page, "BDD-CP-One", el("Name input in Create Space dialog")));
      ...
      await session.step(27, "Then the Create Space dialog should close", () => createDialogCloses(page));
      await session.step(28, "When user picks \"Create Child Space...\" from the context menu of BDD-CP-Root tree node inside browse tree", () => pickFromContextMenu(page, "Create Child Space...", el("BDD-CP-Root tree node inside browse tree")));
      await session.step(29, "And user enters \"BDD-CP-Two\" into Name input in Create Space dialog", () => enterInto(page, "BDD-CP-Two", el("Name input in Create Space dialog")));
      await session.step(30, "And user clicks on OK button in Create Space dialog", () => clickOn(page, el("OK button in Create Space dialog")));
    ...
    await run.scenario("Selecting a space shows its details", async () => {
      ...
      await session.step(40, "And the context panel should show \"BDD-CP-Root\"", () => contextPanelShows(page, "BDD-CP-Root"));
      await session.step(41, "And \"Details\" accordion header in context panel should be visible", () => shouldBe(page, el("\"Details\" accordion header in context panel"), "visible"));
    });
    await run.scenario("The panel carries the sections a space has", async () => {
      await session.step(48, "Then the following elements should be visible:", () => followingShouldBe(page, "visible", [["\"Details\" accordion header in context panel"],["\"Content\" accordion header in context panel"],["\"Sharing\" accordion header in context panel"],["\"Chats\" accordion header in context panel"]]));
      await session.step(53, "And \"Activity\" accordion header in context panel should be present", () => shouldBe(page, el("\"Activity\" accordion header in context panel"), "present"));
    });
    await run.scenario("Clicking one child, then the other, switches the panel", async () => {
      await session.step(56, "When user double-clicks on BDD-CP-Root tree node inside browse tree", () => doubleClickOn(page, el("BDD-CP-Root tree node inside browse tree")));
      await session.step(57, "Then the \"BDD-CP-Root\" view should be current", () => viewIsCurrent(page, "BDD-CP-Root"));
      ...
      await session.step(59, "Then the context panel should show \"BDD-CP-One\"", () => contextPanelShows(page, "BDD-CP-One"));
      await session.step(60, "When user clicks on BDD-CP-Two link in space gallery", () => clickOn(page, el("BDD-CP-Two link in space gallery")));
      await session.step(61, "Then the context panel should show \"BDD-CP-Two\"", () => contextPanelShows(page, "BDD-CP-Two"));
      await session.step(62, "And context panel should not contain text \"BDD-CP-One\"", () => shouldNotContainText(page, el("context panel"), "BDD-CP-One"));
      await session.step(63, "When user clicks on BDD-CP-One link in space gallery", () => clickOn(page, el("BDD-CP-One link in space gallery")));
      await session.step(64, "Then the context panel should show \"BDD-CP-One\"", () => contextPanelShows(page, "BDD-CP-One"));
      await session.step(65, "And context panel should not contain text \"BDD-CP-Two\"", () => shouldNotContainText(page, el("context panel"), "BDD-CP-Two"));
    });
    run.finish();
  });
});
