...
import {clickOn, hoverOver, isExpanded, shouldBe} from '@datagrok-libraries/bdd/bindings/common/steps';
import {browsePanelOpen, closeAllViews, contextPanelOpen, contextPanelShows, openDataset, openProject, saveAsProject, urlShouldContain, viewHoldsViewers, viewIsCurrent} from '@datagrok-libraries/bdd/bindings/platform/steps';
...
test.describe("The Apps and Dashboards sections of the Browse tree", () => {
  ...
  test("The Apps section lists the installed applications", {tag: ["@browse", "@realizes:views.browse"]}, async ({browser}) => {
    ...
    await session.step(35, "And Chem tree node inside browse tree should be visible", () => shouldBe(page, el("Chem tree node inside browse tree"), "visible"));
  ...
  test("Hovering an application does not break its details", {tag: ["@browse", "@realizes:views.browse"]}, async ({browser}) => {
    const page = await session.page(browser);
    await session.step(29, "Given user is logged in", () => loggedIn(page));
    await session.step(30, "And the browse panel is open", () => browsePanelOpen(page));
    await session.step(47, "Given the context panel is open", () => contextPanelOpen(page));
    await session.step(48, "And Apps tree node inside browse tree is expanded", () => isExpanded(page, el("Apps tree node inside browse tree")));
    await session.step(49, "When user hovers over Tutorials tree node inside browse tree", () => hoverOver(page, el("Tutorials tree node inside browse tree")));
    await session.step(50, "And user clicks on Tutorials tree node inside browse tree", () => clickOn(page, el("Tutorials tree node inside browse tree")));
    await session.step(51, "Then the context panel should show \"Tutorials\"", () => contextPanelShows(page, "Tutorials"));
  ...
  test("The Model Hub opens from the Compute group", {tag: ["@browse", "@realizes:views.browse", "@compute"]}, async ({browser}) => {
    ...
    await session.step(29, "Given user is logged in", () => loggedIn(page));
    await session.step(30, "And the browse panel is open", () => browsePanelOpen(page));
    await session.step(59, "Given Apps tree node inside browse tree is expanded", () => isExpanded(page, el("Apps tree node inside browse tree")));
    await session.step(60, "And Apps---Compute tree node inside browse tree is expanded", () => isExpanded(page, el("Apps---Compute tree node inside browse tree")));
    await session.step(61, "When user clicks on Apps---Compute---Model-Hub tree node inside browse tree", () => clickOn(page, el("Apps---Compute---Model-Hub tree node inside browse tree")));
    await session.step(62, "Then the \"Model Hub\" view should be current", () => viewIsCurrent(page, "Model Hub"));
  ...
  test("The Dashboards node opens the list of projects", {tag: ["@browse", "@realizes:views.browse"]}, async ({browser}) => {
    ...
    await session.step(68, "Then the \"Projects\" view should be current", () => viewIsCurrent(page, "Projects"));
    await session.step(69, "And the page address should contain \"/projects\"", () => urlShouldContain(page, "/projects"));
    await session.step(72, "And card of gallery should be visible", () => shouldBe(page, el("card of gallery"), "visible"));
  ...
  test("A dashboard saved from a table view opens again with its viewers", {tag: ["@browse", "@realizes:views.browse"]}, async ({browser}) => {
    ...
    await session.step(29, "Given user is logged in", () => loggedIn(page));
    await session.step(30, "And the browse panel is open", () => browsePanelOpen(page));
    await session.step(77, "Given user opens demog-1000 dataset", () => openDataset(page, ds("demog-1000")));
    await session.step(78, "And user adds a scatter plot viewer", () => addViewer(page, "scatter plot"));
    await session.step(79, "When user saves the current view as project \"BDD-Browse-Dash-{run}\"", () => saveAsProject(page, session.text("BDD-Browse-Dash-{run}")));
    await session.step(80, "And user closes all views", () => closeAllViews(page));
    await session.step(81, "And user opens the \"BDD-Browse-Dash-{run}\" project", () => openProject(page, session.text("BDD-Browse-Dash-{run}")));
    await session.step(82, "Then the current view should hold at least 2 viewers", () => viewHoldsViewers(page, 2));
