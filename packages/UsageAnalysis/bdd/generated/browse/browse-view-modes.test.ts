...
test.describe("Browsing mode and persistent views", () => {
  ...
  test("A new browsing session does not unpin what is already persistent", {tag: ["@browse", "@realizes:views.browse"]}, async ({browser}) => {
    ...
    await session.step(22, "And the browse panel is open", () => browsePanelOpen(page));
    await session.step(23, "And Apps tree node inside browse tree is expanded", () => isExpanded(page, el("Apps tree node inside browse tree")));
    await session.step(46, "Given Files tree node inside browse tree is expanded", () => isExpanded(page, el("Files tree node inside browse tree")));
    await session.step(47, "When user double-clicks on Tutorials tree node inside browse tree", () => doubleClickOn(page, el("Tutorials tree node inside browse tree")));
    await session.step(48, "Then Tutorials view should be visible", () => shouldBe(page, el("Tutorials view"), "visible"));
    ...
    await session.step(50, "Then Projects view should be visible", () => shouldBe(page, el("Projects view"), "visible"));
    await session.step(51, "When user clicks on Files---Demo tree node inside browse tree", () => clickOn(page, el("Files---Demo tree node inside browse tree")));
    await session.step(52, "Then Demo view should be visible", () => shouldBe(page, el("Demo view"), "visible"));
    await session.step(53, "And Tutorials view should be visible", () => shouldBe(page, el("Tutorials view"), "visible"));
    await session.step(54, "And Projects view should be absent", () => shouldBe(page, el("Projects view"), "absent"));
