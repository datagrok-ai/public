...
test.describe("The Files section of the Browse tree", () => {
  ...
  test("The Files section lists its file shares", {tag: ["@browse", "@realizes:views.browse"]}, async ({browser}) => {
    ...
    await session.step(25, "And Files tree node inside browse tree is expanded", () => isExpanded(page, el("Files tree node inside browse tree")));
    await session.step(28, "Then the following elements should be visible:", () => followingShouldBe(page, "visible", [["Files---My-files tree node inside browse tree"],["Files---App-Data tree node inside browse tree"],["Files---Demo tree node inside browse tree"]]));
  ...
  test("A folder opens as a folder view of its own", {tag: ["@browse", "@realizes:views.browse"]}, async ({browser}) => {
    ...
    await session.step(39, "And gallery should contain text \"chem\"", () => shouldContainText(page, el("gallery"), "chem"));
  ...
  test("A tabular file opens as a preview with its rows", {tag: ["@browse", "@realizes:views.browse"]}, async ({browser}) => {
    ...
    await session.step(24, "And the browse panel is open", () => browsePanelOpen(page));
    await session.step(25, "And Files tree node inside browse tree is expanded", () => isExpanded(page, el("Files tree node inside browse tree")));
    await session.step(44, "Given Files---Demo tree node inside browse tree is expanded", () => isExpanded(page, el("Files---Demo tree node inside browse tree")));
    await session.step(45, "When user clicks on Files---Demo---demog.csv tree node inside browse tree", () => clickOn(page, el("Files---Demo---demog.csv tree node inside browse tree")));
    await session.step(46, "Then the \"demog\" view should be current", () => viewIsCurrent(page, "demog"));
    await session.step(47, "And grid should show 5850 rows", () => showsRows(page, el("grid"), 5850));
