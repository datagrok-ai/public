...
import {browsePanelOpen, contextPanelOpen, dialogCloses, galleryCountLower, groupsOnServer, noGroupOnServer, rememberGalleryCount, viewIsCurrent} from '@datagrok-libraries/bdd/bindings/platform/steps';
...
test.describe("A group from creation to deletion", () => {
  ...
  test("A group from creation to deletion", {tag: ["@journey", "@serial", "@groups", "@realizes:views.groups"]}, async ({browser}) => {
    ...
    await run.scenario("A group is created (Groups-04)", async () => {
      ...
      await session.step(57, "When user types \"BDD-GL-Group-{time}\" into gallery search", () => typeInto(page, session.text("BDD-GL-Group-{time}"), el("gallery search")));
      await session.step(58, "Then the gallery counter should be lower than remembered", () => galleryCountLower(page));
      await session.step(59, "And \"BDD-GL-Group-{time}\" link in gallery should be visible", () => shouldBe(page, el(session.text("\"BDD-GL-Group-{time}\" link in gallery")), "visible"));
    ...
    await run.scenario("Properties... renames the group and rewrites its description (Groups-09)", async () => {
      await session.step(64, "When user picks \"Properties...\" from the context menu of \"BDD-GL-Group-{time}\" link in gallery", () => pickFromContextMenu(page, "Properties...", el(session.text("\"BDD-GL-Group-{time}\" link in gallery"))));
      await session.step(65, "Then \"BDD-GL-Group-{time} Properties\" dialog should be visible", () => shouldBe(page, el(session.text("\"BDD-GL-Group-{time} Properties\" dialog")), "visible"));
      await session.step(66, "And Name input in \"BDD-GL-Group-{time} Properties\" dialog should have value \"BDD-GL-Group-{time}\"", () => shouldHaveValue(page, el(session.text("Name input in \"BDD-GL-Group-{time} Properties\" dialog")), session.text("BDD-GL-Group-{time}")));
      await session.step(67, "And Description input in \"BDD-GL-Group-{time} Properties\" dialog should have value \"created by the BDD suite\"", () => shouldHaveValue(page, el(session.text("Description input in \"BDD-GL-Group-{time} Properties\" dialog")), "created by the BDD suite"));
      await session.step(68, "When user types \"BDD-GL-Renamed-{time}\" into Name input in \"BDD-GL-Group-{time} Properties\" dialog", () => typeInto(page, session.text("BDD-GL-Renamed-{time}"), el(session.text("Name input in \"BDD-GL-Group-{time} Properties\" dialog"))));
      await session.step(69, "And user types \"renamed by the BDD suite\" into Description input in \"BDD-GL-Group-{time} Properties\" dialog", () => typeInto(page, "renamed by the BDD suite", el(session.text("Description input in \"BDD-GL-Group-{time} Properties\" dialog"))));
      await session.step(70, "And user clicks on OK button in \"BDD-GL-Group-{time} Properties\" dialog", () => clickOn(page, el(session.text("OK button in \"BDD-GL-Group-{time} Properties\" dialog"))));
      await session.step(71, "Then the \"BDD-GL-Group-{time} Properties\" dialog should close", () => dialogCloses(page, session.text("BDD-GL-Group-{time} Properties")));
      await session.step(72, "And 1 group named \"BDD-GL-Renamed-{time}\" should be on the server", () => groupsOnServer(page, 1, session.text("BDD-GL-Renamed-{time}")));
      await session.step(73, "And 0 groups named \"BDD-GL-Group-{time}\" should be on the server", () => groupsOnServer(page, 0, session.text("BDD-GL-Group-{time}")));
      await session.step(74, "When user types \"BDD-GL-Renamed-{time}\" into gallery search", () => typeInto(page, session.text("BDD-GL-Renamed-{time}"), el("gallery search")));
      await session.step(75, "Then the gallery counter should be lower than remembered", () => galleryCountLower(page));
      await session.step(76, "When user picks \"Properties...\" from the context menu of \"BDD-GL-Renamed-{time}\" link in gallery", () => pickFromContextMenu(page, "Properties...", el(session.text("\"BDD-GL-Renamed-{time}\" link in gallery"))));
      await session.step(77, "Then \"BDD-GL-Renamed-{time} Properties\" dialog should be visible", () => shouldBe(page, el(session.text("\"BDD-GL-Renamed-{time} Properties\" dialog")), "visible"));
      await session.step(78, "And Description input in \"BDD-GL-Renamed-{time} Properties\" dialog should have value \"renamed by the BDD suite\"", () => shouldHaveValue(page, el(session.text("Description input in \"BDD-GL-Renamed-{time} Properties\" dialog")), "renamed by the BDD suite"));
      await session.step(79, "When user clicks on CANCEL button in \"BDD-GL-Renamed-{time} Properties\" dialog", () => clickOn(page, el(session.text("CANCEL button in \"BDD-GL-Renamed-{time} Properties\" dialog"))));
      await session.step(80, "Then the \"BDD-GL-Renamed-{time} Properties\" dialog should close", () => dialogCloses(page, session.text("BDD-GL-Renamed-{time} Properties")));
    ...
    await run.scenario("Delete removes the group after confirmation (Groups-14)", async () => {
      await session.step(85, "When user picks \"Delete\" from the context menu of \"BDD-GL-Renamed-{time}\" link in gallery", () => pickFromContextMenu(page, "Delete", el(session.text("\"BDD-GL-Renamed-{time}\" link in gallery"))));
      ...
      await session.step(89, "And 0 groups named \"BDD-GL-Renamed-{time}\" should be on the server", () => groupsOnServer(page, 0, session.text("BDD-GL-Renamed-{time}")));
      await session.step(90, "When user clears gallery search", () => clearField(page, el("gallery search")));
      await session.step(91, "And user types \"BDD-GL-Renamed-{time}\" into gallery search", () => typeInto(page, session.text("BDD-GL-Renamed-{time}"), el("gallery search")));
      await session.step(92, "Then \"BDD-GL-Renamed-{time}\" link in gallery should be absent", () => shouldBe(page, el(session.text("\"BDD-GL-Renamed-{time}\" link in gallery")), "absent"));
