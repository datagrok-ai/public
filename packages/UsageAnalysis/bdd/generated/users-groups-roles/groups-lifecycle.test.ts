...
test.describe("A group from creation to deletion", () => {
  ...
  test("A group from creation to deletion", {tag: ["@journey", "@serial", "@groups", "@realizes:views.groups"]}, async ({browser}) => {
    ...
    await run.scenario("The New Group dialog, cancelled (Groups-03)", async () => {
      ...
      await session.step(28, "And the following elements should be visible:", () => followingShouldBe(page, "visible", [["Name input in \"Create New Group\" dialog"],["Description input in \"Create New Group\" dialog"]]), [["Name input in \"Create New Group\" dialog"],["Description input in \"Create New Group\" dialog"]]);
