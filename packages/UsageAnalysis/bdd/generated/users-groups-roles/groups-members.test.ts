...
import {adminMemberOnServer, browsePanelOpen, contextPanelOpen, contextPanelShows, dialogCloses, galleryCountLower, groupOnServer, memberOnServer, notMemberOnServer, personalGroupOnServer, plainMemberOnServer, rememberGalleryCount, userOnServer} from '@datagrok-libraries/bdd/bindings/platform/steps';
...
test.describe("A group's members", () => {
  ...
  test("A group's members", {tag: ["@journey", "@serial", "@groups", "@realizes:views.groups"]}, async ({browser}) => {
    ...
    await session.step(17, "And a user \"bddmanaged\" is on the server", () => userOnServer(page, "bddmanaged"));
    ...
    await run.scenario("MANAGE adds a user to the group (Groups-11)", async () => {
      ...
      await session.step(33, "When user types \"bddmanaged\" into membership search", () => typeInto(page, "bddmanaged", el("membership search")));
      await session.step(34, "And user clicks on add button of \"bddmanaged\" membership candidate", () => clickOn(page, el("add button of \"bddmanaged\" membership candidate")));
      await session.step(35, "Then \"bddmanaged\" membership row should be visible", () => shouldBe(page, el("\"bddmanaged\" membership row"), "visible"));
      ...
      await session.step(38, "And \"bddmanaged\" should be a plain member of \"BDD-GM-Group-{time}\" on the server", () => plainMemberOnServer(page, "bddmanaged", session.text("BDD-GM-Group-{time}")));
      await session.step(39, "And \"Members\" section in context panel should contain text \"bddmanaged\"", () => shouldContainText(page, el("\"Members\" section in context panel"), "bddmanaged"));
    ...
    await run.scenario("The Admin box makes the member an admin (Groups-13)", async () => {
      ...
      await session.step(45, "Then checkbox label of \"bddmanaged\" membership row should have text \"Admin\"", () => shouldHaveText(page, el("checkbox label of \"bddmanaged\" membership row"), "Admin"));
      await session.step(46, "And checkbox of \"bddmanaged\" membership row should be unchecked", () => shouldBe(page, el("checkbox of \"bddmanaged\" membership row"), "unchecked"));
      await session.step(47, "When user checks checkbox of \"bddmanaged\" membership row", () => check(page, el("checkbox of \"bddmanaged\" membership row")));
      ...
      await session.step(50, "And \"bddmanaged\" should be an admin member of \"BDD-GM-Group-{time}\" on the server", () => adminMemberOnServer(page, "bddmanaged", session.text("BDD-GM-Group-{time}")));
      ...
      await session.step(52, "Then checkbox of \"bddmanaged\" membership row should be checked", () => shouldBe(page, el("checkbox of \"bddmanaged\" membership row"), "checked"));
    ...
    await run.scenario("Removing a member leaves the others (Groups-12)", async () => {
      ...
      await session.step(72, "Then \"bddmanaged\" membership row should be visible", () => shouldBe(page, el("\"bddmanaged\" membership row"), "visible"));
      await session.step(73, "When user clicks on remove button of \"bddmanaged\" membership row", () => clickOn(page, el("remove button of \"bddmanaged\" membership row")));
      await session.step(74, "Then \"bddmanaged\" membership row should be absent", () => shouldBe(page, el("\"bddmanaged\" membership row"), "absent"));
      ...
      await session.step(77, "And \"bddmanaged\" should not be a member of \"BDD-GM-Group-{time}\" on the server", () => notMemberOnServer(page, "bddmanaged", session.text("BDD-GM-Group-{time}")));
    ...
    await run.scenario("A user's personal group is on the server but not in the Groups view (Groups-18)", async () => {
      await session.step(84, "Then the user \"bddmanaged\" should have a personal group on the server", () => personalGroupOnServer(page, "bddmanaged"));
