...
import {clickOn, enterInto, expand, selectIn, shouldBe, shouldContainText, shouldHaveText, typeInto, visibleCount} from '@datagrok-libraries/bdd/bindings/common/steps';
...
test.describe("Creating a script", () => {
  ...
  test("Creating a script", {tag: ["@journey", "@full-stand", "@serial", "@realizes:views.scripts"]}, async ({browser}) => {
    ...
    await run.scenario("The Signature editor names the script and adds a parameter to its header", async () => {
      ...
      await session.step(64, "And the ribbon of the current view should be ready", () => ribbonReady(page));
      await session.step(65, "When user clicks on \"Open Signature Editor\" icon", () => clickOn(page, el("\"Open Signature Editor\" icon")));
      await session.step(66, "Then PARAMETERS tab should be visible", () => shouldBe(page, el("PARAMETERS tab"), "visible"));
      await session.step(67, "When user enters \"BddScriptCreate{time}\" into Name input", () => enterInto(page, session.text("BddScriptCreate{time}"), el("Name input")));
      await session.step(68, "And user clicks on PARAMETERS tab", () => clickOn(page, el("PARAMETERS tab")));
      await session.step(69, "Then there should be 2 visible \"Add the param\" icon", () => visibleCount(page, 2, el("\"Add the param\" icon")));
      await session.step(70, "When user clicks on first \"Add the param\" icon", () => clickOn(page, el("first \"Add the param\" icon")));
      await session.step(71, "Then there should be 3 visible \"Add the param\" icon", () => visibleCount(page, 3, el("\"Add the param\" icon")));
      await session.step(72, "Then \"Open function editor\" icon should be visible", () => shouldBe(page, el("\"Open function editor\" icon"), "visible"));
      await session.step(73, "When user clicks on \"Open function editor\" icon", () => clickOn(page, el("\"Open function editor\" icon")));
      await session.step(77, "Then \"Open function editor\" icon should be hidden", () => shouldBe(page, el("\"Open function editor\" icon"), "hidden"));
      await session.step(78, "And the \"BddScriptCreate{time}\" view should be current", () => viewIsCurrent(page, session.text("BddScriptCreate{time}")));
      await session.step(79, "And code editor should be visible", () => shouldBe(page, el("code editor"), "visible"));
      await session.step(86, "And the ribbon of the current view should be ready", () => ribbonReady(page));
      await session.step(87, "And code editor should contain the text \"#name: BddScriptCreate{time}\"", () => shouldContainText(page, el("code editor"), session.text("#name: BddScriptCreate{time}")));
      await session.step(88, "And code editor should contain the text \"#input: bool newParam\"", () => shouldContainText(page, el("code editor"), "#input: bool newParam"));
    ...
    await run.scenario("Running with cars answers the number of cells", async () => {
      await session.step(93, "Given user switches to the \"BddScriptCreate{time}\" view", () => switchView(page, session.text("BddScriptCreate{time}")));
      await session.step(94, "When user clicks on \"Run script (F5)\" icon", () => clickOn(page, el("\"Run script (F5)\" icon")));
      await session.step(95, "Then \"BddScriptCreate{time}\" dialog should be visible", () => shouldBe(page, el(session.text("\"BddScriptCreate{time}\" dialog")), "visible"));
      await session.step(96, "When user selects \"cars\" in Table input in \"BddScriptCreate{time}\" dialog", () => selectIn(page, "cars", el(session.text("Table input in \"BddScriptCreate{time}\" dialog"))));
      await session.step(97, "And user clicks on OK button in \"BddScriptCreate{time}\" dialog", () => clickOn(page, el(session.text("OK button in \"BddScriptCreate{time}\" dialog"))));
      await session.step(98, "Then the \"BddScriptCreate{time}\" dialog should close", () => dialogCloses(page, session.text("BddScriptCreate{time}")));
      await session.step(99, "And the script results should show \"count\" as \"510\"", () => scriptResult(page, "count", "510"));
    ...
    await run.scenario("Save stores the script with the parameters of its header", async () => {
      await session.step(104, "When user saves the script", () => saveScript(page));
      await session.step(105, "Then an info balloon containing \"Script saved.\" should have been shown", () => infoBalloonText(page, "Script saved."));
      await session.step(106, "And 1 script named \"BddScriptCreate{time}\" should be on the server", () => scriptsOnServer(page, 1, session.text("BddScriptCreate{time}")));
      await session.step(107, "And the script \"BddScriptCreate{time}\" on the server should have an output \"count\" of type \"int\"", () => scriptHasParam(page, session.text("BddScriptCreate{time}"), "output", "count", "int"));
      await session.step(108, "And the script \"BddScriptCreate{time}\" on the server should have an input \"newParam\" of type \"bool\"", () => scriptHasParam(page, session.text("BddScriptCreate{time}"), "input", "newParam", "bool"));
      await session.step(109, "And the script \"BddScriptCreate{time}\" on the server should have an input \"table\" of type \"dataframe\"", () => scriptHasParam(page, session.text("BddScriptCreate{time}"), "input", "table", "dataframe"));
      await session.step(110, "And the \"BddScriptCreate{time}\" view should be current", () => viewIsCurrent(page, session.text("BddScriptCreate{time}")));
    ...
    await run.scenario("Closing the editor returns to Scripts, where the new script is found", async () => {
      await session.step(114, "When user closes the current view", () => closeCurrentView(page));
      await session.step(115, "Then the \"Scripts\" view should be current", () => viewIsCurrent(page, "Scripts"));
      await session.step(116, "When user types \"BddScriptCreate{time}\" into gallery search", () => typeInto(page, session.text("BddScriptCreate{time}"), el("gallery search")));
      await session.step(117, "Then gallery counter should have text \"1\"", () => shouldHaveText(page, el("gallery counter"), "1"));
      await session.step(118, "And \"BddScriptCreate{time}\" link in gallery should be visible", () => shouldBe(page, el(session.text("\"BddScriptCreate{time}\" link in gallery")), "visible"));
