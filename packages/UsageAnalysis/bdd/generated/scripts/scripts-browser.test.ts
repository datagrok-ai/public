...
import {closeCurrentView, contextPanelOpen, contextPanelShows, dialogCloses, galleryMode, openDataset, paneCountAtLeast, pickSharingUser, scriptOnServer, scriptsView, sharingPaneLists, sharingPaneListsNot, viewIsCurrent} from '@datagrok-libraries/bdd/bindings/platform/steps';
...
test.describe("The Scripts view and a script's context panel", () => {
  ...
  test("The Scripts view and a script's context panel", {tag: ["@journey", "@serial", "@realizes:views.scripts"]}, async ({browser}) => {
    const page = await session.page(browser);
    const run = journey(test, 7, page);
    await session.step(30, "Given user is logged in", () => loggedIn(page));
    await session.step(31, "And a script \"BddScriptBrowser{time}\" is on the server:", () => scriptOnServer(page, session.text("BddScriptBrowser{time}"), "//language: javascript\n//input: dataframe table\n//output: int count\n//output: string newParam\ncount = table.rowCount * table.columns.length;\nnewParam = \"test\";"));
    await session.step(40, "And the context panel is open", () => contextPanelOpen(page));
    await session.step(41, "And user opens the Scripts view", () => scriptsView(page));
    await run.scenario("The view modes and the order of the gallery", async () => {
      await session.step(44, "When user clicks on \"Switch to brief view\" icon inside gallery toolbar", () => clickOn(page, el("\"Switch to brief view\" icon inside gallery toolbar")));
      await session.step(45, "Then the gallery should be in brief mode", () => galleryMode(page, "brief"));
      await session.step(46, "When user clicks on \"Switch to grid view\" icon inside gallery toolbar", () => clickOn(page, el("\"Switch to grid view\" icon inside gallery toolbar")));
      await session.step(47, "Then the gallery should be in grid mode", () => galleryMode(page, "grid"));
      await session.step(48, "When user clicks on \"Switch to card view\" icon inside gallery toolbar", () => clickOn(page, el("\"Switch to card view\" icon inside gallery toolbar")));
      await session.step(49, "Then the gallery should be in card mode", () => galleryMode(page, "card"));
    ...
    await run.scenario("A search narrows the gallery to the script", async () => {
      await session.step(54, "When user types \"BddScriptBrowser{time}\" into gallery search", () => typeInto(page, session.text("BddScriptBrowser{time}"), el("gallery search")));
      await session.step(55, "Then gallery counter should have text \"1\"", () => shouldHaveText(page, el("gallery counter"), "1"));
      await session.step(56, "And \"BddScriptBrowser{time}\" link in gallery should be visible", () => shouldBe(page, el(session.text("\"BddScriptBrowser{time}\" link in gallery")), "visible"));
    ...
    await run.scenario("The script fills the context panel", async () => {
      await session.step(60, "When user clicks on \"BddScriptBrowser{time}\" link in gallery", () => clickOn(page, el(session.text("\"BddScriptBrowser{time}\" link in gallery"))));
      await session.step(61, "Then the context panel should show \"BddScriptBrowser{time}\"", () => contextPanelShows(page, session.text("BddScriptBrowser{time}")));
      await session.step(62, "And the following elements should be visible:", () => followingShouldBe(page, "visible", [["\"Details\" section in context panel"],["\"Script\" section in context panel"],["\"Run\" section in context panel"],["\"Sharing\" section in context panel"],["\"Chats\" section in context panel"]]), [["\"Details\" section in context panel"],["\"Script\" section in context panel"],["\"Run\" section in context panel"],["\"Sharing\" section in context panel"],["\"Chats\" section in context panel"]]);
    ...
    await run.scenario("Edit... opens the script in the editor", async () => {
      await session.step(114, "Given user opens the Scripts view", () => scriptsView(page));
      await session.step(115, "When user types \"BddScriptBrowser{time}\" into gallery search", () => typeInto(page, session.text("BddScriptBrowser{time}"), el("gallery search")));
      await session.step(116, "And user picks \"Edit...\" from the context menu of \"BddScriptBrowser{time}\" link in gallery", () => pickFromContextMenu(page, "Edit...", el(session.text("\"BddScriptBrowser{time}\" link in gallery"))));
      await session.step(117, "Then the \"BddScriptBrowser{time}\" view should be current", () => viewIsCurrent(page, session.text("BddScriptBrowser{time}")));
      await session.step(118, "And code editor should contain the text \"//name: BddScriptBrowser{time}\"", () => shouldContainText(page, el("code editor"), session.text("//name: BddScriptBrowser{time}")));
      await session.step(119, "And no errors should have been logged", () => noErrors(page));
      await session.step(120, "When user closes the current view", () => closeCurrentView(page));
      await session.step(121, "And user opens the Scripts view", () => scriptsView(page));
    });
    run.finish();
  });
});
