...
import {clickModeSwitch, modeSwitchReads, pickPanelMenu, typeIntoCardSearch} from '../../../bindings/filter-panel.js';
...
test.describe("Expression and text filter cards", () => {
  ...
  test("Expression and text filter cards", {tag: ["@journey", "@viewers", "@realizes:viewers.filters"]}, async ({browser}) => {
    ...
    await run.scenario("Two rules from the form combine with OR, and the switch makes them AND", async () => {
      ...
      await session.step(47, "And the AND-OR switch of the \"Expression\" filter card should read \"OR\"", () => modeSwitchReads(page, "Expression", "OR"));
      await session.step(48, "And 446 rows should pass the filter", () => filterPasses(page, 446));
      await session.step(49, "When user clicks on the AND-OR switch of the \"Expression\" filter card", () => clickModeSwitch(page, "Expression"));
      await session.step(50, "Then the AND-OR switch of the \"Expression\" filter card should read \"AND\"", () => modeSwitchReads(page, "Expression", "AND"));
    ...
    await run.scenario("The text card keeps the rows holding any of its terms, or all of them", async () => {
      ...
      await session.step(146, "And the AND-OR switch of the \"Aroma\" filter card should read \"OR\"", () => modeSwitchReads(page, "Aroma", "OR"));
      await session.step(147, "And 106 rows should pass the filter", () => filterPasses(page, 106));
      await session.step(148, "When user clicks on the AND-OR switch of the \"Aroma\" filter card", () => clickModeSwitch(page, "Aroma"));
      await session.step(149, "Then the AND-OR switch of the \"Aroma\" filter card should read \"AND\"", () => modeSwitchReads(page, "Aroma", "AND"));
