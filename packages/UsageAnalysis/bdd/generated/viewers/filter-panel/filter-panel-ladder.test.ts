...
import {addCardFor, enterCardBound, pickCardIndicatorMenu, switchCardBackOn} from '../../../bindings/filter-panel.js';
...
test.describe("Filter panel core ladder", () => {
  ...
  test("Filter panel core ladder", {tag: ["@journey", "@viewers", "@realizes:viewers.filters"]}, async ({browser}) => {
    ...
    await run.scenario("The master toggle keeps a card switched off on its own switched off", async () => {
      ...
      await session.step(133, "When user switches the \"RACE\" filter card back on", () => switchCardBackOn(page, "RACE"));
      await session.step(134, "Then 19 rows should pass the filter", () => filterPasses(page, 19));
      await session.step(135, "And \"RACE\" filter card should be enabled", () => shouldBe(page, el("\"RACE\" filter card"), "enabled"));
      await session.step(136, "And the \"selected categories of RACE\" reading of filter panel should be \"Black\"", () => readingReads(page, "selected categories of RACE", el("filter panel"), "Black"));
    ...
    await run.scenario("Escape on the focused panel switches the cards off and gives them back unchanged", async () => {
      await session.step(141, "When user clicks on empty plot space of filter panel", () => clickEmptySpace(page, el("filter panel")));
      await session.step(142, "And user presses Escape", () => pressKey(page, "Escape"));
      await session.step(143, "Then all rows should pass the filter", () => filterPassesAll(page));
      await session.step(144, "And the \"active\" reading of filter panel should be \"false\"", () => readingReads(page, "active", el("filter panel"), "false"));
      await session.step(145, "And \"AGE\" filter card should be disabled", () => shouldBe(page, el("\"AGE\" filter card"), "disabled"));
      await session.step(146, "And the \"selected categories of RACE\" reading of filter panel should be \"Black\"", () => readingReads(page, "selected categories of RACE", el("filter panel"), "Black"));
      await session.step(147, "And the \"min of AGE\" reading of filter panel should be 30", () => readingIs(page, "min of AGE", el("filter panel"), 30));
      await session.step(148, "And the \"max of AGE\" reading of filter panel should be 60", () => readingIs(page, "max of AGE", el("filter panel"), 60));
      await session.step(149, "And counter of filter panel should be hidden", () => shouldBe(page, el("counter of filter panel"), "hidden"));
      await session.step(150, "When user presses Escape", () => pressKey(page, "Escape"));
      await session.step(151, "Then 19 rows should pass the filter", () => filterPasses(page, 19));
      await session.step(152, "And the \"active\" reading of filter panel should be \"true\"", () => readingReads(page, "active", el("filter panel"), "true"));
      await session.step(153, "And counter of filter panel should have text \"2\"", () => shouldHaveText(page, el("counter of filter panel"), "2"));
    ...
    await run.scenario("The header search hides cards and leaves the rows alone", async () => {
      ...
      await session.step(158, "And user clicks on search icon of filter panel", () => clickOn(page, el("search icon of filter panel")));
      await session.step(159, "And user types \"RACE\" into search of filter panel", () => typeInto(page, "RACE", el("search of filter panel")));
      await session.step(160, "Then \"AGE\" filter card should be hidden", () => shouldBe(page, el("\"AGE\" filter card"), "hidden"));
      await session.step(161, "And \"SEX\" filter card should be hidden", () => shouldBe(page, el("\"SEX\" filter card"), "hidden"));
      await session.step(162, "And \"RACE\" filter card should be visible", () => shouldBe(page, el("\"RACE\" filter card"), "visible"));
      await session.step(163, "And 19 rows should pass the filter", () => filterPasses(page, 19));
      await session.step(164, "And counter of filter panel should have text \"2\"", () => shouldHaveText(page, el("counter of filter panel"), "2"));
      await session.step(165, "When user clears search of filter panel", () => clearField(page, el("search of filter panel")));
      await session.step(166, "Then \"AGE\" filter card should be visible", () => shouldBe(page, el("\"AGE\" filter card"), "visible"));
      await session.step(167, "And \"SEX\" filter card should be visible", () => shouldBe(page, el("\"SEX\" filter card"), "visible"));
      await session.step(168, "And 19 rows should pass the filter", () => filterPasses(page, 19));
    ...
    await run.scenario("The reset icon clears the criteria, keeps the cards and switches every one back on", async () => {
      await session.step(172, "When user hovers over \"AGE\" filter card", () => hoverOver(page, el("\"AGE\" filter card")));
      await session.step(173, "And user unchecks checkbox of \"AGE\" filter card", () => uncheck(page, el("checkbox of \"AGE\" filter card")));
      await session.step(174, "Then 27 rows should pass the filter", () => filterPasses(page, 27));
      await session.step(175, "And \"AGE\" filter card should be disabled", () => shouldBe(page, el("\"AGE\" filter card"), "disabled"));
      await session.step(176, "When user hovers over filter panel", () => hoverOver(page, el("filter panel")));
      await session.step(177, "And user clicks on reset icon of filter panel", () => clickOn(page, el("reset icon of filter panel")));
      await session.step(178, "Then all rows should pass the filter", () => filterPassesAll(page));
      await session.step(179, "And the \"filters\" reading of filter panel should be 0", () => readingIs(page, "filters", el("filter panel"), 0));
      await session.step(180, "And counter of filter panel should be hidden", () => shouldBe(page, el("counter of filter panel"), "hidden"));
      await session.step(181, "And the \"cards\" reading of filter panel should be \"SEX, AGE, RACE\"", () => readingReads(page, "cards", el("filter panel"), "SEX, AGE, RACE"));
      await session.step(182, "And \"AGE\" filter card should be enabled", () => shouldBe(page, el("\"AGE\" filter card"), "enabled"));
      await session.step(183, "And \"RACE\" filter card should be enabled", () => shouldBe(page, el("\"RACE\" filter card"), "enabled"));
      await session.step(184, "And \"SEX\" filter card should be enabled", () => shouldBe(page, el("\"SEX\" filter card"), "enabled"));
      await session.step(185, "And the \"enabled of AGE\" reading of filter panel should be \"true\"", () => readingReads(page, "enabled of AGE", el("filter panel"), "true"));
      await session.step(186, "When user hovers over \"AGE\" filter card", () => hoverOver(page, el("\"AGE\" filter card")));
      await session.step(187, "Then checkbox of \"AGE\" filter card should be checked", () => shouldBe(page, el("checkbox of \"AGE\" filter card"), "checked"));
      await session.step(188, "When user hovers over \"RACE\" filter card", () => hoverOver(page, el("\"RACE\" filter card")));
      await session.step(189, "Then checkbox of \"RACE\" filter card should be checked", () => shouldBe(page, el("checkbox of \"RACE\" filter card"), "checked"));
      await session.step(190, "When user hovers over \"SEX\" filter card", () => hoverOver(page, el("\"SEX\" filter card")));
      await session.step(191, "Then checkbox of \"SEX\" filter card should be checked", () => shouldBe(page, el("checkbox of \"SEX\" filter card"), "checked"));
      await session.step(192, "And the \"filtering of RACE\" reading of filter panel should be \"false\"", () => readingReads(page, "filtering of RACE", el("filter panel"), "false"));
    ...
    await run.scenario("A card's checkbox suspends its criterion and keeps it", async () => {
      await session.step(196, "When user clicks on the \"category Black of RACE\" area of filter panel", () => clickArea(page, "category Black of RACE", el("filter panel")));
      await session.step(197, "Then 27 rows should pass the filter", () => filterPasses(page, 27));
      await session.step(198, "When user hovers over \"RACE\" filter card", () => hoverOver(page, el("\"RACE\" filter card")));
      await session.step(199, "And user unchecks checkbox of \"RACE\" filter card", () => uncheck(page, el("checkbox of \"RACE\" filter card")));
      await session.step(200, "Then all rows should pass the filter", () => filterPassesAll(page));
      await session.step(201, "And \"RACE\" filter card should be disabled", () => shouldBe(page, el("\"RACE\" filter card"), "disabled"));
      await session.step(202, "And the \"enabled of RACE\" reading of filter panel should be \"false\"", () => readingReads(page, "enabled of RACE", el("filter panel"), "false"));
      await session.step(203, "And the \"selected categories of RACE\" reading of filter panel should be \"Black\"", () => readingReads(page, "selected categories of RACE", el("filter panel"), "Black"));
      await session.step(204, "And counter of filter panel should be hidden", () => shouldBe(page, el("counter of filter panel"), "hidden"));
      ...
      await session.step(207, "Then \"RACE\" filter card should be enabled", () => shouldBe(page, el("\"RACE\" filter card"), "enabled"));
      await session.step(208, "And the \"enabled of RACE\" reading of filter panel should be \"true\"", () => readingReads(page, "enabled of RACE", el("filter panel"), "true"));
      await session.step(209, "And all rows should pass the filter", () => filterPassesAll(page));
      await session.step(210, "And no errors should have been logged", () => noErrors(page));
    });
    run.finish();
  });
});
