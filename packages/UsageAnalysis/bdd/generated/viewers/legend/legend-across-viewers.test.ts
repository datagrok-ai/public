...
test.describe("One legend column across seven viewers", () => {
  ...
  test("One legend column across seven viewers", {tag: ["@journey", "@viewers", "@realizes:viewers.legend"]}, async ({browser}) => {
    ...
    await session.step(43, "And user adds a scatter plot viewer with:", () => addViewerWith(page, "scatter plot", [["xColumnName","WEIGHT"],["yColumnName","HEIGHT"],["colorColumnName","RACE"],["Legend Visibility","Always"],["Legend Position","Right"]]));
    await session.step(49, "And user adds a histogram viewer with:", () => addViewerWith(page, "histogram", [["valueColumnName","AGE"],["splitColumnName","RACE"],["Legend Visibility","Always"],["Legend Position","Right"]]));
    await session.step(54, "And user adds a line chart viewer with:", () => addViewerWith(page, "line chart", [["xColumnName","AGE"],["yColumnNames","WEIGHT"],["splitColumnNames","RACE"],["Legend Visibility","Always"],["Legend Position","Right"]]));
    await session.step(60, "And user adds a bar chart viewer with:", () => addViewerWith(page, "bar chart", [["splitColumnName","SEX"],["stackColumnName","RACE"],["Legend Visibility","Always"],["Legend Position","Right"]]));
    await session.step(65, "And user adds a pie chart viewer with:", () => addViewerWith(page, "pie chart", [["categoryColumnName","RACE"],["Legend Visibility","Always"],["Legend Position","Right"]]));
    await session.step(69, "And user adds a trellis plot viewer with:", () => addViewerWith(page, "trellis plot", [["xColumnNames","RACE"],["Viewer Type","Scatter plot"],["Legend Visibility","Always"],["Legend Position","Right"]]));
    ...
    await session.step(75, "And user adds a box plot viewer with:", () => addViewerWith(page, "box plot", [["categoryColumnNames","RACE"],["valueColumnName","AGE"],["markerColorColumnName","RACE"],["Legend Visibility","Always"],["Legend Position","Right"]]));
    ...
    await run.scenario("A categorical coloring switched on in the grid reaches every legend and every canvas", async () => {
      ...
      await session.step(117, "When user colors \"RACE\" column categorically:", () => colorCategorical(page, "RACE", [["Caucasian","#FF00FF"],["Other","#FFFF00"]]));
    ...
    await run.scenario("The palette, the legend column and the visibility come back from a saved layout", async () => {
      ...
      await session.step(199, "And user colors \"RACE\" column categorically:", () => colorCategorical(page, "RACE", [["Caucasian","#2CA02C"],["Asian","#1F77B4"]]));
