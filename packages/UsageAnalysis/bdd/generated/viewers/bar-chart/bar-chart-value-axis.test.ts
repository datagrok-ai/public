...
test.describe("Bar chart value axis range and scale", () => {
  ...
  test("Bar chart value axis range and scale", {tag: ["@journey", "@viewers", "@realizes:viewers.bar-chart"]}, async ({browser}) => {
    ...
    await run.scenario("Clearing Min and Max restores the full range", async () => {
      await session.step(60, "When user sets properties of bar chart viewer:", () => setProperties(page, el("bar chart viewer"), [["Min",""],["Max",""]]), [["Min",""],["Max",""]]);
      await session.step(63, "Then properties of bar chart viewer should be:", () => propertiesShouldBe(page, el("bar chart viewer"), [["Min",""],["Max",""],["Axis Type","linear"]]), [["Min",""],["Max",""],["Axis Type","linear"]]);
