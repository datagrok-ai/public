...
test.describe("PC plot in-chart range filter", () => {
  ...
  test("PC plot in-chart range filter", {tag: ["@journey", "@viewers", "@realizes:viewers.pc-plot"]}, async ({browser}) => {
    ...
    await session.step(19, "And user adds a pc plot viewer with:", () => addViewerWith(page, "pc plot", [["Column Names","AGE, HEIGHT, WEIGHT"]]));
    ...
    await run.scenario("Changing a histogram's column leaves the plot's filter alone (github-972)", async () => {
      await session.step(103, "When user adds a histogram viewer with:", () => addViewerWith(page, "histogram", [["Value","AGE"]]));
    ...
    await run.scenario("Closing the plot releases the filter it contributed", async () => {
      ...
      await session.step(142, "When user adds a pc plot viewer with:", () => addViewerWith(page, "pc plot", [["Column Names","AGE, HEIGHT, WEIGHT"]]));
