...
test.describe("PC plot colouring, legend and colour scale", () => {
  ...
  test("PC plot colouring, legend and colour scale", {tag: ["@journey", "@viewers", "@realizes:viewers.pc-plot"]}, async ({browser}) => {
    ...
    await session.step(17, "And user adds a pc plot viewer with:", () => addViewerWith(page, "pc plot", [["Column Names","AGE, HEIGHT, WEIGHT"]]));
    ...
    await run.scenario("Legend Position moves the legend and Legend Visibility takes it away", async () => {
      ...
      await session.step(60, "When user sets properties of pc plot viewer:", () => setProperties(page, el("pc plot viewer"), [["Legend Position","Auto"],["Color",""]]));
    ...
    await run.scenario("Color Min and Color Max clamp the scale and recolour the lines", async () => {
      await session.step(98, "When user sets properties of pc plot viewer:", () => setProperties(page, el("pc plot viewer"), [["Color Min","30"],["Color Max","60"]]));
      await session.step(101, "Then \"Color Min\" property of pc plot viewer should be \"30\"", () => propertyShouldBe(page, "Color Min", el("pc plot viewer"), "30"));
      await session.step(102, "And the \"color scale\" area of pc plot viewer should have repainted", () => areaRepainted(page, "color scale", el("pc plot viewer")));
      await session.step(103, "And pc plot viewer should have repainted by at least 2000 pixels", () => repaintedBy(page, el("pc plot viewer"), 2000));
      await session.step(104, "When user sets \"Color Axis Type\" property of pc plot viewer to \"logarithmic\"", () => setProperty(page, "Color Axis Type", el("pc plot viewer"), "logarithmic"));
      await session.step(105, "Then the \"color scale\" area of pc plot viewer should have repainted", () => areaRepainted(page, "color scale", el("pc plot viewer")));
      await session.step(106, "When user sets properties of pc plot viewer:", () => setProperties(page, el("pc plot viewer"), [["Color Axis Type","linear"],["Color Min",""],["Color Max",""]]));
      await session.step(110, "Then pc plot viewer should have repainted by at least 2000 pixels", () => repaintedBy(page, el("pc plot viewer"), 2000));
      await session.step(111, "When user sets \"Color\" property of pc plot viewer to \"\"", () => setProperty(page, "Color", el("pc plot viewer"), ""));
      await session.step(112, "Then pc plot viewer should not have a \"color scale\" area", () => hasNoArea(page, el("pc plot viewer"), "color scale"));
    ...
    await run.scenario("A conditional colour coding on the column hands the plot its bins", async () => {
      await session.step(116, "When user sets \"Color\" property of pc plot viewer to \"HEIGHT\"", () => setProperty(page, "Color", el("pc plot viewer"), "HEIGHT"));
      await session.step(117, "Then pc plot viewer should have a \"color scale\" area", () => hasArea(page, el("pc plot viewer"), "color scale"));
      await session.step(118, "When user colors \"HEIGHT\" column conditionally:", () => colorConditional(page, "HEIGHT", [["20-150","#00FF00"],["150-250","#FFA500"]]));
      await session.step(121, "Then \"HEIGHT\" column should be color-coded conditionally", () => colorCodedAs(page, "HEIGHT", "conditionally"));
      await session.step(122, "And legend of pc plot viewer should be visible", () => shouldBe(page, el("legend of pc plot viewer"), "visible"));
      await session.step(123, "And the legend of pc plot viewer should list 3 items", () => legendLists(page, el("pc plot viewer"), 3));
      await session.step(124, "And \"20-150\" legend item in legend of pc plot viewer should be visible", () => shouldBe(page, el("\"20-150\" legend item in legend of pc plot viewer"), "visible"));
      await session.step(125, "And \"150-250\" legend item in legend of pc plot viewer should be visible", () => shouldBe(page, el("\"150-250\" legend item in legend of pc plot viewer"), "visible"));
      await session.step(126, "And pc plot viewer should not have a \"color scale\" area", () => hasNoArea(page, el("pc plot viewer"), "color scale"));
      await session.step(127, "When user removes the coloring of \"HEIGHT\" column", () => colorOff(page, "HEIGHT"));
      await session.step(128, "Then \"HEIGHT\" column should have no color coding", () => noColorCoding(page, "HEIGHT"));
      await session.step(129, "And legend of pc plot viewer should be hidden", () => shouldBe(page, el("legend of pc plot viewer"), "hidden"));
      await session.step(130, "And pc plot viewer should have a \"color scale\" area", () => hasArea(page, el("pc plot viewer"), "color scale"));
      await session.step(131, "When user sets \"Color\" property of pc plot viewer to \"\"", () => setProperty(page, "Color", el("pc plot viewer"), ""));
    ...
    await run.scenario("HEIGHT's colour coding switched categorical, numerical and off leaves no legend behind", async () => {
      await session.step(135, "When user sets \"Color\" property of pc plot viewer to \"HEIGHT\"", () => setProperty(page, "Color", el("pc plot viewer"), "HEIGHT"));
      await session.step(136, "And user colors \"HEIGHT\" column categorically again", () => colorAgain(page, "HEIGHT", "categorically"));
      await session.step(137, "Then \"HEIGHT\" column should be color-coded categorically", () => colorCodedCategorically(page, "HEIGHT"));
      await session.step(138, "And pc plot viewer should be painted", () => painted(page, el("pc plot viewer")));
      await session.step(139, "When user colors \"HEIGHT\" column linearly again", () => colorAgain(page, "HEIGHT", "linearly"));
      await session.step(140, "Then \"HEIGHT\" column should be color-coded linearly", () => colorCodedAs(page, "HEIGHT", "linearly"));
      await session.step(141, "And legend of pc plot viewer should be hidden", () => shouldBe(page, el("legend of pc plot viewer"), "hidden"));
      await session.step(142, "And pc plot viewer should have a \"color scale\" area", () => hasArea(page, el("pc plot viewer"), "color scale"));
      await session.step(143, "When user removes the coloring of \"HEIGHT\" column", () => colorOff(page, "HEIGHT"));
      await session.step(144, "Then \"HEIGHT\" column should have no color coding", () => noColorCoding(page, "HEIGHT"));
      await session.step(145, "And legend of pc plot viewer should be hidden", () => shouldBe(page, el("legend of pc plot viewer"), "hidden"));
      await session.step(146, "And pc plot viewer should have a \"color scale\" area", () => hasArea(page, el("pc plot viewer"), "color scale"));
      await session.step(147, "And pc plot viewer should show 1000 rows", () => showsRows(page, el("pc plot viewer"), 1000));
      await session.step(148, "When user sets \"Color\" property of pc plot viewer to \"\"", () => setProperty(page, "Color", el("pc plot viewer"), ""));
      await session.step(149, "Then pc plot viewer should not have a \"color scale\" area", () => hasNoArea(page, el("pc plot viewer"), "color scale"));
    ...
    await run.scenario("A DateTime colour column is split by its Color Map", async () => {
      await session.step(153, "When user sets properties of pc plot viewer:", () => setProperties(page, el("pc plot viewer"), [["Color","STARTED"],["Color Map","year"]]));
      await session.step(156, "Then legend of pc plot viewer should be visible", () => shouldBe(page, el("legend of pc plot viewer"), "visible"));
      await session.step(157, "And the legend of pc plot viewer should list 3 items", () => legendLists(page, el("pc plot viewer"), 3));
      await session.step(158, "And \"1990\" legend item in legend of pc plot viewer should be visible", () => shouldBe(page, el("\"1990\" legend item in legend of pc plot viewer"), "visible"));
      await session.step(159, "And pc plot viewer should show 1000 rows", () => showsRows(page, el("pc plot viewer"), 1000));
      await session.step(160, "When user sets \"Color Map\" property of pc plot viewer to \"quarter\"", () => setProperty(page, "Color Map", el("pc plot viewer"), "quarter"));
      await session.step(161, "Then the legend of pc plot viewer should list 4 items", () => legendLists(page, el("pc plot viewer"), 4));
      await session.step(162, "And \"Q1\" legend item in legend of pc plot viewer should be visible", () => shouldBe(page, el("\"Q1\" legend item in legend of pc plot viewer"), "visible"));
      await session.step(163, "And pc plot viewer should have repainted by at least 1000 pixels", () => repaintedBy(page, el("pc plot viewer"), 1000));
      await session.step(164, "When user sets \"Color Map\" property of pc plot viewer to \"month\"", () => setProperty(page, "Color Map", el("pc plot viewer"), "month"));
      await session.step(165, "Then the legend of pc plot viewer should list 12 items", () => legendLists(page, el("pc plot viewer"), 12));
      await session.step(166, "And \"January\" legend item in legend of pc plot viewer should be visible", () => shouldBe(page, el("\"January\" legend item in legend of pc plot viewer"), "visible"));
      await session.step(167, "When user sets properties of pc plot viewer:", () => setProperties(page, el("pc plot viewer"), [["Color Map",""],["Color",""]]));
      await session.step(170, "Then legend of pc plot viewer should be hidden", () => shouldBe(page, el("legend of pc plot viewer"), "hidden"));
      await session.step(171, "And pc plot viewer should show 1000 rows", () => showsRows(page, el("pc plot viewer"), 1000));
