...
import {hoverOver, shouldContainText} from '@datagrok-libraries/bdd/bindings/common/steps';
...
test.describe("PC plot axes, vertical scale and chrome", () => {
  ...
  test("PC plot axes, vertical scale and chrome", {tag: ["@journey", "@viewers", "@realizes:viewers.pc-plot"]}, async ({browser}) => {
    ...
    await session.step(25, "And user adds a pc plot viewer with:", () => addViewerWith(page, "pc plot", [["Column Names","AGE, HEIGHT, WEIGHT"]]), [["Column Names","AGE, HEIGHT, WEIGHT"]]);
    ...
    await run.scenario("Line width, label orientation and the horizontal margin change what is drawn", async () => {
      await session.step(97, "When user sets \"Line Width\" property of pc plot viewer to \"3\"", () => setProperty(page, "Line Width", el("pc plot viewer"), "3"));
      await session.step(98, "Then pc plot viewer should have more ink than before", () => moreInk(page, el("pc plot viewer")));
      await session.step(99, "When user sets \"Line Width\" property of pc plot viewer to \"1\"", () => setProperty(page, "Line Width", el("pc plot viewer"), "1"));
      await session.step(100, "Then pc plot viewer should have less ink than before", () => lessInk(page, el("pc plot viewer")));
      await session.step(101, "When user makes row 1 current", () => makeRowCurrent(page, 1));
      await session.step(102, "And user sets \"Current Line Width\" property of pc plot viewer to \"8\"", () => setProperty(page, "Current Line Width", el("pc plot viewer"), "8"));
      await session.step(103, "Then the \"current row\" reading of pc plot viewer should be 1", () => readingIs(page, "current row", el("pc plot viewer"), 1));
      await session.step(104, "And pc plot viewer should have repainted by at least 500 pixels", () => repaintedBy(page, el("pc plot viewer"), 500));
      await session.step(105, "When user sets \"Current Line Width\" property of pc plot viewer to \"2\"", () => setProperty(page, "Current Line Width", el("pc plot viewer"), "2"));
      await session.step(106, "Then pc plot viewer should have repainted by at least 500 pixels", () => repaintedBy(page, el("pc plot viewer"), 500));
      await session.step(107, "When user sets \"Horz Margin\" property of pc plot viewer to \"90\"", () => setProperty(page, "Horz Margin", el("pc plot viewer"), "90"));
      await session.step(108, "Then the \"view\" area of pc plot viewer should be narrower than before", () => areaNarrower(page, "view", el("pc plot viewer")));
      await session.step(109, "And pc plot viewer should have repainted by at least 1000 pixels", () => repaintedBy(page, el("pc plot viewer"), 1000));
      await session.step(110, "When user sets \"Horz Margin\" property of pc plot viewer to \"40\"", () => setProperty(page, "Horz Margin", el("pc plot viewer"), "40"));
      await session.step(111, "Then the \"view\" area of pc plot viewer should be wider than before", () => areaWider(page, "view", el("pc plot viewer")));
      await session.step(112, "When user sets \"Labels Orientation\" property of pc plot viewer to \"Vert\"", () => setProperty(page, "Labels Orientation", el("pc plot viewer"), "Vert"));
      await session.step(113, "Then the \"view\" area of pc plot viewer should be shorter than before", () => areaShorter(page, "view", el("pc plot viewer")));
      await session.step(114, "When user sets \"Labels Orientation\" property of pc plot viewer to \"Auto\"", () => setProperty(page, "Labels Orientation", el("pc plot viewer"), "Auto"));
      await session.step(115, "Then the \"view\" area of pc plot viewer should be taller than before", () => areaTaller(page, "view", el("pc plot viewer")));
    ...
    await run.scenario("Show Min Max and Show Labels drop the axis chrome", async () => {
      await session.step(119, "When user sets \"Show Min Max\" property of pc plot viewer to \"false\"", () => setProperty(page, "Show Min Max", el("pc plot viewer"), "false"));
      await session.step(120, "Then pc plot viewer should not have an \"axis min \\\"AGE\\\"\" area", () => hasNoArea(page, el("pc plot viewer"), "axis min \"AGE\""));
      await session.step(121, "And pc plot viewer should not have an \"axis max \\\"AGE\\\"\" area", () => hasNoArea(page, el("pc plot viewer"), "axis max \"AGE\""));
      await session.step(122, "And pc plot viewer should have repainted by at least 200 pixels", () => repaintedBy(page, el("pc plot viewer"), 200));
      await session.step(123, "When user sets \"Show Labels\" property of pc plot viewer to \"false\"", () => setProperty(page, "Show Labels", el("pc plot viewer"), "false"));
      await session.step(124, "Then pc plot viewer should not have an \"axis label \\\"AGE\\\"\" area", () => hasNoArea(page, el("pc plot viewer"), "axis label \"AGE\""));
      await session.step(125, "And pc plot viewer should have repainted by at least 200 pixels", () => repaintedBy(page, el("pc plot viewer"), 200));
      await session.step(126, "When user sets properties of pc plot viewer:", () => setProperties(page, el("pc plot viewer"), [["Show Min Max","true"],["Show Labels","true"]]), [["Show Min Max","true"],["Show Labels","true"]]);
      await session.step(129, "Then pc plot viewer should have an \"axis min \\\"AGE\\\"\" area", () => hasArea(page, el("pc plot viewer"), "axis min \"AGE\""));
      await session.step(130, "And pc plot viewer should have an \"axis label \\\"AGE\\\"\" area", () => hasArea(page, el("pc plot viewer"), "axis label \"AGE\""));
    ...
    await run.scenario("Filter > Show Filters hides the in-chart range sliders", async () => {
      await session.step(134, "When user hovers over pc plot viewer", () => hoverOver(page, el("pc plot viewer")));
      await session.step(135, "Then pc plot viewer should have a \"range slider \\\"AGE\\\"\" area", () => hasArea(page, el("pc plot viewer"), "range slider \"AGE\""));
      await session.step(136, "And pc plot viewer should have a \"range max handle \\\"AGE\\\"\" area", () => hasArea(page, el("pc plot viewer"), "range max handle \"AGE\""));
      await session.step(137, "And pc plot viewer should have a \"range slider \\\"WEIGHT\\\"\" area", () => hasArea(page, el("pc plot viewer"), "range slider \"WEIGHT\""));
      await session.step(138, "When user picks \"Filter > Show Filters\" from the context menu of pc plot viewer", () => pickFromContextMenu(page, "Filter > Show Filters", el("pc plot viewer")));
      await session.step(139, "Then \"Show Filters\" property of pc plot viewer should be \"false\"", () => propertyShouldBe(page, "Show Filters", el("pc plot viewer"), "false"));
      await session.step(140, "When user hovers over pc plot viewer", () => hoverOver(page, el("pc plot viewer")));
      await session.step(141, "Then pc plot viewer should not have a \"range slider \\\"AGE\\\"\" area", () => hasNoArea(page, el("pc plot viewer"), "range slider \"AGE\""));
      await session.step(142, "And pc plot viewer should not have a \"range max handle \\\"AGE\\\"\" area", () => hasNoArea(page, el("pc plot viewer"), "range max handle \"AGE\""));
      await session.step(143, "And the \"range max of \\\"AGE\\\"\" reading of pc plot viewer should be 89", () => readingIs(page, "range max of \"AGE\"", el("pc plot viewer"), 89));
      await session.step(144, "When user picks \"Filter > Show Filters\" from the context menu of pc plot viewer", () => pickFromContextMenu(page, "Filter > Show Filters", el("pc plot viewer")));
      await session.step(145, "Then \"Show Filters\" property of pc plot viewer should be \"true\"", () => propertyShouldBe(page, "Show Filters", el("pc plot viewer"), "true"));
      await session.step(146, "When user hovers over pc plot viewer", () => hoverOver(page, el("pc plot viewer")));
      await session.step(147, "Then pc plot viewer should have a \"range slider \\\"AGE\\\"\" area", () => hasArea(page, el("pc plot viewer"), "range slider \"AGE\""));
    ...
    await run.scenario("To Script > To JavaScript prints the call that rebuilds the plot", async () => {
      await session.step(151, "Given the package autostarts have completed", () => autostartsCompleted(page));
      await session.step(152, "When user picks \"To Script > To JavaScript\" from the context menu of pc plot viewer", () => pickFromContextMenu(page, "To Script > To JavaScript", el("pc plot viewer")));
      await session.step(153, "Then balloon should contain text \"addViewer\"", () => shouldContainText(page, el("balloon"), "addViewer"));
