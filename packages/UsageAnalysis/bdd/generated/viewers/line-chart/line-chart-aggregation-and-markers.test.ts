...
test.describe("Line chart aggregation, whiskers and markers", () => {
  ...
  test("Line chart aggregation, whiskers and markers", {tag: ["@journey", "@viewers", "@realizes:viewers.line-chart"]}, async ({browser}) => {
    ...
    await session.step(18, "And user adds a line chart viewer with:", () => addViewerWith(page, "line chart", [["xColumnName","Competition assay Date"],["xMap","year quarter"],["yColumnNames","Chemical Space X"]]), [["xColumnName","Competition assay Date"],["xMap","year quarter"],["yColumnNames","Chemical Space X"]]);
    ...
    await run.scenario("Whiskers on a chart that is not aggregated are not drawn whatever the property says", async () => {
      ...
      await session.step(58, "When user sets properties of line chart viewer:", () => setProperties(page, el("line chart viewer"), [["xMap",""],["xColumnName","CAST Idea ID"]]), [["xMap",""],["xColumnName","CAST Idea ID"]]);
      ...
      await session.step(65, "When user sets properties of line chart viewer:", () => setProperties(page, el("line chart viewer"), [["whiskersType",""],["xColumnName","Competition assay Date"],["xMap","year quarter"]]), [["whiskersType",""],["xColumnName","Competition assay Date"],["xMap","year quarter"]]);
    ...
    await run.scenario("A size column is reported and a marker type change redraws", async () => {
      ...
      await session.step(92, "When user sets properties of line chart viewer:", () => setProperties(page, el("line chart viewer"), [["markersSizeColumnName",""],["markerType","circle"]]), [["markersSizeColumnName",""],["markerType","circle"]]);
    ...
    await run.scenario("The aggregation selector shows up only for a multi-axis aggregated split", async () => {
      ...
      await session.step(113, "When user sets properties of line chart viewer:", () => setProperties(page, el("line chart viewer"), [["yColumnNames","Chemical Space X, TPSA"],["multiAxis","true"]]), [["yColumnNames","Chemical Space X, TPSA"],["multiAxis","true"]]);
      ...
      await session.step(121, "When user sets properties of line chart viewer:", () => setProperties(page, el("line chart viewer"), [["yColumnNames","Chemical Space X"],["splitColumnNames",""]]), [["yColumnNames","Chemical Space X"],["splitColumnNames",""]]);
