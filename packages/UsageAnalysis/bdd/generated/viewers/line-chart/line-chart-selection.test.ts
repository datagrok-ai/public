...
test.describe("Line chart selection and the row markers", () => {
  ...
  test("Line chart selection and the row markers", {tag: ["@journey", "@viewers", "@realizes:viewers.line-chart"]}, async ({browser}) => {
    ...
    await run.scenario("A drag that ends where it started selects an empty band", async () => {
      ...
      await session.step(71, "When user drags a lasso over the \"plot\" area of line chart viewer", () => dragLasso(page, "plot", el("line chart viewer")));
      await session.step(72, "Then the \"rows selected\" reading of line chart viewer should be 0", () => readingIs(page, "rows selected", el("line chart viewer"), 0));
      await session.step(73, "And line chart viewer should show no selection highlight", () => noHighlight(page, el("line chart viewer")));
    ...
    await run.scenario("Show Selected Rows decides whether the selection is painted at all", async () => {
      await session.step(77, "When user selects rows where \"Stereo Category\" is \"R_ONE\"", () => selectWhereIs(page, "Stereo Category", "R_ONE"));
      await session.step(78, "Then the \"rows selected\" reading of line chart viewer should be 36", () => readingIs(page, "rows selected", el("line chart viewer"), 36));
      await session.step(79, "And line chart viewer should show a selection highlight", () => someHighlight(page, el("line chart viewer")));
      await session.step(80, "When user sets \"showSelectedRows\" property of line chart viewer to \"false\"", () => setProperty(page, "showSelectedRows", el("line chart viewer"), "false"));
      await session.step(81, "Then line chart viewer should show less selection highlight than before", () => lessHighlight(page, el("line chart viewer")));
      await session.step(82, "And the \"rows selected\" reading of line chart viewer should be 36", () => readingIs(page, "rows selected", el("line chart viewer"), 36));
      await session.step(83, "When user sets \"showSelectedRows\" property of line chart viewer to \"true\"", () => setProperty(page, "showSelectedRows", el("line chart viewer"), "true"));
      await session.step(84, "Then line chart viewer should show more selection highlight than before", () => moreHighlight(page, el("line chart viewer")));
      await session.step(85, "When user clears the row selection", () => clearSelection(page));
      await session.step(86, "Then the \"rows selected\" reading of line chart viewer should be 0", () => readingIs(page, "rows selected", el("line chart viewer"), 0));
    ...
    await run.scenario("Opening and dismissing the context menu leaves the selection and the current row alone", async () => {
      await session.step(90, "When user selects rows where \"Stereo Category\" is \"S_ABS\"", () => selectWhereIs(page, "Stereo Category", "S_ABS"));
      await session.step(91, "And user makes row 7 current", () => makeRowCurrent(page, 7));
      await session.step(92, "Then the \"rows selected\" reading of line chart viewer should be 2", () => readingIs(page, "rows selected", el("line chart viewer"), 2));
      await session.step(93, "When user opens the context menu of line chart viewer", () => openContextMenu(page, el("line chart viewer")));
      ...
      await session.step(96, "And row 7 should be current", () => currentRowIs(page, 7));
      await session.step(97, "And 2 rows should be selected", () => selectedRowCount(page, 2));
      await session.step(98, "When user clears the row selection", () => clearSelection(page));
    ...
    await run.scenario("The current-row line is drawn only when it is asked for", async () => {
      await session.step(102, "When user makes row 40 current", () => makeRowCurrent(page, 40));
      await session.step(103, "And user sets \"showCurrentRowLine\" property of line chart viewer to \"true\"", () => setProperty(page, "showCurrentRowLine", el("line chart viewer"), "true"));
      await session.step(104, "Then line chart viewer should have more ink than before", () => moreInk(page, el("line chart viewer")));
      await session.step(105, "When user makes row 80 current", () => makeRowCurrent(page, 80));
      await session.step(106, "Then line chart viewer should have repainted", () => repainted(page, el("line chart viewer")));
      await session.step(107, "When user sets \"showCurrentRowLine\" property of line chart viewer to \"false\"", () => setProperty(page, "showCurrentRowLine", el("line chart viewer"), "false"));
      await session.step(108, "Then line chart viewer should have less ink than before", () => lessInk(page, el("line chart viewer")));
    ...
    await run.scenario("Selecting through the data and reading it off the chart agree", async () => {
      await session.step(112, "When user selects rows where \"Stereo Category\" is one of \"S_ABS, S_PART\"", () => selectWhereOneOf(page, "Stereo Category", "S_ABS, S_PART"));
      await session.step(113, "Then 12 rows should be selected", () => selectedRowCount(page, 12));
      await session.step(114, "And the \"rows selected\" reading of line chart viewer should be 12", () => readingIs(page, "rows selected", el("line chart viewer"), 12));
      await session.step(115, "And line chart viewer should show a selection highlight", () => someHighlight(page, el("line chart viewer")));
      await session.step(116, "When user selects no rows", () => selectNoRows(page));
      await session.step(117, "Then the \"rows selected\" reading of line chart viewer should be 0", () => readingIs(page, "rows selected", el("line chart viewer"), 0));
      await session.step(118, "And line chart viewer should show no selection highlight", () => noHighlight(page, el("line chart viewer")));
    ...
    await run.scenario("After picking Lasso Tool and closing the menu, a Shift-drag selects the same X band", async () => {
      await session.step(122, "Given user clears the row selection", () => clearSelection(page));
      await session.step(123, "When user picks \"Tools > Lasso Tool\" from the context menu of line chart viewer", () => pickFromContextMenu(page, "Tools > Lasso Tool", el("line chart viewer")));
      await session.step(124, "Then \"lassoTool\" property of line chart viewer should be \"true\"", () => propertyShouldBe(page, "lassoTool", el("line chart viewer"), "true"));
      await session.step(125, "And the \"region drawing mode\" reading of line chart viewer should be \"false\"", () => readingReads(page, "region drawing mode", el("line chart viewer"), "false"));
      await session.step(126, "When user closes the context menu", () => closeContextMenu(page));
      await session.step(127, "And user drags a selection box over the \"plot\" area of line chart viewer", () => dragSelectionOverArea(page, "plot", el("line chart viewer")));
      await session.step(128, "Then the \"rows selected\" reading of line chart viewer should be 82", () => readingIs(page, "rows selected", el("line chart viewer"), 82));
      await session.step(129, "When user clears the row selection", () => clearSelection(page));
      await session.step(130, "And user sets \"lassoTool\" property of line chart viewer to \"false\"", () => setProperty(page, "lassoTool", el("line chart viewer"), "false"));
