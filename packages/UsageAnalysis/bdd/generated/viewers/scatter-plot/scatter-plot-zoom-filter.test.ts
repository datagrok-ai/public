...
test.describe("Scatter plot zoom and filter synchronization", () => {
  ...
  test("Scatter plot zoom and filter synchronization", {tag: ["@journey", "@viewers", "@realizes:viewers.scatter-plot"]}, async ({browser}) => {
    ...
    await run.scenario("A logarithmic axis alone filters nothing; Filter Out Invalid drops the non-positive rows", async () => {
      ...
      await session.step(84, "And user sets properties of scatter plot viewer:", () => setProperties(page, el("scatter plot viewer"), [["X","WEIGHT"],["Y","AGE_SHIFT"]]));
      ...
      await session.step(96, "When user sets properties of scatter plot viewer:", () => setProperties(page, el("scatter plot viewer"), [["Y Axis Type","linear"],["Filter Out Invalid","true"]]));
      ...
      await session.step(101, "When user sets properties of scatter plot viewer:", () => setProperties(page, el("scatter plot viewer"), [["Filter Out Invalid","false"],["X","WEIGHT"],["Y","HEIGHT"]]));
    ...
    await run.scenario("A large jitter on a logarithmic axis filters no row", async () => {
      await session.step(130, "When user sets properties of scatter plot viewer:", () => setProperties(page, el("scatter plot viewer"), [["jitterSize","16"],["jitterSizeY","17"]]));
      ...
      await session.step(139, "When user sets properties of scatter plot viewer:", () => setProperties(page, el("scatter plot viewer"), [["Y Axis Type","linear"],["jitterSize","0"],["jitterSizeY","0"]]));
