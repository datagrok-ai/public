...
test.describe("Scatter plot tooltip and marker labels", () => {
  ...
  test("Scatter plot tooltip and marker labels", {tag: ["@journey", "@viewers", "@realizes:viewers.scatter-plot"]}, async ({browser}) => {
    ...
    await session.step(19, "And user adds a scatter plot viewer with:", () => addViewerWith(page, "scatter plot", [["X","WEIGHT"],["Y","HEIGHT"]]));
    await session.step(22, "Then scatter plot viewer should show 872 rows", () => showsRows(page, el("scatter plot viewer"), 872));
    await session.step(23, "And properties of scatter plot viewer should be:", () => propertiesShouldBe(page, el("scatter plot viewer"), [["Show Tooltip","inherit from table"],["Label Columns",""]]));
    ...
    await run.scenario("A custom column list replaces what the tooltip shows", async () => {
      ...
      await session.step(54, "And user sets properties of scatter plot viewer:", () => setProperties(page, el("scatter plot viewer"), [["Show Tooltip","inherit from table"],["Row Tooltip",""],["Data Values","Merge"],["showLabels","Auto"]]));
    ...
    await run.scenario("Labels are drawn for every row, then only for the selected ones", async () => {
      ...
      await session.step(78, "When user sets properties of scatter plot viewer:", () => setProperties(page, el("scatter plot viewer"), [["Label Columns",""],["Show Labels For","All"]]));
    ...
    await run.scenario("Labels on a datetime axis survive a filter that keeps one category and a selection", async () => {
      await session.step(85, "When user sets properties of scatter plot viewer:", () => setProperties(page, el("scatter plot viewer"), [["X","STARTED"],["Y","RACE"],["Label Columns","USUBJID"],["Show Labels For","Selected"]]));
      ...
      await session.step(104, "When user sets properties of scatter plot viewer:", () => setProperties(page, el("scatter plot viewer"), [["X","WEIGHT"],["Y","HEIGHT"],["Label Columns",""],["Show Labels For","All"]]));
