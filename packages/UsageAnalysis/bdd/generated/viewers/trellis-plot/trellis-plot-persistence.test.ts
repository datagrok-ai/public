...
test.describe("Trellis plot through a layout and a project", () => {
  ...
  test("Two trellises keep their own settings through one layout", {tag: ["@viewers", "@realizes:viewers.trellis-plot"]}, async ({browser}) => {
    ...
    await session.step(20, "And user adds a trellis plot viewer with:", () => addViewerWith(page, "trellis plot", [["X Column Names","SEX, CONTROL"],["Y Column Names","RACE"],["Viewer Type","Pie chart"]]));
    ...
    await session.step(28, "When user sets properties of second trellis plot viewer:", () => setProperties(page, el("second trellis plot viewer"), [["X Column Names","RACE"],["Y Column Names","SEX"],["Viewer Type","Bar chart"]]));
    await session.step(32, "Then the open tableview should have 2 trellis plot viewers", () => viewerCount(page, 2, "trellis plot"));
    await session.step(33, "And properties of first trellis plot viewer should be:", () => propertiesShouldBe(page, el("first trellis plot viewer"), [["X Column Names","SEX, CONTROL"],["Viewer Type","Pie chart"]]));
    await session.step(36, "And properties of second trellis plot viewer should be:", () => propertiesShouldBe(page, el("second trellis plot viewer"), [["X Column Names","RACE"],["Viewer Type","Bar chart"]]));
    ...
    await session.step(45, "And properties of first trellis plot viewer should be:", () => propertiesShouldBe(page, el("first trellis plot viewer"), [["X Column Names","SEX, CONTROL"],["Y Column Names","RACE"],["Viewer Type","Pie chart"]]));
    await session.step(49, "And properties of second trellis plot viewer should be:", () => propertiesShouldBe(page, el("second trellis plot viewer"), [["X Column Names","RACE"],["Y Column Names","SEX"],["Viewer Type","Bar chart"]]));
    ...
    await session.step(59, "And properties of trellis plot viewer should be:", () => propertiesShouldBe(page, el("trellis plot viewer"), [["X Column Names","SEX, CONTROL"],["Y Column Names","RACE"],["Viewer Type","Pie chart"]]));
  ...
  test("A selection behind Row Source Selected does not come back with the project, and the trellis still draws", {tag: ["@viewers", "@realizes:viewers.trellis-plot"]}, async ({browser}) => {
    ...
    await session.step(76, "And properties of trellis plot viewer should be:", () => propertiesShouldBe(page, el("trellis plot viewer"), [["X Column Names","SEX, CONTROL"],["Y Column Names","RACE"],["Viewer Type","Pie chart"],["On Click","Select"],["Row Source","Selected"]]));
  ...
  test("With nothing selected a Selected trellis comes back from a project alive", {tag: ["@viewers", "@realizes:viewers.trellis-plot"]}, async ({browser}) => {
    ...
    await session.step(20, "And user adds a trellis plot viewer with:", () => addViewerWith(page, "trellis plot", [["X Column Names","SEX, CONTROL"],["Y Column Names","RACE"],["Viewer Type","Pie chart"]]));
    await session.step(24, "Then the \"cells\" reading of trellis plot viewer should be 16", () => readingIs(page, "cells", el("trellis plot viewer"), 16));
    await session.step(95, "When user sets properties of trellis plot viewer:", () => setProperties(page, el("trellis plot viewer"), [["On Click","Select"],["Row Source","Selected"]]));
