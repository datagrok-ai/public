...
test.describe("Trellis plot global scale, axes and range sliders", () => {
  ...
  test("Trellis plot global scale, axes and range sliders", {tag: ["@journey", "@viewers", "@realizes:viewers.trellis-plot"]}, async ({browser}) => {
    ...
    await session.step(19, "And user adds a trellis plot viewer with:", () => addViewerWith(page, "trellis plot", [["X Column Names","SEX"],["Y Column Names","RACE"],["Viewer Type","Scatter plot"]]));
    ...
    await run.scenario("Show X and Y Axes obey Always, Never and Auto", async () => {
      await session.step(43, "When user sets properties of trellis plot viewer:", () => setProperties(page, el("trellis plot viewer"), [["Show X Axes","Always"],["Show Y Axes","Always"]]));
      ...
      await session.step(51, "When user sets properties of trellis plot viewer:", () => setProperties(page, el("trellis plot viewer"), [["Show X Axes","Never"],["Show Y Axes","Never"]]));
      ...
      await session.step(58, "When user sets properties of trellis plot viewer:", () => setProperties(page, el("trellis plot viewer"), [["Show X Axes","Auto"],["Show Y Axes","Auto"]]));
    ...
    await run.scenario("Reset Inner Range Sliders is offered only while a slider exists", async () => {
      ...
      await session.step(81, "And user sets properties of trellis plot viewer:", () => setProperties(page, el("trellis plot viewer"), [["Show X Axes","Never"],["Show Y Axes","Never"]]));
      ...
      await session.step(88, "And user sets properties of trellis plot viewer:", () => setProperties(page, el("trellis plot viewer"), [["Show X Axes","Always"],["Show Y Axes","Always"]]));
    ...
    await run.scenario("The wheel zooms a cell only once Allow Zoom says so", async () => {
      await session.step(111, "When user sets properties of trellis plot viewer:", () => setProperties(page, el("trellis plot viewer"), [["Global Scale","false"]]));
