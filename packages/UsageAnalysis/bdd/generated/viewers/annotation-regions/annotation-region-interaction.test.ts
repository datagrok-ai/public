...
/* ---
generated: features/viewers/annotation-regions/annotation-region-interaction.feature
...
test.describe("Annotation region interaction", () => {
  const session = feature(test, "features/viewers/annotation-regions/annotation-region-interaction.feature", import.meta.url);
  test("Annotation region interaction", {tag: ["@journey", "@viewers", "@realizes:viewers.scatter-plot"]}, async ({browser}) => {
    ...
    await run.scenario("Control drops a selected region's rows and Shift adds a region's rows", async () => {
      ...
      await session.step(53, "When user clicks on the \"region Outer\" area of scatter plot viewer", () => clickArea(page, "region Outer", el("scatter plot viewer")));
      await session.step(54, "And user clicks on the \"region Inner\" area of scatter plot viewer holding Shift", () => clickAreaHolding(page, "region Inner", el("scatter plot viewer"), "Shift"));
    ...
    await run.scenario("Swapped axes keep the regions and another column drops them", async () => {
      ...
      await session.step(109, "Then scatter plot viewer should have a \"region Outer\" area", () => hasArea(page, el("scatter plot viewer"), "region Outer"));
      await session.step(110, "And scatter plot viewer should have a \"region Inner\" area", () => hasArea(page, el("scatter plot viewer"), "region Inner"));
      ...
      await session.step(119, "Then scatter plot viewer should have a \"region Outer\" area", () => hasArea(page, el("scatter plot viewer"), "region Outer"));
      await session.step(120, "And scatter plot viewer should have a \"region Inner\" area", () => hasArea(page, el("scatter plot viewer"), "region Inner"));
    ...
    await run.scenario("A column rename follows into the regions", async () => {
      await session.step(124, "When user renames \"AGE\" column to \"AGE (years)\"", () => renameColumn(page, "AGE", "AGE (years)"));
      await session.step(125, "Then \"annotationRegions\" property of scatter plot viewer should contain \"\\\"x\\\":\\\"AGE (years)\\\"\"", () => propertyShouldContain(page, "annotationRegions", el("scatter plot viewer"), "\"x\":\"AGE (years)\""));
      await session.step(126, "And scatter plot viewer should have a \"region Outer\" area", () => hasArea(page, el("scatter plot viewer"), "region Outer"));
      await session.step(127, "And scatter plot viewer should have a \"region Inner\" area", () => hasArea(page, el("scatter plot viewer"), "region Inner"));
      await session.step(128, "When user renames \"AGE (years)\" column to \"AGE\"", () => renameColumn(page, "AGE (years)", "AGE"));
      await session.step(129, "Then \"annotationRegions\" property of scatter plot viewer should contain \"\\\"x\\\":\\\"AGE\\\"\"", () => propertyShouldContain(page, "annotationRegions", el("scatter plot viewer"), "\"x\":\"AGE\""));
      await session.step(130, "And scatter plot viewer should have a \"region Outer\" area", () => hasArea(page, el("scatter plot viewer"), "region Outer"));
      await session.step(131, "And scatter plot viewer should have a \"region Inner\" area", () => hasArea(page, el("scatter plot viewer"), "region Inner"));
    ...
    await run.scenario("Escape leaves the drawing mode with the regions untouched", async () => {
      await session.step(135, "When user sets \"showViewerAnnotationRegions\" property of scatter plot viewer to \"false\"", () => setProperty(page, "showViewerAnnotationRegions", el("scatter plot viewer"), "false"));
      await session.step(136, "And user moves the pointer away from scatter plot viewer", () => pointerAway(page, el("scatter plot viewer")));
      await session.step(137, "And user picks \"Tools > Draw Annotation Region\" from the context menu of scatter plot viewer", () => pickFromContextMenu(page, "Tools > Draw Annotation Region", el("scatter plot viewer")));
      await session.step(138, "Then the \"region drawing mode\" reading of scatter plot viewer should be \"true\"", () => readingReads(page, "region drawing mode", el("scatter plot viewer"), "true"));
      await session.step(139, "When user presses Escape", () => pressKey(page, "Escape"));
      await session.step(140, "Then the \"region drawing mode\" reading of scatter plot viewer should be \"false\"", () => readingReads(page, "region drawing mode", el("scatter plot viewer"), "false"));
      await session.step(141, "And the \"viewer regions\" reading of scatter plot viewer should be 2", () => readingIs(page, "viewer regions", el("scatter plot viewer"), 2));
      await session.step(142, "When user sets \"showViewerAnnotationRegions\" property of scatter plot viewer to \"true\"", () => setProperty(page, "showViewerAnnotationRegions", el("scatter plot viewer"), "true"));
      await session.step(143, "Then scatter plot viewer should have a \"region Outer\" area", () => hasArea(page, el("scatter plot viewer"), "region Outer"));
      await session.step(144, "And scatter plot viewer should have a \"region Inner\" area", () => hasArea(page, el("scatter plot viewer"), "region Inner"));
