...
test.describe("Annotation regions", () => {
  ...
  test("Annotation regions", {tag: ["@journey", "@viewers", "@realizes:viewers.scatter-plot", "@realizes:viewers.histogram", "@realizes:viewers.line-chart"]}, async ({browser}) => {
    ...
    await run.scenario("Show Annotation Regions in the Tools menu switches both kinds at once", async () => {
      ...
      await session.step(68, "And \"showDataframeAnnotationRegions\" property of scatter plot viewer should be \"true\"", () => propertyShouldBe(page, "showDataframeAnnotationRegions", el("scatter plot viewer"), "true"));
      await session.step(69, "And the \"regions shown\" reading of scatter plot viewer should be 1", () => readingIs(page, "regions shown", el("scatter plot viewer"), 1));
      await session.step(70, "When user picks \"Tools > Show Annotation Regions\" from the context menu of the \"empty space\" area of scatter plot viewer", () => pickFromAreaContextMenu(page, "Tools > Show Annotation Regions", "empty space", el("scatter plot viewer")));
      await session.step(71, "Then \"showViewerAnnotationRegions\" property of scatter plot viewer should be \"false\"", () => propertyShouldBe(page, "showViewerAnnotationRegions", el("scatter plot viewer"), "false"));
      await session.step(72, "And the \"regions shown\" reading of scatter plot viewer should be 0", () => readingIs(page, "regions shown", el("scatter plot viewer"), 0));
      await session.step(73, "When user picks \"Tools > Show Annotation Regions\" from the context menu of the \"empty space\" area of scatter plot viewer", () => pickFromAreaContextMenu(page, "Tools > Show Annotation Regions", "empty space", el("scatter plot viewer")));
      await session.step(74, "Then \"showViewerAnnotationRegions\" property of scatter plot viewer should be \"true\"", () => propertyShouldBe(page, "showViewerAnnotationRegions", el("scatter plot viewer"), "true"));
      await session.step(75, "And the \"regions shown\" reading of scatter plot viewer should be 1", () => readingIs(page, "regions shown", el("scatter plot viewer"), 1));
    ...
    await run.scenario("A histogram locks the categorical axis and stores the band as a formula region", async () => {
      await session.step(79, "Given user adds a histogram viewer with:", () => addViewerWith(page, "histogram", [["valueColumnName","AGE"]]), [["valueColumnName","AGE"]]);
      await session.step(81, "Then the \"viewer regions\" reading of histogram viewer should be 0", () => readingIs(page, "viewer regions", el("histogram viewer"), 0));
      await session.step(82, "When user picks \"Tools > Draw Annotation Region\" from the context menu of histogram viewer", () => pickFromContextMenu(page, "Tools > Draw Annotation Region", el("histogram viewer")));
      await session.step(83, "And user drags across the \"view\" area of histogram viewer", () => dragAcrossArea(page, "view", el("histogram viewer")));
      await session.step(84, "Then the \"viewer regions\" reading of histogram viewer should be 1", () => readingIs(page, "viewer regions", el("histogram viewer"), 1));
      await session.step(85, "And the \"regions shown\" reading of histogram viewer should be 1", () => readingIs(page, "regions shown", el("histogram viewer"), 1));
      await session.step(86, "And histogram viewer should have a \"region 1\" area", () => hasArea(page, el("histogram viewer"), "region 1"));
      await session.step(87, "And \"annotationRegions\" property of histogram viewer should contain \"formula\"", () => propertyShouldContain(page, "annotationRegions", el("histogram viewer"), "formula"));
      await session.step(88, "And \"annotationRegions\" property of histogram viewer should contain \"${AGE}\"", () => propertyShouldContain(page, "annotationRegions", el("histogram viewer"), "${AGE}"));
      await session.step(89, "When user clicks OK button in \"Formula Lines\" dialog", () => clickOn(page, el("OK button in \"Formula Lines\" dialog")));
    ...
    await run.scenario("The value axis offers Add Line, Add Band and Add Region", async () => {
      await session.step(93, "When user right-clicks on the \"x axis\" area of histogram viewer", () => rightClickArea(page, "x axis", el("histogram viewer")));
      await session.step(94, "Then the open menu should list \"Annotations > Add Line\"", () => menuLists(page, "Annotations > Add Line"));
      await session.step(95, "And the open menu should list \"Annotations > Add Band\"", () => menuLists(page, "Annotations > Add Band"));
      await session.step(96, "And the open menu should list \"Annotations > Add Region\"", () => menuLists(page, "Annotations > Add Region"));
    ...
    await run.scenario("A multi-axis line chart offers no Draw Annotation Region", async () => {
      await session.step(101, "Given user adds a line chart viewer with:", () => addViewerWith(page, "line chart", [["xColumnName","AGE"],["yColumnNames","WEIGHT, HEIGHT"],["multiAxis","false"]]), [["xColumnName","AGE"],["yColumnNames","WEIGHT, HEIGHT"],["multiAxis","false"]]);
      await session.step(105, "When user opens the context menu of line chart viewer", () => openContextMenu(page, el("line chart viewer")));
      await session.step(106, "Then the open menu should list \"Tools > Draw Annotation Region\"", () => menuLists(page, "Tools > Draw Annotation Region"));
      await session.step(107, "When user closes the context menu", () => closeContextMenu(page));
      await session.step(108, "And user sets \"multiAxis\" property of line chart viewer to \"true\"", () => setProperty(page, "multiAxis", el("line chart viewer"), "true"));
      await session.step(109, "And user opens the context menu of line chart viewer", () => openContextMenu(page, el("line chart viewer")));
      await session.step(110, "Then the open menu should not list \"Tools > Draw Annotation Region\"", () => menuDoesNotList(page, "Tools > Draw Annotation Region"));
      await session.step(111, "When user closes the context menu", () => closeContextMenu(page));
