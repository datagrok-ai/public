...
import {addViewer, clickArea, hasArea, hasNoArea, noBalloons, noErrors, propertyShouldBe, readingIs, readingReads, readingsEqual, setProperty} from '@datagrok-libraries/bdd/bindings/tiers/viewers/steps';
...
test.describe("Forms viewer field lifecycle and number format", () => {
  ...
  test("Forms viewer field lifecycle and number format", {tag: ["@journey", "@viewers", "@realizes:viewers.forms"]}, async ({browser}) => {
    ...
    await run.scenario("An empty field set draws nothing and says nothing", async () => {
      ...
      await session.step(85, "When user sets \"fieldsColumnNames\" property of forms viewer to \"RACE, AGE, SEX\"", () => setProperty(page, "fieldsColumnNames", el("forms viewer"), "RACE, AGE, SEX"));
      await session.step(86, "Then the \"fields shown\" reading of forms viewer should be 3", () => readingIs(page, "fields shown", el("forms viewer"), 3));
    });
    await run.scenario("A renamed column carries its field to the new name", async () => {
      await session.step(89, "Then the \"fields\" reading of forms viewer should be \"RACE, AGE, SEX\"", () => readingReads(page, "fields", el("forms viewer"), "RACE, AGE, SEX"));
      await session.step(90, "When user renames \"SEX\" column to \"GENDER\"", () => renameColumn(page, "SEX", "GENDER"));
      await session.step(91, "Then the table should have a column \"GENDER\"", () => hasColumn(page, "GENDER"));
      await session.step(92, "And the \"fields\" reading of forms viewer should be \"RACE, AGE, GENDER\"", () => readingReads(page, "fields", el("forms viewer"), "RACE, AGE, GENDER"));
      await session.step(93, "And the \"header labels\" reading of forms viewer should be \"RACE, AGE, GENDER\"", () => readingReads(page, "header labels", el("forms viewer"), "RACE, AGE, GENDER"));
      await session.step(94, "And forms viewer should have a \"field GENDER of card 1\" area", () => hasArea(page, el("forms viewer"), "field GENDER of card 1"));
      await session.step(95, "And forms viewer should not have a \"field SEX of card 1\" area", () => hasNoArea(page, el("forms viewer"), "field SEX of card 1"));
      await session.step(96, "And the \"GENDER of card 1\" reading of forms viewer should be \"F\"", () => readingReads(page, "GENDER of card 1", el("forms viewer"), "F"));
      await session.step(97, "And no error or warning balloon should have been shown", () => noBalloons(page));
      await session.step(98, "When user renames \"GENDER\" column to \"SEX\"", () => renameColumn(page, "GENDER", "SEX"));
      await session.step(99, "Then the \"fields\" reading of forms viewer should be \"RACE, AGE, SEX\"", () => readingReads(page, "fields", el("forms viewer"), "RACE, AGE, SEX"));
    ...
    await run.scenario("A rename to a service name drops the field", async () => {
      await session.step(103, "Then the \"fields\" reading of forms viewer should be \"RACE, AGE, SEX\"", () => readingReads(page, "fields", el("forms viewer"), "RACE, AGE, SEX"));
      await session.step(104, "When user renames \"SEX\" column to \"~SERVICE\"", () => renameColumn(page, "SEX", "~SERVICE"));
      await session.step(105, "Then the \"fields\" reading of forms viewer should be \"RACE, AGE\"", () => readingReads(page, "fields", el("forms viewer"), "RACE, AGE"));
      await session.step(106, "And the \"header labels\" reading of forms viewer should be \"RACE, AGE\"", () => readingReads(page, "header labels", el("forms viewer"), "RACE, AGE"));
      await session.step(107, "And the \"header labels\" reading of forms viewer should not contain \"~\"", () => readingNotContains(page, "header labels", el("forms viewer"), "~"));
      await session.step(108, "And the \"fields shown\" reading of forms viewer should be 2", () => readingIs(page, "fields shown", el("forms viewer"), 2));
      await session.step(109, "And forms viewer should not have a \"field ~SERVICE of card 1\" area", () => hasNoArea(page, el("forms viewer"), "field ~SERVICE of card 1"));
      await session.step(110, "And no error or warning balloon should have been shown", () => noBalloons(page));
      await session.step(111, "When user renames \"~SERVICE\" column to \"SEX\"", () => renameColumn(page, "~SERVICE", "SEX"));
      await session.step(112, "Then the table should have a column \"SEX\"", () => hasColumn(page, "SEX"));
    ...
    await run.scenario("A column that leaves the table takes its field with it", async () => {
      await session.step(116, "When user sets \"fieldsColumnNames\" property of forms viewer to \"RACE, AGE, SEX\"", () => setProperty(page, "fieldsColumnNames", el("forms viewer"), "RACE, AGE, SEX"));
      await session.step(117, "Then the \"fields\" reading of forms viewer should be \"RACE, AGE, SEX\"", () => readingReads(page, "fields", el("forms viewer"), "RACE, AGE, SEX"));
      await session.step(118, "When user removes \"RACE\" column", () => removeColumn(page, "RACE"));
      await session.step(119, "Then the table should not have a column \"RACE\"", () => hasNoColumn(page, "RACE"));
      await session.step(120, "And the \"fields\" reading of forms viewer should be \"AGE, SEX\"", () => readingReads(page, "fields", el("forms viewer"), "AGE, SEX"));
      await session.step(121, "And the \"header labels\" reading of forms viewer should be \"AGE, SEX\"", () => readingReads(page, "header labels", el("forms viewer"), "AGE, SEX"));
      await session.step(122, "And the \"fields shown\" reading of forms viewer should be 2", () => readingIs(page, "fields shown", el("forms viewer"), 2));
      await session.step(123, "And forms viewer should not have a \"field RACE of card 1\" area", () => hasNoArea(page, el("forms viewer"), "field RACE of card 1"));
      await session.step(124, "And the \"AGE of card 1\" reading of forms viewer should be \"26\"", () => readingReads(page, "AGE of card 1", el("forms viewer"), "26"));
