...
test.describe("Box plot selection and highlight", () => {
  ...
  test("Box plot selection and highlight", {tag: ["@journey", "@viewers", "@realizes:viewers.box-plot"]}, async ({browser}) => {
    ...
    await run.scenario("A marker click sets the current row", async () => {
      ...
      await session.step(24, "And \"RACE\" of the current row should be \"Asian\"", () => currentRowValue(page, "RACE", "Asian"));
    });
    await run.scenario("A Shift-drag selects a band and highlights it", async () => {
      await session.step(27, "When user clears the row selection", () => clearSelection(page));
      await session.step(28, "And user drags a selection box over the \"Caucasian values\" area of box plot viewer", () => dragSelectionOverArea(page, "Caucasian values", el("box plot viewer")));
      await session.step(29, "Then some rows where \"RACE\" is \"Caucasian\" should be selected", () => someOfSelected(page, "RACE", "Caucasian"));
      await session.step(30, "And box plot viewer should show more selection highlight than before", () => moreHighlight(page, el("box plot viewer")));
    });
    await run.scenario("The highlight survives a categorical coloring", async () => {
      await session.step(33, "When user clears the row selection", () => clearSelection(page));
      await session.step(34, "And user sets \"Marker Color Column\" property of box plot viewer to \"SEX\"", () => setProperty(page, "Marker Color Column", el("box plot viewer"), "SEX"));
      await session.step(35, "And user drags a selection box over the \"Caucasian values\" area of box plot viewer", () => dragSelectionOverArea(page, "Caucasian values", el("box plot viewer")));
      await session.step(36, "Then some rows where \"RACE\" is \"Caucasian\" should be selected", () => someOfSelected(page, "RACE", "Caucasian"));
      await session.step(37, "And box plot viewer should show more selection highlight than before", () => moreHighlight(page, el("box plot viewer")));
      await session.step(38, "When user sets \"Marker Color Column\" property of box plot viewer to \"\"", () => setProperty(page, "Marker Color Column", el("box plot viewer"), ""));
    });
    await run.scenario("Category labels select categories", async () => {
      ...
      await session.step(42, "And user clicks on the \"category Caucasian\" area of box plot viewer", () => clickArea(page, "category Caucasian", el("box plot viewer")));
      await session.step(43, "Then only rows where \"RACE\" is \"Caucasian\" should be selected", () => onlyOfSelected(page, "RACE", "Caucasian"));
      await session.step(44, "When user clicks on the \"category Black\" area of box plot viewer holding Control", () => clickAreaHolding(page, "category Black", el("box plot viewer"), "Control"));
      await session.step(45, "Then only rows where \"RACE\" is one of \"Black, Caucasian\" should be selected", () => onlyOfAnySelected(page, "RACE", "Black, Caucasian"));
    ...
    await run.scenario("Show Selected Rows and Row Source gate the highlight", async () => {
      ...
      await session.step(71, "And \"Marker Color Column\" property of box plot viewer should be \"RACE\"", () => propertyShouldBe(page, "Marker Color Column", el("box plot viewer"), "RACE"));
      await session.step(72, "When user sets \"Show Selected Rows\" property of box plot viewer to \"true\"", () => setProperty(page, "Show Selected Rows", el("box plot viewer"), "true"));
      await session.step(73, "Then box plot viewer should show more selection highlight than before", () => moreHighlight(page, el("box plot viewer")));
    ...
    await run.scenario("The hover tooltip and Show Mouse Over Point", async () => {
      await session.step(83, "When user hovers over the \"marker\" area of box plot viewer", () => hoverArea(page, "marker", el("box plot viewer")));
      await session.step(84, "Then tooltip should be visible", () => shouldBe(page, el("tooltip"), "visible"));
      await session.step(85, "When user moves the pointer away from box plot viewer", () => pointerAway(page, el("box plot viewer")));
      await session.step(86, "And user sets \"Show Mouse Over Point\" property of box plot viewer to \"false\"", () => setProperty(page, "Show Mouse Over Point", el("box plot viewer"), "false"));
      await session.step(87, "And user hovers over the \"marker\" area of box plot viewer", () => hoverArea(page, "marker", el("box plot viewer")));
      await session.step(88, "Then box plot viewer should not have repainted", () => notRepainted(page, el("box plot viewer")));
