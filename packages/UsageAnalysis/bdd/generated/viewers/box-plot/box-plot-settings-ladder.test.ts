...
test.describe("Box plot settings ladder", () => {
  ...
  test("Box plot settings ladder", {tag: ["@journey", "@viewers", "@realizes:viewers.box-plot"]}, async ({browser}) => {
    ...
    await run.scenario("The ladder survives a layout round-trip", async () => {
      await session.step(84, "When user saves the layout of the current table view", () => saveLayout(page));
      await session.step(85, "And user clicks on close icon of box plot viewer", () => clickOn(page, el("close icon of box plot viewer")));
      await session.step(86, "Then box plot viewer should be absent", () => shouldBe(page, el("box plot viewer"), "absent"));
      await session.step(87, "When user adds a scatter plot viewer", () => addViewer(page, "scatter plot"));
      await session.step(88, "Then scatter plot viewer should be visible", () => shouldBe(page, el("scatter plot viewer"), "visible"));
    ...
    await run.scenario("The ladder survives a project round-trip", async () => {
      ...
      await session.step(108, "Then box plot viewer should show a narrower value range than before", () => narrowerRange(page, el("box plot viewer")));
      await session.step(109, "When user remembers the value range of box plot viewer", () => rememberRange(page, el("box plot viewer")));
      await session.step(110, "And user saves the current view as project \"bdd box plot ladder\"", () => saveAsProject(page, "bdd box plot ladder"));
