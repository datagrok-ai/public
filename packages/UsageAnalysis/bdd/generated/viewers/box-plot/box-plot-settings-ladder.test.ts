...
test.describe("Box plot settings ladder", () => {
  ...
  test("Box plot settings ladder", {tag: ["@journey", "@viewers", "@realizes:viewers.box-plot"]}, async ({browser}) => {
    ...
    await run.scenario("An explicit coloring survives a value change", async () => {
      ...
      await session.step(40, "Then properties of box plot viewer should be:", () => propertiesShouldBe(page, el("box plot viewer"), [["Category 1","SEX"],["Marker Color Column","HEIGHT"]]), [["Category 1","SEX"],["Marker Color Column","HEIGHT"]]);
    });
    await run.scenario("Value limits and the log axis", async () => {
      await session.step(45, "When user sets properties of box plot viewer:", () => setProperties(page, el("box plot viewer"), [["Value Min","20"],["Value Max","60"]]), [["Value Min","20"],["Value Max","60"]]);
      await session.step(48, "Then box plot viewer should show a narrower value range than before", () => narrowerRange(page, el("box plot viewer")));
      await session.step(49, "And properties of box plot viewer should be:", () => propertiesShouldBe(page, el("box plot viewer"), [["Value Min","20"],["Value Max","60"]]), [["Value Min","20"],["Value Max","60"]]);
      await session.step(52, "When user sets properties of box plot viewer:", () => setProperties(page, el("box plot viewer"), [["Value Min",""],["Value Max",""]]), [["Value Min",""],["Value Max",""]]);
      await session.step(55, "Then box plot viewer should show a wider value range than before", () => widerRange(page, el("box plot viewer")));
      await session.step(56, "When user sets \"Axis Type\" property of box plot viewer to \"logarithmic\"", () => setProperty(page, "Axis Type", el("box plot viewer"), "logarithmic"));
      await session.step(57, "Then \"Axis Type\" property of box plot viewer should be \"logarithmic\"", () => propertyShouldBe(page, "Axis Type", el("box plot viewer"), "logarithmic"));
      await session.step(58, "And box plot viewer should have repainted", () => repainted(page, el("box plot viewer")));
      await session.step(59, "And no errors should have been logged", () => noErrors(page));
      await session.step(60, "And the value range of box plot viewer should lie within \"WEIGHT\" column", () => rangeWithinColumn(page, el("box plot viewer"), "WEIGHT"));
      await session.step(61, "When user sets properties of box plot viewer:", () => setProperties(page, el("box plot viewer"), [["Invert Y Axis","true"],["Plot Style","violin"]]), [["Invert Y Axis","true"],["Plot Style","violin"]]);
      await session.step(64, "Then properties of box plot viewer should be:", () => propertiesShouldBe(page, el("box plot viewer"), [["Invert Y Axis","true"],["Plot Style","violin"]]), [["Invert Y Axis","true"],["Plot Style","violin"]]);
      await session.step(67, "And box plot viewer should have repainted", () => repainted(page, el("box plot viewer")));
    });
    await run.scenario("A zoom survives a coloring change", async () => {
      await session.step(70, "When user zooms into the value axis of box plot viewer", () => zoomValueAxis(page, el("box plot viewer")));
      await session.step(71, "Then box plot viewer should show a narrower value range than before", () => narrowerRange(page, el("box plot viewer")));
      ...
      await session.step(73, "Then \"Marker Color Column\" property of box plot viewer should be \"SEX\"", () => propertyShouldBe(page, "Marker Color Column", el("box plot viewer"), "SEX"));
      await session.step(74, "And box plot viewer should show the same value range as before", () => sameRange(page, el("box plot viewer")));
    });
    await run.scenario("The ladder survives a layout round-trip", async () => {
      await session.step(77, "When user saves the layout of the current table view", () => saveLayout(page));
      await session.step(78, "And user clicks on close icon of box plot viewer", () => clickOn(page, el("close icon of box plot viewer")));
      await session.step(79, "Then box plot viewer should be absent", () => shouldBe(page, el("box plot viewer"), "absent"));
      await session.step(80, "When user adds a scatter plot viewer", () => addViewer(page, "scatter plot"));
      await session.step(81, "Then scatter plot viewer should be visible", () => shouldBe(page, el("scatter plot viewer"), "visible"));
      await session.step(82, "When user loads the saved layout", () => loadLayout(page));
      await session.step(83, "Then box plot viewer should be visible", () => shouldBe(page, el("box plot viewer"), "visible"));
      await session.step(84, "And scatter plot viewer should be absent", () => shouldBe(page, el("scatter plot viewer"), "absent"));
      await session.step(85, "And properties of box plot viewer should be:", () => propertiesShouldBe(page, el("box plot viewer"), [["Value","WEIGHT"],["Category 1","SEX"],["Category 2","RACE"],["Show Minor Categories","true"],["Show All Categories","true"],["Marker Color Column","SEX"],["Invert Color Scheme","true"],["Color Min","20"],["Color Max","80"],["Axis Type","logarithmic"],["Invert Y Axis","true"],["Plot Style","violin"]]), [["Value","WEIGHT"],["Category 1","SEX"],["Category 2","RACE"],["Show Minor Categories","true"],["Show All Categories","true"],["Marker Color Column","SEX"],["Invert Color Scheme","true"],["Color Min","20"],["Color Max","80"],["Axis Type","logarithmic"],["Invert Y Axis","true"],["Plot Style","violin"]]);
    });
    await run.scenario("The ladder survives a project round-trip", async () => {
      await session.step(100, "When user zooms into the value axis of box plot viewer", () => zoomValueAxis(page, el("box plot viewer")));
      await session.step(101, "Then box plot viewer should show a narrower value range than before", () => narrowerRange(page, el("box plot viewer")));
      await session.step(102, "When user remembers the value range of box plot viewer", () => rememberRange(page, el("box plot viewer")));
      await session.step(103, "And user saves the current view as project \"bdd box plot ladder\"", () => saveAsProject(page, "bdd box plot ladder"));
      await session.step(104, "And user closes all views", () => closeAllViews(page));
      await session.step(105, "And user opens the \"bdd box plot ladder\" project", () => openProject(page, "bdd box plot ladder"));
      await session.step(106, "Then box plot viewer should be visible", () => shouldBe(page, el("box plot viewer"), "visible"));
      await session.step(107, "And properties of box plot viewer should be:", () => propertiesShouldBe(page, el("box plot viewer"), [["Value","WEIGHT"],["Category 1","SEX"],["Category 2","RACE"],["Marker Color Column","SEX"],["Invert Color Scheme","true"],["Color Min","20"],["Color Max","80"],["Axis Type","logarithmic"],["Invert Y Axis","true"],["Plot Style","violin"]]), [["Value","WEIGHT"],["Category 1","SEX"],["Category 2","RACE"],["Marker Color Column","SEX"],["Invert Color Scheme","true"],["Color Min","20"],["Color Max","80"],["Axis Type","logarithmic"],["Invert Y Axis","true"],["Plot Style","violin"]]);
      await session.step(118, "And box plot viewer should show the remembered value range", () => rememberedRange(page, el("box plot viewer")));
    });
    run.finish();
  });
});
