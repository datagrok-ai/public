...
test.describe("Row Source MouseOverGroup on seven viewers", () => {
  ...
  test("scatter plot is empty until a group of pie chart is hovered, then shows that group within its filter [viewer=scatter plot, source=pie chart, small group=slice Asian, small rows=5, large group=slice Caucasian, large rows=463]", {tag: ["@viewers", "@realizes:viewers.scatter-plot", "@realizes:viewers.line-chart", "@realizes:viewers.histogram", "@realizes:viewers.bar-chart", "@realizes:viewers.pie-chart", "@realizes:viewers.box-plot", "@realizes:viewers.pc-plot"]}, async ({browser}) => {
    ...
    await session.step(16, "And user adds a scatter plot viewer with:", () => addViewerWith(page, "scatter plot", [["xColumnName","AGE"],["yColumnName","WEIGHT"],["colorColumnName","RACE"],["filter","${AGE} > 44"]]));
    await session.step(21, "And user adds a line chart viewer with:", () => addViewerWith(page, "line chart", [["xColumnName","AGE"],["yColumnNames","WEIGHT"],["filter","${AGE} > 44"]]));
    await session.step(25, "And user adds a histogram viewer with:", () => addViewerWith(page, "histogram", [["valueColumnName","AGE"],["filter","${AGE} > 44"]]));
    await session.step(28, "And user adds a bar chart viewer with:", () => addViewerWith(page, "bar chart", [["valueColumnName","AGE"],["splitColumnName","RACE"],["filter","${AGE} > 44"]]));
    await session.step(32, "And user adds a pie chart viewer with:", () => addViewerWith(page, "pie chart", [["categoryColumnName","RACE"],["filter","${AGE} > 44"]]));
    await session.step(35, "And user adds a box plot viewer with:", () => addViewerWith(page, "box plot", [["categoryColumnNames","RACE"],["valueColumnName","AGE"],["filter","${AGE} > 44"]]));
    await session.step(39, "And user adds a pc plot viewer with:", () => addViewerWith(page, "pc plot", [["columnNames","AGE, WEIGHT"],["filter","${AGE} > 44"]]));
