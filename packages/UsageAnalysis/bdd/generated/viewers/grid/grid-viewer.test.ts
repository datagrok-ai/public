...
test.describe("A second grid as a viewer, and column tooltips", () => {
  ...
  test("A second grid as a viewer, and column tooltips", {tag: ["@journey", "@viewers", "@realizes:viewers.grid"]}, async ({browser}) => {
    ...
    await run.scenario("The column tooltip menu marks the setting that is on, and Columns lists the chosen columns", async () => {
      ...
      await session.step(78, "When user picks \"Tooltip > Current Column > Columns\" from the context menu of the \"header AGE\" area of grid", () => pickFromAreaContextMenu(page, "Tooltip > Current Column > Columns", "header AGE", el("grid")));
      await session.step(79, "Then \"Select columns...\" dialog should be visible", () => shouldBe(page, el("\"Select columns...\" dialog"), "visible"));
      await session.step(80, "When user clicks on All link in \"Select columns...\" dialog", () => clickOn(page, el("All link in \"Select columns...\" dialog")));
      ...
      await session.step(82, "Then \"Select columns...\" dialog should be hidden", () => shouldBe(page, el("\"Select columns...\" dialog"), "hidden"));
      await session.step(83, "When user hovers over the \"cell 3 of AGE\" area of grid", () => hoverArea(page, "cell 3 of AGE", el("grid")));
      await session.step(84, "Then the tooltip should show columns \"USUBJID, AGE, SEX, RACE, DIS_POP, HEIGHT, WEIGHT, DEMOG, CONTROL, STARTED, SEVERITY\"", () => tooltipColumns(page, "USUBJID, AGE, SEX, RACE, DIS_POP, HEIGHT, WEIGHT, DEMOG, CONTROL, STARTED, SEVERITY"));
      await session.step(85, "And the tooltip should show \"AGE\" as \"58\"", () => tooltipValue(page, "AGE", "58"));
      await session.step(86, "When user moves the pointer away from grid", () => pointerAway(page, el("grid")));
      await session.step(87, "And user right-clicks on the \"header AGE\" area of grid", () => rightClickArea(page, "header AGE", el("grid")));
      await session.step(88, "And user hovers over \"Tooltip\" menu item in context menu", () => hoverOver(page, el("\"Tooltip\" menu item in context menu")));
      await session.step(89, "And user hovers over \"Current Column\" menu item in context menu", () => hoverOver(page, el("\"Current Column\" menu item in context menu")));
      await session.step(90, "Then \"Columns\" menu item in context menu should be selected", () => shouldBe(page, el("\"Columns\" menu item in context menu"), "selected"));
      await session.step(91, "When user closes the context menu", () => closeContextMenu(page));
      await session.step(92, "And user picks \"Tooltip > Current Column > None\" from the context menu of the \"header AGE\" area of grid", () => pickFromAreaContextMenu(page, "Tooltip > Current Column > None", "header AGE", el("grid")));
      ...
      await session.step(96, "Then \"None\" menu item in context menu should be selected", () => shouldBe(page, el("\"None\" menu item in context menu"), "selected"));
      await session.step(97, "And \"Default\" menu item in context menu should not be selected", () => shouldNotBe(page, el("\"Default\" menu item in context menu"), "selected"));
      await session.step(98, "When user closes the context menu", () => closeContextMenu(page));
      ...
      await session.step(102, "And user clicks on \"Default\" menu item in context menu", () => clickOn(page, el("\"Default\" menu item in context menu")));
      await session.step(103, "And user right-clicks on the \"header AGE\" area of grid", () => rightClickArea(page, "header AGE", el("grid")));
      await session.step(104, "And user hovers over \"Tooltip\" menu item in context menu", () => hoverOver(page, el("\"Tooltip\" menu item in context menu")));
      await session.step(105, "And user hovers over \"Current Column\" menu item in context menu", () => hoverOver(page, el("\"Current Column\" menu item in context menu")));
      await session.step(106, "Then \"Default\" menu item in context menu should be selected", () => shouldBe(page, el("\"Default\" menu item in context menu"), "selected"));
      await session.step(107, "And \"None\" menu item in context menu should not be selected", () => shouldNotBe(page, el("\"None\" menu item in context menu"), "selected"));
      await session.step(108, "When user closes the context menu", () => closeContextMenu(page));
    });
    await run.scenario("Pick Up and Apply carry the grid's look to a second grid", async () => {
      await session.step(111, "When user adds a grid viewer", () => addViewer(page, "grid"));
      await session.step(112, "Then the open tableview should have 2 grid viewers", () => viewerCount(page, 2, "grid"));
      await session.step(113, "When user picks \"Grid Color Coding > All\" from the context menu of the \"cell 3 of AGE\" area of grid", () => pickFromAreaContextMenu(page, "Grid Color Coding > All", "cell 3 of AGE", el("grid")));
      await session.step(114, "And user moves the pointer away from grid", () => pointerAway(page, el("grid")));
      await session.step(115, "Then \"Color Coding\" property of grid should be \"All\"", () => propertyShouldBe(page, "Color Coding", el("grid"), "All"));
      await session.step(116, "And \"Color Coding\" property of second grid viewer should be \"Auto\"", () => propertyShouldBe(page, "Color Coding", el("second grid viewer"), "Auto"));
      await session.step(117, "When user remembers the \"color of cell 2 of HEIGHT\" reading of grid", () => rememberReading(page, "color of cell 2 of HEIGHT", el("grid")));
      await session.step(118, "Then the \"color of cell 2 of HEIGHT\" reading of grid should not be \"#ffffff\"", () => readingDoesNotRead(page, "color of cell 2 of HEIGHT", el("grid"), "#ffffff"));
      await session.step(119, "And the \"color of cell 2 of HEIGHT\" reading of second grid viewer should not be as remembered", () => readingNotAsRemembered(page, "color of cell 2 of HEIGHT", el("second grid viewer")));
      await session.step(120, "When user picks \"Pick Up / Apply > Pick Up\" from the context menu of the \"cell 3 of AGE\" area of grid", () => pickFromAreaContextMenu(page, "Pick Up / Apply > Pick Up", "cell 3 of AGE", el("grid")));
      await session.step(121, "And user picks \"Pick Up / Apply > Apply\" from the context menu of the \"cell 3 of AGE\" area of second grid viewer", () => pickFromAreaContextMenu(page, "Pick Up / Apply > Apply", "cell 3 of AGE", el("second grid viewer")));
      await session.step(122, "Then \"Color Coding\" property of second grid viewer should be \"All\"", () => propertyShouldBe(page, "Color Coding", el("second grid viewer"), "All"));
      await session.step(123, "And the \"color of cell 2 of HEIGHT\" reading of second grid viewer should be as remembered", () => readingAsRemembered(page, "color of cell 2 of HEIGHT", el("second grid viewer")));
      await session.step(124, "When user picks \"Grid Color Coding > Auto\" from the context menu of the \"cell 3 of AGE\" area of grid", () => pickFromAreaContextMenu(page, "Grid Color Coding > Auto", "cell 3 of AGE", el("grid")));
      await session.step(125, "And user clicks on close icon of second grid viewer", () => clickOn(page, el("close icon of second grid viewer")));
      await session.step(126, "Then the open tableview should have 1 grid viewer", () => viewerCount(page, 1, "grid"));
