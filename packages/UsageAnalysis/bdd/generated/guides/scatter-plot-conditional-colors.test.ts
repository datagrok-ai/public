...
test.describe("Color a scatter plot by conditions, then change one condition's color", () => {
  ...
  test("Switch the color column to conditional coding and recolor one range from the legend", {tag: ["@guide", "@help:visualize/viewers"]}, async ({browser}) => {
    ...
    await session.step(14, "And user adds a scatter plot viewer with:", () => addViewerWith(page, "scatter plot", [["X","WEIGHT"],["Y","HEIGHT"],["Color","AGE"]]), [["X","WEIGHT"],["Y","HEIGHT"],["Color","AGE"]]);
    await session.step(18, "Then scatter plot viewer should have a \"color scale\" area", () => hasArea(page, el("scatter plot viewer"), "color scale"));
    await session.step(19, "When user picks \"Color Coding > Conditional\" from the context menu of the \"header AGE\" area of grid", () => pickFromAreaContextMenu(page, "Color Coding > Conditional", "header AGE", el("grid")));
    await session.step(20, "And user presses Escape", () => pressKey(page, "Escape"));
    await session.step(21, "Then \"AGE\" column should be color-coded conditionally", () => colorCodedAs(page, "AGE", "conditionally"));
    await session.step(22, "And context menu should be hidden", () => shouldBe(page, el("context menu"), "hidden"));
