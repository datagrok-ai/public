...
test.describe("Color a scatter plot by conditions, then change one condition's color", () => {
  ...
  test("Switch the color column to conditional coding and recolor one range from the legend", {tag: ["@guide", "@help:visualize/viewers"]}, async ({browser}) => {
    ...
    await session.step(25, "When user hovers over \"18 - 35.75\" legend item in legend of scatter plot viewer", () => hoverOver(page, el("\"18 - 35.75\" legend item in legend of scatter plot viewer")));
    await session.step(26, "And user clicks on color picker icon", () => clickOn(page, el("color picker icon")));
    await session.step(27, "Then \"18 - 35.75\" dialog should be visible", () => shouldBe(page, el("\"18 - 35.75\" dialog"), "visible"));
    await session.step(28, "When user picks the color \"#9467BD\" in the color picker dialog", () => pickColorSwatch(page, "#9467BD"));
    await session.step(29, "And user clicks on OK button in \"18 - 35.75\" dialog", () => clickOn(page, el("OK button in \"18 - 35.75\" dialog")));
    await session.step(30, "Then the \"18 - 35.75\" item in the legend of scatter plot viewer should be colored \"#9467BD\"", () => legendItemColor(page, "18 - 35.75", el("scatter plot viewer"), "#9467BD"));
    await session.step(31, "And the \"view\" area of scatter plot viewer should contain the color \"#9467BD\"", () => areaColor(page, "view", el("scatter plot viewer"), "#9467BD"));
  });
});
