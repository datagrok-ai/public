...
test.describe("Open a demo dataset", () => {
  ...
  test("Open demog.csv from the Demo files", {tag: ["@guide", "@help:access/files"]}, async ({browser}) => {
    ...
    await session.step(10, "When user expands Files tree node inside browse tree", () => expand(page, el("Files tree node inside browse tree")));
    await session.step(11, "And user expands Files---Demo tree node inside browse tree", () => expand(page, el("Files---Demo tree node inside browse tree")));
    await session.step(12, "And user clicks on Files---Demo---demog.csv tree node inside browse tree", () => clickOn(page, el("Files---Demo---demog.csv tree node inside browse tree")));
    await session.step(13, "Then the \"demog\" view should be current", () => viewIsCurrent(page, "demog"));
    await session.step(14, "And grid should show 5850 rows", () => showsRows(page, el("grid"), 5850));
  });
});
