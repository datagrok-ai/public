...
test.describe("Importing an OpenAPI (Swagger) file as a connection", () => {
  ...
  test("The imported file is a connection with a query per operation", {tag: ["@connections", "@full-stand", "@serial"]}, async ({browser}) => {
    ...
    await session.step(49, "When user opens the context menu of Platform---Functions---OpenAPI---BDD-Conn-Swagger tree node inside browse tree", () => openContextMenu(page, el("Platform---Functions---OpenAPI---BDD-Conn-Swagger tree node inside browse tree")));
    await session.step(50, "Then the open menu should list \"Edit...\"", () => menuLists(page, "Edit..."));
    await session.step(51, "And the open menu should list \"Test connection\"", () => menuLists(page, "Test connection"));
  ...
  test("With its API key the connection's query returns the weather", {tag: ["@connections", "@full-stand", "@serial", "@needs-credentials"]}, async ({browser}) => {
    ...
    await session.step(35, "And Platform---Functions tree node inside browse tree is expanded", () => isExpanded(page, el("Platform---Functions tree node inside browse tree")));
    await session.step(36, "And Platform---Functions---OpenAPI tree node inside browse tree is expanded", () => isExpanded(page, el("Platform---Functions---OpenAPI tree node inside browse tree")));
    await session.step(57, "When user right-clicks on Platform---Functions---OpenAPI---BDD-Conn-Swagger tree node inside browse tree", () => rightClickOn(page, el("Platform---Functions---OpenAPI---BDD-Conn-Swagger tree node inside browse tree")));
    await session.step(58, "And user picks \"Edit...\" from the open menu", () => pickFromOpenMenu(page, "Edit..."));
    await session.step(59, "Then \"Edit Connection\" dialog should be visible", () => shouldBe(page, el("\"Edit Connection\" dialog"), "visible"));
    await session.step(60, "And Url input in \"Edit Connection\" dialog should have value \"https://api.openweathermap.org/data/2.5\"", () => shouldHaveValue(page, el("Url input in \"Edit Connection\" dialog"), "https://api.openweathermap.org/data/2.5"));
    await session.step(61, "When user enters the DG_OPENWEATHERMAP_API_KEY secret into ApiKey input in \"Edit Connection\" dialog", () => enterSecret(page, "DG_OPENWEATHERMAP_API_KEY", el("ApiKey input in \"Edit Connection\" dialog")));
    await session.step(62, "And user clicks on OK button in \"Edit Connection\" dialog", () => clickOn(page, el("OK button in \"Edit Connection\" dialog")));
    await session.step(63, "Then the \"Edit Connection\" dialog should close", () => dialogCloses(page, "Edit Connection"));
    await session.step(64, "Given Platform---Functions---OpenAPI---BDD-Conn-Swagger tree node inside browse tree is expanded", () => isExpanded(page, el("Platform---Functions---OpenAPI---BDD-Conn-Swagger tree node inside browse tree")));
    await session.step(65, "When user right-clicks on Platform---Functions---OpenAPI---BDD-Conn-Swagger---Current-Weather-Data-By-City-Name tree node inside browse tree", () => rightClickOn(page, el("Platform---Functions---OpenAPI---BDD-Conn-Swagger---Current-Weather-Data-By-City-Name tree node inside browse tree")));
    await session.step(66, "And user picks \"Run\" from the open menu", () => pickFromOpenMenu(page, "Run"));
    await session.step(67, "Then \"Current Weather Data By City Name\" dialog should be visible", () => shouldBe(page, el("\"Current Weather Data By City Name\" dialog"), "visible"));
    await session.step(68, "When user enters \"London\" into Q input in \"Current Weather Data By City Name\" dialog", () => enterInto(page, "London", el("Q input in \"Current Weather Data By City Name\" dialog")));
    await session.step(69, "Given user watches the task bar", () => watchTaskBar(page));
    await session.step(70, "When user clicks on OK button in \"Current Weather Data By City Name\" dialog", () => clickOn(page, el("OK button in \"Current Weather Data By City Name\" dialog")));
    await session.step(71, "Then the task bar should have finished \"Running Current Weather Data By City Name\"", () => taskBarFinished(page, "Running Current Weather Data By City Name"));
    await session.step(72, "And the table should have 1 row", () => rowCount(page, 1));
    await session.step(73, "And the table should have a column \"weather/main\"", () => hasColumn(page, "weather/main"));
