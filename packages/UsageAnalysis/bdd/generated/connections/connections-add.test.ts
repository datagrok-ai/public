...
test.describe("Adding a database connection", () => {
  ...
  test("A connection without a password is saved, and its TEST fails for the password", {tag: ["@connections"]}, async ({browser}) => {
    ...
    await session.step(86, "Given Databases---Postgres tree node inside browse tree is expanded", () => isExpanded(page, el("Databases---Postgres tree node inside browse tree")));
    await session.step(87, "Then Databases---Postgres---BDD-Conn-Add-{run} tree node inside browse tree should be visible", () => shouldBe(page, el(session.text("Databases---Postgres---BDD-Conn-Add-{run} tree node inside browse tree")), "visible"));
  });
  test("A connection with the right credentials passes its TEST and is saved", {tag: ["@connections", "@needs-credentials"]}, async ({browser}) => {
    ...
    await session.step(25, "And the browse panel is open", () => browsePanelOpen(page));
    await session.step(26, "And Databases tree node inside browse tree is expanded", () => isExpanded(page, el("Databases tree node inside browse tree")));
    await session.step(91, "Given no connection named \"BDD-Conn-Add-Ok-{run}\" is on the server", () => noConnectionOnServer(page, session.text("BDD-Conn-Add-Ok-{run}")));
    await session.step(92, "When user right-clicks on Databases---Postgres tree node inside browse tree", () => rightClickOn(page, el("Databases---Postgres tree node inside browse tree")));
    await session.step(93, "And user picks \"New connection...\" from the open menu", () => pickFromOpenMenu(page, "New connection..."));
    await session.step(94, "Then \"Add new connection\" dialog should be visible", () => shouldBe(page, el("\"Add new connection\" dialog"), "visible"));
    await session.step(95, "When user enters \"BDD-Conn-Add-Ok-{run}\" into Name input in \"Add new connection\" dialog", () => enterInto(page, session.text("BDD-Conn-Add-Ok-{run}"), el("Name input in \"Add new connection\" dialog")));
    await session.step(96, "And user enters \"db.datagrok.ai\" into Server input in \"Add new connection\" dialog", () => enterInto(page, "db.datagrok.ai", el("Server input in \"Add new connection\" dialog")));
    await session.step(97, "And user enters \"54322\" into Port input in \"Add new connection\" dialog", () => enterInto(page, "54322", el("Port input in \"Add new connection\" dialog")));
    await session.step(98, "And user enters \"northwind\" into Db input in \"Add new connection\" dialog", () => enterInto(page, "northwind", el("Db input in \"Add new connection\" dialog")));
    await session.step(99, "And user enters the DG_PG_LOGIN secret into Login input in \"Add new connection\" dialog", () => enterSecret(page, "DG_PG_LOGIN", el("Login input in \"Add new connection\" dialog")));
    await session.step(100, "And user enters the DG_PG_PASSWORD secret into Password input in \"Add new connection\" dialog", () => enterSecret(page, "DG_PG_PASSWORD", el("Password input in \"Add new connection\" dialog")));
    await session.step(101, "Given user watches the task bar", () => watchTaskBar(page));
    await session.step(102, "When user clicks on TEST button in \"Add new connection\" dialog", () => clickOn(page, el("TEST button in \"Add new connection\" dialog")));
    await session.step(103, "Then the task bar should have shown \"Testing\"", () => taskBarShown(page, "Testing"));
    await session.step(104, "And the connection test should have ended on an info balloon containing \"connected successfully\"", () => connectionTestEnded(page, "info", "connected successfully"));
    await session.step(105, "When user clicks on OK button in \"Add new connection\" dialog", () => clickOn(page, el("OK button in \"Add new connection\" dialog")));
    await session.step(106, "Then the \"Add new connection\" dialog should close", () => dialogCloses(page, "Add new connection"));
    await session.step(107, "And 1 connection named \"BDD-Conn-Add-Ok-{run}\" should be on the server", () => connectionsOnServer(page, 1, session.text("BDD-Conn-Add-Ok-{run}")));
    await session.step(108, "Given Databases---Postgres tree node inside browse tree is expanded", () => isExpanded(page, el("Databases---Postgres tree node inside browse tree")));
    await session.step(109, "Then Databases---Postgres---BDD-Conn-Add-Ok-{run} tree node inside browse tree should be visible", () => shouldBe(page, el(session.text("Databases---Postgres---BDD-Conn-Add-Ok-{run} tree node inside browse tree")), "visible"));
