...
test.describe("Identifiers configured on a connection", () => {
  ...
  test("Identifiers configured on a connection", {tag: ["@connections", "@needs-credentials", "@journey"]}, async ({browser}) => {
    ...
    await session.step(33, "And user enters \"datagrok\" into Login input in \"Edit Connection\" dialog", () => enterInto(page, "datagrok", el("Login input in \"Edit Connection\" dialog")));
    ...
    await run.scenario("The table's column comes with the identifier's semantic type", async () => {
      await session.step(62, "When user reloads the page", () => reloadPage(page));
      await session.step(63, "Given the browse panel is open", () => browsePanelOpen(page));
      await session.step(64, "And Databases---Postgres---BDD-Conn-Ident-{time} tree node inside browse tree is expanded", () => isExpanded(page, el(session.text("Databases---Postgres---BDD-Conn-Ident-{time} tree node inside browse tree"))));
      await session.step(65, "And Databases---Postgres---BDD-Conn-Ident-{time}---Schemas tree node inside browse tree is expanded", () => isExpanded(page, el(session.text("Databases---Postgres---BDD-Conn-Ident-{time}---Schemas tree node inside browse tree"))));
      await session.step(66, "And Databases---Postgres---BDD-Conn-Ident-{time}---Schemas---public tree node inside browse tree is expanded", () => isExpanded(page, el(session.text("Databases---Postgres---BDD-Conn-Ident-{time}---Schemas---public tree node inside browse tree"))));
      await session.step(67, "When user right-clicks on Databases---Postgres---BDD-Conn-Ident-{time}---Schemas---public---customers tree node inside browse tree", () => rightClickOn(page, el(session.text("Databases---Postgres---BDD-Conn-Ident-{time}---Schemas---public---customers tree node inside browse tree"))));
      await session.step(68, "And user picks \"Get All\" from the open menu", () => pickFromOpenMenu(page, "Get All"));
      await session.step(69, "Then the \"customers\" view should be current", () => viewIsCurrent(page, "customers"));
      await session.step(70, "And the table should have 91 rows", () => rowCount(page, 91));
      await session.step(71, "And \"customerid\" column should have semantic type \"CUSTOMER_ID\"", () => columnSemType(page, "customerid", "CUSTOMER_ID"));
