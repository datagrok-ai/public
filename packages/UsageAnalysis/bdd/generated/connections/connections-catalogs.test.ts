...
test.describe("The catalogs of an MS SQL connection", () => {
  ...
  test("The catalogs of an MS SQL connection", {tag: ["@connections", "@full-stand", "@serial", "@journey", "@known-failure"]}, async ({browser}) => {
    ...
    await run.scenario("Clearing the comment in the pane clears it on the server", async () => {
      await session.step(68, "Then the \"tempdb\" catalog of the \"MSSQLTest\" connection should have the comment \"BDD comment {run}\"", () => catalogCommentIs(page, "tempdb", "MSSQLTest", session.text("BDD comment {run}")));
      await session.step(69, "When user clears Comment input in context panel", () => clearField(page, el("Comment input in context panel")));
      await session.step(70, "And user clicks on SAVE button in context panel", () => clickOn(page, el("SAVE button in context panel")));
      await session.step(71, "Then the \"tempdb\" catalog of the \"MSSQLTest\" connection should have the comment \"\"", () => catalogCommentIs(page, "tempdb", "MSSQLTest", ""));
