...
Feature: A group's members
  ...
  Background:
    ...
    And a new user "opavlenko{time}g" with email "opavlenko+{time}g@datagrok.ai" is on the server
  ...
  Scenario: MANAGE adds a user to the group (Groups-11)
    ...
    When user types "opavlenko{time}g" into membership search
    And user clicks on add button of "opavlenko{time}g" membership candidate
    Then "opavlenko{time}g" membership row should be visible
    ...
    And "opavlenko{time}g" should be a plain member of "BDD-GM-Group-{time}" on the server
    And "Members" section in context panel should contain text "opavlenko{time}g"
    And no errors should have been logged
    And no error or warning balloon should have been shown

  Scenario: The Admin box makes the member an admin (Groups-13)
    When user clicks on MANAGE button in "Members" section in context panel
    Then checkbox label of "opavlenko{time}g" membership row should have text "Admin"
    And checkbox of "opavlenko{time}g" membership row should be unchecked
    When user checks checkbox of "opavlenko{time}g" membership row
    And user clicks on SAVE button in "BDD-GM-Group-{time} members" dialog
    Then the "BDD-GM-Group-{time} members" dialog should close
    And "opavlenko{time}g" should be an admin member of "BDD-GM-Group-{time}" on the server
    When user clicks on MANAGE button in "Members" section in context panel
    Then checkbox of "opavlenko{time}g" membership row should be checked
  ...
  Scenario: Removing a member leaves the others (Groups-12)
    When user clicks on MANAGE button in "Members" section in context panel
    Then "opavlenko{time}g" membership row should be visible
    When user clicks on remove button of "opavlenko{time}g" membership row
    Then "opavlenko{time}g" membership row should be absent
    When user clicks on SAVE button in "BDD-GM-Group-{time} members" dialog
    Then the "BDD-GM-Group-{time} members" dialog should close
