...
Feature: Importing an OpenAPI (Swagger) file as a connection
  ...
  the connection (and the queries that go with it) before and after — checked gone.

  Not translated, and why: running an imported query — it calls api.openweathermap.org, an outside
  web service (see the bdd library's CLAUDE.md, "What never becomes a feature"). Dropping the file
  from the desktop onto the page — a synthetic drop never reaches the platform's drop zone (only an
  OS drag does), so the file is opened through the Browse panel's "Open local file", which runs the
  same import. The old spec's fallback "a grid or any balloon" and "the first child node,
  ...
  Background:
    ...
    When user uploads "fixtures/bdd-swagger.yaml" through "Open local file" icon inside browse toolbar
