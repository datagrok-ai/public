...
Feature: Working with what a space holds
  ...
  Rule backgrounds require the fixture's view before each file operation. If setup fails while
  Demo is still open, later scenarios must fail before touching the original demo files.
  ...
  Rule: File operations apply to the test space
    Background:
      Then the "BDD-Ops" view should be current
  ...
  Rule: The copied file opens from the destination space
    Background:
      Then the "BDD-Ops-Copy" view should be current
