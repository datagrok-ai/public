...
group and role features are `@serial`: their gallery searches are fuzzy and bring up each other's
fixtures, so they take turns while the rest runs in parallel. Every TestTrack case there is
