...
* Tests: Translated the Queries, Scripts and Connections material into BDD — 31 features / 99 scenarios in `bdd/features/{queries,scripts,connections}/`, from the TestTrack cases, the `playwright-public` specs and the dev-only harness suites, including the cases neither could automate: a query from creation to deletion on Postgres and MS SQL, parameterized and visual queries, transformations, the Post-Process and Layout tabs, every script language template, the script browser, editor and signature, connection browsing, editing, credentials, schemas, catalogs, identifiers, providers, SPARQL, an OpenAPI import and writes through an external provider. New package bindings for the three areas and a shared layout cleanup; the library gained the queries/scripts/connections fixture sources, a row-count claim that waits for the view instead of throwing, and the chat and server-request helpers the suites had been re-implementing. One scenario carries `@known-failure`: a catalog's Comment is not cleared when it is emptied in the same context-panel session that saved it (GROK-20981)
