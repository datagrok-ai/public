...
* BDD: Added the Browse suite — 9 features, 44 scenarios over the Browse panel, its toolbar, the tree and its keyboard, browsing versus persistent views, Files, My stuff, Platform, Databases, Apps, Dashboards, the context panel, the context menus and the per-section error matrix. Translated from the manual cases Browse-Nav/Tree/View/Files/MyStuff/Platform/DB/Apps/Dash/CtxPanel/Fav/ModelHub/Node-*. A tree node below the top level is addressed by its full path (`Files---Demo`) because Demo, My files and App Data each name more than one node. Reviewed backwards against the specs they replace, which found and fixed: a missing error-balloon floor in every scenario, an expand outline that a tree remembering its state turned into a no-op, a "keeps its state across views" case that never left the panel, collapse claims with no positive before them, and a gallery claim satisfied by a container built with the view. Library: a Dart tree node's selection (published only as a CSS class) is now read; the gallery element names its cards; and the saved-project step deletes the ViewInfo it saved, which it had been leaking on every run
