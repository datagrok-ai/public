...
* GROK-20977: Tests: Added `bdd/features/viewers/filter-panel/expression-filter-all-columns.feature` — the Expression card with Column set to All Columns over a table written in the feature, one column per type (text, a 32-bit double of short decimals, a 64-bit double, an integer, a date): text with "equals" and "=", in any case and with spaces around it, a number with "=", a comparison across the numeric columns, a date with "equals", a value no column holds. Every scenario claims the committed rule as well as the rows; "equals" with a number and "= 0.47" over a 32-bit column, fixed under GROK-20977, keep the rows that hold the value
* Tests: Translated the Queries, Scripts and Connections material into BDD — 23 features / 77 scenarios in `bdd/features/{queries,scripts,connections}/`, from the TestTrack cases and the `playwright-public` specs: a query from creation to deletion, parameterized and visual queries, transformations (read back from the server), the Post-Process and Layout tabs, a query result saved as a project, every column of a schema in the context panel, the Scripts view, editor, Signature Editor and a run from every table source and the console, and the connection dialogs, browser, schema view, SPARQL provider and OpenAPI import. Script runs are JavaScript and Grok; R/Python/Octave/Julia/NodeJS/Pyodide runs, connection TEST answers and cases that need database credentials are left out by the bdd scope rule. New package bindings for the three areas and a shared layout cleanup; the library gained the queries/scripts/connections fixture sources and server claims, the code editor, console and browser-alert steps, a row-count claim that waits for the view, the context menu aimed at the visible part of a clipped row, and the chat and server-request helpers the suites had been re-implementing
* Tests: Translated the Annotation Regions and Formula Lines TestTrack sections into `bdd/features/viewers/annotation-regions/` and `bdd/features/viewers/formula-lines/` — ten new features, 66 scenarios: hovering and clicking regions (an overlap selects the rows both share, Control and Shift compose the selection), where a region title is drawn and what must not move it (GROK-20157, GROK-20158), regions on the line chart and the density plot, drawing on every viewer and editing from the region's own menu, a layout round trip, the Tools menu, the Annotations group of a numeric axis, the Formula Lines dialog and the look it writes, and the fixed defects around formula lines (GROK-19334, GROK-16214, GROK-20458). New library phrases for it: an edge or corner of an area (`left edge of x axis`, `top left corner of region Older`), the overlap of two areas, an area's place remembered and compared after a change, an area lying inside, above or beside another, a table tag set through the API, and a between-selection claim. Three scenarios carry `@known-failure`: a band title the strip cannot take is not drawn in the data (GROK-20388), Auto Layout off loses it the same way, and a resized line chart drops its band title
* Tests: Closed the gaps between the grid features and the TestTrack grid scenarios: four new features in `bdd/features/viewers/grid/` — `grid-persistence` (appearance and geometry back from a layout over a fresh view and from a project), `grid-pinning` (every item of the Pin menu, moved here from `grid-columns`), `grid-column-groups` (Group columns..., the band and its colour, the colours back from a layout and the groups back from a project, GROK-17441/17442/17505/18213) and `grid-forms-column` (Design a Form..., the two HTML forms) — and the existing ones extended with the two-level Sort dialog, the Order or Hide Columns type filter (GROK-19333/20167), the status-bar column manager across tables (GROK-19332), Tab wrapping at the row edges, linked column resizing, sideways scrolling (GROK-19753), the tooltip Columns and None settings, Pick Up / Apply, the Style background against colour coding (GROK-18638), Selected Rows Color, mouse selection, auto-append, Editable by and the clipboard's column order. New library phrases for it: the `editor` part of a property row (the colour picker step then finds the swatch in the popup it opens as well as in a dialog), the clipboard's lines, and the own and the second account's login typed into an input, the status-bar column manager as `column manager`, and the grid's overlay canvas as `grid overlay` - the element a key has to reach for Escape to clear the current row. The second table of `grid-viewer` is a 100-row subset of demog-1000 instead of spgi-100
...
* Tests: Translated the Users, Groups and Roles TestTrack specs and `playwright-public/user groups` into `bdd/features/users-groups-roles/` — 9 features / 65 scenarios. The features share two fixture users made once per stand (a user cannot be deleted), only the creation feature adds users, and every membership is claimed on the server as well as in the pane that shows it
* Tests: Translated the Legend TestTrack section's manual cases into features: `legend/legend-across-viewers`, `legend-filtering`, `legend-placement` and `legend-structures`, plus new scenarios in the scatter plot and line chart legend features — seven viewers with one legend column, colors from the grid and from the legend's picker, filters of every kind, the splitter, Position Auto, Visibility Auto, corners and the mini icon, molecules in the legend, each through a layout and a project; the known-failure mark on the layout that holds a histogram viewer is off (the filter panel's save before its first build was fixed in the core on 2026-09-15), and in review the corner claim reads the mode the legend publishes instead of its inline styles, and the splitter drag shares the area drag's pointer sequence
* Tests: Closed the remaining Viewers BDD gaps against their TestTrack mds — seven new features (line chart layout and styling, Row Source MouseOverGroup and rebound, the scatter plot's conditional legend, trellis plot persistence, docking by the title bar) and twenty-five extended; in review the viewer menu is walked group by group (a series' "Chart type" no longer lands on the chart-wide "Chart Type"), ordinals count visible viewers only (the Home page keeps hidden ones), and the docking drop waits for the wheel item's hover mark, which the core now keeps above the tab strip's drop zones; the GROK-20900 known-failure mark is off (fixed in the core on 2026-09-15).
* Tests: The filter panel's suspended card advertised its own on/off checkbox as disabled (`aria-disabled` on the whole card host, so Playwright and a screen reader could never switch it back on): the attribute is on the card body now. The AND/OR word of a text, expression, multi-value or combined boolean card is `filter-mode-toggle`, and a hierarchical node's box carries `aria-checked` (true, false, mixed) — three package steps that read the DOM around those gaps are gone
* Tests: The filter panel's summary scenario now states the product's real behaviour instead of a defect: the full list of active filters, the viewers' share included, is the tooltip of the "?" icon in the Filters title bar, where every source names itself as "column: criterion". Its known-failure tag is gone, the scatter plot's zoom, the bar click, the pie slice, the PC plot slider and the trellis cell are each claimed by the line they write, and `help icon of <viewer>` is a phrase of the library now
* Tests: Closed the filter panel's BDD gaps against its TestTrack md — five new features (Expression and Text cards, the cloned view, linked tables, saved states, the indicator with viewers filtering) and new scenarios in the ladder, entry points, categorical, hierarchical, combined boolean and compose-with-viewers features: header drags (a pinned one too), Column Properties' Add filter, card reordering, hidden columns, Select Columns, Multi Value and Filter to Column, Radio/Multi-Select, a pasted list, Ctrl+Z, the tri-state tree, Escape on the panel, and one click-filter outline over the bar chart, pie chart, trellis and PC plot
