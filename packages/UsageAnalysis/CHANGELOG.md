...
* GROK-20760: Tests dashboards: Scoped the build window to a branch, defaulting to `master` — every pipeline can run on any branch, so an ad-hoc run sat in the trunk column and read as master. `TestsDashboard` takes a `branchFilter` input; the `/release` dashboard gets a Branch picker in the ribbon beside Environment, backed by the new `TestBranches` query (branches that reported in the last 60 days, newest first). Pick "(all branches)" for the previous behaviour; both queries also return the branch, and `TestsDashboard` carries it as a per-build column tag alongside the instance
