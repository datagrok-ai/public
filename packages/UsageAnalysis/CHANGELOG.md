...
* Tests: Translated the Users, Groups and Roles TestTrack specs and `playwright-public/user groups` into `bdd/features/users-groups-roles/` — 9 features / 65 scenarios. Each run makes its own user (`opavlenko<time>`) instead of reusing a shared one, so memberships, disabling and favorites need no revert, and every membership is claimed on the server as well as in the pane that shows it
