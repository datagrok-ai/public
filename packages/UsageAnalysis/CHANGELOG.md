...
* GROK-20877: Metrics: Added an Errors panel — the window's error events grouped by message from `grok.dapi.admin.getMetrics().errors`, ordered by the users they hit, then the hours they kept recurring, then count, with users and hours colour-banded, a last-seen column and an all / server / client toggle (the endpoint returns the top groups of every source, so package errors reported by the browser always have a place next to the server's); the Errors card reads the same endpoint (count, distinct users, delta vs the previous window, per-source breakdown in the tooltip) instead of the `MetricsErrorsCount` query, which is removed; the count includes server-side errors with no session, which that query excluded, so it is higher than before. Both are in the e-mail snapshot (`errors.csv`, `errors_summary.csv`)
