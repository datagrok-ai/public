...
test('Chem: Chemical Space', async ({page}) => {
  ...
  await softStep('Run Chemical Space with defaults', async () => {
    ...
    await page.evaluate(async () => {
      ...
      const cs = Array.from(menuItems).find(m => m.textContent!.trim() === 'Chemical Space...');
      cs!.closest('.d4-menu-item')!.dispatchEvent(new MouseEvent('click', {bubbles: true}));
    });
  ...
  await softStep('Run Chemical Space with t-SNE', async () => {
    const colsBefore = await page.evaluate(() => grok.shell.tv.dataFrame.columns.length);
    await page.evaluate(async () => {
      const chemMenu = document.querySelector('[name="div-Chem"]');
      chemMenu!.dispatchEvent(new MouseEvent('click', {bubbles: true}));
      ...
      for (const s of selects) {
        if (Array.from(s.options).some(o => o.text === 't-SNE')) {
          s.value = 't-SNE';
