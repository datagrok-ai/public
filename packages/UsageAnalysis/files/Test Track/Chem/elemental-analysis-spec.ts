...
test('Chem: Elemental Analysis', async ({page}) => {
  ...
  await softStep('Open Elemental Analysis dialog', async () => {
    await page.evaluate(async () => {
      ...
      const ea = Array.from(menuItems).find(m => m.textContent!.trim() === 'Elemental Analysis...');
      ea!.closest('.d4-menu-item')!.dispatchEvent(new MouseEvent('click', {bubbles: true}));
    });
    await page.locator('.d4-dialog').waitFor({timeout: 5000});
  });

  await softStep('Turn all checkboxes on and compute', async () => {
