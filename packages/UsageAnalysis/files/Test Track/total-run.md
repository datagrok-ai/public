...
**Total**: 187 tests · Run: 138/187 (74%) · Playwright: 105/187 (56%) · Mean Browser: 1m 45s · Mean Spec Gen: 9.7s · Mean Spec Run: 32s · Mean Total: 2m 21s
...
| Charts | 3 | 3/3 (100%) | 3/3 (100%) | PARTIAL | 1m 48s | 19s | 23s | 2m 30s |
...
| Viewers | 49 | 42/49 (86%) | 42/49 (86%) | PARTIAL | 3m 0s | 17s | 41s | 3m 57s |

## All Tests

**Total**: 187 tests · 84 PASS / 35 PARTIAL / 11 FAIL / 1 AMBIGUOUS / 7 SKIP / 49 NO RUN · Mean Browser: 1m 45s · Mean Spec Gen: 9.7s · Mean Spec Run: 32s · Mean Total: 2m 21s
...
| Charts | [radar](Charts/radar-run.md) | PASS → PASS | The Radar viewer (Charts package) reproduced cleanly on dev for both earthquakes.csv and demog.csv. Switching tables,... | 4m 14s | 50s | 10.8s | 5m 15s | +3m 42s | +47s | -3.2s | +4m 25s |
| Charts | [sunburst](Charts/sunburst-run.md) | PARTIAL → PARTIAL | 9 of 9 steps attempted: 8 passed, 1 ambiguous (canvas multi-selection), 1 skipped (old layout). Playwright spec passe... | 50s | 3.0s | 42s | 1m 34s | +0s | +0.0s | +0s | +0s |
| Charts | [tree](Charts/tree-run.md) | PASS → PASS | All 4 steps of the Tree viewer collaborative filtering scenario passed. The Tree viewer correctly maintains selection... | 20s | 3.0s | 17s | 40s | +0s | +0.0s | +0s | +0s |
| Chem | [activity-cliffs](Chem/activity-cliffs-run.md) | PASS → PASS | Activity Cliffs computation works correctly on SPGI.csv. UMAP embedding produces a scatter plot with molecule thumbna... | 50s | 3.0s | 57s | 1m 50s | +0s | +0.0s | +0s | +0s |
| Chem | [Advanced/scaffold-tree](Chem/Advanced/scaffold-tree-run.md) | FAIL → FAIL | Steps 1-2 passed: SPGI.csv loaded and the Scaffold Tree viewer was added with the "empty" state. However, adding scaf... | 30s |  |  | 30s | +0s | — | — | +0s |
| Chem | [Advanced/scaffold-tree-functions](Chem/Advanced/scaffold-tree-functions-run.md) | FAIL → FAIL | Steps 1-2 and the toolbar check passed. The scaffold tree viewer opens correctly with the "empty" state message. Howe... | 2m 0s | 3.0s |  | 2m 3s | +0s | +0.0s | — | +0s |
| Chem | [Advanced/similarity-search](Chem/Advanced/similarity-search-run.md) | PASS → PASS | All 4 steps passed. The Similarity Search viewer opened correctly showing similar molecules with Tanimoto/Morgan simi... | 35s | 3.0s |  | 38s | +0s | +0.0s | — | +0s |
| Chem | [Advanced/structure-filter](Chem/Advanced/structure-filter-run.md) | PASS → PASS | All 4 core steps passed. The Structure filter panel opened with 43 filters for SPGI.csv. Benzene substructure search... | 40s | 3.0s | 24s | 1m 7s | +0s | +0.0s | +0s | +0s |
| Chem | [calculate](Chem/calculate-run.md) | PASS → PASS | Descriptors calculation works correctly on dev server. Dialog shows all descriptor categories, pre-selected descripto... | 25s | 3.0s | 24s | 52s | +0s | +0.0s | +0s | +0s |
| Chem | [chemical-space](Chem/chemical-space-run.md) | PASS → PASS | Chemical Space works correctly with both default (UMAP) and modified (t-SNE) parameters. Scatter plots are generated... | 45s | 3.0s | 44s | 1m 32s | +0s | +0.0s | +0s | +0s |
| Chem | [chemprop](Chem/chemprop-run.md) | PARTIAL → PARTIAL | Steps 1-3 passed: mol1K.sdf loaded correctly, Predictive Model view opened, and columns were configured. However, mod... | 2m 0s | 3.0s |  | 2m 3s | +0s | +0.0s | — | +0s |
| Chem | [elemental-analysis](Chem/elemental-analysis-run.md) | PASS → PASS | All 4 steps passed. Elemental Analysis computed element counts (H, C, N, O, F, S, Cl, Br, I) and Molecule Charge for... | 30s | 3.0s | 21s | 54s | +0s | +0.0s | +0s | +0s |
| Chem | [filter-panel](Chem/filter-panel-run.md) | PARTIAL → PARTIAL | Core substructure filtering works: drawing benzene in the structure filter correctly filters to 1356/3624 rows with h... | 20s | 3.0s | 29s | 52s | +0s | +0.0s | +0s | +0s |
| Chem | [info-panels](Chem/info-panels-run.md) | PASS → PASS | All 5 steps passed. The canonical_smiles column Context Panel shows 11 info panels including Chemistry, all expanding... | 40s | 3.0s | 39s | 1m 22s | +0s | +0.0s | +0s | +0s |
| Chem | [mmp](Chem/mmp-run.md) | FAIL → FAIL | Steps 1-2 passed (dataset opened, MMP dialog opened). Step 3 failed: the MMP analysis function crashed with a `TypeEr... | 1m 50s | 3.0s |  | 1m 53s | +0s | +0.0s | — | +0s |
| Chem | [r-group-analysis](Chem/r-group-analysis-run.md) | PASS → PASS | All tested steps passed. The R-Groups Analysis dialog opened correctly, MCS computation completed, and the trellis pl... | 45s | 3.0s | 38s | 1m 26s | +0s | +0.0s | +0s | +0s |
| Chem | [sketcher](Chem/sketcher-run.md) | PASS → PASS | Core steps 1-6 passed: smiles.csv opened, sketcher dialog displayed with OpenChemLib, C1CCCCC1 entered and rendered,... | 25s | 3.0s | 13s | 41s | +0s | +0.0s | +0s | +0s |
...
| DiffStudio | [catalog](DiffStudio/catalog-run.md) | PASS → PASS | All 6 steps passed in both MCP and Playwright runs. The full DiffStudio catalog workflow works end-to-end on dev.data... | 45s | 5.0s | 32s | 1m 22s | +0s | +0.0s | +0s | +0s |
| DiffStudio | [cyclic-models](DiffStudio/cyclic-models-run.md) | PASS → PASS | All 4 steps passed in both MCP and Playwright runs. The PK-PD cyclic model loads correctly, Multiaxis/Facet plot tabs... | 30s | 3.0s | 23s | 56s | +0s | +0.0s | +0s | +0s |
| DiffStudio | [files-and-sharing](DiffStudio/files-and-sharing-run.md) | PASS → PASS | All 3 functional steps passed. The pk.ivp file opened from the Files browser as a DiffStudio view. Modifying Step (0.... | 43s | 3.0s | 24s | 1m 10s | +0s | +0.0s | +0s | +0s |
| DiffStudio | [fitting](DiffStudio/fitting-run.md) | PASS → PASS | All 6 steps passed. The Bioreactor model loaded from Library, Fit view opened correctly, Process mode dropdown update... | 1m 30s | 5.0s | 28s | 2m 3s | +0s | +0.0s | +0s | +0s |
| DiffStudio | [open-model](DiffStudio/open-model-run.md) | PASS → PASS | All 6 steps passed. DiffStudio opened correctly, Bioreactor loaded from Library with 13 columns and 1001 rows. Both M... | 25s | 3.0s | 18s | 46s | +0s | +0.0s | +0s | +0s |
| DiffStudio | [scripting](DiffStudio/scripting-run.md) | PASS → PASS | All 5 steps passed. The DiffStudio Edit toggle and `</>` export icon work correctly, producing a JS script with all m... | 50s | 3.0s | 49s | 1m 42s | +0s | +0.0s | +0s | +0s |
| DiffStudio | [sensitivity-analysis](DiffStudio/sensitivity-analysis-run.md) | PASS → PASS | All 4 steps passed. The Sensitivity Analysis view opened correctly from the ribbon. Process mode switching updated FF... | 30s | 3.0s | 23s | 56s | +0s | +0.0s | +0s | +0s |
| DiffStudio | [stages](DiffStudio/stages-run.md) | PASS → PASS | All 4 steps passed. The Acid Production (GA-production) model loaded from Library with 2-stage simulation (1st stage... | 25s | 3.0s | 17s | 45s | +0s | +0.0s | +0s | +0s |
| EDA | [anova](EDA/anova-run.md) | PASS → PASS | All 3 steps passed successfully. The ANOVA dialog opened correctly with default parameters (Category=RACE, Feature=AG... | 12s | 2.0s | 3.0s | 17s | +0s | +0.0s | +0s | +0s |
...
| EDA | [multivariate-analysis](EDA/multivariate-analysis-run.md) | PASS → PASS | All 3 steps passed. The Multivariate Analysis (PLS) dialog opened with correct defaults. After clicking RUN, all 5 ex... | 10s | 2.0s | 7.0s | 19s | +0s | +0.0s | +0s | +0s |
| EDA | [pareto-front-viewer](EDA/pareto-front-viewer-run.md) | PARTIAL → PARTIAL | 6 of 7 steps passed (1 partial due to missing test file). The Pareto Front viewer works correctly: non-numeric column... | 20s | 2.0s | 9.0s | 31s | +0s | +0.0s | +0s | +0s |
| EDA | [pca](EDA/pca-run.md) | PASS → PASS | All 5 steps passed. PCA runs correctly twice — first without preprocessing (raw scores), then with Center+Scale (norm... | 13s | 2.0s | 3.0s | 18s | +0s | +0.0s | +0s | +0s |
| EDA | [pls](EDA/pls-run.md) | PASS → PASS | All 4 steps passed. PLS dialog opened with correct defaults (Predict=price, Using=15 numeric columns, Components=3).... | 10s | 2.0s | 4.0s | 16s | +0s | +0.0s | +0s | +0s |
...
| Viewers | [3d-scatter-plot](Viewers/3d-scatter-plot-run.md) | PASS → PASS | All four scenario steps passed against dev. 3D Scatter plot opens by clicking the Viewers toolbox icon, click-on-canv... | 1m 55s | 30s | 6.0s | 2m 31s | +0s | +0s | +0s | +0s |
...
| Viewers | [form](Viewers/form-run.md) | PASS → PASS | All three scenarios passed in both the MCP run and the generated Playwright spec. The spec needed one revision after... | 1m 30s | 5.0s | 35s | 2m 10s | +0s | +0.0s | +0s | +0s |
| Viewers | [grid](Viewers/grid-run.md) | PARTIAL → PARTIAL | The Grid scenario was exercised primarily via the JS API due to the canvas-based nature of grid interactions (headers... | 6m 0s | 1m 0s | 28s | 7m 28s | +0s | +0s | +0s | +0s |
| Viewers | [grid-viewer](Viewers/grid-viewer-run.md) | PASS → PASS | All 16 steps completed. UI path used where DOM selectors were reliable (Toolbox icon, SAVE dialog, viewer icons). JS... | 4m 0s | 1m 0s | 58s | 5m 58s | +0s | +0s | +0s | +0s |
| Viewers | [heatmap](Viewers/heatmap-run.md) | PARTIAL → PARTIAL | Steps 1, 2, 5 passed cleanly. Step 3 (table switching) requires JS-API fallback because the property panel's `<select... | 1m 50s | 5.0s | 18s | 2m 13s | +0s | +0.0s | +0s | +0s |
...
| Viewers | [Legend/filtering](Viewers/Legend/filtering-run.md) | PASS → PASS | All legend filtering behaviors work correctly. Legends dynamically update when dataframe filters, in-viewer filters,... | 55s | 5.0s | 36s | 1m 36s | +0s | +0.0s | +0s | +0s |
...
| Viewers | [Legend/visibility-and-positioning](Viewers/Legend/visibility-and-positioning-run.md) | PASS → PASS | All legend visibility and positioning features work correctly. Legends display on viewers with color/split columns, c... | 1m 25s | 5.0s | 44s | 2m 14s | +0s | +0.0s | +0s | +0s |
...
| Viewers | [matrix-plot](Viewers/matrix-plot-run.md) | PASS → PASS | Steps 1-2, 5, 7 passed: Matrix plot viewer added to SPGI, properties accessible (xColumnNames, yColumnNames, innerVie... | 20s | 3.0s | 11s | 34s | +0s | +0.0s | +0s | +0s |
...
| Viewers | [pivot-table](Viewers/pivot-table-run.md) | PASS → PASS | All 10 scenario steps passed. Pivot table viewer behaves correctly on `demog` and `SPGI` — defaults match the documen... | 45s | 5.0s | 22s | 1m 12s | +0s | +0.0s | +0s | +0s |
...
| Viewers | [tile](Viewers/tile-run.md) | PASS → PASS | All 6 scenario sub-steps passed on dev.datagrok.ai. Tile viewer correctly rebinds across dataframes, persists layouts... | 5m 0s | 30s | 1m 6s | 6m 36s | +0s | +0s | +0s | +0s |
| Viewers | [tree-map-viewer](Viewers/tree-map-viewer-run.md) | PARTIAL → PARTIAL | All 9 scenario steps were reproduced. 7 pure PASS, 1 AMBIGUOUS (step 7 — heavy category skew masked shift/ctrl semant... | 45s | 2.0s | 7.4s | 54s | +0s | +0.0s | +0s | +0s |
...
| Viewers | [word-cloud](Viewers/word-cloud-run.md) | new: PASS | All tested steps passed. Word cloud viewer added to SPGI with column set to "Primary Series Name". Properties include... | 20s | 3.0s | 13s | 36s | 20s (new) | 3.0s (new) | 13s (new) | 36s (new) |
| Viewers | word-cloud-tests | NO RUN → NO RUN |  |  |  |  |  | — | — | — | — |
| Viewers | ~~world-cloud~~ | removed: PASS |  |  |  |  |  | 20s (removed) | 3.0s (removed) | 13s (removed) | 36s (removed) |
...
**Total**: Tests Δ **+1** · Run Δ **+0 (+0%)** · Playwright Δ **+0 (+0%)** · Status **PARTIAL → PARTIAL** · Browser Δ **+3s** · Spec Gen Δ **+0.6s** · Spec Run Δ **+0s** · Total Δ **+3s**
...
| Charts | +0 | +0 (+0%) | +0 (+0%) | PARTIAL → PARTIAL | +1m 14s | +16s | -1s | +1m 29s |
...
| Viewers | +1 | +0 (-2%) | +0 (-2%) | PARTIAL → PARTIAL | +0s | +0s | +0s | +0s |

### Per-Test Changes

| Folder | Test | Status | Browser Δ | Spec Gen Δ | Spec Run Δ | Total Δ |
|--------|------|--------|-----------|------------|------------|---------|
| Charts | radar | PASS → PASS | +3m 42s | +47s | -3.2s | +4m 25s |
| Viewers | word-cloud | new: PASS | 20s (new) | 3.0s (new) | 13s (new) | 36s (new) |
| Viewers | word-cloud-tests | new: NO RUN | — | — | — | — |
| Viewers | ~~world-cloud~~ | removed: PASS | 20s (removed) | 3.0s (removed) | 13s (removed) | 36s (removed) |
...
Run coverage is 74% (138/187). No FAIL folders overall, but 13 folders are PARTIAL and 6 have no run data. 49 scenarios remain unexecuted (48 in the 6 NO DATA folders plus the new word-cloud-tests scenario in Viewers).
