...
// TestTrack/General for reference only. Reason for removal: it trains a Chemprop model through the
// Chem-managed on-demand `chem-chemprop` Docker container (a long GNN training run). The training does
// not complete on the minimal ui_tests CI stack AND does not complete on dev either within the 15-min
// (900_000ms) step budget — "Save model test_chemprop" times out, so Apply/Browse/Share/Delete cascade.
// Restore it to CI once the chem-chemprop container is pre-built/warmed on the stack and the training run
// is fast/reliable enough for a Playwright budget.
test('Models / Chemprop model — Train, Apply, Container, Browse', async ({page}) => {
