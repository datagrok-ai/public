...
async function gotoPostgresAndExpand(page: import('@playwright/test').Page) {
  await page.goto(`${baseUrl}/browse/databases/Postgres`, {waitUntil: 'networkidle', timeout: 30000});
  ...
  await page.evaluate(async () => {
    ...
    for (let i = 0; i < 20; i++) {
      pg = Array.from(document.querySelectorAll('.d4-tree-view-group-label'))
    ...
    for (let i = 0; i < 5; i++) {
      const showMore = Array.from(node.querySelectorAll('.d4-tree-view-group-label, label, a'))
        .find((el) => el.textContent?.trim() === 'Show more') as HTMLElement | undefined;
      if (!showMore) break;
      showMore.click();
...
async function rightClickConnection(page: import('@playwright/test').Page, name: string) {
  return await page.evaluate(async (n) => {
    ...
    for (let i = 0; i < 60; i++) {
      tp = Array.from(document.querySelectorAll('.d4-tree-view-group-label, label, .d4-link-label'))
...
async function clickMenuItem(page: import('@playwright/test').Page, text: string) {
  return await page.evaluate(async (t) => {
    const target = Array.from(document.querySelectorAll('.d4-menu-item'))
      .find((el) => el.textContent?.trim() === t) as HTMLElement | undefined;
    if (!target) return {clicked: false};
    const inner = target.querySelector('.d4-menu-item-label') as HTMLElement | null;
    const el = inner ?? target;
    for (const evtType of ['pointerdown', 'mousedown', 'pointerup', 'mouseup', 'click']) {
      el.dispatchEvent(new MouseEvent(evtType, {bubbles: true, cancelable: true, button: 0}));
    }
...
async function setDialogInput(page: import('@playwright/test').Page, labelText: string, value: string) {
  return await page.evaluate(({l, v}) => {
    ...
    const inp = (lbl?.closest('.d4-input-base, .d4-flex-row, div')?.querySelector('input, select, .d4-combo-popup') as HTMLInputElement | HTMLSelectElement | null);
...
async function clickDialogButton(page: import('@playwright/test').Page, name: string) {
  return await page.evaluate((n) => {
    const dialogs = Array.from(document.querySelectorAll('.d4-dialog'));
    const dialog = dialogs[dialogs.length - 1];
    if (!dialog) return {clicked: false};
    const candidates = Array.from(dialog.querySelectorAll('button, .ui-btn, .d4-dialog-footer .ui-btn, [name^="button-"]'));
...
test('Connections / Identifiers', async ({page}) => {
  ...
  await softStep('Step 1: Browse > Databases > Postgres tree visible', async () => {
    const visible = await page.evaluate(() => !!Array.from(document.querySelectorAll('.d4-tree-view-group-label'))
      .find((el) => el.textContent?.trim() === 'Postgres'));
    expect(visible).toBe(true);
  });

  await softStep('Step 2-3: Right-click test_postgres → Configure Identifiers... opens schema dialog', async () => {
    ...
    await expect(page.locator('.d4-dialog .d4-dialog-title')).toContainText('Identifiers Configuration', {timeout: 10000});
  });
  ...
  await softStep('Step 4: Set Schema = public, click OK', async () => {
    ...
    expect(r.set, `Schema input not found in schema dialog: ${(r as any).reason}`).toBe(true);
  ...
  await softStep('Step 5: Add identifier (CUSTOMER_ID, customers, customerid, [A-Z]{5})', async () => {
    ...
    const add = await page.evaluate(() => {
      ...
      const addBtn = (Array.from(dialog.querySelectorAll('button, .ui-btn, .grok-icon, i, [name*="add"]'))
        .find((b) => {
          const t = b.textContent?.trim() ?? '';
          const n = b.getAttribute('name') ?? '';
          return t === 'ADD' || t === 'Add' || t === '+' || /add/i.test(n);
        }) ?? dialog.querySelector('[name="icon-plus"], .fa-plus, .grok-icon-plus')) as HTMLElement | null;
      if (!addBtn) return {clicked: false, reason: 'no add button'};
      addBtn.click();
      return {clicked: true};
    });
    expect(add.clicked, `Add identifier row not found: ${(add as any).reason}`).toBe(true);
    await page.waitForTimeout(500);

    // Fill the four fields. Names guessed based on scenario wording.
    const fillResults: Record<string, any> = {};
    for (const [label, value] of [['Semantic Type', 'CUSTOMER_ID'], ['Table', 'customers'], ['Column', 'customerid'], ['MatchRegexp', '[A-Z]{5}']]) {
      const r = await setDialogInput(page, label, value);
      fillResults[label] = r;
    }
    expect(Object.values(fillResults).every((r) => r.set), `Fill failures: ${JSON.stringify(fillResults)}`).toBe(true);
  });
  ...
  await softStep('Step 8-9: Open customers table from test_postgres', async () => {
    ...
    const result = await page.evaluate(async () => {
      const tp = await (window as any).grok.dapi.connections.find('af9bcf40-21a0-11f1-89e2-7b1321b80948');
      ...
      return {opened: true, semType: col?.semType ?? null, type: col?.type ?? null, sample: col ? [col.get(0), col.get(1), col.get(2)] : null};
    });
    expect(result.opened, `Could not open customers: ${(result as any).reason}`).toBe(true);
  });

  await softStep('Step 10: customerid column is highlighted (semType = CUSTOMER_ID)', async () => {
    const semType = await page.evaluate(() => {
      const tv = (window as any).grok.shell.tv;
      return tv?.dataFrame?.col('customerid')?.semType ?? null;
    });
    expect(semType, `customerid semType expected CUSTOMER_ID, got ${semType}`).toBe('CUSTOMER_ID');
  });

  await softStep('Step 11: Click customerid header → Context Panel shows semantic type', async () => {
  ...
  await softStep('Step 12: Remove identifiers configuration on test_postgres', async () => {
    await gotoPostgresAndExpand(page);
    const r1 = await rightClickConnection(page, TEST_POSTGRES);
    expect(r1.found).toBe(true);
    ...
    const removeText = items.find((t) => /remove.*identif/i.test(t)) ?? items.find((t) => /^Configure Identifiers/.test(t));
    expect(removeText, `Removal menu item not found. Items: ${items.join(' | ')}`).toBeTruthy();
  ...
  await softStep('Step 13-14: Reload + verify customerid not highlighted (semType empty/non-CUSTOMER_ID)', async () => {
    await page.reload({waitUntil: 'networkidle', timeout: 30000});
    await page.waitForTimeout(4000);
    ...
    expect(semType, `customerid semType after removal — should NOT be CUSTOMER_ID, got ${semType}`).not.toBe('CUSTOMER_ID');
  });
