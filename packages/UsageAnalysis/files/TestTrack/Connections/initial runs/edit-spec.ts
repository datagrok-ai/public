...
test("Connections / Edit", async ({ page }) => {
  ...
  await softStep("Step 1: Reload the tree", async () => {
    ...
    await expect(
      page
        .locator('[data-link="/db/null.test_postgres"]')
        .or(page.locator('[name="span-test-postgres"]')),
      ...
      await page.evaluate(async () => {
        ...
        const target = (document.querySelector(
          ...
          Array.from(document.querySelectorAll(".d4-link-label, label"))
            .filter((el) => el.textContent?.trim() === "test_postgres")
        ...
        const edit = Array.from(
          document.querySelectorAll(".d4-menu-item"),
  ...
  await softStep(
    "Steps 5-6: Set wrong credentials, verify Test connection fails",
    async () => {
      await page.evaluate(async () => {
        const sync = document.querySelector(
          '[name="icon-sync"]',
      ...
      expect(result).toMatch(
        /password authentication failed for user "wronguser"/,
      );
    },
  );
