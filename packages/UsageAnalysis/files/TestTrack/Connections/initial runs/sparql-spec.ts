...
test("Connections / Sparql", async ({ page }) => {
  ...
  await softStep(
    "Step 1: Browse > Databases, press ellipsis to list providers without connections",
    async () => {
      ...
      await page.evaluate(async () => {
        const databasesNode = Array.from(
          ...
          .find((l) => l.textContent?.trim() === "Databases")
          ?.closest(".d4-tree-view-group") as HTMLElement | undefined;
        const ellipsis = databasesNode?.querySelector(
          '[name="icon-ellipsis-h"]',
        ) as HTMLElement | null;
        ellipsis?.click();
  ...
  await softStep(
    "Step 2: Right-click Sparql, choose New connection...",
    async () => {
      await page.evaluate(async () => {
        const sparql = Array.from(
          document.querySelectorAll(".d4-tree-view-group-label"),
        ).find((l) => l.textContent?.trim() === "Sparql") as
          | HTMLElement
          | undefined;
        sparql?.scrollIntoView({ block: "center" });
        sparql
          ?.closest(".d4-tree-view-node")
          ?.dispatchEvent(
  ...
  await softStep(
    "Step 4: Endpoint + Requires Server=true, leave Prefixes empty",
    async () => {
      await page.locator('[name="input-Endpoint"]').click();
      await page.keyboard.press("Control+A");
      await page.keyboard.type(
        "http://data.ontotext.com/repositories/data-last",
      );

      await page.evaluate(() => {
        const cb = document.querySelector(
          '[name="input-Requires-Server"]',
      ...
      const state = await page.evaluate(() => ({
        endpoint: (
          document.querySelector('[name="input-Endpoint"]') as HTMLInputElement
        )?.value,
        requiresServer: (
        ...
        prefixes:
          (document.querySelector('[name="Prefixes"]') as HTMLTextAreaElement)
            ?.value ?? "",
      }));
  ...
  await softStep(
    'Step 5: Click TEST — expect "connected successfully"',
    async () => {
  ...
  await softStep(
    ...
    async () => {
      ...
      await page.evaluate(async () => {
        (
          document.querySelector(
            '[name="icon-sync"], [name="icon-refresh"]',
          ) as HTMLElement | null
        )?.click();
        await new Promise((r) => setTimeout(r, 1500));
        ...
        let sparql = labels().find(
          (l) => l.textContent?.trim() === "Sparql",
        ) as HTMLElement | undefined;
        if (!sparql) {
          const databasesNode = labels()
            .find((l) => l.textContent?.trim() === "Databases")
            ?.closest(".d4-tree-view-group") as HTMLElement | undefined;
          (
            databasesNode?.querySelector(
              '[name="icon-ellipsis-h"]',
            ) as HTMLElement | null
          )?.click();
          await new Promise((r) => setTimeout(r, 1500));
          sparql = labels().find((l) => l.textContent?.trim() === "Sparql") as
            | HTMLElement
            | undefined;
        }
      ...
      await page.evaluate(async () => {
        const conn = Array.from(
          document.querySelectorAll(
            ".d4-tree-view-group-label, .d4-tree-view-item-label, .d4-link-label, label",
          ),
        ).find((l) => l.textContent?.trim() === "test_sparql") as
          | HTMLElement
          | undefined;
        conn?.scrollIntoView({ block: "center" });
        const target = (conn?.closest(".d4-tree-view-node") ??
          conn) as HTMLElement | null;
        ...
        (
          document.querySelector('[name="div-Delete..."]') as HTMLElement | null
        )?.click();
        await new Promise((r) => setTimeout(r, 1000));
        (
          document.querySelector(
            '.d4-dialog [name="button-DELETE"]',
          ) as HTMLElement | null
        )?.click();
