...
const ensureConnection = async (
  ...
  return await page.evaluate(
    async ({ friendlyName, slug }) => {
      const grok = (window as any).grok;
      const DG = (window as any).DG;
      const list = await grok.dapi.connections
        .filter(`name = "${slug}"`)
        .list();
      if (list.length > 0) {
        const conn = list[0];
...
const deleteConnectionByDataLink = async (page: Page, dataLink: string) => {
  await page.evaluate(async (link) => {
    ...
    const del = Array.from(document.querySelectorAll(".d4-menu-item")).find(
    ...
    const confirmBtn = (dialog?.querySelector('[name="button-DELETE"]') ??
      Array.from(dialog?.querySelectorAll("button, .ui-btn") ?? []).find(
        (b) => b.textContent?.trim() === "DELETE",
      )) as HTMLElement | undefined;
    confirmBtn?.click();
    await new Promise((res) => setTimeout(res, 2500));
    (
      document.querySelector('[name="icon-sync"]') as HTMLElement | null
    )?.click();
...
test("Connections / Delete", async ({ page }) => {
  ...
  const tp2Nq = await ensureConnection(
    page,
    "test_postgres_2",
    "test_postgres_2",
  );
  ...
  await softStep("Steps 1-2: Delete new_test_postgres", async () => {
    await expect(page.locator(`[data-link="${ntpLink}"]`)).toBeVisible({
      timeout: 10000,
    });
    await deleteConnectionByDataLink(page, ntpLink);
    const present = await page.evaluate(
      (link) => !!document.querySelector(`[data-link="${link}"]`),
      ntpLink,
    );
  ...
  await softStep("Steps 3-4: Delete test_postgres_2", async () => {
    await expect(page.locator(`[data-link="${tp2Link}"]`)).toBeVisible({
      timeout: 10000,
    });
