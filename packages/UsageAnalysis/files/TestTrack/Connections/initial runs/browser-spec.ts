...
test("Connections / Browser", async ({ page }) => {
  ...
  await softStep("Step 1: Go to Browse > Databases", async () => {
    await page.goto(`${baseUrl}/db?browse=db`, {
      waitUntil: "networkidle",
      timeout: 30_000,
    });
    await page.waitForTimeout(2500);
    await expect(
      page.locator("label", { hasText: /^New_test$/ }).first(),
    ).toBeVisible({ timeout: 15_000 });
  });

  await softStep(
    "Step 2: Filter templates icon — toggle each template",
    async () => {
      ...
      for (const t of ["All", "Created by me", "Used by me", "Invalid"]) {
        await page
          .locator("span.d4-tag", { hasText: new RegExp(`^${t}$`) })
          .first()
          .click();
  ...
  await softStep("Step 3: Type new_test in the search field", async () => {
    const search = page.locator(
      'input[placeholder="Search connections by name or by #tags"]',
    );
  ...
  await softStep("Step 4: Click the found connection", async () => {
    ...
    await expect(
      page.locator('.grok-prop-panel [name="label-New-test"]'),
    ).toBeVisible({ timeout: 5000 });
  });

  await softStep("Step 5.1: Details pane shows correct values", async () => {
    const details = page.locator(
      '.grok-prop-panel .d4-accordion-pane[d4-title="Details"]',
    );
  ...
  await softStep(
    ...
    async () => {
      ...
      if ((await shareItem.count()) > 0) await shareItem.first().click();
      ...
      const result = await page.evaluate(async () => {
        ...
        const conn = await g.dapi.connections
          .filter('name = "new_test"')
          .first();
      ...
      const sharingHeader = page.locator(
        '.grok-prop-panel .d4-accordion-pane[d4-title="Sharing"] .d4-accordion-pane-header',
      );
      if (
        !(await sharingHeader.evaluate((el) =>
          el.classList.contains("expanded"),
        ))
      )
        await sharingHeader.click();
      await page.waitForTimeout(1500);
      const sharing = page.locator(
        '.grok-prop-panel .d4-accordion-pane[d4-title="Sharing"] .d4-accordion-pane-content',
      );
  ...
  await softStep("Step 5.3: Activity has the expected events", async () => {
    const header = page.locator(
      '.grok-prop-panel .d4-accordion-pane[d4-title="Activity"] .d4-accordion-pane-header',
    );
    if (!(await header.evaluate((el) => el.classList.contains("expanded"))))
      await header.click();
    await page.waitForTimeout(1500);
    const pane = page.locator(
      '.grok-prop-panel .d4-accordion-pane[d4-title="Activity"] .d4-accordion-pane-content',
    );
  ...
  await softStep("Step 5.4: Send a chat message", async () => {
    const header = page.locator(
      '.grok-prop-panel .d4-accordion-pane[d4-title="Chats"] .d4-accordion-pane-header',
    );
    if (!(await header.evaluate((el) => el.classList.contains("expanded"))))
      await header.click();
    await page.waitForTimeout(1500);
    const ta = page.locator(
      '.grok-prop-panel .d4-accordion-pane[d4-title="Chats"] textarea[placeholder]',
    );
    await ta.click();
    ...
    const pane = page.locator(
      '.grok-prop-panel .d4-accordion-pane[d4-title="Chats"] .d4-accordion-pane-content',
    );
  ...
  await softStep("Step 6: Open the connection name dropdown menu", async () => {
    await page
      .locator('.grok-prop-panel [name="icon-context-arrow-down"]')
      .click();
    await page.waitForTimeout(800);
    await expect(page.locator('[name="div-Browse"]')).toBeVisible({
      timeout: 5000,
    });
