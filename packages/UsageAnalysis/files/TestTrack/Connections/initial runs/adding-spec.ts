...
test("Connections / Adding", async ({ page }) => {
  ...
  await softStep("Step 1: Go to Browse > Databases", async () => {
    ...
    await expect(
      page
        .locator(".d4-tree-view-group-label", { hasText: /^Postgres$/ })
        .first(),
    ).toBeVisible({ timeout: 15_000 });
  });

  await softStep(
    "Step 2: Right-click Postgres → New connection...",
    async () => {
      await page.evaluate(() => {
  ...
  await softStep(
    ...
    async () => {
      ...
      expect(await page.locator('[name="input-Name"]').inputValue()).toBe(
        "test_postgres",
      );
      expect(await page.locator('[name="input-Server"]').inputValue()).toBe(
        "db.datagrok.ai",
      );
      expect(await page.locator('[name="input-Port"]').inputValue()).toBe(
        "54322",
      );
      expect(await page.locator('[name="input-Db"]').inputValue()).toBe(
        "northwind",
      );
      expect(await page.locator('[name="input-Login"]').inputValue()).toBe(
        "datagrok",
      );
    },
  );
  ...
  await softStep(
    "Step 5: Click TEST — dialog stays open (auth error expected with placeholder)",
    async () => {
      await page.evaluate(() => {
        const btn = Array.from(
          document.querySelectorAll(".d4-dialog button"),
        ).find((b) => b.textContent?.trim() === "TEST") as
          | HTMLButtonElement
          | undefined;
        btn?.click();
      });
  ...
  await softStep("Step 6: Click OK — connection saved", async () => {
    await page.evaluate(() => {
      const ok = Array.from(
        document.querySelectorAll(".d4-dialog button"),
      ).find((b) => b.textContent?.trim() === "OK") as
        | HTMLButtonElement
        | undefined;
      ok?.click();
    });
    ...
    const exists = await page.evaluate(async () => {
      const g: any = (window as any).grok;
      const c = await g.dapi.connections
        .filter('name = "test_postgres"')
        .first();
      return !!c;
    });
  ...
  await softStep(
    "Step 7: Create test_postgres_2 from the NEW CONNECTION... button",
    async () => {
      await page.evaluate(() => {
        const btn = Array.from(
          document.querySelectorAll("button.ui-btn-ok"),
        ).find(
          (b) => b.textContent?.trim().toLowerCase() === "new connection...",
