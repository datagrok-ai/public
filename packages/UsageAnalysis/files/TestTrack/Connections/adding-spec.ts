...
test('Connections / Adding', async ({page}) => {
  ...
  await softStep('Steps 2-4: Right-click Postgres, fill Add connection dialog', async () => {
    await page.evaluate(async () => {
      ...
      (postgresNode?.closest('.d4-tree-view-node') as HTMLElement)?.dispatchEvent(
  ...
  await softStep('Step 6: Click OK — connection saved', async () => {
    await page.evaluate(() => {
      ...
      (ok as HTMLElement | undefined)?.click();
    });
  ...
  await softStep('Step 7: Create test_postgres_2', async () => {
    await page.evaluate(() => {
