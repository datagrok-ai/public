 ...
 */
test('Queries — columns inspection on PostgresDart NorthwindTest and Postgres Northwind', async ({page}) => {
  ...
  const walkAndInspect = async (provider: string, connection: string) => {
    return await page.evaluate(async ({provider, connection}: {provider: string; connection: string}) => {
      ...
      const expandIfNeeded = (node: Element | null) => {
        ...
        if (!tri) (node.querySelector('.d4-tree-view-group-label, .d4-tree-view-item-label') as HTMLElement)?.click();
        return false;
      };

      const findLabelInHost = (host: Element | null | undefined, text: string) =>
        host && Array.from(host.querySelectorAll(':scope > .d4-tree-view-group > .d4-tree-view-node > .d4-tree-view-group-label'))
          .find((e) => e.textContent?.trim() === text) as HTMLElement | undefined;

      const stages: string[] = [];
      const tables: {table?: string; columnCount: number; clicks: number}[] = [];
      ...
      const providerNode = await waitForSelector(`[name="tree-Databases---${provider}"]`);
      ...
      const connNode = await waitForSelector(`[name="tree-Databases---${provider}---${connection}"]`);
      ...
      const connHost = connGroup?.querySelector(':scope > .d4-tree-view-group-host');
      ...
      const schemasHost = schemasGroup?.querySelector(':scope > .d4-tree-view-group-host');
      ...
      const publicHost = publicGroup?.querySelector(':scope > .d4-tree-view-group-host');
      const tableGroups = publicHost ? Array.from(publicHost.querySelectorAll(':scope > .d4-tree-view-group')) : [];
      stages.push(`tables:${tableGroups.length}`);

      // Expand each table, click each column, then collapse.
      for (const tg of tableGroups) {
        const label = tg.querySelector(':scope > .d4-tree-view-node > .d4-tree-view-group-label')?.textContent?.trim();
        const tnode = tg.querySelector(':scope > .d4-tree-view-node');
        const tri = tnode?.querySelector('.d4-tree-view-tri') as HTMLElement | null;
        if (tri) tri.click(); else (tnode as HTMLElement | null)?.click();
        await wait(900);
        const thost = tg.querySelector(':scope > .d4-tree-view-group-host');
        const colEls = thost
          ? Array.from(thost.querySelectorAll('.d4-tree-view-item-label, .d4-tree-view-group-label'))
          : [];
        let clicks = 0;
      ...
      const balloons = Array.from(document.querySelectorAll('.d4-balloon, .grok-balloon'))
        .filter((b) => (b as HTMLElement).offsetParent !== null)
        .map((b) => (b.textContent?.trim() ?? '').slice(0, 160));
      const accordionErrors = Array.from(document.querySelectorAll('.grok-prop-panel .d4-accordion-pane'))
        .filter((a) => !!a.querySelector('.d4-error, .grok-error, .d4-accordion-error'))
        .map((a) => a.querySelector('.d4-accordion-pane-header')?.textContent?.trim());

      return {ok: true, stages, tableCount: tables.length, totalClicks: tables.reduce((s, t) => s + t.clicks, 0), balloons, accordionErrors};
    }, {provider, connection});
  };

  // ---------- Part 1: PostgresDart → NorthwindTest ----------
  let part1: any;
  await softStep('Browse → Databases → PostgresDart → NorthwindTest → Schemas → public', async () => {
  ...
  await softStep('Part 1 — expand each DB table, click each column, no errors on Context Panel', async () => {
  ...
  await page.evaluate(() => {
    const node = document.querySelector('[name="tree-Databases---PostgresDart---NorthwindTest"]') as HTMLElement | null;
    const tri = node?.querySelector('.d4-tree-view-tri.d4-tree-view-tri-expanded') as HTMLElement | null;
    if (tri) tri.click();
  });
  await page.waitForTimeout(500);

  let part2: any;
  await softStep('Browse → Databases → Postgres → Northwind → Schemas → public', async () => {
  ...
  await softStep('Part 2 — expand each DB table, click each column, no errors on Context Panel', async () => {
