...
test("Queries — delete new_test_query via context menu", async ({ page }) => {
  ...
  const seedId = await page.evaluate(async () => {
    ...
    let q = await (window as any).grok.dapi.queries
      .filter('friendlyName = "new_test_query"')
      .first()
      .catch(() => null);
    if (!q) {
  ...
  await softStep(
    "Right-click new_test_query → Delete → confirm DELETE",
    async () => {
      ...
      await page.evaluate(async () => {
        ...
        const card = (label.closest(
          ".grok-gallery-grid-item, .d4-gallery-card, .d4-gallery-item, .grok-entity, .grok-item, .d4-tile, .d4-card",
