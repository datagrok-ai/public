...
test("Queries — edit an existing SQL query", async ({ page }) => {
  ...
  const seedId = await page.evaluate(async () => {
    ...
    try {
      const list = await (window as any).grok.dapi.queries
        .filter(
          'name in ("test_query", "TestQuery", "new_test_query", "NewTestQuery")',
        )
        .list();
      for (const q of list)
        try {
    ...
    const conn = await (window as any).grok.dapi.connections.find(
      "a2d74603-7594-56ea-a2bd-844b2fd16ee7",
    );
  ...
  await softStep("Right-click test_query → Edit... (open editor)", async () => {
    ...
    await page.goto(
      `${process.env.DATAGROK_URL ?? "http://localhost:8888"}/queries/Dbtests.PostgresTest?browse=db`,
    );
    await page
      .locator('div[name="div-TestQuery"]')
      .waitFor({ timeout: 30_000 });

    const opened = await page.evaluate(async () => {
      const card = document.querySelector(
        'div[name="div-TestQuery"]',
      ) as HTMLElement | null;
  ...
  await softStep(
    "Run via Ribbon Play button → inline preview grid",
    async () => {
  ...
  await softStep(
    "Run via Toolbox → Actions → Run query... → new view",
    async () => {
    ...
    const q = await page.evaluate(async (id) => {
      ...
      for (let i = 0; i < 50; i++) {
        ...
        if (
          ...
          qq.friendlyName === "new_test_query"
        )
          return {
            ok: true,
            name: qq.name,
