...
test("Queries — query post-process balloon and saved layout", async ({
  page,
}) => {
  ...
  await page.evaluate(async () => {
    ...
    if (!w.__capturedInfo) {
      ...
      w.grok.shell.info = (msg: any) => {
        w.__capturedInfo.push(String(msg));
        return orig(msg);
      };
    }
    ...
    try {
      ...
      for (const q of list)
        try {
          await w.grok.dapi.queries.delete(q);
        } catch (e) {}
    } catch (e) {}
  });
  ...
  await softStep(
    "Browse → Databases → Postgres → NorthwindTest → right-click → New Query...",
    async () => {
      const result = await page.evaluate(async () => {
        ...
        const node = nwGroup.querySelector(
          ":scope > .d4-tree-view-node",
        ) as HTMLElement | null;
        if (!node) return { ok: false, missing: "NorthwindTest tree-node" };
        (
          node.querySelector(".d4-tree-view-group-label") as HTMLElement | null
        )?.click();
  ...
  await softStep(
    "Set name = Test_Postprocessing and body = select * from products",
    async () => {
  ...
  await softStep(
    "Post-Process: replace line 7 with grok.shell.info(result.rowCount);",
    async () => {
      const result = await page.evaluate(async () => {
        const tab = Array.from(
          document.querySelectorAll(".d4-tab-host .d4-tab-header"),
        ).find((el) => el.textContent?.trim() === "Post-Process") as
          | HTMLElement
          | undefined;
  ...
  await softStep("Layout: add scatter plot and correlation plot", async () => {
    const result = await page.evaluate(async () => {
      const tab = Array.from(
        document.querySelectorAll(".d4-tab-host .d4-tab-header"),
      ).find((el) => el.textContent?.trim() === "Layout") as
        | HTMLElement
        | undefined;
      ...
      const sIcon = document.querySelector(
        '[name="icon-scatter-plot"]',
      ) as HTMLElement | null;
      ...
      const cIcon = document.querySelector(
        '[name="icon-correlation-plot"]',
      ) as HTMLElement | null;
  ...
  await softStep("Close all and re-open Browse", async () => {
    ...
    await page.waitForFunction(
      () => {
        return (
          document.querySelectorAll(".d4-tree-view-group-label").length > 5
        );
      },
      undefined,
      { timeout: 30_000 },
  ...
  await softStep(
    "Preview Test_Postprocessing — both viewers + green balloon 77",
    async () => {
      const result = await page.evaluate(async () => {
        const findChildGroupByLabel = (
          parent: ParentNode,
        ...
        if (!tpGroup)
          return { ok: false, missing: "Test_Postprocessing in tree" };
        (window as any).__capturedInfo = [];
        const lbl = tpGroup.querySelector(
        ...
        for (let i = 0; i < 80; i++) {
          ...
          if (v?.type === "TableView" && v?.dataFrame?.rowCount > 0) {
            ...
            const viewers = Array.from(
              (window as any).grok.shell.tv.viewers,
            ).map((x: any) => x.type);
            if (
              captured.length > 0 &&
              viewers.includes("Scatter plot") &&
              viewers.includes("Correlation plot")
            )
              return {
                ok: true,
                viewers,
        ...
        return {
          ...
          viewers:
            v?.type === "TableView"
              ? Array.from((window as any).grok.shell.tv.viewers).map(
                  (x: any) => x.type,
                )
              : null,
          rowCount: v?.dataFrame?.rowCount,
  ...
  await softStep("Right-click Test_Postprocessing → Edit...", async () => {
    const result = await page.evaluate(async () => {
      ...
      const node = tpGroup.querySelector(
        ":scope > .d4-tree-view-node",
      ) as HTMLElement | null;
    ...
    const result = await page.evaluate(async () => {
      ...
      while (Date.now() < deadline) {
        ...
        for (let i = 0; i < 12; i++) {
          await new Promise((r) => setTimeout(r, 250));
          const captured = [...((window as any).__capturedInfo ?? [])];
          if (captured.length > 0) return { ok: true, attempts, captured };
        }
      }
  ...
  await softStep(
    "Run from Layout tab — green balloon 77 + both viewers",
    async () => {
      const result = await page.evaluate(async () => {
        const tab = Array.from(
          document.querySelectorAll(".d4-tab-host .d4-tab-header"),
