...
test("Queries — Visual Query Advanced (post-process, layout, edit, refresh, project)", async ({
  page,
}) => {
  ...
  await softStep(
    "1. Create new visual query — open editor on customers table",
    async () => {
      const result = await page.evaluate(async () => {
        ...
        const tableNode = customersGroup.querySelector(
        ...
        while (Date.now() < menuDeadline) {
          ...
          ).find((el) => el.textContent?.trim() === "New Visual Query...") as
            | HTMLElement
            | undefined;
          if (target) break;
        }
        ...
        for (let i = 0; i < 80; i++) {
          ...
          const titles = Array.from(
            document.querySelectorAll(".grok-pivot-column-panel"),
          ).map((p) =>
            p
              .querySelector(".grok-pivot-column-tags-title")
              ?.textContent?.trim(),
          );
  ...
  await softStep(
    "1. Set Group by = companyname and a Where parameter (companyname contains a, exposed)",
    async () => {
      const result = await page.evaluate(async () => {
        const wait = (ms: number) => new Promise((r) => setTimeout(r, ms));
        const pickFromPicker = async (rowIndex: number) => {
          const grids = Array.from(
            document.querySelectorAll('[name="viewer-Grid"]'),
          ).filter((g) => {
            const r = g.getBoundingClientRect();
            return (
              r.width > 50 && r.width < 250 && r.height > 80 && r.height < 350
            );
          });
          const popup = grids[0];
          ...
          for (const t of [
            ...
            const Ctor =
              isPtr && (window as any).PointerEvent ? PointerEvent : MouseEvent;
            canv.dispatchEvent(
              new Ctor(t, {
                bubbles: true,
                cancelable: true,
        ...
        const groupByPanel = Array.from(
          document.querySelectorAll(".grok-pivot-column-panel"),
        ).find(
          (p) =>
            p
              .querySelector(".grok-pivot-column-tags-title")
              ?.textContent?.trim() === "Group by",
        ) as Element;
        (
        ...
        const wherePanel = Array.from(
          document.querySelectorAll(".grok-pivot-column-panel"),
        ).find(
          (p) =>
            p
              .querySelector(".grok-pivot-column-tags-title")
              ?.textContent?.trim() === "Where",
        ) as Element;
        (
          wherePanel.querySelector(".grok-icon.fa-plus") as HTMLElement
        )?.click();
        await wait(1200);
        await pickFromPicker(1);
        const wherePanel2 = Array.from(
          document.querySelectorAll(".grok-pivot-column-panel"),
        ).find(
          (p) =>
            p
        ...
        const conditionInput = Array.from(
          wherePanel2.querySelectorAll("input.ui-input-editor"),
        ).find(
        ...
        conditionInput.blur();
        await wait(400);
        const checkbox = wherePanel2.querySelector(
          'input[type="checkbox"]',
        ) as HTMLInputElement;
  ...
  await softStep(
    "2. Run the query (ribbon Play button) — preview shows rows",
    async () => {
      const result = await page.evaluate(async () => {
        const wait = (ms: number) => new Promise((r) => setTimeout(r, ms));
        const play = Array.from(document.querySelectorAll("i.grok-icon")).find(
          (i) => i.classList.contains("fa-play"),
        ) as HTMLElement;
        play?.click();
        await wait(2500);
        const editor = (
          Array.from((window as any).grok.shell.views) as any[]
        ).find((v) => v.type === "DataQueryView");
  ...
  await softStep("3-4. Set custom name and Save", async () => {
    const result = await page.evaluate(async (n) => {
      ...
      const nameInput = Array.from(
        ...
        (i) =>
          (i as HTMLInputElement).value === "customers" &&
      ...
      for (let i = 0; i < 20; i++) {
        await wait(800);
        q = await (window as any).grok.dapi.queries
          .filter(`friendlyName = "${n}"`)
          .first()
          .catch(() => null);
        if (q) break;
      }
  ...
  await softStep("5. Share the query with All users (read)", async () => {
    ...
    const result = await page.evaluate(async (id) => {
      ...
      const allUsers = await w.grok.dapi.groups
  ...
  await softStep(
    "6. Add post-process: grok.shell.info(result.rowCount)",
    async () => {
      const result = await page.evaluate(async () => {
        const wait = (ms: number) => new Promise((r) => setTimeout(r, ms));
        const ppTab = Array.from(
  ...
  await softStep("8-9. Save the query and Close all", async () => {
    ...
    const views = await page.evaluate(() =>
      Array.from((window as any).grok.shell.views).map((v: any) => v.type),
    );
    expect(views.length).toBeLessThanOrEqual(1);
  });

  await softStep(
    "10. Click query to preview — verify name and post-process executes",
    async () => {
      if (!ctx.queryId) return;
      const result = await page.evaluate(async (id) => {
        const wait = (ms: number) => new Promise((r) => setTimeout(r, ms));
        ...
        const observer = new MutationObserver((muts) => {
          for (const m of muts)
            for (const n of m.addedNodes) {
              ...
              el?.querySelectorAll?.(".d4-balloon-content")?.forEach?.((c) =>
                balloons.push(c.textContent?.trim() ?? ""),
              );
            }
        });
  ...
  await softStep(
    "11. Edit the query — change Where condition, change layout, save",
    async () => {
      if (!ctx.queryId) return;
      await page.goto(
        `${process.env.DATAGROK_URL ?? "http://localhost:8888"}/query/${ctx.queryId}/edit`,
      );
      await page.waitForFunction(
        () => document.querySelector(".grok-preloader") == null,
        null,
        { timeout: 120_000 },
      );
      ...
      await page.waitForFunction(
        () => {
          try {
            return (window as any).grok?.shell?.v?.type === "DataQueryView";
          } catch {
            return false;
          }
        },
      ...
      const result = await page.evaluate(async () => {
        ...
        if (!wherePanel)
          return { ok: false, missing: "Where panel after edit-page load" };
        const conditionInput = Array.from(
          wherePanel.querySelectorAll("input.ui-input-editor"),
        ).find(
          (i) => (i as HTMLInputElement).type === "text",
        ) as HTMLInputElement;
        if (!conditionInput)
          return { ok: false, missing: "Where condition input" };
        conditionInput.focus();
        document.execCommand("selectAll");
        ...
        await wait(300);
        (
          document.querySelector('[name="button-Save"]') as HTMLElement
        )?.click();
        await wait(3500);
        const viewerEls = Array.from(
          document.querySelectorAll('[name^="viewer-"]'),
        ).map((e) => e.getAttribute("name"));
  ...
  await softStep(
    "12-13. Close all, click query to preview again — verify post-process",
    async () => {
      if (!ctx.queryId) return;
      const result = await page.evaluate(async (id) => {
        const wait = (ms: number) => new Promise((r) => setTimeout(r, ms));
        ...
        const balloons = Array.from(
          document.querySelectorAll(".d4-balloon-content"),
  ...
  await softStep(
    "14-15. Run the query and refresh with a different parameter (q.prepare path)",
    async () => {
      if (!ctx.queryId) return;
      const result = await page.evaluate(async (id) => {
        const wait = (ms: number) => new Promise((r) => setTimeout(r, ms));
  ...
  await softStep(
    "16-18. Add viewers, delete some rows, refresh (Enrich) — restored",
    async () => {
      if (!ctx.queryId) return;
      const result = await page.evaluate(async (id) => {
        const wait = (ms: number) => new Promise((r) => setTimeout(r, ms));
        ...
        return {
          beforeRows,
          afterDelete: df.rowCount,
          restored: fresh.rowCount,
          beforeCols,
          afterCols: df.columns.length,
          restoredCols: fresh.columns.length,
          colDeleted,
          viewerEls,
        };
      }, ctx.queryId);
      expect(result.afterDelete).toBe(
        result.beforeRows - Math.min(5, result.beforeRows),
      );
  ...
  await softStep("19. Save the project (top SAVE button)", async () => {
    const result = await page.evaluate(async (n) => {
      ...
      const beforeIds = new Set(
        (
          Array.from(
            await (window as any).grok.dapi.projects.list({ pageSize: 20 }),
          ) as any[]
        ).map((p) => p.id),
      );
      ...
      const errors = Array.from(
        document.querySelectorAll(".d4-balloon.error"),
      ).map((b) => b.textContent?.trim());
      return {
  ...
  await softStep("20-21. Close all and reopen the saved project", async () => {
    ...
    await page.goto(
      `${process.env.DATAGROK_URL ?? "http://localhost:8888"}/p/${ctx.projectId}`,
    );
    await page.waitForFunction(
      () => document.querySelector(".grok-preloader") == null,
      null,
