...
test("Queries — query layout (Layout tab, preview, project save/restore)", async ({
  page,
}) => {
  ...
  const ids = await page.evaluate(async () => {
    ...
    const q = await (window as any).grok.dapi.queries
      .filter(
        'name = "PostgresAll" and connection.friendlyName = "NorthwindTest"',
      )
      .first()
      .catch(() => null);
    return {
  ...
  await page.evaluate(async () => {
    try {
      const list = await (window as any).grok.dapi.projects
        .filter('name in ("query-layout-test-claude", "QueryLayoutTestClaude")')
        .list();
      for (const p of list)
        try {
          await (window as any).grok.dapi.projects.delete(p);
        } catch (e) {}
    } catch (e) {}
  });
  ...
  await softStep("Open the Layout tab", async () => {
    await page
      .locator('.d4-tab-header[name="Layout"]')
      .first()
      .click({ force: true });
    const result = await page.evaluate(async () => {
      for (let i = 0; i < 50; i++) {
        if (document.querySelector('.d4-tab-header[name="Layout"].selected'))
          break;
        await new Promise((r) => setTimeout(r, 200));
      }
      for (let i = 0; i < 80; i++) {
      ...
      const result = await page.evaluate(async () => {
        ...
        for (let i = 0; i < 80; i++) {
          if (document.querySelector('[name="viewer-Grid"] canvas')) {
            ...
            return {
              ok: true,
              rows: (window as any).grok.shell.v?.dataFrame?.rowCount,
            };
          }
          await new Promise((r) => setTimeout(r, 300));
        }
  ...
  await softStep(
    "Add four viewers (Histogram, Bar chart, Scatter plot, Pie chart)",
    async () => {
  ...
  await softStep("Add two more viewers (Box plot, Tree map)", async () => {
    const viewers = await page.evaluate(async () => {
      (
        document.querySelector('[name="icon-box-plot"]') as HTMLElement
      )?.click();
      await new Promise((r) => setTimeout(r, 1500));
      (
        document.querySelector('[name="icon-tree-map"]') as HTMLElement
      )?.click();
      await new Promise((r) => setTimeout(r, 1500));
      return Array.from(document.querySelectorAll('[name^="viewer-"]')).map(
        (e) => e.getAttribute("name"),
    ...
    expect(viewers).toEqual(
      expect.arrayContaining(["viewer-Box-plot", "viewer-Tree-map"]),
    );
  });
  ...
  await softStep(
    "Save the project — SAVE → name → OK (Cancel Share dialog)",
    async () => {
      await page.locator('[name="button-Save"]').first().click();
      ...
      const dialogInfo = await page.evaluate(() => {
        ...
        const inputs = Array.from(
          dlg?.querySelectorAll("input, textarea") ?? [],
        ).map((i: any) => ({
          type: i.type,
      ...
      const nameInput = page
        .locator(".d4-dialog input, .d4-dialog textarea")
        .nth(nameInputIdx);
      ...
      const lookup = await page.evaluate(async () => {
        ...
        return {
          id: null,
          recentNames: list.slice(0, 10).map((p: any) => p.name),
        };
      });
      projectId = lookup.id;
  ...
  await softStep("Toolbox → Source → Refresh — layout unchanged", async () => {
    ...
    const result = await page.evaluate(async () => {
      ...
      const before = Array.from(tv.viewers)
        .map((v: any) => v.type)
        .sort();
      ...
      if (sourceSection) {
        const header = sourceSection.querySelector(
          ".d4-accordion-pane-header",
        ) as HTMLElement | null;
        const content = sourceSection.querySelector(
      ...
      for (let i = 0; i < 10; i++) {
        refreshBtn = Array.from(document.querySelectorAll("button")).find(
          (b) =>
      ...
      const after = Array.from((window as any).grok.shell.tv.viewers)
        .map((v: any) => v.type)
        .sort();
      return {
        ok: true,
