...
test("Queries — New Visual Query from the customers table", async ({
  page,
}) => {
  ...
  await softStep(
    ...
    async () => {
      const result = await page.evaluate(async () => {
        ...
        return {
          ok: true,
          hasCustomers: !!findChildGroupByLabel(scope, "customers"),
        };
      });
  ...
  await softStep(
    "Right-click customers → New Visual Query… opens visual query editor",
    async () => {
      const result = await page.evaluate(async () => {
        const findChildGroupByLabel = (
          parent: ParentNode,
  ...
  await softStep("Set Group by to companyname", async () => {
    const result = await page.evaluate(async () => {
      ...
      const groupByPanel = panels.find(
        (p) =>
          p
            .querySelector(".grok-pivot-column-tags-title")
      ...
      const plus = groupByPanel.querySelector(
        ".grok-icon.fa-plus",
      ) as HTMLElement | null;
      ...
      await wait(1000);
      const tags = Array.from(groupByPanel.querySelectorAll(".d4-tag")).map(
  ...
  await softStep(
    "Order by picker shows only companyname when Group by has companyname",
    async () => {
      const result = await page.evaluate(async () => {
        const wait = (ms: number) => new Promise((r) => setTimeout(r, ms));
        const panels = Array.from(
        ...
        const orderByPanel = panels.find(
          (p) =>
            p
              .querySelector(".grok-pivot-column-tags-title")
              ?.textContent?.trim() === "Order by",
        );
        if (!orderByPanel) return { ok: false, missing: "Order by panel" };
        const plus = orderByPanel.querySelector(
          ".grok-icon.fa-plus",
        ) as HTMLElement | null;
        plus?.click();
        await wait(1500);
    ...
    const result = await page.evaluate(async () => {
      ...
      ["mouseenter", "mousemove", "mouseover"].forEach((t) =>
        nwMi.dispatchEvent(
          new MouseEvent(t, {
            bubbles: true,
            clientX: nwRect.left + nwRect.width / 2,
            clientY: nwRect.top + nwRect.height / 2,
            view: window,
          }),
        ),
      );
      ...
      const publicItem = items2.find(
        (el) => el.textContent?.trim() === "public",
      ) as HTMLElement | undefined;
      ...
      ["mouseenter", "mousemove", "mouseover"].forEach((t) =>
        pMi.dispatchEvent(
          new MouseEvent(t, {
            bubbles: true,
            clientX: pRect.left + pRect.width / 2,
            clientY: pRect.top + pRect.height / 2,
            view: window,
          }),
        ),
      );
      await wait(900);
      const items3 = Array.from(
        document.querySelectorAll(".d4-menu-popup .d4-menu-item-label"),
      ).filter((el) => (el as HTMLElement).offsetParent !== null);
      const ordersItem = items3.find(
      ...
      await wait(2500);
      const tags = Array.from(document.querySelectorAll(".d4-tag")).map((t) =>
  ...
  await softStep("Open Debug tab and run Debug query — no errors", async () => {
    const result = await page.evaluate(async () => {
      ...
      const debugTab = tabs.find((t) => t.textContent?.trim() === "Debug") as
        | HTMLElement
        | undefined;
      if (!debugTab) return { ok: false, missing: "Debug tab" };
      debugTab.click();
      await wait(1500);
      const bug = document.querySelector(
        '[name="icon-bug"]',
      ) as HTMLElement | null;
      ...
      for (let i = 0; i < 40; i++) {
        ...
        if (
          debugContent.includes("Call was started") ||
          debugContent.includes("STATEMENT EXECUTION")
        )
          return { ok: true };
      }
  ...
  await softStep("Toolbox → Actions → Run query… opens new view", async () => {
    const result = await page.evaluate(async () => {
      ...
      const tabs = Array.from(document.querySelectorAll(".d4-tab-header"));
      const queryTab = tabs.find((t) => t.textContent?.trim() === "Query") as
        | HTMLElement
        | undefined;
      queryTab?.click();
      await wait(800);
  ...
  await softStep("Save the query", async () => {
    ...
    await page.evaluate((name) => {
      const input = Array.from(
        ...
        (el) =>
          (el as HTMLInputElement).value === "customers" &&
          (el as HTMLElement).offsetParent !== null,
