...
test('Queries — transformations on the Products query', async ({page}) => {
  ...
  const transformationScript = 'AddNewColumn(t, "${productid}", "${productid}", "auto", false, false, "")';
  ...
  await softStep('Open Products query editor (Edit...)', async () => {
    ...
    await page.locator('[name="Transformations"][data-source="tab-pane-Transformations"]').waitFor({timeout: 30_000});
  ...
  await softStep('Open Transformations tab', async () => {
    await page.locator('[name="Transformations"][data-source="tab-pane-Transformations"]').click();
    await page.waitForTimeout(1500);
    await page.locator('[name="span-AddNewColumn"]').waitFor({timeout: 30_000});
  });

  await softStep('Add new column ${productid}; verify added to transformation script', async () => {
    ...
    const result = await page.evaluate(async ({id, script}) => {
      const w = window as any;
  ...
  await softStep('Toolbox → Actions → Run query... — parameter dialog appears', async () => {
    await page.locator('[name="Toolbox"]').first().click();
    await page.locator('[name="pane-Actions"] label.d4-link-action').first().waitFor({state: 'visible', timeout: 60_000});
    await page.waitForTimeout(2000);
    await page.locator('[name="pane-Actions"] label.d4-link-action').first().click();
    await page.locator('.d4-dialog .d4-dialog-title:has-text("Products")').waitFor({timeout: 30_000});
    const title = await page.evaluate(() =>
      document.querySelector('.d4-dialog .d4-dialog-title')?.textContent?.trim());
    expect(title).toBe('Products');
  });

  await softStep('Close all', async () => {
    await page.evaluate(async () => {
      const cancelBtn = document.querySelector('.d4-dialog [name="button-CANCEL"]') as HTMLElement | null;
      cancelBtn?.click();
  ...
  await softStep('Run the query — TableView opens (transformation chain applied if persisted)', async () => {
    ...
    for (let i = 0; i < 30; i++) {
      ...
      if (v === 'TableView') { ranOk = true; break; }
    }
    if (!ranOk) {
    ...
    const result = await page.evaluate(async () => {
      for (let i = 0; i < 300; i++) {
        ...
        if (w.grok.shell.tv && !candidates.includes(w.grok.shell.tv)) candidates.push(w.grok.shell.tv);
        for (const c of candidates) {
          if (c?.type === 'TableView' && c.dataFrame) {
            const cols = c.dataFrame.columns.toList().map((cl: any) => cl.name);
  ...
  await softStep('Delete transformations created during this test', async () => {
  ...
  await softStep('Refresh view — Transformations tab opens cleanly', async () => {
    await page.goto(`${process.env.DATAGROK_URL}/query/${queryId}`);
    await page.locator('[name="Transformations"][data-source="tab-pane-Transformations"]').waitFor({timeout: 30_000});
    await page.locator('[name="Transformations"][data-source="tab-pane-Transformations"]').click();
    await page.waitForTimeout(2000);
