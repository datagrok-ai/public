...
test('Queries — New SQL Query from the products table', async ({page}) => {
  ...
  await softStep('Browse → Databases → Postgres → NorthwindTest → Schemas → public', async () => {
    const result = await page.evaluate(async () => {
      ...
      return {ok: true, hasProducts: !!findChildGroupByLabel(scope, 'products')};
    });
    expect(result.ok).toBe(true);
    expect(result.hasProducts).toBe(true);
  });

  await softStep('Right-click products → New SQL Query… opens query editor', async () => {
    const result = await page.evaluate(async () => {
      const findChildGroupByLabel = (parent: ParentNode, label: string): Element | null => {
        const candidates = Array.from(parent.querySelectorAll('.d4-tree-view-group-label'))
          .filter((el) => el.textContent?.trim() === label);
      ...
      const tableNode = productsGroup.querySelector(':scope > .d4-tree-view-node') as HTMLElement | null;
      ...
      while (Date.now() < menuDeadline) {
        ...
        target = Array.from(document.querySelectorAll('.d4-menu-popup .d4-menu-item-label'))
          .find((el) => el.textContent?.trim() === 'New SQL Query...') as HTMLElement | undefined;
        if (target) break;
      }
      if (!target) return {ok: false, missing: 'New SQL Query... menu item'};
      target.click();
      ...
      for (let i = 0; i < 80; i++) {
        ...
        if (cm && cm.CodeMirror) {
          ...
          return {ok: true, viewType: v?.type, cm: cm.CodeMirror.getValue()};
        }
      }
  ...
  await softStep('Run via ribbon Play button — inline grid appears', async () => {
    const result = await page.evaluate(async () => {
  ...
  await softStep('Run via Toolbox → Actions → Run query… — new view opens', async () => {
    const result = await page.evaluate(async () => {
      const w: any = window;
      const before = Array.from(w.grok.shell.views).length;
      const link = Array.from(document.querySelectorAll('label.d4-link-action'))
        .find((el) => el.textContent?.trim() === 'Run query...' && (el as HTMLElement).offsetParent !== null) as HTMLElement | undefined;
        ...
        if (after > before) {
          ...
          for (let j = 0; j < 30; j++) {
            ...
            if (v?.type === 'TableView' && v?.dataFrame?.rowCount > 0)
              return {ok: true, before, after, rowCount: v.dataFrame.rowCount, colCount: v.dataFrame.columns.length, name: v.name};
          }
  ...
  await softStep('Save the query', async () => {
    ...
    await page.evaluate((name) => {
      const input = Array.from(document.querySelectorAll('input.ui-input-editor'))
        .find((el) => (el as HTMLInputElement).value === 'products' && (el as HTMLElement).offsetParent !== null) as HTMLInputElement | undefined;
    ...
    const found = await page.evaluate(async (n) => {
      const q = await (window as any).grok.dapi.queries.filter(`name = "${n}"`).first().catch(() => null);
      return !!q;
    }, queryName);
