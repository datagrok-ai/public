...
test('Queries — MS SQL: Add / Edit / Browse / Delete', async ({page}) => {
  ...
  await page.goto(`${process.env.DATAGROK_URL ?? 'https://dev.datagrok.ai'}/browse`);

  let savedQueryId: string | null = null;

  await softStep('Part 1.1 — Browse → Databases → MS SQL → right-click NorthwindTest → New Query', async () => {
    ...
    await page.locator('[name="tree-Databases"]').first().waitFor({state: 'attached', timeout: 30_000});
    await page.evaluate(async () => {
      // Expand Databases — click the expander triangle, not the label
      for (let i = 0; i < 30; i++) {
        const exp = document.querySelector('[name="tree-expander-Databases"]') as HTMLElement | null;
        const node = document.querySelector('[name="tree-Databases"]') as HTMLElement | null;
      ...
      for (let i = 0; i < 30; i++) {
        const exp = document.querySelector('[name="tree-expander-Databases---MS-SQL"]') as HTMLElement | null;
        const node = document.querySelector('[name="tree-Databases---MS-SQL"]') as HTMLElement | null;
        if (document.querySelector('[name="tree-Databases---MS-SQL---NorthwindTest"]')) break;
    ...
    await page.locator('[name="tree-Databases---MS-SQL---NorthwindTest"]').first()
      .waitFor({state: 'attached', timeout: 30_000});
    const opened = await page.evaluate(async () => {
      const nw = document.querySelector('[name="tree-Databases---MS-SQL---NorthwindTest"]') as HTMLElement | null;
      if (!nw) return { ok: false, stage: 'no-northwindtest' };
      nw.scrollIntoView({block: 'center'});
      nw.dispatchEvent(new MouseEvent('contextmenu', {bubbles: true, cancelable: true, button: 2}));
      for (let i = 0; i < 25; i++) {
    ...
    await page.locator('input[name="input-Name"]').waitFor({timeout: 30_000});
  });
  ...
  await softStep('Part 1.5 — Run via Toolbox → Actions → Run query… (new view)', async () => {
    const result = await page.evaluate(async () => {
      const beforeViews = Array.from((window as any).grok.shell.views).length;
      const item = Array.from(document.querySelectorAll('label, .d4-link-label'))
    ...
    const saved = await page.evaluate(async () => {
      for (let i = 0; i < 30; i++) {
        const q = await (window as any).grok.dapi.queries
          .filter('friendlyName = "test_query_ms_sql"').first().catch(() => null);
        if (q && q.id) return { ok: true, id: q.id, name: q.name, friendly: q.friendlyName, body: q.query };
        await new Promise((r) => setTimeout(r, 500));
      }
      return { ok: false };
    });
  ...
  await softStep('Part 2.1 — Refresh Browse → Edit query (open editor)', async () => {
    ...
    await page.goto(`${process.env.DATAGROK_URL ?? 'https://dev.datagrok.ai'}/query/${savedQueryId}`);
  ...
  await softStep('Part 2.2 — Change name to new_test_query_ms_sql', async () => {
    const nameInput = page.locator('input[name="input-Name"]').first();
    await nameInput.click();
    await page.keyboard.press('Control+a');
  ...
  await softStep('Part 2.5 — Save the renamed query', async () => {
    ...
    const saved = await page.evaluate(async (id) => {
      for (let i = 0; i < 30; i++) {
        ...
        if (q && q.friendlyName === 'new_test_query_ms_sql' && q.query === 'select * from orders')
          return { ok: true, name: q.name, friendly: q.friendlyName, body: q.query };
        await new Promise((r) => setTimeout(r, 500));
      }
      return { ok: false };
    }, savedQueryId);
  ...
  await softStep('Part 3.1/3.2 — Browse → MS SQL → NorthwindTest, expand and search', async () => {
    await page.goto(`${process.env.DATAGROK_URL ?? 'https://dev.datagrok.ai'}/browse`);
    const result = await page.evaluate(async () => {
      for (let i = 0; i < 30; i++) {
      ...
      const ms = document.querySelector('[name="tree-Databases---MS-SQL"]') as HTMLElement | null;
      ms?.click();
      ms?.dispatchEvent(new MouseEvent('dblclick', {bubbles: true, cancelable: true}));
      // Wait for NorthwindTest to mount
      for (let i = 0; i < 30; i++) {
        if (document.querySelector('[name="tree-Databases---MS-SQL---NorthwindTest"]')) break;
        await new Promise((r) => setTimeout(r, 300));
      }
      const exp = document.querySelector('[name="tree-expander-Databases---MS-SQL---NorthwindTest"]') as HTMLElement | null;
      exp?.click();
      // Wait briefly for children to mount
      for (let i = 0; i < 20; i++) {
        const children = document.querySelectorAll('[name^="tree-Databases---MS-SQL---NorthwindTest---"]');
        if (children.length > 0) break;
        await new Promise((r) => setTimeout(r, 300));
      }
      const labels = [...new Set(Array.from(
        document.querySelectorAll('[name^="tree-Databases---MS-SQL---NorthwindTest---"] .d4-tree-view-group-label, [name^="tree-Databases---MS-SQL---NorthwindTest---"] .d4-tree-view-item-label'),
      ).map((e) => e.textContent?.trim()).filter(Boolean))];
      const containsNew = labels.some((l) => l?.includes('new_test_query_ms_sql'));
  ...
  await softStep('Part 3.3 — Context Panel tabs for the saved query', async () => {
