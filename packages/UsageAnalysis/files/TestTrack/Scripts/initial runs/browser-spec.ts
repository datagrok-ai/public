...
test("Scripts Browser — context panel accordions, share, chat, ACF run+edit", async ({
  page,
}) => {
  ...
  await softStep(
    ...
    async () => {
      ...
      await page.waitForFunction(
        () => (window as any).grok?.shell?.v?.name === "Scripts",
        null,
        { timeout: 30000 },
      );
  ...
  await softStep(
    "3. Click script — context pane shows all accordions",
    async () => {
      await page.evaluate(async () => {
        const titleEl = Array.from(
          document.querySelectorAll(".grok-gallery-grid-item-title"),
        ).find((e) => e.textContent?.trim() === "testRscript") as HTMLElement;
        const card = (titleEl?.closest(".grok-gallery-grid-item") ||
      ...
      await page.waitForFunction(
        () => {
          ...
          return ["Details", "Script", "Run", "Sharing", "Chats"].every(
            (want) => headers.some((h) => h.startsWith(want)),
          );
        },
        null,
        { timeout: 30000 },
  ...
  await softStep(
    "3A. Details accordion: Created by, Inputs, Outputs",
    async () => {
      const text = await page.evaluate(async () => {
        const details = Array.from(
          document.querySelectorAll(".d4-accordion-pane-header"),
        ).find((e) =>
          e.textContent?.trim().startsWith("Details"),
        ) as HTMLElement;
  ...
  await softStep(
    "3C. Share with selenium user — Sharing pane lists recipient",
    async () => {
      const text = await page.evaluate(async () => {
        const s = await grok.dapi.scripts
          .filter('name = "testRscript"')
          .first();
        const target = await grok.dapi.users
          .filter('login = "selenium"')
          .first()
          .catch(() => null);
        if (target)
          await grok.dapi.permissions.grant(s, (target as any).group, false);
        await new Promise((r) => setTimeout(r, 1500));
        const titleEl = Array.from(
          document.querySelectorAll(".grok-gallery-grid-item-title"),
        ).find((e) => e.textContent?.trim() === "testRscript") as HTMLElement;
        ...
        const sharing = Array.from(
          document.querySelectorAll(".d4-accordion-pane-header"),
        ).find((e) => e.textContent?.trim() === "Sharing") as
          | HTMLElement
          | undefined;
  ...
  await softStep("3D. Activity pane shows audit entries", async () => {
    const text = await page.evaluate(async () => {
      const activity = Array.from(
        document.querySelectorAll(".d4-accordion-pane-header"),
      ).find((e) => e.textContent?.trim().startsWith("Activity")) as
        | HTMLElement
        | undefined;
      if (activity && !activity.classList.contains("expanded"))
        activity.click();
      await new Promise((r) => setTimeout(r, 1500));
  ...
  await softStep("3E. Send message to chat", async () => {
    const result = await page.evaluate(async () => {
      const chats = Array.from(
        document.querySelectorAll(".d4-accordion-pane-header"),
      ).find((e) => e.textContent?.trim() === "Chats") as
        | HTMLElement
        | undefined;
  ...
  await softStep(
    "4. Script browser — gallery + sort + search controls present",
    async () => {
  ...
  await softStep("5a. Open TSLA.csv (precondition for ACF)", async () => {
    const ok = await page.evaluate(async () => {
      let df = null;
      for (const path of [
        "System:DemoFiles/finance/TSLA.csv",
        "System:DemoFiles/TSLA.csv",
        "System:DemoFiles/stocks/TSLA.csv",
      ]) {
        try {
  ...
  await softStep(
    "5b. Right-click ACF card → Run... → set Columns → OK",
    async () => {
      await page.evaluate(async () => {
        grok.shell.route("/scripts");
        for (let i = 0; i < 30; i++) {
      ...
      const balloons = await page.evaluate(async () => {
        ...
        const visibleAcf = dlgs.find(
          (d) =>
            d.querySelector(".d4-dialog-header")?.textContent?.trim() ===
              "ACF" && getComputedStyle(d).display !== "none",
        );
        visibleAcf
          ?.querySelector<HTMLButtonElement>('[name="button-OK"]')
          ?.click();
  ...
  await softStep(
    "5c. Right-click ACF → Edit... → ScriptView opens",
    async () => {
      await page.evaluate(async () => {
        for (const d of (DG.Dialog as any).getOpenDialogs())
          try {
  ...
  await softStep("5d. Editor Context Panel — Details tab", async () => {
    const text = await page.evaluate(async () => {
      const acf = await grok.dapi.scripts
        .filter('name = "ACF"')
        .first()
        .catch(() => null);
  ...
  await softStep(
    "5e. Editor Context Panel — Script tab body visible",
    async () => {
      const info = await page.evaluate(async () => {
        const scrHdr = Array.from(
          document.querySelectorAll(".d4-accordion-pane-header"),
        ).find((e) =>
          e.textContent?.trim().startsWith("Script"),
        ) as HTMLElement;
        if (scrHdr && !scrHdr.classList.contains("expanded")) scrHdr.click();
        await new Promise((r) => setTimeout(r, 1500));
        const ta = scrHdr?.parentElement?.querySelector(
          "textarea",
  ...
  await softStep(
    "5f. Editor Context Panel — Activity tab populated",
    async () => {
      const text = await page.evaluate(async () => {
        const act = Array.from(
          document.querySelectorAll(".d4-accordion-pane-header"),
        ).find((e) =>
          e.textContent?.trim().startsWith("Activity"),
        ) as HTMLElement;
