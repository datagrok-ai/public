...
test("Scripts Edit — testRscript", async ({ page }) => {
  ...
  await softStep(
    ...
    async () => {
      ...
      await page.waitForFunction(
        () => !!document.querySelector('[name="button-New"]'),
        null,
        { timeout: 15000 },
      );
  ...
  await softStep("2. Find testRscript and double-click", async () => {
    await page.evaluate(async () => {
      ...
      const card = (label?.closest(
        ".grok-gallery-grid-item, .grok-entity-card",
      ) || label) as HTMLElement;
      card.dispatchEvent(
        new MouseEvent("dblclick", { bubbles: true, view: window }),
      );
    });
    await page.waitForFunction(
      () => (window as any).grok?.shell?.v?.type === "ScriptView",
      null,
      { timeout: 30000 },
    );
    await page.waitForFunction(
  ...
  await softStep(
    '6. Double-click testRscript again; verify newParam="test"',
    async () => {
      await page.waitForFunction(
        () =>
          Array.from(
      ...
      await page.waitForFunction(
        () => (window as any).grok?.shell?.v?.name === "testRscript",
        null,
        { timeout: 30000 },
      );
      await page.waitForFunction(
