...
test("Scripts Delete — testRscript", async ({ page }) => {
  ...
  await softStep("3. Right-click the script, select Delete", async () => {
    const cardLocator = page
      .locator(".grok-gallery-grid-item")
      .filter({
        has: page.locator(".grok-gallery-grid-item-title", {
          hasText: /^testRscript$/,
        }),
      })
      .first();
    ...
    const delLocator = page
      .locator(".d4-menu-item", { hasText: /^Delete$/ })
      .first();
  ...
  await softStep("4. Click YES in the confirmation dialog", async () => {
    ...
    const dialogHandled = await page.evaluate(async () => {
      ...
      for (let i = 0; i < 60; i++) {
        const dialog = Array.from(document.querySelectorAll(".d4-dialog")).find(
          (d) =>
            isVisible(d) && (d.textContent || "").includes("Delete script"),
        );
        if (dialog) {
          const yes = dialog.querySelector(
            '[name="button-YES"]',
          ) as HTMLElement | null;
          if (yes) {
            yes.click();
          ...
          const byText = Array.from(
            dialog.querySelectorAll('button, .ui-btn, [role="button"]'),
          ).find((b) => /^YES$/i.test((b.textContent || "").trim())) as
            | HTMLElement
            | undefined;
          if (byText) {
            byText.click();
  ...
  await softStep("5. Check script is no longer present", async () => {
    const remaining = await page.evaluate(async () => {
      for (let i = 0; i < 30; i++) {
        const found = await grok.dapi.scripts
          .filter('name = "testRscript"')
          .list();
