...
test("Scripts Run — context menu + console", async ({ page }) => {
    ...
    async () => {
      const balloons = await page.evaluate(async () => {
        ...
        for (let i = 0; i < 30; i++) {
          ...
          if (
            dlgs.length > 0 &&
            dlgs.some((d) => d.querySelector('[name="button-OK"]'))
          )
            break;
        ...
        return Array.from(document.querySelectorAll(".grok-balloon"))
  ...
  await softStep("7. Open Datagrok console (~)", async () => {
    ...
    const ok = await page.evaluate(
      () =>
        !!document.querySelector(".d4-console-wrapper") &&
        !!Array.from(
          document.querySelectorAll('input[placeholder*="Enter command"]'),
  ...
  await softStep(
    ...
    async () => {
      await page.evaluate(async () => {
        ...
        if (!grok.shell.tables.some((t: any) => t.name === "cars")) {
          ...
          if (df) {
            ...
            if (d) {
              d.name = "cars";
              grok.shell.addTableView(d);
            }
          }
        }
        const tbl = grok.shell.tables.find(
          (t: any) => t.name === "cars" || t.name === "Table",
        );
      ...
      const hasNewParam = lines.some((l: string) =>
        /newParam:\s*"test"/.test(l),
      );
