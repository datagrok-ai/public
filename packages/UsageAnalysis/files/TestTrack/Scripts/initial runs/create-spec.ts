...
test("Scripting — create R script with signature editor + all-language smoke test", async ({
  page,
}) => {
  ...
  await page.evaluate(async () => {
    ...
    const existing = await grok.dapi.scripts
      .filter('friendlyName = "testRscript"')
      .list();
  ...
  await softStep("1. Browse > Platform > Functions > Scripts", async () => {
    await page.evaluate(async () => {
      document
        .querySelector<HTMLElement>('[name="tree-expander-Platform"]')
        ?.click();
      await new Promise((r) => setTimeout(r, 600));
      document
        .querySelector<HTMLElement>(
          '[name="tree-expander-Platform---Functions"]',
        )
        ?.click();
      await new Promise((r) => setTimeout(r, 600));
      const lbl = Array.from(
        document.querySelectorAll(".d4-tree-view-item-label"),
      ).find(
        (e) => e.textContent?.trim().toLowerCase() === "scripts",
      ) as HTMLElement;
      lbl?.click();
      for (let i = 0; i < 30; i++) {
  ...
  await softStep("3. Click Open script sample table (asterisk)", async () => {
    await page.evaluate(async () => {
      ...
      for (let i = 0; i < 30; i++) {
        if (
          Array.from(grok.shell.tables).some((t: any) => /^cars/.test(t.name))
        )
          break;
        await new Promise((r) => setTimeout(r, 200));
      }
      await new Promise((r) => setTimeout(r, 800));
      const sv = Array.from(grok.shell.views).find(
        (v: any) => v.type === "ScriptView",
      );
      if (sv) (grok.shell as any).v = sv;
    });
    const carsLoaded = await page.evaluate(() =>
      Array.from(grok.shell.tables).some((t: any) => /^cars/.test(t.name)),
    );
  ...
  await softStep("4. Click Signature editor (magic-wand)", async () => {
    if (sigEditorAvailable) {
      await page.evaluate(async () => {
        document
          .querySelector<HTMLElement>("i.fal.fa-magic, i.fa-magic")
          ?.click();
        for (let i = 0; i < 60; i++) {
  ...
  await softStep("6. Switch to Parameters tab", async () => {
    if (sigEditorAvailable) {
      ...
      const selected = await page.evaluate(() =>
        Array.from(document.querySelectorAll(".d4-tab-header.selected")).map(
    ...
    if (sigEditorAvailable) {
      ...
      await page.evaluate(async () => {
        const plus = Array.from(
          document.querySelectorAll("i.fal.fa-plus, i.fa-plus"),
        ).filter((i) => (i as HTMLElement).offsetParent !== null);
        const btn = (plus[1] as HTMLElement)?.closest(
          "button",
        ) as HTMLElement | null;
        btn?.click();
        await new Promise((r) => setTimeout(r, 800));
  ...
  await softStep(
    "9. Click Open function editor (return to script body)",
    async () => {
      if (sigEditorAvailable) {
        await page.evaluate(async () => {
          document
            .querySelector<HTMLElement>("i.fal.fa-code, i.fa-code")
            ?.click();
  ...
  await softStep("10. Click Play, choose cars, OK — script runs", async () => {
    await page.evaluate(async () => {
      ...
      const sel = document.querySelector<HTMLSelectElement>(
        '.d4-dialog select[name="input-Table"]',
      );
      ...
      await new Promise((r) => setTimeout(r, 400));
      document
        .querySelector<HTMLElement>('.d4-dialog [name="button-OK"]')
        ?.click();
      for (let i = 0; i < 100; i++) {
        if (!document.querySelector(".d4-dialog")) break;
        await new Promise((r) => setTimeout(r, 200));
  ...
  await softStep(
    "11. Click Save — script persisted with parsed params",
    async () => {
      await page.evaluate(async () => {
      ...
      const probe = await page.evaluate(async () => {
        const list = await grok.dapi.scripts
          .filter('friendlyName = "testRscript"')
          .list();
        const s: any = list[0];
        if (!s) return { count: 0 };
        return {
          count: list.length,
          outputs: (s.outputs ?? []).map((p: any) => ({
            name: p.name,
            type: p.propertyType,
          })),
          inputs: (s.inputs ?? []).map((p: any) => ({
            name: p.name,
            type: p.propertyType,
          })),
        };
  ...
  await softStep("12. Close script view via x", async () => {
    await page.evaluate(async () => {
      const handle = document.querySelector<HTMLElement>(
        '[name="view-handle: testRscript"]',
      );
      const closeBtn = handle?.querySelector<HTMLElement>(
    ...
    const closed = await page.evaluate(
      () =>
        !Array.from(grok.shell.views).some((v: any) => v.type === "ScriptView"),
    );
  ...
  const runLang = async (menuLabel: string) => {
    return await page.evaluate(async (label) => {
      ...
      const scriptsV = Array.from(grok.shell.views).find(
        (v: any) => v.type === "scripts",
      );
      ...
      if (!item) {
        log.errors.push(`menu item not found: ${label}`);
        return log;
      }
      item.click();
      for (let i = 0; i < 60; i++) {
      ...
      const ast = document.querySelector<HTMLElement>(
        'i[name="icon-asterisk"]',
      );
      ...
      if (dialogOpened) {
        ...
        const sels = Array.from(
          document.querySelectorAll(".d4-dialog select"),
  ...
  await softStep("14. R script: open sample, run, no errors", async () => {
    ...
    expect(
      log.errors.filter((e: string) => /Value not defined/i.test(e)),
    ).toEqual([]);
  });
  ...
  await softStep(
    "18. JavaScript script: run (alert dialog), no errors",
    async () => {
  ...
  await softStep(
    "20. Pyodide script: open sample, run, no errors",
    async () => {
