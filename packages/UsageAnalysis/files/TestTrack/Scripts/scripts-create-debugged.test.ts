...
/** Fast reset: closeAll + dismiss dialogs/toasts, then navigate to Scripts browser. */
async function resetToScripts(page: Page) {
  ...
  // Click Scripts in the tree if visible, otherwise fall back to full navigation
...
async function runScriptWithTable(page: Page, tableName: string) {
  ...
  await okBtn.click();
  // Wait for execution to finish (balloon or grid update) instead of fixed timeout
...
test.describe.serial('Scripts: Create', () => {
  ...
  test.beforeAll(async ({ browser }) => {
    ...
    // Navigate to Scripts once — expand tree for subsequent tests
    await openScriptsBrowser(page);
  });
