...
async function gridCellPoint(page: Page, column: string, rowIdx: number): Promise<{x: number; y: number} | null> {
  return page.evaluate(async ({col, row}) => {
    ...
    return {x: Math.round(rect.left + xLocal), y: Math.round(rect.top + yLocal)};
  }, {col: column, row: rowIdx});
}

async function gridHeaderPoint(page: Page, column: string): Promise<{x: number; y: number} | null> {
  return page.evaluate(async (col) => {
    const grid = grok.shell.tv.grid;
    ...
    const rect = overlay.getBoundingClientRect();
    return {
      x: Math.round(rect.left + (gc.left + gc.right) / 2),
...
async function openColumnOptionsPopup(page: Page, column: string): Promise<ColumnPopup> {
  ...
  expect(pt, `the ${column} grid column header must be locatable for the column-options hover`).not.toBeNull();
...
async function observeSketchClearing(page: Page, windowMs: number): Promise<ClearingObservation> {
  ...
  while (Date.now() - started < windowMs) {
    ...
    if (last.drawn === 0) {
      ...
      if (++zeroes >= 3)
        return {clearedAfterMs: clearedAt, observedMs: Date.now() - started, samples,
          lastDrawn: 0, lastDrawnColumns: []};
    }
    else { zeroes = 0; clearedAt = null; }
    await page.waitForTimeout(500);
  }
  return {clearedAfterMs: null, observedMs: Date.now() - started, samples,
...
async function removeSubstructureCards(page: Page, columns: string[]): Promise<number> {
  const removed = await page.evaluate(() => {
    ...
    const targets = cards.filter((c) => !!c.querySelector('.chem-filter'));
    for (const c of targets) {
      const icon = c.querySelector('[name="icon-times"]') as HTMLElement | null;
      if (!icon)
        throw new Error('a substructure card in the panel carries no [name="icon-times"] remove icon, so its '
          + 'removal was never actuated: ' + (c.querySelector('.d4-filter-column-name')?.textContent ?? '?').trim());
      icon.click();
    }
...
async function setCardEnabled(page: Page, on: boolean): Promise<boolean> {
  return page.evaluate(async ({want, col, source}) => {
    ...
    if (!cb || !card) return false;
    if (cb.checked !== want) cb.click();
    await new Promise((r) => setTimeout(r, 800));
    return cb.checked === want && card.classList.contains('d4-filter-disabled') === !want;
  }, {want: on, col: CHEM_COL, source: SUBSTRUCTURE_SOURCE});
}
...
async function setAlignHighlight(page: Page, on: boolean): Promise<AlignHighlightState> {
  return page.evaluate((want) => {
    const dlg = [...document.querySelectorAll('.d4-dialog')]
      .find((d) => !!d.querySelector('input[placeholder*="SMILES"]'));
    const box = dlg?.querySelector('.chem-sketcher-filter-options');
    const boxes = ['input-Align', 'input-Highlight']
      .map((n) => box?.querySelector(`[name="${n}"]`) as HTMLInputElement | null);
    if (!box || boxes.some((cb) => cb?.type !== 'checkbox'))
      return {set: false, align: false, highlight: false};
    for (const cb of boxes)
      if (cb!.checked !== want) cb!.click();
    return {set: true, align: boxes[0]!.checked, highlight: boxes[1]!.checked};
  }, on);
}
...
async function setFilterAsYouDraw(page: Page, on: boolean): Promise<{set: boolean; value: boolean}> {
  return page.evaluate((want) => {
    const dlg = [...document.querySelectorAll('.d4-dialog')]
      .find((d) => !!d.querySelector('input[placeholder*="SMILES"]'));
    const box = dlg?.querySelector('.chem-sketcher-filter-options');
    const cb = box?.querySelector('[name="input-Filter-as-you-draw"]') as HTMLInputElement | null;
    if (!cb || cb.type !== 'checkbox') return {set: false, value: false};
    if (cb.checked !== want) cb.click();
    return {set: true, value: cb.checked};
  }, on);
}
...
test('Filter panel — Chem package filters', async ({page}) => {
  ...
  await softStep('Scenario 1, Step 6: GROK-20001: Current Value > Use as filter adds exactly one card', async () => {
    ...
    expect(clicked.row >= 0 && clicked.row < CHEM_FULL,
      `the right-click must make a real data row current (read ${clicked.row})`).toBe(true);
  ...
  await softStep('Scenario 1, Step 8: Reset clears filtering and the sketched substructure state', async () => {
    ...
    console.log(`Step 8 sketch-area clearing: ${clearing.clearedAfterMs === null
      ? `NOT cleared — ${clearing.lastDrawn} card body/bodies (${clearing.lastDrawnColumns.join(', ')}) were still ` +
        `painting a structure across the whole ${clearing.observedMs} ms observation window (${clearing.samples} reads)`
  ...
  await softStep('Scenario 1, Step 9: github-3445: realignment repaints on the spot; "Filter as you draw" gates the sketch', async () => {
    ...
    const heldNoPropagation = async (label: string, expected: number) => {
      for (const waitMs of [3000, 4000]) {
        ...
        expect((await v.diffCanvasColors(page, 'Grid', GRID_CANVAS)).deltaPx,
          `${label}: with "Filter as you draw" CLEARED, a further edit must not repaint the structure ` +
          `column while the dialog stays open (read ${waitMs} ms after the edit)`).toBe(0);
        expect(await trueCount(page),
          `${label}: with "Filter as you draw" CLEARED, a further edit must not move the row set while ` +
          `the dialog stays open (read ${waitMs} ms after the edit; expected ${expected})`).toBe(expected);
      }
    };
    ...
    expect(await trueCount(page),
      'Half A: with "Filter as you draw" CLEARED the sketch must not reach the grid while the dialog is open — ' +
      'the row set must still be the full one').toBe(CHEM_FULL);
    ...
    expect(CHEM_PROBE_2, 'the further edit must be a DIFFERENT structure, or "it did not reach the grid" ' +
      'and "it did" would look the same').not.toBe(CHEM_PROBE);
