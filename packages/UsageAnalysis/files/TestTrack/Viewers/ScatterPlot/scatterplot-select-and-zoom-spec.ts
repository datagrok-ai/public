...
async function setChoiceProp(
  ...
  await v.pollValue(() => propCellText(page, viewCell),
    (t) => t.toLowerCase() === value.toLowerCase(), 2500, 100);
...
async function resetViewFromMenu(page: Page, from?: Rect): Promise<void> {
  ...
  await v.pollValue(() => page.locator('.d4-menu-popup:visible').count(), (n) => n === 0, 2000, 100);
...
async function waitProp(page: Page, name: string, value: any, timeoutMs = 4000): Promise<void> {
  await v.pollValue(() => page.evaluate((n: string) => {
    const sp = grok.shell.tv.viewers.find((x: any) => x.type === 'Scatter plot') as any;
    return sp.props[n];
  }, name), (cur) => cur === value, timeoutMs, 120);
}
...
async function waitSliderHandle(
  ...
  return v.pollValue(() => sliderHandlePoint(page, slider, handle), (p) => p !== null, 3000, 150);
}
