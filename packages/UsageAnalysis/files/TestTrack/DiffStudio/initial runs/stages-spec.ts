...
test("DiffStudio Stages (Acid Production): Load, Multiaxis+Facet, modify input, tooltips", async ({
  page,
}) => {
  ...
  await softStep(
    "Step 1: Open Diff Studio + Acid Production from Library",
    async () => {
      await page.evaluate(async () => {
      ...
      await page.waitForTimeout(2000);
      const card = page
        .locator(".diff-studio-hub-card", { hasText: "Acid Production" })
        .first();
      ...
      await page.waitForFunction(
        () =>
          ...
          grok.shell.v?.name === "Acid Production",
        null,
        { timeout: 30000 },
      );
      await page.waitForTimeout(2000);
  ...
  await softStep(
    "Step 3: Modify 1-st stage input; URL updates live",
    async () => {
      const inp = page.locator(
        '[name="input-host-1-st-stage"] input.ui-input-editor',
      );
  ...
  await softStep(
    "Step 4: Tooltips on inputs (1-st stage, biomass, glucose)",
    async () => {
