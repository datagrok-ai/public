...
test("DiffStudio Sensitivity Analysis: Bioreactor, Process mode cascade, Run, 4 viewers", async ({
  page,
}) => {
  ...
  await softStep(
    "Step 1: Open Diff Studio + Bioreactor; Edit toggle OFF",
    async () => {
      await page.evaluate(async () => {
      ...
      const hasProcessMode = await page
        .locator('[name="input-host-Process-mode"]')
        .count();
  ...
  await softStep("Step 2: Open Sensitivity Analysis view", async () => {
    ...
    await page.waitForFunction(
      () =>
        ...
        document.body.innerText.includes("Sensitivity Analysis"),
      null,
      { timeout: 30000 },
    );
    await page.waitForTimeout(2000);
    const sa = await page.evaluate(() => ({
      ...
      hasProcessMode: !!document.querySelector(
        '[name="input-host-Process-mode"]',
      ),
    }));
  ...
  await softStep(
    "Step 3: Modify Process mode; FFox/KKox/others cascade-update",
    async () => {
      const before = await page.evaluate(() => ({
        ...
        mea: (
          document.querySelector(
            '[name="input-host-MEAthiol"] input.ui-input-editor',
          ) as HTMLInputElement
        )?.value,
        gas: (
          document.querySelector(
            '[name="input-host-Gas"] input.ui-input-editor',
          ) as HTMLInputElement
        )?.value,
      }));
      await page.evaluate(() => {
  ...
  await softStep(
    ...
    async () => {
      ...
      await page.evaluate(() => {
        (
          document.querySelector(
            '[name="input-host-FFox"] .ui-input-switch',
          ) as HTMLElement
        )?.click();
        (
          document.querySelector(
            '[name="input-host-KKox"] .ui-input-switch',
          ) as HTMLElement
        )?.click();
      });
      await page.waitForTimeout(1500);
