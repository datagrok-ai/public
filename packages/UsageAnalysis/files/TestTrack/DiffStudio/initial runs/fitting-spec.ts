...
test("DiffStudio Fitting (Bioreactor): Fit view, Process mode cascade, max edits, CSV upload, Run", async ({
  page,
}) => {
  ...
  await softStep("Step 2: Click Fit icon — Fitting view opens", async () => {
    await page.evaluate(() => {
      const item = Array.from(
        document.querySelectorAll(".d4-ribbon-item"),
    ...
    await page.waitForFunction(
      () =>
        grok.shell.v?.name?.includes("fitting") ||
        document.body.innerText.includes("Fitting"),
      null,
      { timeout: 30000 },
    );
    await page.waitForTimeout(2000);
    const hasFittingForm = await page.evaluate(
      () =>
        !!Array.from(document.querySelectorAll(".form-title")).find(
          (t) => t.textContent?.trim() === "Fit",
        ),
    );
    ...
    const before = await page.evaluate(() => ({
      ffox: (
        document.querySelector(
          '[name="input-host-FFox"] input.ui-input-editor',
        ) as HTMLInputElement
      )?.value,
      kkox: (
        document.querySelector(
          '[name="input-host-KKox"] input.ui-input-editor',
        ) as HTMLInputElement
      )?.value,
    }));
    await page.evaluate(() => {
  ...
  await softStep(
    "Step 4: Set Process mode to Default; enable switchers; set FFox max=1.0, FKox max=3",
    async () => {
      await page.evaluate(() => {
        const select = document.querySelector(
          '[name="input-host-Process-mode"] select',
      ...
      await page.evaluate(() => {
        ...
        const switches = Array.from(
          document.querySelectorAll(".sa-switch-input"),
        );
        for (const s of switches) {
          ...
          if (targets.includes(label)) {
            ...
            if (!switchEl.classList.contains("ui-input-switch-on"))
              switchEl.click();
          }
        }
      });
      ...
      const vals = await page.evaluate(() => ({
        ffoxMax: (
          document.querySelector(
            '[name="input-host-FFox-(max)"] input.ui-input-editor',
          ) as HTMLInputElement
        )?.value,
        fkoxMax: (
          document.querySelector(
            '[name="input-host-FKox-(max)"] input.ui-input-editor',
          ) as HTMLInputElement
        )?.value,
      }));
  ...
  await softStep(
    "Step 5: Load bioreactor-experiment.csv and select in Bioreactor table input",
    async () => {
      await page.evaluate(async () => {
        ...
        const fitView = Array.from(grok.shell.views).find((v) =>
          v.name?.includes("fitting"),
        );
        if (fitView) grok.shell.v = fitView;
        await new Promise((r) => setTimeout(r, 1500));
        const select = document.querySelector(
          '[name="input-host-Bioreactor"] select',
        ) as HTMLSelectElement;
    ...
    async () => {
      test
        ...
        .annotations.push({
          type: "blocker",
          description:
            "Run icon clicks but fit never populates the result grid (0 rows after 2m, no balloon/error). " +
