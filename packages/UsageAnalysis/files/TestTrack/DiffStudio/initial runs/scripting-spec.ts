...
test("DiffStudio Scripting: Edit toggle, </> JS view, Run, Save with //tags: model, Model Hub", async ({
  page,
}) => {
  ...
  await softStep(
    "Step 1: Turn on Edit toggle; click </> to open JS script view",
    async () => {
      await page.evaluate(() => {
        const editRibbonItem = Array.from(
          document.querySelectorAll(".d4-ribbon-item"),
        ).find((el) => el.textContent?.trim() === "Edit") as HTMLElement;
        const editor = editRibbonItem?.querySelector(
          ".ui-input-bool-switch .ui-input-editor",
        ) as HTMLElement;
        editor?.click();
      });
      await page.waitForTimeout(2000);
      const switchOn = await page.evaluate(() =>
        document
          ...
          ?.classList.contains("ui-input-switch-on"),
      );
      expect(switchOn).toBe(true);

      await page.evaluate(() => {
        const span = Array.from(
          document.querySelectorAll(".d4-ribbon-name"),
  ...
  await softStep(
    "Step 2: Run the script; adjust Final input; live update",
    async () => {
      await page.locator('[name="icon-play"]').click();
      await page.waitForSelector('[name="input-host-Final"] input', {
        timeout: 30000,
      });
      await page.waitForTimeout(2000);
      ...
      const missingFacet = await page.evaluate(
        () =>
          !document.body.innerText.includes("Facet") &&
          !document.querySelector('[name="input-host-Process-mode"]'),
      );
  ...
  await softStep("Step 3: Add //tags: model; Save the script", async () => {
    await page.evaluate(() => {
      const scriptView = Array.from(grok.shell.views).find(
        (v) => v.type === "ScriptView",
      );
  ...
  await softStep(
    "Step 4: Access Model in Model Hub (Compute2:modelCatalog)",
    async () => {
      await page.evaluate(async () => {
        const f = DG.Func.find({
          package: "Compute2",
      ...
      const hasBioreactor = await page.evaluate(
        () =>
          !!Array.from(document.querySelectorAll(".d4-list-item")).find(
            (el) => el.textContent?.trim() === "Bioreactor",
          ),
      );
  ...
  await softStep(
    "Step 5: Interact with saved model; adjust Final",
    async () => {
      await page.evaluate(async () => {
        const items = Array.from(
          document.querySelectorAll(".d4-list-item"),
        ).filter(
          (el) => el.textContent?.trim() === "Bioreactor",
        ) as HTMLElement[];
