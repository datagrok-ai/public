...
test("DiffStudio Catalog: PK-PD load, save to Model Hub, refresh, run, modify dose", async ({
  page,
}) => {
  ...
  await softStep("Step 1: Open PK-PD from Diff Studio Library", async () => {
    ...
    await page.waitForTimeout(2000);
    const card = page
      .locator(".diff-studio-hub-card", { hasText: "PK-PD" })
      .first();
    await card.waitFor({ timeout: 15000 });
    await card.dblclick();
    await page.waitForFunction(() => grok.shell.v?.name === "PK-PD", null, {
      timeout: 30000,
    });
    await page.waitForTimeout(2000);
  });
  ...
  await softStep("Step 2: Click Save to Model Hub icon", async () => {
    ...
    await page
      .waitForFunction(
        () =>
          Array.from(
            document.querySelectorAll(".d4-balloon, .grok-notification"),
          ).some(
            (b) =>
              b.textContent?.includes("Saved to Library") ||
              b.textContent?.includes("PK-PD.ivp"),
          ),
        null,
        { timeout: 15000 },
      )
  ...
  await softStep("Step 3: Open Model Hub (Apps > Model Hub)", async () => {
    ...
    await page.waitForFunction(() => grok.shell.v?.name === "Model Hub", null, {
      timeout: 30000,
    });
    await page.waitForTimeout(3000);
    const pkpdCount = await page.evaluate(
      () =>
        Array.from(document.querySelectorAll(".d4-list-item")).filter(
          (e) => e.textContent?.trim() === "PK-PD",
        ).length,
    );
  ...
  await softStep("Step 4: Click Refresh icon; catalog reloads", async () => {
    await page.evaluate(() => {
      const refresh = Array.from(document.querySelectorAll(".grok-icon")).find(
        (e) => e.className.includes("fa-sync"),
      ) as HTMLElement;
      refresh?.click();
    });
    await page.waitForTimeout(3000);
  ...
  await softStep(
    "Step 6: Modify dose input; verify live update and URL",
    async () => {
      const inp = page.locator(
        '[name="input-host-dose"] input.ui-input-editor',
      );
