...
test("DiffStudio Files & Sharing: pk.ivp preview, modify Step/Count, URL reload", async ({
  page,
  context,
}) => {
  ...
  await softStep("Step 1: Open pk.ivp file preview", async () => {
    await page.evaluate(async () => {
      const previewFn = DG.Func.find({
        name: "previewIvp",
        package: "DiffStudio",
      })[0];
      const info = await grok.dapi.files.list(
        "System:AppData/DiffStudio/library/",
        false,
        "pk.ivp",
      );
      const file = info[0];
    ...
    await page.waitForFunction(() => grok.shell.v?.name === "PK", null, {
      timeout: 30000,
    });
    await page.waitForTimeout(2000);
  ...
  await softStep(
    "Step 2: Set Step=0.1 (slider+textbox) and Count=4 (clicker)",
    async () => {
      const stepInp = page.locator(
        '[name="input-host-step"] input.ui-input-editor',
      );
      await stepInp.click();
  ...
  await softStep(
    "Step 3: Open URL in a new tab; model loads with same inputs",
    async () => {
      const url = page.url();
