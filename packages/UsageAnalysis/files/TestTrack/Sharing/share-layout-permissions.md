...
original_path: public/packages/UsageAnalysis/files/TestTrack/sharing/share-layout-permissions.md
migration_date: '2026-06-11'
source_text_fixes:
  - typo-form-to-from-block-d-step-2
  - typo-form-to-from-block-f-step-1
  - typo-form-to-from-block-f-step-3
  - typo-open-gallery-capitalization-block-d-step-3
candidate_helpers: []
...
- `sharing.entity-types.layout` is in `manual_only[]` (atlas rev 2): two-actor test requires two independent authenticated browser sessions; cannot be reduced to single-session Playwright automation.
- Atlas `source_classes[].layout`: `external_deps: []` — ViewLayout is standalone; no transitive entity dependency at share time. This explains why Block B shows no "This entity will also be shared..." cascade notice.
- Block C advanced editor shows the standard View/Edit/Delete/Share column set only — no entity-specific permission column for layouts (applying a layout is a View-level operation).
- `sharing.browse-shared-with-me` is also in `manual_only[]` — covered here in Block D step 2 as part of the two-actor flow.
- Step mapping: all 14 numbered steps (Block A×2, Block B×3, Block C×2, Block D×3, Block E×3, Block F×4) are preserved 1:1 in the migrated body. No steps dropped or deferred.
- No SCOPE_REDUCTION proposals: every block covers a distinct sub_feature interaction; none can be removed without losing coverage of a listed `sub_features_covered` entry.
