...
for (const ds of datasets) {
  test(`Bio Sequence Activity Cliffs on ${ds.name}`, async ({page}) => {
    ...
    await softStep(`${ds.name}: Open Bio > Analyze > Activity Cliffs (defaults)`, async () => {
      // Note: scenario text says "Bio > Search > Sequence Activity Cliffs" but the actual
      // menu path on the live platform is "Bio > Analyze > Activity Cliffs..."
      // (the function is internally named "Sequence Activity Cliffs").
      await page.locator('[name="div-Bio"]').hover();
      await page.locator('[name="div-Bio---Analyze"]').hover();
      await page.locator('[name="div-Bio---Analyze---Activity-Cliffs..."]').click();
    ...
    await softStep(`${ds.name}: Change Method to t-SNE and Similarity to Levenshtein`, async () => {
      // During MCP run we verified the named hosts: input-host-Method (UMAP/t-SNE/MCL),
