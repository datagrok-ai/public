...
select
  ...
  r.result,
  r.duration,
  coalesce(t.owner, p.package_author, '') as owner
...
r.date_time = (select max(_r.date_time) from test_runs _r where _r.test_name = r.test_name and _r.build_name = r.build_name and not _r.stress_test) and not r.stress_test and r.ci_cd
