...
import {map, SYNTHESIZERS} from '../structures-works/map';
...
async function saveTableAsSdFile(table: DG.DataFrame) {
  ...
  for (let i = 0; i < table.rowCount; i++) {
    ...
    if (typeColumn.get(i) == SEQUENCE_TYPES.DUPLEX) {
      ...
      const as = sequenceToMolV3000(obj.AS, true, true, format, monomersLib) +
      '\n' + `> <Sequence>\nAnti Sense\n\n`;
      const ss = sequenceToMolV3000(obj.SS, false, true, format, monomersLib) +
      '\n' + `> <Sequence>\nSense Strand\n\n`;
