...
import {
  DEFAULT_PHOSPHOROTHIOATE, DEFAULT_SEQUENCE_LENGTH, MAX_SEQUENCE_LENGTH, USER_STORAGE_KEY, SENSE_STRAND, ANTISENSE_STRAND, STRAND_LABEL, STRANDS, TERMINAL, TERMINAL_KEYS, THREE_PRIME_END, FIVE_PRIME_END, PATTERN_FIELD as FIELD
...
class PatternBlock {
  ...
  getHtmlElements(): HTMLElement[] {
    ...
    const patternControlsBlock = [
      ...
      this.strandLengthInput[ANTISENSE_STRAND].root,
  ...
  private get createAsStrandInput(): DG.InputBase<boolean> {
    const createAsStrand = ui.boolInput(
      `${STRAND_LABEL.ANTISENSE_STRAND} strand`, true, (value: boolean) => {
        this.onCreateAsStrand.next(value)
      // modificationSection[ANTISENSE_STRAND].hidden = !v;
      // strandColumnInputDiv[ANTISENSE_STRAND].hidden = !v;
      // asLengthDiv.hidden = !v;
      // asModificationDiv.hidden = !v;
      // asExampleDiv.hidden = !v;
      // firstPto[ANTISENSE_STRAND].root.hidden = !v;
      // updateSvgScheme();
    });
  ...
  private get strandLengthInput(): {[strand: string]: DG.InputBase<number | null>} {
    const strandLengthInput = Object.fromEntries(STRANDS.map(
      (strand) => {
        const input = ui.intInput(
          `${STRAND_LABEL[strand]} length`, DEFAULT_SEQUENCE_LENGTH,
    ...
    this.onCreateAsStrand.subscribe((createAsCriterion: boolean) => {
      $(strandLengthInput[ANTISENSE_STRAND].root).toggle(createAsCriterion);
    })
