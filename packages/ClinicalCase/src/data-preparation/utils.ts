...
export function getUniqueValues(df: DG.DataFrame, colName: string) {
  ...
  const column = df.columns.byName(colName);
...
export function addDataFromDmDomain(df: DG.DataFrame, dm: DG.DataFrame,
  columnsToExtract: string[], columnsToExtractFromDm: string[], subjIdColName = SUBJECT_ID) {
  ...
  const withArm = grok.data.joinTables(df, dm, [subjIdColName],
    [SUBJECT_ID], columnsToExtract, columnsToExtractFromDm, DG.JOIN_TYPE.LEFT, false);
...
export function createPivotedDataframe(df: DG.DataFrame, groupByCols: string[],
  pivotCol: string, aggregatedColName: string, splitBy: string[]) {
  return df
...
export function createPivotedDataframeAvg(df: DG.DataFrame, groupByCols: string[],
  pivotCol: string, aggregatedColName: string, splitBy: string[]) {
  return df
    .groupBy(groupByCols.concat(splitBy))
    .pivot(pivotCol)
...
export function replaceNullsWithValues(df: DG.DataFrame, newValue: any) {
  ...
  columns.forEach((col) => {
    const column = df.columns.byName(col);
    const rowCount = df.rowCount;
    for (let i = 0; i < rowCount; i++) {
      if (column.isNone(i))
...
export function createTotalValuesForRowsAndCols(initTable: DG.DataFrame, groupCol: string, totalColName: string) {
  ...
  for (const column of totalValuesTable.columns) {
