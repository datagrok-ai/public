import {SUBJECT_ID, VISIT} from '../constants/columns-constants';
...
export class PatientVisit {
  ...
  updateSubjectVisitDomains(domains: any) {
    this.atVisit.forEach((it) => {
      if (domains[it] && domains[it].columns.names().includes(VISIT)) {
        this[it] = domains[it]
          .groupBy(domains[it].columns.names())
          .where(`${SUBJECT_ID} = ${this.currentSubjId} and ${VISIT} = ${this.currentVisitName}`)
          .aggregate();
      }
    });
