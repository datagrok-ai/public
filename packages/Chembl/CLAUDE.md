...
- **`src/demo.ts`** — `_demoDatabasesChembl()` builds a two-tab view (input form + raw SQL) that
  reruns `AtcClassificationWithSubstructure` whenever an input changes.
...
| **ATC classification** | `atc_classification` (`level1..level4_description`, `level5` code, `who_name`), `molecule_atc_classification` | WHO Anatomical Therapeutic Chemical hierarchy: anatomical group → therapeutic → pharmacological → chemical subgroup → substance. Level 3 often repeats the level 2 label and level 4 labels recur under different parents, so filter on every chosen level. | Powers the `AtcClassification*` queries and the `Database Queries` demo. |
