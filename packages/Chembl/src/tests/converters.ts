...
async function compare(input: string, expectArr: Array<any>, callMethod: string, columnName: string) {
  ...
  const dfResult: DG.DataFrame = await grok.functions.call(callMethod, {'ids': df});
...
category('Converting queries', () => {
  ...
  test('converter InchiKey to Chembl', async () => {
    ...
    await compare(INCHI_KEY_CSV, expectArr, `${_package.name}:inchiKeyToChembl`, 'chembl_id');
  }, {stressTest: true});
  ...
  test('converter InchiKey to Smiles', async () => {
    ...
    await compare(INCHI_KEY_CSV, expectArr, `${_package.name}:inchiKeyToSmiles`, 'canonical_smiles');
  }, {stressTest: true});
  ...
  test('converter InchiKey to Inchi', async () => {
    ...
    await compare(INCHI_KEY_CSV, expectArr, `${_package.name}:inchiKeyToInchi`, 'standard_inchi');
  }, {stressTest: true});
  ...
  test('converter Chembl to Smiles', async () => {
    ...
    await compare(CHEMBL_ID_CSV, expectArr, `${_package.name}:chemblToSmiles`, 'canonical_smiles');
  }, {stressTest: true});
  ...
  test('converter Chembl to Inchi', async () => {
    ...
    await compare(CHEMBL_ID_CSV, expectArr, `${_package.name}:chemblToInchi`, 'standard_inchi');
  }, {stressTest: true});
  ...
  test('converter Chembl to InchiKey', async () => {
    ...
    await compare(CHEMBL_ID_CSV, expectArr, `${_package.name}:chemblToInchiKey`, 'standard_inchi_key');
  }, {stressTest: true});

  test('chemblIdToSmiles', async () => {
    const query = `${_package.name}:chemblIdToSmiles`;
  ...
  test('molregnoToSmiles', async () => {
    const query = `${_package.name}:molregnoToSmiles`;
