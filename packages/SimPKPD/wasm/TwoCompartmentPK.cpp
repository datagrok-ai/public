...
extern "C" {
     int solveTwoCompartmentPK(float initial, float final, float step,
...
namespace TwoCompartmentPK
{
    ...
    VectorXd _f(double t, VectorXd & _y) noexcept
    {
        ...
        // output computation
        _res(0) =  - KA * depot;
        _res(1) =  KA * depot - CL * C2 - Q * C2 + Q * C3;
        _res(2) =  Q * C2 - Q * C3;
...
int solveTwoCompartmentPK(float initial, float final, float step,
    ...
    float * _solution, int _solutionRowCount, int _solutionColCount) noexcept
{
    using namespace TwoCompartmentPK;
