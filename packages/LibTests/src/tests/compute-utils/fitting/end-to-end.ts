// End-to-end recovery tests via runOptimizer.
//
// Uses synthetic JS-language scripts with known (a*, b*, ...) and asserts
// the optimizer recovers them within tolerance. Each test runs once per
// executor in ALL_EXECUTORS — the worker arm exercises the same fixtures
// via runOptimizer's `executor` parameter.
...
import {ALL_EXECUTORS} from './executors';
...
for (const executor of ALL_EXECUTORS) {
  category(`ComputeUtils: Fitting / End-to-end (${executor})`, () => {
