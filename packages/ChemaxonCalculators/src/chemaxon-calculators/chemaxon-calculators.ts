...
const _STORAGE_NAME = 'chemaxon_calculations';
const _KEY = 'selected';

export function getCalculationsSingle(smiles: string): DG.Widget {
...
function openModelsDialog(selected: any, onOK: any): void {
  ...
  for (const groupName of keys) {
    ...
    for (const calculator of categories[groupName]['calculators']) {
      const item = group.item(calculator['name'], calculator);
      item.enableCheckBox(selected.includes(calculator['name']));
      items.push(item);
      item.root.appendChild(ui.divText('hello'));
  ...
  ui.dialog('Chemaxon Calculators')
