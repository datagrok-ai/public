...
There is a queryable knowledge graph of every plugin, function, script, query, library, doc page, tutorial, changelog entry, and Jira ticket in this repository. It lives at `.kg/`, committed directly in the `public` repo as a ~2.4 MB compressed binary (`kg.kuzu.xz`). Stored file paths are anchored at `./` — e.g. a graph result `packages/Bio/src/foo.ts` is on disk at `packages/Bio/src/foo.ts`. Use it as your **first lookup** when you need to:
...
## How to query — just run `qq.py` with system Python

Run it with whatever Python you have (`python3` or `py`) — **no setup needed**. On the
first call, `qq.py` self-installs the query engine into a local `.kg/.venv` (one-time,
~30s), restores the `kg.kuzu` database from the committed `kg.kuzu.xz` (~1s), and starts
a persistent server on `127.0.0.1:7475`. Every call after that returns in ~5–50ms.
You never run a bootstrap step or `pip install` yourself — `qq.py` handles it.

```bash
# First call: self-installs engine + unpacks DB + spawns server, then queries
...
python3 .kg/scripts/qq.py "MATCH (n) RETURN label(n), count(n) ORDER BY 2 DESC LIMIT 5"

# Server status
python3 .kg/scripts/qq.py                    # show node/edge counts + uptime

# Raw JSON (for piping into another script)
python3 .kg/scripts/qq.py --json "MATCH (f:Feature) WHERE f.name CONTAINS 'Activity' RETURN f.id, f.name"

# Restart (e.g. after a rebuild)
python3 .kg/scripts/qq.py --restart
python3 .kg/scripts/qq.py --stop             # release the DB lock
```
...
```powershell
python3 .kg/scripts/query.py --demos
```
