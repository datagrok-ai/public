...
7. **Invoke a server-side function.**
   ...
   404s — the actual segment is `/public/v1/functions/{name}/call`
   (drift `DG-FACT-DRIFT-090`). Body is the parameter object as JSON.
   ```bash
   curl -fsS -X POST \
        -H "Authorization: $API_KEY" \
        -H "Content-Type: application/json" \
...
- **`404 Not Found` calling a function.** You followed the article path
  `/public/v1/{name}/call` literally (drift `DG-FACT-DRIFT-090`). Fix:
...
- Knowledge:
  - `docs/_internal/knowledge/knowledge-graph.md` — facts
    ...
    URL), and drift `DG-FACT-DRIFT-090` (function-call path).
- Related skills:
