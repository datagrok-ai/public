...
4. **Inspect available properties via `getProperties()` — use `columnTypeFilter`.**
   ...
   backward-compat alias (`DG-FACT-DRIFT-078`):
   ```typescript
...
- **`p.columnFilter` is `undefined` in `getProperties()`.** Accessor
  was renamed to `columnTypeFilter`; no alias (`DG-FACT-DRIFT-078`).
...
- Knowledge: `docs/_internal/knowledge/knowledge-graph.md` — facts
  `DG-FACT-198`…`DG-FACT-206`, drifts `DG-FACT-DRIFT-078`/`079`/`080`.
- Reference packages:
