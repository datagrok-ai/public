...
1. **Substructure search → BitSet.**
   ...
   shown in the article is not read by the wrapper (drift
   `DG-FACT-DRIFT-093`).
...
3. **Score every row, or prime the fingerprint cache.**
   ...
   `DG-FACT-DRIFT-094`) of length equal to the input column; null
...
4. **Pick a diverse subset.**
   ...
   despite what the article shows (drift `DG-FACT-DRIFT-091`).
...
5. **Compute the Maximum Common Substructure (MCS).**
   ...
   Column where a DataFrame is expected and will fail at runtime (drift
   `DG-FACT-DRIFT-092`).
...
- **`searchSubstructure` is missing rows you expected.** Unparseable
  ...
  dropped by the wrapper (drift `DG-FACT-DRIFT-093`).
- **`mcs(column)` throws / returns garbage.** Signature is
  `(DataFrame, columnName, ...)`, not `(Column)` (drift
  `DG-FACT-DRIFT-092`). Fix: pass the table and column name string.
- **`diversitySearch(col, METRIC_TANIMOTO, 10)` — `METRIC_TANIMOTO is
  undefined`.** The constant doesn't exist; second arg is a settings
  object (drift `DG-FACT-DRIFT-091`). Fix: `(col, { limit: 10 })`.
- **`getSimilarities(col, null)` returns nothing useful.** Cache-priming
  expects `''`, not `null` (drift `DG-FACT-DRIFT-094`).
...
- Knowledge:
  - `docs/_internal/knowledge/knowledge-graph.md` — facts `DG-FACT-235`
    through `DG-FACT-241`, drifts `DG-FACT-DRIFT-091..094`.
- Reference samples:
