...
1. **Identify the extension type from the intended behavior.**
   ...
   | Autostart hook | `autostart` (optional `meta.autostartImmediate: true`) | function-roles.md §Autostart | `packages/Chembl`, `packages/PowerPack` | `DG-FACT-DRIFT-021` |
...
3. **Declare the function role explicitly in the source.**
   ...
   undocumented and inconsistent with canonical packages
   (drift `DG-FACT-DRIFT-021`) — split into two functions or pick one.
   Decorator form is canonical for modern packages (drifts
   `DG-FACT-DRIFT-028` for fileExporter, `DG-FACT-DRIFT-036` for
   folderViewer, `DG-FACT-DRIFT-039` for tooltip, `DG-FACT-DRIFT-042`
   for cellRenderer); the codegen still emits header-form into
   `package.g.ts`.
   ```typescript
...
- **Compound `init, autostart` annotation.** Article
  `register-identifiers.md` shows it but no canonical package uses it
  (drift `DG-FACT-DRIFT-021`). Fix: declare ONE role per function;
...
- Knowledge: `docs/_internal/knowledge/knowledge-graph.md` — facts
  ...
  `173`, `186-197`, `213-217`); drifts `DG-FACT-DRIFT-021` (init+autostart
  compound), `028/036/039/042` (article-vs-canonical decorator drift).
