...
3. **Run docking from the UI.** Open the dialog from the top menu —
   ...
   shortens this to "Chem > Autodock"; drift `DG-FACT-DRIFT-095`).
   Fields:
...
- **Article says "Chem > Autodock", menu is empty.** The actual menu
  path is **Chem | Docking | AutoDock...** (drift `DG-FACT-DRIFT-095`).
- **Field labelled "Conformations" in the article doesn't exist.**
  The dialog input is **Poses** (parameter name `poses`, drift
  `DG-FACT-DRIFT-095`).
...
- Knowledge:
  - `docs/_internal/knowledge/knowledge-graph.md` — facts
    `DG-FACT-242`..`DG-FACT-247`, drift `DG-FACT-DRIFT-095`.
- Reference packages:
