...
2. **Pick the entry point from the task.**
   ...
   | Custom semantic-type renderer | `DG.ObjectHandler` (NOT `EntityMeta`) | `DG-FACT-059/060`, drift `DG-FACT-DRIFT-087` |
...
- **`grok.testData(...)` throws "is not a function".** The article's
  ...
  rows?, cols?)` (drift `DG-FACT-DRIFT-088`, see also `DG-FACT-008` for the
  allowed dataset-name enum). Fix: prefix with `grok.data.`.
- **`extends DG.EntityMeta` fails to compile.** That class does not
  ...
  `DG.ObjectHandler.register(handler)` (drift `DG-FACT-DRIFT-087`,
  knowledge `DG-FACT-059/060`). Fix: subclass `ObjectHandler`; see the
...
- **`grok.dapi.fetchProxy(...)` example URL `restcountries.eu` returns
  ...
  moved to `restcountries.com` (drift `DG-FACT-DRIFT-089`). Fix: swap the
...
- Knowledge: `docs/_internal/knowledge/knowledge-graph.md` — facts
  ...
  drifts `DG-FACT-DRIFT-087` (EntityMeta → ObjectHandler),
  `DG-FACT-DRIFT-088` (testData prefix), `DG-FACT-DRIFT-089` (dead URL).
