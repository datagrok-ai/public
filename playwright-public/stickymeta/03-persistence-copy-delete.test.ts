...
test('Sticky Meta: persistence across copy / clone / reload, and delete', async ({ page }) => {
  ...
  try {
    ...
    // CI injects auth via a token (no DATAGROK_LOGIN/PASSWORD), so the UI relogin can't run there —
    // and the logout would invalidate the shared token for any following test. Run this sub-scenario
    // only when interactive credentials are available (dev). Refresh persistence (3.3b) above already
    // covers reload survival without a logout.
    if (LOGIN && PASSWORD) {
