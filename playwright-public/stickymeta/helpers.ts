...
export async function gotoHome(page: Page): Promise<void> {
  ...
  // CI authenticates via an injected token and sets no DATAGROK_LOGIN/PASSWORD, so a UI relogin is
  // only possible on dev. If the login form is shown without credentials available, there is nothing
  // to self-heal with — fall through to the ribbon wait (which surfaces the real auth problem).
  if (await loginInput.isVisible().catch(() => false) && process.env.DATAGROK_LOGIN && process.env.DATAGROK_PASSWORD) {
...
export async function apiDeleteSchema(page: Page, name: string): Promise<void> {
  await page.evaluate(async (name) => {
    ...
    if (!schema) return;
    const id = (window as any).grok_Entity_Get_Id(schema.dart);
