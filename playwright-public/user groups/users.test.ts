...
// management-target entities, so this set actively CREATES users (Users-05/07) and seeds the
...
// the per-test pages don't race on that shared server-side state.
test.describe.configure({ mode: 'serial' });

test.describe('Users View (Users-*)', () => {
  // Seed the shared targets once for the whole file (fresh CI DB has none of them).
  test.beforeAll(async ({ browser }) => {
    const ctx = await browser.newContext({ storageState: 'e2e/.auth.json' });
    const page = await ctx.newPage();
    try {
      await ensureUserSeeded(page, TARGET_USER);
      await ensureGroupSeeded(page, TEST_GROUP);
      await ensureRoleSeeded(page, TEST_ROLE);
    } finally {
      await ctx.close();
    }
  });
  ...
  // Users-03 + Users-04 + Users-06 + Users-08: every NEW create dialog, fields + validation,
  // each cancelled (the dialog/validation surface — actual creation is Users-05/07 below).
  ...
  // Users-05: create a real user through the UI and confirm it lands in the gallery. The fresh CI
  // DB makes this safe (the instance is torn down after the run); users have no hard delete.
  test('Users-05 — create a new user via the UI (active creation)', async ({ page }) => {
  ...
  test('Users-07 — create a new service user via the UI (active creation)', async ({ page }) => {
    const login = `ci_svc_${STAMP}`;

    await createServiceUserViaUI(page, login);

    await openPlatformView(page, 'Users');
    await searchAndWaitCard(page, 'users', login);
    await expect(galleryCardByName(page, login), 'created service user should appear in the gallery').toBeVisible();
  });

  test('Users-09 — search by name filters list and updates URL', async ({ page }) => {
    const before = await readGalleryCount(page);
    await searchGallery(page, 'users', TARGET_USER);
    await expect(page).toHaveURL(new RegExp(`/users\\?q=${TARGET_USER}`));
