...
test('Pie chart tests', async ({page}) => {
  ...
  await softStep('Donut mode', async () => {
    const result = await page.evaluate(async () => {
      ...
      const rendered = (act: () => void) => w.__settled('viewer:Pie chart.onViewerRendered', act, 2000);
      const r: any[] = [];

      const asPie = canvas.toDataURL();
      await rendered(() => { pie.props.mode = 'Donut'; });
      r.push(pie.props.mode, canvas.toDataURL() !== asPie);

      await rendered(() => { pie.props.centerLabel = 'Race'; });
      const withCenterLabel = canvas.toDataURL();
      await rendered(() => { pie.props.showCenterLabel = false; });
