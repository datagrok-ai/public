...
test('Trellis plot — layout round-trip, project save dialog, Multi Curve inner viewer, To Script', async ({page}) => {
  ...
  await softStep('To Script', async () => {
    ...
    // "To Script" is contributed by the DevTools plugin's context-menu hook; the minimal CI stack may
    // not load DevTools. A menu that opened with its usual items but without "To Script" is that case.
    const menu = await page.evaluate(() => {
      const popup = [...document.querySelectorAll('.d4-menu-popup')].pop();
      const labels = [...(popup?.querySelectorAll('.d4-menu-item-label') ?? [])].map((e) => (e.textContent ?? '').trim());
      return {count: labels.length, hasToScript: labels.includes('To Script')};
    });
    if (!menu.hasToScript && menu.count > 15) {
