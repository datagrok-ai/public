...
import {armBalloonRecorderProved, expectNoBalloonSinceArmed} from '@datagrok-libraries/test/src/playwright/balloons';
import {PIVOT, openPivot, pivotProps, rowChips} from './pivot-helpers';
...
// The event is armed BEFORE the click; for a click that must not filter, the cap is the hold.
async function clickAggregateCell(page: Page, rowIdx: number) {
  const r = await innerGridRect(page);
  const filtered = await v.armEvent(page, 'df.onRowsFiltered', 500);
...
async function ctrlClickAggregateCell(page: Page, rowIdx: number) {
  const r = await innerGridRect(page);
  const selected = await v.armEvent(page, 'df.onSelectionChanged', 500);
...
// the pivot re-aggregates on the filter event; its render is armed before the filter so it cannot be missed
async function filterToMales(page: Page, renderCapMs = 900) {
  const rendered = await v.armEvent(page, 'viewer:Pivot table.onViewerRendered', renderCapMs);
  await page.evaluate(() => {
    const df = grok.shell.tv.dataFrame;
...
const avgAgeByDisPop = (page: Page, filtered: boolean) => page.evaluate((useFilter: boolean) => {
  const df = grok.shell.tv.dataFrame;
  const q = df.groupBy(['DIS_POP']).avg('AGE');
  const g = (useFilter ? q.whereRowMask(df.filter) : q).aggregate();
...
async function resetFilterAndSelection(page: Page) {
  await page.evaluate(() => {
    const df = grok.shell.tv.dataFrame;
  ...
  }), (s) => s.sel === 0 && s.filt === DEMOG_ROWS, 2000, 50);
}

const selectedGroups = (page: Page) => page.evaluate(() => {
  const df = grok.shell.tv.dataFrame;
  const disPop = df.col('DIS_POP');
  const selected = new Set<string>();
  for (let i = 0; i < df.rowCount; i++) if (df.selection.get(i)) selected.add(disPop.get(i));
  return {sel: df.selection.trueCount as number, keys: [...selected], filt: df.filter.trueCount as number};
});
...
test('Pivot Table — Row Source, filter and selection links', async ({page}) => {
  ...
  await softStep('Scenario 1 Step 4: after a SEX = "M" panel filter, the pivot aggregate matches an independent groupBy on the filtered df', async () => {
    const baseline = await avgAgeByDisPop(page, false);

    await page.evaluate(() => grok.shell.tv.getFiltersGroup());
    await page.locator('[name="viewer-Filters"] .d4-filter').first().waitFor({timeout: 15000});
    await filterToMales(page, 900);

    const filtered = await avgAgeByDisPop(page, true);
  ...
  await softStep('Scenario 1 Step 6: clear the SEX filter and close the published table before the next scenario', async () => {
    ...
    await page.waitForFunction(() => grok.shell.tv?.dataFrame?.rowCount === 5850, null, {timeout: 5000});
    await page.evaluate(() => grok.shell.tv.dataFrame.filter.setAll(true));
    expect(await v.pollValue(() => srcFilterCount(page), (n) => n === DEMOG_ROWS, 2000, 50)).toBe(DEMOG_ROWS);
  });

  await softStep('Scenario 2 Step 3: Row Source = All with filteringEnabled = true does not widen the aggregated row set', async () => {
    await filterToMales(page);
  ...
  await softStep('Scenario 2 Step 5: a real cell click narrows df.filter.trueCount to a smaller positive value', async () => {
    ...
    const after = await v.pollValue(() => srcFilterCount(page), (n) => n > 0 && n < before, 3000, 100);
  ...
  await softStep('Scenario 4 Step 4: at Row Source = Filtered, a Ctrl+click on an aggregate row selects that group\'s source rows (df.selection)', async () => {
    await resetFilterAndSelection(page);
    await setupSingleKey(page, 'Filtered', true);
    expect(await srcSelectionCount(page)).toBe(0);

    expect(await rowChips(page, 'Group by')).toEqual(['DIS_POP']);
    expect(await rowChips(page, 'Aggregate')).toEqual(['avg(AGE)']);
    ...
    const after = await v.pollValue(() => selectedGroups(page), (s) => s.sel > 0 && s.keys.length === 1, 3000, 100);
