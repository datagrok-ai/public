...
import {isLocalBootNoise, openDatagrok, specTestOptions, softStep} from '@datagrok-libraries/test/src/playwright/spec-login';
...
test('Grid — Row Selection, Filter, and Keyboard Navigation', async ({page}) => {
  ...
  try {
    ...
    await softStep('Step 3 — click row 5 data cell: sets current row, no selection', async () => {
      ...
      const p = await g.cellCenter(page, col, 5);
      await page.mouse.click(p.x, p.y);
      await waitDfEvent(page, 'onCurrentCellChanged', 300);
      const r = await page.evaluate(() => {
        const df = grok.shell.tv.dataFrame;
        const grid = grok.shell.tv.grid;
        return {currentRowIdx: df.currentRowIdx, expectedTableIdx: grid.gridRowToTable(5), selTrueCount: df.selection.trueCount};
      });
    ...
    await softStep('Step 4 — drag-select data rows 5..10: range select 6 rows', async () => {
      ...
      const r = await page.evaluate(() => {
        ...
        const grid = grok.shell.tv.grid;
        return {
          selected: Array.from(df.selection.getSelectedIndexes()) as number[],
    ...
    await softStep('Step 10 — rapid Ctrl+clicks across 5+ column headers: no console error (GROK-17455)', async () => {
      ...
      for (const col of ['AGE', 'HEIGHT', 'WEIGHT', 'SEX', 'RACE', 'DIS_POP']) {
        ...
        await page.mouse.click(p.x, p.y, {modifiers: ['Control']} as any);
    ...
    await softStep('Step 11 — Shift+drag over data cells: selects rows only, no column selection', async () => {
      ...
      const start = await g.cellCenter(page, 'AGE', 2);
      const end = await g.cellCenter(page, 'AGE', 6);
    ...
    await softStep('Step 13 — arrows: Down x2 then Right x2 move current row/col', async () => {
      const p = await g.cellCenter(page, 'AGE', 4);
      await page.mouse.click(p.x, p.y);
    ...
    await softStep('Step 17 (also proves the Step 5 toggle-membership channel) — Space toggles current row selection membership', async () => {
      const p = await g.cellCenter(page, 'AGE', 3);
      await page.mouse.click(p.x, p.y);
      await waitDfEvent(page, 'onCurrentCellChanged', 200);
      await page.evaluate(() => grok.shell.tv.dataFrame.selection.setAll(false));
      await g.focusGrid(page);
    ...
    await softStep('Step 22 — 2 pinned rows + scroll + drag-select: selected indices are the intended table rows', async () => {
      ...
      await page.evaluate(async () => {
        ...
        await (window as any).__quiet('viewer:Grid.onAfterDrawContent', 200, 500);
      });
      await page.evaluate(() => grok.shell.tv.dataFrame.selection.setAll(false));
      await g.focusGrid(page);
      ...
      const geom = await page.evaluate(() => {
        ...
        return {start: {x: stripX, y: rowY(startSlot)}, end: {x: stripX, y: rowY(startSlot + 2)}, intended};
      });
      ...
      await page.mouse.up();
      await waitDfEvent(page, 'onSelectionChanged', 400);
      const r = await page.evaluate(() => ({selected: Array.from(grok.shell.tv.dataFrame.selection.getSelectedIndexes()) as number[]}));
      const selectedSet = [...r.selected].sort((a, b) => a - b);
    ...
    await softStep('Step 24 — sort + filter: navigate via arrows, gridRowToTable yields correct table indices', async () => {
      const filtCount = await page.evaluate(async () => {
        ...
        await w.__settled('df.onRowsFiltered', () =>
          fg.updateOrAdd({type: DG.FILTER_TYPE.HISTOGRAM, column: 'AGE', min: 31, max: 999}), 1200);

        grid.props.rowSource = 'Filtered';
        await w.__quiet('viewer:Grid.onAfterDrawContent', 200, 400);
        await new Promise<void>((resolve) => {
          const sub = grid.onRowsSorted.subscribe(() => { sub.unsubscribe(); resolve(); });
          setTimeout(() => { sub.unsubscribe(); resolve(); }, 600);
          grid.sort(['AGE'], [true]);
        });
        w.__filtBaseline = df.filter.trueCount;
    ...
    await softStep('Step 25 — keyboard range-select under sort+filter: selected indices are the intended table rows', async () => {
      await page.evaluate(() => grok.shell.tv.dataFrame.selection.setAll(false));
      const anchorCell = await g.cellCenter(page, 'AGE', 0);
    ...
    await softStep('Step 26 — rowSource All → Filtered → Selected → All; df.filter unchanged throughout', async () => {
      const setup = await page.evaluate(async () => {
        ...
        df.filter.setAll(true);
        await w.__settled('df.onRowsFiltered', () =>
          fg.updateOrAdd({type: DG.FILTER_TYPE.CATEGORICAL, column: 'SEX', selected: ['M']}), 1200);
        df.selection.setAll(false);
      ...
      // the visible row count settles a beat after rowSource changes, so wait for it to leave the previous value
      const setSource = (src: string) => page.evaluate(async (s) => {
        const w = window as any;
        const grid = grok.shell.tv.grid;
        const df = grok.shell.tv.dataFrame;
        const visible = () => {
        ...
        grid.props.rowSource = s;
        return {visible: await w.__moved(visible, before, 400, 50), filter: df.filter.trueCount};
      }, src);

      const filtered = await setSource('Filtered');
      expect(filtered.visible).toBe(setup.filterCount);

      const selected = await setSource('Selected');
      expect(selected.visible).toBe(setup.selCount);

      const all = await setSource('All');
      const rowCount = await page.evaluate(() => grok.shell.tv.dataFrame.rowCount);
      expect(all.visible).toBe(rowCount);

      expect(filtered.filter).toBe(setup.filterCount);
      expect(selected.filter).toBe(setup.filterCount);
      expect(all.filter).toBe(setup.filterCount);
    });
    ...
    await softStep('Step 27 — Ctrl+F search: matching cells tint, then the filter is restored', async () => {
      ...
      const restored = await page.evaluate(async () => {
        ...
        const df = grok.shell.tv.dataFrame;
        df.filter.setAll(true);
        return (await w.__poll(() => df.filter.trueCount, (n: number) => n === df.rowCount, 400, 50)) === df.rowCount;
      });
