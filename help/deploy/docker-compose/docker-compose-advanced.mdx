...
2. Pin image versions for the second stand. Any tag from
   ...
   [Images and versions](../images.md) for the latest pinned set, or
   [release history](../releases/release-history.md) for older releases.

   Each service has its own environment variable: `DATAGROK_VERSION`,
   `GROK_PIPE_VERSION`, `GROK_SPAWNER_VERSION`, `GROK_CONNECT_VERSION`,
   `JKG_VERSION`, `GROK_REGISTRY_PROXY_VERSION`. They all default to
   `bleeding-edge`. To pin a stand to the latest stable release:
   ...
   export GROK_PIPE_VERSION=1.19.0
   export GROK_SPAWNER_VERSION=2.16.0
