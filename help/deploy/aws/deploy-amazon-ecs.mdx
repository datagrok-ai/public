...
sidebar_position: 2
description: Deploy Datagrok on an AWS ECS Fargate cluster with RDS and S3 using a CloudFormation template.
keywords:
  - aws ecs
  - fargate
...
`RabbitmqVersion`) for picking the container image tag. The defaults match the
[EKS template](deploy-amazon-eks.mdx) — both templates ship the same images. See
[Images and versions](../images.md) for the canonical set and pinning conventions across
deployment paths.
