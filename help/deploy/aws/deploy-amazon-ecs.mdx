...
:::caution Avoid for new installations

The ECS path is stable and supported, but is targeted for deprecation in a future release. For new stands, prefer the
[AWS CloudFormation (EKS)](deploy-amazon-eks.mdx) template — it provisions the same RDS + S3 logical resources
and installs the [Helm chart](../k8s/install-helm-chart.md) on EKS. The two templates share resource IDs, so an
existing ECS stack can be migrated to EKS in place via *Update stack → Replace current template* without
re-creating the database or bucket.
...
defaults track the most recent Datagrok release on Docker Hub and match the
[EKS template defaults](deploy-amazon-eks.mdx#service-versions) — both templates ship the same
images:
...
[Release history](../releases/release-history.md) lists all available tags.

## Update Datagrok components

You can update your Datagrok deployment without re-creating infrastructure. Before updating, back up
the database and persistent storage.

### How to update

1. Click **Update** > **Replace current template**, and provide the new template URL corresponding
   to your deployment configuration.
1. Specify new image tag versions (see [Release history](../releases/release-history.md)).
1. Click **Next**, skip optional settings, and proceed to **Review**.
1. If the stack enters a failed state (e.g., `UPDATE_ROLLBACK_IN_PROGRESS`),
   [check events](https://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/troubleshooting.html#basic-ts-guide)
   for details.
...
- Customisations applied via stack parameters persist; environment-variable overrides applied
  outside the stack do not.
