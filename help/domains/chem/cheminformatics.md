...
deployed, reused by other scientists, and incorporated in pipelines or info panels.<!--corr: what is
info panel? a link/description needed --> Predictive models for _toxicity_ and _drug-likeness_ are also
supported.
...
Though it might seem that cheminformatics covers all the "molecular" needs <!--corr: a very vague formulation-->,
...
to: docking and molecular dynamics, systems biology and pharmacology, bioinformatics, proteomics.<!--corr:
...
users the set of tools for work with such graphs. <!--corr: Which ones? Examples, please-->  From the
...
of 2D projections of descriptor vectors, or _planar maps_<!--corr: is it a correct term?-->. Such maps
...
structure-property relationship (QSPR) models. Datagrok provides the following properties<!--corr: related
to QSPR?--> : formula, drug likeness, acceptor count, donor count, logP, logD, polar surface area, rotatable
...
in each element possible<!--corr: what element? what structure?-->. It can also handle special chemical
queries for subgraphs. This is apt in filtering of chemical datasets, because a subgraph may not be related
to existing structure (e.g. query can have a single aromatic atom or bond)<!--corr: what?--> . Each time
...
of similar compounds. MCS feature is integrated into several Datagrok tools<!--corr: list!-->, with sketching
options.
...
The methods described above<!--corr: examples, please--> assume that descriptors and graphs correspond
...
compound identification and lead compound optimization. (Q)SAR<!--corr: what is it?--> studies are performed
...
Datagrok supports machine learning [predictive models](../../learn/data-science.md#predictive-modeling),
...
The developed model<!--corr: which one??--> is able to get the possible hits from the screened dataset
