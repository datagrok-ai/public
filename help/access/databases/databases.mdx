...
You can cache query results to improve query performance. To learn more, see [Caching function results](../../develop/function_results_cache.md)
...
The syntax for defining query parameters is based on [functions syntax](../../datagrok/concepts/functions/func-params-annotation.md)
...
A special query input type is a [filter pattern](../../datagrok/concepts/functions/func-params-annotation.md) that allows you to enter free-text conditions
like "this week" for dates, or ">50" for numbers.
...
1. Select how to store data:
    ...
    * Store the data as a generation script by toggling the **Data sync** control. The query re-executes each time the project is opened. To learn more about dynamic data updates in projects, see [Dynamic data](../../collaborate/project.md#dynamic-data).
...
In Datagrok, certain objects called _entities_ have a [set of operations](../../datagrok/concepts/objects.md) that can be applied to them. These entities are connections, queries, tables, and table columns, all of which can be shared, assigned permissions, annotated, and more.
...
To open debug section follow the next steps:

<img alt="Open debug section" src={require('./debug-section-open-saved-query.gif').default} width="800px"/>

If you have a saved data query:

1. Go to **Data > Databases**.
2. In the **Database Manager**, click the desired connector, choose saved connection, and click on saved query.
3. Activate **Context Panel** with F2 keyboard shortcut or with the help of **Windows > Context Panel**.
4. Click on **Debug** section.

:::tip

Additionally, you can open debug window by right-clicking on saved query and clicking on Debug

:::

<img alt="Open debug section" src={require('./debug-section-open-new-query.gif').default} width="800px"/>

If you don't have a saved query:

1. Go to **Data > Databases**.
2. In the **Database Manager**, click the desired connector, choose saved connection and click **Add query**.
3. Write your query and click on Debug icon

### Debug section parameters

There are two parameters that can significantly influence execution time of query - **Fetch size** and **Init fetch size**.

**Fetch size**. Gives the JDBC driver a hint as to the number of rows that should be fetched from the database
when more rows are needed for this ResultSet object. If the fetch size specified is zero, the JDBC driver ignores the value
and is free to make its own best guess as to what the fetch size should be.
Grok Connect under the hood not only uses this value as fetch size for ResultSet but also as the number
of rows that will be sent to the server in one WebSocket message. And consequently, this value for User-Agent equals
the number of rows that will be rendered on the screen by 1 workaround.
By default, this value is calculated automatically by Grok Connect based on the average size of the row to achieve a maximum message size of 10 Mb
that is sent to the server by WebSocket. This parameter can be specified in megabytes to limit the maximum message size.

:::note

To get consistent results for debugging **Fetch size** parameter should be specified.

:::

To specify **Fetch size** for debugging use the corresponding input field in the debug section.
If you want to run the query with this parameter every time, add to the query declaration an additional meta parameter.
...
Or if you want to set it in megabytes:
...
**Init fetch size**. It is the same parameter as Fetch size, but it is set on Statement and
influences the time taken to execute the Statement and the time of rendering of first rows of the DataFrame on the screen.
The default value is 100.
To specify **Init fetch size** for debug use the corresponding input field in the debug section.

:::note

**Init fetch size** will not influence debug run. So it is not mandatory as Fetch size.
...
Init fetch size can be set only in numbers.

:::

**Optimize fetch size**. When checked disables other inputs and runs several queries with different fetch sizes and displays in the end times of execution for each run.

### Debug run

When you press the **Debug** button with **Optimize fetch size** unchecked, under the hood Grok Connect runs 3 queries.
First two without processing ResultSet and the last with processing. It adds 2 additional parameters to the Log Summary (see [Query logs](#query-logs)) of the query:

1. **DRY_RUN** - time taken on reading ResultSet without interpretation of its values and filling of columns of DataFrame.
2. **COLUMN_FILL** - difference between **RESULT_SET_PROCESSING_WITH_COLUMN_FILL** and **DRY_RUN**. It is close to real
time taken to read ResultSet and process all its values.

In general, Debug run helps to estimate the overhead of the interpretation of ResultSet values and filling of DataFrame columns.

:::note

The stability of network connection or in the case of local database load of SSD/HDD can significantly influence the result.
So for best evaluation try to run several consecutive Debug runs.

:::

### Query logs

When you run a query in debug mode you get results of execution and logs. Log Summary consists of various parameters that describe times of different stages of query execution.
These metrics are helpful in the evaluation of the influence of different factors such as Networking throughput,
the complexity of data etc. Logs can be viewed for saved queries at any time in the Log section.
...
* Tutorials
  * [Adding parameters to functions](../../datagrok/concepts/functions/func-params-enhancement.md)
