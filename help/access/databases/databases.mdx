...
Query debugging helps you fix and improve query performance.

To start debugging:

1. Right-click on the saved query and select **Debug**. The form appears.
2. Fill in **Fetch size** input. This parameter sets the number of rows the JDBC driver fetches from the database and sends to the server in one WebSocket message. It also affects how many rows are shown on the screen in one workaround. You can also specify this parameter in megabytes to limit the message size.
3. Optionally fill in **Init fetch size** input. This parameter is the same as **Fetch size**, but it applies to [Statement](https://docs.oracle.com/javase/8/docs/api/java/sql/Statement.html) and affects how fast the Statement executes and the first rows of the DataFrame appear on the screen.
4. Check **Optimize fetch size** to run the query with several predefined **Fetch sizes** and get execution times.
5. Press **OK**. Debugging starts.

:::note

You can debug queries in [Query Editor](#query-editor) by clicking on the "bug" icon on top of the editor. Also, you can start debugging from the **Context Panel** using the **Debug** section.

:::

When you finish debugging with **Optimize fetch size** off, you will see the execution results, **Summary**, and detailed logs. **Summary** shows the times of different stages of query execution. These metrics help you evaluate how factors like network speed, data complexity, etc. affect performance. **Summary** also explains what each metric means.

:::note

The network connection or the local disk load can affect the result a lot. For better evaluation, run **Debug** several times in a row.
...
To set a constant value for **Fetch size** and **Init fetch size** after analyzing **Summary** or **Optimize fetch size** results, follow these steps:

1. Open the saved query in [Query Editor](#query-editor).
2. Add the meta parameters *initConnectFetchSize* or *connectFetchSize* with the desired value to the query declaration.
3. Save query.
