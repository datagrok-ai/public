...
**Bin...** option lets you automatically bin numerical columns into ranges, given the desired number of bins.

**APPLY COLOR CODING** option lets you color-code the rows of the source column based on the category they belong to.

**SAVE COLUMN** Creates a "category" column and assigns each row the category of the
first matching pattern. If the color coding checkbox is enabled, the added column will contain specified color coding. This option is mostly useful when categories do not overlap. If they do, consider using
**SAVE MATRIX**.
