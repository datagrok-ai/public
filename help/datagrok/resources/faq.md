...
Open **Browse**. **Databases** lists every connection shared with you, **Files** lists file shares, and **Spaces** lists team content. Anything missing hasn't been shared with you yet. See [Browse](../navigation/views/browse.md#browse-tree).
...
* Statistical analysis support: [statistics viewer](../../visualize/viewers/statistics.md), [correlation plot](../../visualize/viewers/correlation-plot.md), scatter plot with [regression lines](../../visualize/viewers/scatter-plot.md#regression-line), [statistical hypothesis testing](../../visualize/table-view-1.md#statistical-hypothesis-testing)
...
Datagrok supports several [color coding modes](../../visualize/viewers/grid#color-code-columns) applied at the column level, with colors automatically synchronized across viewers:
...
**Domain-specific color coding**: For chemical data analysis, the [Scaffold tree](../solutions/domains/chem/chem.md#scaffold-tree-analysis) assigns colors to scaffold structures that can be used as the color source in [scatterplot](../../visualize//viewers/scatter-plot.md). Scaffold colors follow inheritance rules where child nodes inherit parent colors unless explicitly overridden.
...
Joining produces one wider table. Linking keeps both tables and synchronizes the current row, selection, or filter between them through key columns. Use a join when you want a single flat table to chart, and a link when the detail side has many rows per master row and you want to browse them. See [Join or link](../../transform/link-tables.md#joining-or-linking).
...
The grid restores the saved order by column name. The order isn't saved for tables with more than 1,000 columns, and pivoted results order new columns by first appearance. Sort in the query for a stable order.
