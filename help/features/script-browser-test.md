...
The script browser allows you to view previously created [scripts](../features/scripting.md), 
...
1. Run "ACF" [R Script](../features/scripting.md) from its context menu (input table "TSLA.csv" must be opened)
   * Script runs successfully after selecting the required input parameters

1. Choose "Edit" item from context menu for "ACF" [R Script](../features/scripting.md)  
   * Script edit tab has been opened

1. Share "ACF" [R Script](../features/scripting.md) from its context menu to another user or user group
