...
4. Create a default package in your branch [called](../develop.md#naming-conventions)
...
* via the [console](../../datagrok/navigation#console): press `~` key anywhere inside Datagrok, the Console will
...
3. <a name="detectors"></a> Define a `detectNucleotides` semantic type detector function as part of the special
   ...
   iterate [on `column.categories`](../advanced/data-frame.md#work-with-categories)
...
7. Now let’s put the result of the previously created `complement` function into an [info panel](../how-to/add-info-panel.md):
   ...
   properly (see an example [here](../how-to/add-info-panel.md#functions)).
...
2. Implement the function `CountSubsequenceJS` in JavaScript, which does the same as
   [`CountSubsequencePython`](#scripting-with-server-functions). Follow the same conventions on the parameters in the comments block and returning a
