...
1. As we use Docusaurus to generate the API documentation, `js-api` must be installed too. It is part of
the repository's pnpm workspace, so run this once at the repository root (`corepack enable` first if
