...
* **Link** - connects steps. Links can pass data directly from input of one step to output of another step, or apply some transformations.
  * Link source: where to take data from
  * Link target: where to put data
  * Link handler: a TypeScript function performing data transformation. Using handlers, the user can apply small non-interactive transformation to data between steps.
...
in a real time and are used to trigger links.
...
transformation to the data, when makind a separate script step is and overkill.
