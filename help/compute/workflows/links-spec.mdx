...
`FLAGS` are an optional comma-separated list inside parentheses. Omit the
parentheses entirely if no flags are needed (`NAME:PATH`). The recognized flags
are:
...
`PATH` is a mandatory path, where items are delimited by `/`. A
query with no path segments (just a `Name`) resolves to the
link-defining node; this form is used by
[`pipelineValidator`](link-types#pipeline-validators) links to target
the host pipeline. For
...
Three self-ref shorthands are accepted: `NAME:`, `NAME:.`, and `NAME:./`
are all equivalent to bare `NAME`. The `./` prefix is also accepted at
the start of a path, so `NAME:./path` is equivalent to `NAME:path`.
Mid-path `.` and leading `/` remain forbidden.
...
Reference selectors anchor a `from`/`to` query relative to the link's
[base](link-types#base) path. The **base path** is declared by the link's
`base` field; it is itself an LQL query (conventionally named `base`) that
matches one or more workflow nodes. Reference selectors then resolve relative
to each base match, with `@base` standing in for the base query name (or
whatever name was given to the base query).

A special `expand` selector is available for base queries. Selector `expand`
is working like `all` in terms of matching, but has a special meaning. For
each match a separate link instance will be created (provided all other
queries are matching as well). Each link instance will be triggered separately
and pass data only from its own matched inputs to own matched outputs.
Reference selectors take the base query name as the first argument (prefixed
with `@`), and an id list as the second.
...
In this example (assuming that mylink1 is defined directly in
...
are colored via a red border. Note that base queries target
scripts or workflows, not script IO.
...
Matching the first preceding item with any of the specified IDs before the
link instance base path.

#### `after`

Matching the first following item with any of the specified IDs after the
link instance base path.

#### `modifiers`

For [before](#before) and [after](#after) special suffixes are
available, written immediately after the selector name:

- `*` — match _all_ items in the selector's direction, not just the first one.
  Example: `after*(@base,scriptX)` matches every `scriptX` that follows the
  base match.
- `+` — match the _immediately adjacent_ item in the selector's direction
  (the next or previous sibling, with no other steps in between).
  Example: `after+(@base,scriptX)` matches `scriptX` only when it is the very
  next sibling of the base match.

Additionally, [before](#before) and [after](#after) selectors accept a third
parameter called "stop IDs" — a `|`-separated list of node ids that terminates
the scan. As soon as the scan (in the selector's direction) encounters a node
whose id appears in the stop list, matching stops without descending further,
even if a matching id appears later. This is useful for confining a relative
match to a region between two known steps. Example:
`before(@base,prepData,resetStep)` matches the nearest preceding `prepData`,
but gives up if a `resetStep` is encountered first.


### Tag selectors

All selectors can use [tags](configuration#tags) instead of IDs for
matching (including `expand` in base query), by using the `#`
...
Tag and id selectors can be mixed within a single query, in any order: each
segment is resolved against the result set of the previous one.

```
#all(tag1&tag2)/last(script1|script2)/input1
```

Matches all items with both `tag1` and `tag2` first, regardless of how deep
they are nested, then descends into each match and picks the _last_ direct
child whose id is `script1` or `script2`, and finally targets the `input1`
script IO on that child.

The id selector can also come first:

```
#first(workflowA)/#all(metric)/result
```

Picks the first direct child with id `workflowA`, then matches all descendants
tagged `metric`, then the `result` IO on each.

Note that you can match only nested items. Tags that are higher
...
io. All other path parts cannot use template expansion. The name of
