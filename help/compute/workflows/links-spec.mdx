...
This section provides Link Query Language (abbreviated as LQL)
description. Note that queries could be rather complicated, but links
must still propagate data only forward, and it is up to query author
to ensure it is the case in any possible workflow state.
...
The same as `all` in terms of matching, but is used in base path
only. Will generate several link instances (one per each match).
...
__TODO__ ui gif and state diagramm with base paths and links
...
name as the first argument, and id list as the second. It is possible
to mix reference and plain selectors, in the exmaple above first
selector is used in a short form. Note that base queries are targeted
...
__TODO__ ui gif and state diagramm with base paths
...
If there is a need to pass many io items of one script to another,
this could be done using templates. For exmaple, the following:
