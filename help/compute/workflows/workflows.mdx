---
title: "Low-code workflows"
---

:::caution Package dependency

Ensure that the `Compute2` and `WebComponents` packages are installed

:::


Every Datagrok [script](../scripting) is a function that consumes and returns data.
On its own, a script is a one-shot computation: run it, get the output.

To enable interactive multi-step data processing, Datagrok provides the low-code
workflow capabilities. A workflow consists of a series of connected **steps**;
outputs from one step can serve as inputs for another. Users can execute these
workflows step-by-step, examining intermediate results and adjusting parameters
for subsequent steps.

Throughout these docs, the terms **step**, **script node**, and **node** all
refer to the same thing — a single position in the workflow tree. A step is
usually backed by a Datagrok script, but it can also be a nested workflow, an
[action step](./configuration#action-steps), or a reference to another workflow.

## Datagrok low-code workflow capabilities

* Organize a series of steps into a workflow.
* Change inputs and review outputs for each step
* Ensure workflow consistency -- identify and rerun all steps whose input data has changed.
* Dynamically modify the workflow scheme: change step order, add/remove steps, apply the same step several times
* Put restrictions on step order and connections
* Make nested workflows — use a workflow as a step of another workflow.

Simply put, workflows create a flexible sequence of steps that users can modify,
with automatic input population based on outputs from previous steps.
Any individual script can serve as a building block within a workflow.
The platform automatically generates a user interface for each script using
[RichFunctionView](../scripting/advanced-scripting/rich-function-view).

## Where to go from here

Start with the [overview](./overview) for terms and concepts, then walk through
the [Tutorial and examples](./examples) to build a workflow end-to-end.
