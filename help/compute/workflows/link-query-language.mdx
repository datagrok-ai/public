...
- For every base match, `from` resolves `same(@base, workflow1|workflow2)`
  and `same(@base, script1)` back to the same nodes the base picked, then
  takes `output1` on that script.
