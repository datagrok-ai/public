...
travels upward past the link's host workflow.
...
When source-output and target-input sets share names, listing them twice
is redundant. A `(template)` query can use `inputs(...)` or `outputs(...)`
as its last segment to pull the IO list from a script's declared
inputs/outputs. Expansion happens at **config-processing time**: the
deferred entry is replaced by N concrete bare-list entries before any
runtime matching runs.

The first argument is the script's **nqName** (package-qualified function
name, e.g. `MyPkg:MyScript`). An optional second argument is a
pipe-separated exclusion list.

```
{
  from: "in_(template):script1/outputs(MyPkg:Script1)",
  to:   "out_(template):script2/inputs(MyPkg:Script2)"
}
```
...
```
from: "in_(template):script1/outputs(MyPkg:Script1, debug|verbose)"
to:   "out_(template):script2/inputs(MyPkg:Script2, opts)"
```

`outputs(...)` always pulls the script's outputs and `inputs(...)` always
pulls its inputs, regardless of whether the selector appears in `from` or
`to`. There is no parse-time direction check.

**Pairing is purely positional and uses no name check.** After expansion,
the deferred entry becomes N bare-list entries, in the order the script
declared its IOs. The default handler then pairs the i-th expanded source
entry with the i-th expanded target entry — names on the two sides are
not cross-checked. Concretely:

- Source and target IOs share names AND match the declaration order →
  pairing is correct.
- Source and target IOs share names but declaration order differs → the
  link **silently miswires** (e.g. source `a` ends up at target `b`).
- Source and target IOs don't share names at all → the link still writes
  positionally, transferring values into target slots that have no
  semantic relationship to the source.

If you can't guarantee declaration-order alignment, use the explicit
bare-list template form (`script1/a|b|c` → `script2/x|y|z`), which gives
you full control over the pairing.

**Custom handlers.** No contract change — custom handlers receive
expansion arrays in the script's declared IO order. Existing custom
