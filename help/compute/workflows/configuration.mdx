...
[LQL queries](./links-spec) at the same path level as a script IO — but it
...
Same as the [common `id`](#id) — used in [links](./links-spec) for paths.
...
[tag selectors](./links-spec#tag-selectors).
...
* **LQL paths resolve.** Every `from` / `to` / `base` segment matches a step
  `id`, a script IO name, or a [tag](./links-spec#tag-selectors) that exists
