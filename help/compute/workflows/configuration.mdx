...
In this section the configuration object will be described in depth.
...
another [workflow](#workflows), a [workflow
...
Init hook specification, described [here](./link-types).
...
to store data from onInit hook, or some derived state based on scripts
inputs/outputs. These states could be used in links the same way as
...
A list of strings. The primary use case if for annotating that an item
has a certain structural compatibility, like interfaces concept in
programming. Could be matched in [links](./link-spec#tag-selectors).
...
A key-value object if input initial restrictions. Read more about
restrictions [here](./link-types).
