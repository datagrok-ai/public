...
be either a [LQL](./link-spec) string or an array of 0 or more
[LQL](./link-spec) strings. All from [LQL](./link-spec)queries should
...
be either a [LQL](./link-spec) string or an array of 0 or more
[LQL](./link-spec) strings. All to [LQL](./link-spec)queries should
have at least one match, otherwise a link will not be created.
...
Could be either a [LQL](./link-spec) string or an array of 0 or more
[LQL](./link-spec) strings. If all [LQL](./link-spec) queries match,
...
Could be either a [LQL](./link-spec) string or an array of exactly 1
...
a default one. For each [LQL](./link-spec) input query the first matched input is
passed to the corresponding by order [LQL](./link-spec) output. It is also possible
...
Could be either a [LQL](./link-spec) string or an array of multiple
...
There are also integrations avaliable with
