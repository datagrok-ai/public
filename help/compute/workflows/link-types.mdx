...
before those with a lower one. This is primary used to order data
links between script own inputs, since they doesn't have a well
...
trigger consistency changes. User has either to manual review, apply
changes and rerun scripts. Or do it via "rerun with consistent" on a
scope of workflow, nested workflow or a scipt.
...
Each of those field could have an array of `ValidationItem` items with
the following type:
