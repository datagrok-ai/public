...
Two extra operations are registered: `columns` (a dataframe and an
optional filter `numerical`, `categorical`, a column type or a semantic
type; returns column names) and `len` (length of a string, array or
dataframe row count). Truthiness of `when` follows JSON Logic rules.
`when` omitted or `null` means the rule is always on.
