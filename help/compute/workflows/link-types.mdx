...
(for example, user clicks a button); links handlers will be triggered if any of its
input is changed.

Links and actions runtime creation is based on matching
[Link Query Language](./links-spec.mdx) or LQL for short. So until workflow state
...
Could be either a [LQL](./links-spec) string or an array of 0 or more
[LQL](./links-spec) strings. All from [LQL](./links-spec)queries should
have at least one match, otherwise a link will not be created.

#### `to`

Describes link outputs via a [Link Query Language](./links-spec) Could
be either a [LQL](./links-spec) string or an array of 0 or more
[LQL](./links-spec) strings. All to [LQL](./links-spec)queries should
have at least one match, otherwise a link will not be created.
...
Could be either a [LQL](./links-spec) string or an array of 0 or more
[LQL](./links-spec) strings. If all [LQL](./links-spec) queries match,
the link will not be created. Queries are targeting script nodes or
workflows.
...
Could be either a [LQL](./links-spec) string or an array of exactly 1
[LQL](./links-spec) string. Queries are targeting script nodes or
...
links between script own inputs, since they don't have a well
defined order in the workflow.
...
a default one. For each [LQL](./links-spec) input query the first matched input is
passed to the corresponding by order [LQL](./links-spec) output. It is also possible
...
__TODO__ ui consistency examples gifs
...
Could be either a [LQL](./links-spec) string or an array of multiple
[LQL](./links-spec) strings. Queries are targeting script nodes or
...
hooks](./configuration#viewershook) and have a type `meta`.
...
Also an input annotation (__TODO__: link to annotation doc) should
include `optional: true`

For choice inputs (__TODO__: link to doc) it is possible to update an
items list:
...
[sensitivity-analysis](../compute#sensitivity-analysis) and
[optimization](../compute#input-parameter-optimization)
...
once when a workflow is started [programmaticly](./code-usage) and
exited via accept button. Otherwise, the same as [data
links](#data-links).
