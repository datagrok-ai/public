...
| File | Demonstrates |
|------|--------------|
| `.claude/skills/create-workflow/examples/minimal-static.ts` | A static pipeline with no links — the smallest configuration that works. |
| `.claude/skills/create-workflow/examples/dynamic-with-links.ts` | A [dynamic workflow](./configuration#dynamic-workflows) with `stepTypes`, `initialSteps`, an `all()` selector, and a link handler that propagates outputs to every downstream instance. |
| `.claude/skills/create-workflow/examples/validators-and-meta.ts` | Cross-field [validators](./link-types#validators), [meta links](./link-types#metadata-links) that hide/show advanced inputs, and a user-triggered reset [action](./link-types#data-actions). |
