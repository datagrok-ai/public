...
Each Datagrok script has the **header**. Header contains **header parameters** \-
...
Datagrok automatically injects _input_ parameters' values before the script starts
and captures _output_ parameters' values when the script finishes.

Datagrok supports all standard types of scalar parameters (namely, `int`, `float`, `bool`, `string`).
In addition, Datagrok offers high-level parameters designed to interact with dataframes.
You can pass to the script the whole dataframe (`datarframe` type), dataframe column (`column`), list of columns (`column_list`),
or a binary file (`blob`).
...
* The `column` input parameter allows you to select one column from the dataframe.
  In the script the `column` parameters is string variable containing header name of the selected columns.
* The `column_list` input parameter allows you to select multiple columns from the dataframe.
  In the script the `column_list` parameters is a list of strings containing header names of the selected columns.
...
At the moment, there is no way to specify semantic types for the column in the output dataframe.
However, Datagrok sematic type detectors work excellent and assign the correct semantic types 
in the most part of cases. 
...
If the script throws an unhandled exception, Datagrok catches it and displays an error popup (shown on the right).
