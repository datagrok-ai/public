...
```javascript
//output: dataframe tempOnTime { viewer: Line chart }
```
...
```javascript
//input: dataframe temp_data { viewer: Line chart(block: 100, x: "Time (hours)") | Grid(block: 100, rowHeight: 35) }
```
...
grok.shell.info(`Data row count: ${temp_data.rowCount}`);
```
