...
- Built-in editor opens for newly created scripts (see [how to create a script](#how-to-create))
...
```javascript
//output: dataframe output_DF { viewer: Line chart | Scatter plot }
```
...
output_DF = input_DF
output_DF.name = "DF with linechart"
```
...
Use Rich function view only when you are sure that script works as expected. Features of Rich function view affect GUI only and has no effect on computations.
...
View includes 4 tabs in total since there is separate tab for input dataframe viewers.
...
The historical runs are immutable. Thus, if you load historical run and then run it again changing input values, the platform will create a new historical run.
...
external consumption. To find our how to use it, select the function and expand the `REST` pane on the context panel
...
At any moment of the time you can delete a script:
- Find to in [Scripts section](https://public.datagrok.ai/scripts).
