...
- To run an existing script, find it in [Scripts secton](https://public.datagrok.ai/scripts), right click on script card and choose **Run** action
...
- JS scripts run locally in the browser, without sending it to the server

### JavaScript

There are a set of [script examples](https://public.datagrok.ai/js/samples) available publicly. Feel free to play around with them to explore different Datagrok capabilities.

For the scripts created via user interface, it is possible to use debugging output. Output could be written:
...
Inspector provides a wide range of additional information for development.
...
Since scripts runs are independent, it is important provide `pkg load %PACKAGE_NAME%` in your scripts.
...
```javascript
//output: dataframe dummy_data { viewer: Line chart(markerType: star, markerSize: 15) | Scatter plot(showRegressionLine: true) }
```
...
dummy_data = temp_data.clone();
grok.shell.info(`Data row count: ${dummy_data.rowCount}`);
```
...
Use `RichFunctionView` only when you are sure that script works as expected. Features of `RichFunctionView` affect only user interface and has no effect on computations.
...
You may control access rights to your scripts. All sharing features are located in Sharing dialog. 
