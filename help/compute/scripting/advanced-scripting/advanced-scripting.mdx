---
title: "Advanced scripting"
sidebar_position: 3
description: Overview of advanced script development options including custom UI, packaging, and integration.
keywords:
  - rich function view
  - convert script to package
  - shared conda environments
  - rest api integration
  - embed script as iframe
  - autocomplete and validators
---

```mdx-code-block
import Tabs from '@theme/Tabs';
import TabItem from '@theme/TabItem';
import BrowserWindow from '@site/src/components/browser-window';
```

This section contains advanced options for script development.

* Using the [Rich Function View](./rich-function-view.md) UI editor,
  you can create a complex full-featured interface for your script.
* You can [convert your script to a package function](./convert-script-to-package-function.mdx),
  and [share Python and R environments across scripts](./share-envs.mdx).
* [Automate inputs](./automate-inputs.md) with suggestions, autocompletions, and validators.
* Some details regarding default script environments are given in the section
  [Under the hood](./under-the-hood.mdx).
* Advanced integration options allow you to
  [call your script via REST](./call-via-rest.md) or [embed it as iFrame](./embed-as-iframe.mdx).






