...
sidebar_position: 8
description: Specify inline or package-based Conda environments to control script dependencies and language versions.
keywords:
  - conda yaml config
  - inline environment
  - package environment folder
  - environment reuse
  - md5 hash environment
...
to a one-liner YAML following the standard Conda YAML config (omit its name).
...
You can store your configurations in a package's `environment` folder. They should use Conda YAML format.
