...
Datagrok expects that all variables are defined in the script code and have proper values by the end of script run.
...
- and takes time to be created before script run.
...
to an one-liner YAML following the standard Conda YAML config (omit it's name). 
...
If there is a ready-to-use environment with the same environment config, Datagrok will pick it up and reuse it. 
...
Environment references are supported in both package and ad-hoc scripts.

#### Sharing environments

Environment referencing is a handy way to share and manage environments. 
You share common environments through a dedicated package, 
and one Conda environment will be re-used by many users, which is more space- and time-efficient.
...
In the future, Datagrok script editor will show all available environments in a dropdown list.
...
that sometimes it takes a large time to resolve dependencies. 
...
Single output will be assigned to `result` directly. Multiple outputs will be united into single object.
...
* Save and access [run history](#accessing-history)
...
You can use `category` keyword to group output parameters,
simular to the [input categories](scripting-for-non-developers#grouping-inputs).
In case of output parameters the `category` tag specifies 
...
so to save results to the [run history](#accessing-history) you should press **Save** button manually.
...
When enabled, server saves each script run. The saved script run includes both the input and output values of the run.
...
This specific capability is very useful when you're developing a 
model, predicting behavior of some real-world process.
Using this mode, 
you can create in the run history a "fake" run by uploading data from some external source instead of 
running script. Saved historical run with external data will have the `experimental` tag.
...
RichFunctionView automatically collects all data fields you need to proved to mimic script run,
