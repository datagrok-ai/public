...
export class DataFrame {
   ...
   * @returns {DataFrame} */
  static fromColumns(columns: Column[]) {
  ...
  static fromObjects(list: object[]) {
   ...
   * @param {string=} format
   * @returns {Column} */
  changeColumnType(column: Column, newType: Type, format = null) {
   ...
   * @returns {Int32Array}
   * */
  getSortedOrder(sortByColumnIds: object[], sortOrders = null, rowMask = null) {
   ...
   * */
  unpivot(copyColumnNames: string[], mergeColumnNames: string[], categoryColumnName = 'Category', valueColumnName = 'Value') {
   ...
   * @returns {DataFrame}
   * */
  join(t2: DataFrame, keyColumns1: string[], keyColumns2: string[], valueColumns1: string[], valueColumns2: string[], joinType: JoinType, inPlace: boolean) {
   ...
   * @returns {DataFrame}
   * */
  append(t2: DataFrame, inPlace = false, columnsToAppend = null) {
    return new DataFrame(api.grok_DataFrame_Append(this.d, t2.d, inPlace, columnsToAppend));
  }

  /** @returns {Observable} */ _event(event: string) {
  ...
  getDensity(xBins: number, yBins: number, xColName: string, yColName: string) {
...
export class Column {
   ...
   * @param {boolean} notify
   */
  set(i: number, x: any, notify = true) {
  ...
  * values() {
   ...
   *  @param {string} format
   *  @returns {Column} */
  convertTo(newType: string, format = null) {
...
constructor(d: any) {
   ...
   * @returns {Column[]} */
  bySemTypesExact(semTypes: SemType[]) {
    let columns = [];
   ...
   * @param {int} index
   * @returns {Column} */
  insert(column: Column, notify = true, index: number) {
   ...
   * @returns {Column} */
  addNew(name: string, type: ColumnType) {
   ...
   * @param {string} name
   * @returns {Column} */
  addNewString(name: string) { return this.addNew(name, TYPE.STRING); }

  /** Adds a new integer column
   * @param {string} name
   * @returns {Column} */
  addNewInt(name: string) { return this.addNew(name, TYPE.INT); }

  /** Adds a new float column
   * @param {string} name
   * @returns {Column} */
  addNewFloat(name: string) { return this.addNew(name, TYPE.FLOAT); }

  /** Adds a new qualified number column
   * @param {string} name
   * @returns {Column} */
  addNewQnum(name: string) { return this.addNew(name, TYPE.QNUM); }

  /** Adds a new datetime column
   * @param {string} name
   * @returns {Column} */
  addNewDateTime(name: string) { return this.addNew(name, TYPE.DATE_TIME); }

  /** Adds a new boolean column
   * @param {string} name
   * @returns {Column} */
  addNewBool(name: string) { return this.addNew(name, TYPE.BOOL); }
   ...
   * @returns {Column} */
  addNewVirtual(name: string, getValue: Function, type = TYPE.OBJECT) {
  ...
  replace(columnToReplace: Column | string, newColumn: Column) {
...
export class RowList {
  ...
  removeAt(idx: number, count = 1, notify = true) {
   ...
   * @param notify - Whether a change notification should be fired. */
  insertAt(idx: number, count = 1, notify = true) {
  ...
  * [Symbol.iterator]() {
   ...
   * @param {number} idx - Row index.
   * @param values - List of values (length and types should match columns) */
  setValues(idx: number, values: any[]) {
   ...
   * */
  _applyPredicate(bitset: BitSet, rowPredicate: RowPredicate) {
    for (let row of this) {
   ...
   * @param {RowPredicate} rowPredicate
   * */
  select(rowPredicate: RowPredicate) {
   ...
   * @param {RowPredicate} rowPredicate
   * */
  filter(rowPredicate: RowPredicate) {
...
export class Qnum {
   ...
   * @returns {number}
   * */
  static create(value: number, q = QNUM_EXACT) {
