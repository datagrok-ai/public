...
export class DataQuery extends Func {
  ...
  /** The connection this query runs on. */
  ...
  /** Script that post-processes the query result. */
...
export class TableQuery extends DataQuery {
   ...
   * @param connection - DataConnection to query table from */
...
export class TableQueryBuilder {
   ...
   * @param fieldAlias - Name of the resulting column. Default value is agg(colName). */
   ...
   * @param fields - Array of fields to group by */
   ...
   * @param fields - Array of fields to pivot on */
  pivotOn(fields: string[]): TableQueryBuilder {
    return toJs(api.grok_DbTableQueryBuilder_PivotOn(this.dart, fields));
  }

  /** Adds a where clause to the query.
   * @param field - Field name. If you join to other tables use correct alias or table name as prefix, e.g. <table alias>.<field>
   * @param pattern - Pattern to test field values against
   ...
   * @param field - Field to sort based on
   ...
   * @param n - Number of records to select */
...
export class DataConnection extends Entity {
  /** Connection parameters (server, database, and so on) as a key-value map. */
  parameters: any;
  ...
  /** Starts a {@link TableQueryBuilder} over [table] of this connection. */
  buildQuery(table: string): TableQueryBuilder {
    return TableQueryBuilder.from(table, this);
  }

  /** Creates an empty {@link TableQuery} over this connection. */
   ...
   * @param parameters - Connection properties */
