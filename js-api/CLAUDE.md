...
```bash
pnpm install            # once, at the root of public/ (js-api is part of the pnpm workspace)
pnpm run build          # grok tsc (TypeScript 7, emits dist/ with declarations) && grok build --skip-check && smoke test
pnpm run build-docker   # browser bundle only (grok build --skip-check --only=browser)
pnpm run build-node     # Node bundle only
pnpm run typecheck      # tsc --noEmit
pnpm run lint           # ESLint over the TypeScript sources
pnpm test               # Node unit tests over the built bundle
pnpm run check-links    # every ApiSamples link in the JSDoc must resolve
pnpm run inventory      # surface inventory ratchet (see CI/CD below)
```
...
`tsc` (TypeScript 7 via `grok tsc`) emits the npm package's JavaScript and declarations into `dist/`;
consumers resolve `datagrok-api/dg`, `datagrok-api/src/...` through the `exports` map in `package.json`.
Two rspack bundles in `rspack.config.js` (swc, no ts-loader):
...
2. **Browser target** - Outputs `js-api.js` to `../../core/client/xamgle/web/js/api` for browser use,
   downlevelled to Chrome 50 (Dartium)

### Regenerating after a JS API change

After editing TypeScript sources here, regenerate the bundle Datagrok ships:

1. Make sure your reddata working copy has the matching `public/` submodule
   commit checked out.
2. From the root of `public/`, run `pnpm install` once (or after dependency bumps),
   then `pnpm run build` in `public/js-api/` (or `grok build` there).
3. The browser target writes `js-api.js` and `js-api.js.map` into
   `core/client/xamgle/web/js/api/`. Commit both the autogenerated files in
   reddata **and** the matching submodule bump together.

On Windows, autogenerated files may show as modified purely because of CRLF/LF
line-ending differences — diff the file content (not the raw bytes) before
re-committing.

### CI/CD

`public/.github/workflows/js-api.yml` triggers on changes under `js-api/**`.
The workflow builds js-api, runs `check-links`, `inventory`, `lint` and `test`, version-checks
`package.json`, and publishes to npm when the version was bumped (it must install through the pnpm
workspace and pack with `pnpm pack`, which rewrites the `workspace:`/`catalog:` specifiers; see
`public/.github.md`). TypeDoc-based reference docs are generated by the Docusaurus pipeline
(`public/.github/workflows/docusaurus.yaml`), not by this workflow.

**Surface inventory ratchet.** `scripts/inventory.cjs` walks the hand-written and generated
sources with the TypeScript compiler and counts documented / tested (ApiTests) / sampled
(ApiSamples) members, `any` in signatures and "dark" members (none of the three). `pnpm run
inventory` fails when any count in `scripts/inventory-baseline.json` grows, so a new member
must arrive documented and typed; when a PR improves a number, commit the new baseline
(`node scripts/inventory.cjs --update-baseline`). `--out <dir>` writes the per-file and
per-class tables and the samples-per-member index (a sample's optional first line
`//api: DG.Class.member, ui.fn` names what it demonstrates). `grok-core audit js-api` runs
the same check from the monorepo. The audit that produced it: `core/docs/reviews/js-api-audit/`.
