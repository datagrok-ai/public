...
The workflow builds js-api, version-checks `package.json`, and publishes to npm when the version was
bumped (it must install through the pnpm workspace and pack with `pnpm pack`, which rewrites the
`workspace:`/`catalog:` specifiers; see `public/.github.md`). TypeDoc-based reference docs are
generated by the Docusaurus pipeline (`public/.github/workflows/docusaurus.yaml`),
