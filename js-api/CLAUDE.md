...
npm test                # Node unit tests over the built bundle
npm run check-links     # every ApiSamples link in the JSDoc must resolve
npm run inventory       # surface inventory ratchet (see CI/CD below)
```
...
1. Make sure your reddata working copy has the matching `public/` submodule
   commit checked out.
2. From `public/js-api/`, run `npm install` once (or after dependency bumps),
   then `npm run build` (or `webpack`).
...
The workflow runs `npm run build-ts`, `check-links`, `inventory`, `lint` and `test`,
version-checks `package.json`, and publishes to npm when the version was bumped.
TypeDoc-based reference docs are generated by the Docusaurus pipeline
...
(ApiSamples) members, `any` in signatures and "dark" members (none of the three). `npm run
