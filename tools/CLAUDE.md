...
- `schema.ts` - Generate (from `@grok.decorators.entity` classes, read with ts-morph in `utils/entity-classes.ts`), seal, check, diff, migrate, squash and print the DDL of entity schema manifests (`databases/<schema>/schema.json`); the pure snapshot/diff/scaffold library is `utils/domain-snapshot.ts`
