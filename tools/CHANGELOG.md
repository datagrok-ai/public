...
* GROK-20799: `grok schema seal|check|diff|migrate` — entity schema snapshots for `databases/<schema>/schema.json` manifests: `seal` writes the canonical, hashed `databases/<schema>/snapshot.json` (byte-compatible with the snapshot the server records on deploy), `check` exits 1 with the change list when a manifest changed without a reseal, `diff` prints the classified changes, and `migrate --name <x>` scaffolds `databases/<schema>/migrations/NNNN_<x>.sql` and its `down/` twin for the physical changes the deployer refuses (additive ones it applies itself) and reseals. `--server [alias]` diffs against the snapshot recorded on a server instead of the sealed file.
