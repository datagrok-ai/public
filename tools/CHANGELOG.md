...
* `grok s migrate` — refuses to push a bundle whose pull dropped entities, instead of promoting less than was asked for; a table whose `.d42` data could not be read counts as dropped, and byte transfers get their own 10-minute deadline (`GROK_HTTP_BYTES_TIMEOUT`) so a large table is not cut off by the request one.
