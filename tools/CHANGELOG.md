...
* GROK-20799: `grok schema generate|seal|check|diff|migrate|squash|ddl` — entity schema snapshots for `databases/<schema>/schema.json` manifests: `generate` derives the manifests from classes decorated `@grok.decorators.entity` / `@grok.decorators.column` (`src/**/*.ts`, read statically with ts-morph; property order = column order, types inferred from the property types), `ddl` prints the CREATE TABLE DDL of every table in dependency order, `seal` writes the canonical, hashed `databases/<schema>/snapshot.json` (byte-compatible with the snapshot the server records on deploy), `check` exits 1 with the change list when a manifest changed without a reseal, `diff` prints the classified changes, and `migrate --name <x>` scaffolds `databases/<schema>/NNNN_<x>.sql` (run on deploy) and its `down/` twin with every physical change (additive ones too, so a script that moves data is self-contained) and reseals; `squash --name <x> --all|--from N --to N` folds consecutive scripts into one (`--all --discard --yes` deletes them all: the manifest becomes the baseline). `--server [alias]` diffs against the snapshot recorded on a server instead of the sealed file.
