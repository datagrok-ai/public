...
/** Renders a diff as an up and a down script (MIGRATIONS.md §5): every physical change gets
 * its statements — the additive ones too, so a script that moves data between them is
 * self-contained (the deployer's own repeat is a no-op) — and the down script reverses
 * them in reverse order. Registry-only changes are listed in the header. */
export function scaffold(changes: Change[], schema: string, options: ScaffoldOptions = {}): {up: string, down: string} {
  ...
  return {
    up: script(schema, changes, physical, false, options),
    down: script(schema, changes, [...physical].reverse(), true, options),
  };
}

// --- Squashing migration scripts ------------------------------------------------------

/** The transition line every generated script carries as its second line. */
export const migrationHeader = /^--\s*ems-migration:\s*from=(\S+)\s+to=(\S+)/m;
...
export function parseMigrationHeader(text: string): {from: string, to: string} | null {
  const m = migrationHeader.exec(text);
  return m == null ? null : {from: m[1], to: m[2]};
}

/** The first break in the chain — each script's `to` must be the next one's `from` — or null. */
export function chainGap(scripts: MigrationScript[]): string | null {
  for (let i = 0; i + 1 < scripts.length; i++) {
    if (scripts[i].to !== scripts[i + 1].from) {
      return `${scripts[i].file} ends at ${shortHash(scripts[i].to)}… but ` +
        `${scripts[i + 1].file} starts from ${shortHash(scripts[i + 1].from)}…`;
    }
  }
  return null;
}

/** One source of a squashed script: its body verbatim minus its own transition line. */
function section(file: string, body: string | undefined): string[] {
  const lines = [`-- ==== ${file} ====`];
  if (body == null)
    lines.push(`-- TODO: no down script for ${file.replace(/^down\//, '')}`);
  else
    lines.push(body.replace(/^--\s*ems-migration:.*(?:\r?\n|$)/m, '').replace(/\s+$/, ''));
  lines.push('');
  return lines;
}

/** One up and one down script covering [scripts] (a continuous chain, in order): the up
 * bodies in order, the down twins in reverse order, each verbatim minus its transition line. */
export function squashScripts(scripts: MigrationScript[], schema: string, name: string): {up: string, down: string} {
  const first = scripts[0], last = scripts[scripts.length - 1];
  const title = `-- ${schema}: ${name} (squashed from ${first.file} .. ${last.file})`;
  const trailer = ['-- Generated by grok schema squash', '-- Review every statement before committing.', ''];
  const up = [title, `-- ems-migration: from=${first.from} to=${last.to}`, ...trailer];
  for (const s of scripts)
    up.push(...section(s.file, s.up));
  const down = [`${title} (down)`, `-- ems-migration: from=${last.to} to=${first.from}`, ...trailer];
