...
describe('kg gen (conventions.md §11.2)', () => {
  ...
  it('writes a note instead of a tree when there are no homes', () => {
    const md = generateFeatures(system, {homes: [], pages: [], stubs: [], errors: [], warnings: [], unresolvedExternal: [], scanned: 0, annotatedPages: 0, citations: {doc: 0, code: 0, media: 0}, media: new Map(), mediaInventory: {files: new Map(), ok: true, conflicted: new Set()}, index: {byId: new Map(), byAlias: new Map()}});
...
describe('grok kg command', () => {
  ...
  it('refuses a nameless report, a bad --output, a stray argument and gen --types-only with exit 1', async () => {
    ...
    for (const [argv, message] of [
      [{_: ['kg', 'report'], kg: kgRoot}, /^grok kg report needs a report name: orphans, stale, coverage, proposed, media, diff, replay$/],
