...
const domainUiImports = ['domainHandler', 'DomainAppView', 'DomainAppViewOptions', 'DomainEntityAppView',
  'DomainEntityAppViewOptions', 'DomainGrid', 'DomainGridOptions', 'EntityListOptions', 'EntityListWidget'];
...
function generateDomainUiCode(manifest: any, dbImports: string[]): string {
  ...
  for (const tableName of Object.keys(manifest.tables)) {
    ...
    decls.push(
      ...
      [`/** Options of {@link ${type}Ui.appView}. */`,
      ...
      ['/**',
        ...
        `  get client(): DG.DomainTableClient<${type}Row, ${type}Insert, ${type}Column, ${type}Expand> {`,
        ...
        '    return domainHandler(this.table);',
        '  }',
        '',
        '  /** The browse/CRUD page: list, search, New, and a deep-linkable query. */',
        `  appView(options?: ${type}AppViewOptions): DomainAppView {`,
        '    return new DomainAppView(this.client, options);',
        '  }',
        '',
        '  /** The row page: form, detail tabs, history. */',
        '  entityView(id: string, options?: DomainEntityAppViewOptions): DomainEntityAppView {',
        '    return new DomainEntityAppView(this.client, id, options);',
        '  }',
        '',
        '  /** An editable grid: batch editing, one-transaction save. */',
        `  grid(options?: ${type}GridOptions): Promise<DomainGrid> {`,
        '    return DomainGrid.create(this.client, options);',
        '  }',
        '',
        '  /** A list of rows (cards / brief / grid); null when the table is not viewable. */',
        `  list(options?: ${type}ListOptions): Promise<EntityListWidget | null> {`,
        '    return EntityListWidget.create(this.client, options);',
        '  }',
        '',
